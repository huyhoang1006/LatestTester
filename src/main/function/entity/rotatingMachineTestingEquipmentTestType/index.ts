      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get rotatingMachineTestingEquipmentTestType by id failed'
          })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'RotatingMachineTestingEquipmentTestType not found'
          })
        return resolve({
          success: true,
          data: row,
          message: 'Get rotatingMachineTestingEquipmentTestType by id completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Insert rotatingMachineTestingEquipmentTestType failed'
          })
        return resolve({
          success: true,
          data: data,
          message: 'Insert rotatingMachineTestingEquipmentTestType completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Update rotatingMachineTestingEquipmentTestType failed'
          })
        return resolve({
          success: true,
          data: data,
          message: 'Update rotatingMachineTestingEquipmentTestType completed'
        })
      }
    )
  })
...
export const deleteRotatingMachineTestingEquipmentTestTypeByIdTransaction = async (
  mrid: string,
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM rotating_machine_testing_equipment_test_type WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Delete rotatingMachineTestingEquipmentTestType failed'
          })
        if (this.changes === 0)
          return resolve({
            success: false,
        ...
        return resolve({
          success: true,
          data: null,
          message: 'Delete rotatingMachineTestingEquipmentTestType completed'
        })
      }
    )
  })
