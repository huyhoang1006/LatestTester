...
export const updateUserById = async (
    user_id: string,
    user: { role: string | null; permission: string | null; username: string | null; token: string | null; group_user: string | null }
) => {
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE user SET
...
export const deleteUserById = async (user_id: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM user WHERE user_id = ?', [user_id], function (this: { lastID: number; changes: number }, err: Error | null) {
