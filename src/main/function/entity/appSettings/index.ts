...
export const insertAppSettingsTransaction = (data: { key: string; value: unknown }, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const updateAppSettingsTransaction = (data: { key: string; value: unknown }, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const deleteAppSettingsPsrTransaction = (key: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM app_settings WHERE key=?', [key], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) {
