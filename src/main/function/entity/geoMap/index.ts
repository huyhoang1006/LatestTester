...
export const getGeoMapByOrganisationId = async (organisationId: string) => {
    return new Promise((resolve, reject) => {
        db.all("SELECT * FROM geo_map WHERE organisation_id=?", [organisationId], (err: any, rows: any) => {
...
export const insertGeoMapTransaction = async (geoMap: any, dbsql: any) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            function (err: any) {
                if (err) return reject({ success: false, err, message: 'Insert geo map transaction failed' })
                return resolve({ success: true, data: geoMap, message: 'Insert geo map transaction completed' })
            }
        )
    })
}
...
export const updateGeoMapByIdTransaction = async (mrid: string, geoMap: any, dbsql: any) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            [geoMap.organisation_id, geoMap.x, geoMap.y, geoMap.z, mrid],
            function (err: any) {
                if (err) return reject({ success: false, err, message: 'Update geo map transaction failed' })
                return resolve({ success: true, data: geoMap, message: 'Update geo map transaction completed' })
            }
        )
    })
}
