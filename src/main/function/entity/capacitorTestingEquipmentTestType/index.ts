      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get capacitorTestingEquipmentTestType by id failed'
          })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'CapacitorTestingEquipmentTestType not found'
          })
        return resolve({
          success: true,
          data: row,
          message: 'Get capacitorTestingEquipmentTestType by id completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Insert capacitorTestingEquipmentTestType failed'
          })
        return resolve({
          success: true,
          data: data,
          message: 'Insert capacitorTestingEquipmentTestType completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Update capacitorTestingEquipmentTestType failed'
          })
        return resolve({
          success: true,
          data: data,
          message: 'Update capacitorTestingEquipmentTestType completed'
        })
      }
    )
  })
...
export const deleteCapacitorTestingEquipmentTestTypeByIdTransaction = async (
  mrid: string,
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM capacitor_testing_equipment_test_type WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Delete capacitorTestingEquipmentTestType failed'
          })
        if (this.changes === 0)
          return resolve({
            success: false,
        ...
        return resolve({
          success: true,
          data: null,
          message: 'Delete capacitorTestingEquipmentTestType completed'
        })
      }
    )
  })
