...
export const insertCurrentTransformerEntity: any = async (old_entity: any, entity: any) => {
  try {
    ...
    const toAddVoltage = (entity.voltage || []).filter(
      (v: any) => v.mrid && !oldVoltageIds.includes(v.mrid)
    )
    ...
    const _newCurrentFlowIds = (entity.currentFlow || [])
      .map((c: any) => c.mrid)
      .filter((id: any) => id)
    void _newCurrentFlowIds
    const oldCurrentFlowIds = (old_entity.currentFlow || [])
      .map((c: any) => c.mrid)
      .filter((id: any) => id)
    const toAddCurrentFlow = (entity.currentFlow || []).filter(
      (c: any) => c.mrid && !oldCurrentFlowIds.includes(c.mrid)
    )
    ...
    const toAddSeconds = (entity.seconds || []).filter(
      (s: any) => s.mrid && !oldSecondsIds.includes(s.mrid)
    )
    ...
    const _newFrequencyIds = (entity.frequency || [])
      .map((f: any) => f.mrid)
      .filter((id: any) => id)
    void _newFrequencyIds
    const oldFrequencyIds = (old_entity.frequency || [])
      .map((f: any) => f.mrid)
      .filter((id: any) => id)
    const toAddFrequency = (entity.frequency || []).filter(
      (f: any) => f.mrid && !oldFrequencyIds.includes(f.mrid)
    )
    ...
    const _newResistanceIds = (entity.resistance || [])
      .map((r: any) => r.mrid)
      .filter((id: any) => id)
    void _newResistanceIds
    const oldResistanceIds = (old_entity.resistance || [])
      .map((r: any) => r.mrid)
      .filter((id: any) => id)
    const toAddResistance = (entity.resistance || []).filter(
      (r: any) => r.mrid && !oldResistanceIds.includes(r.mrid)
    )
    ...
    const toAddPercent = (entity.percent || []).filter(
      (p: any) => p.mrid && !oldPercentIds.includes(p.mrid)
    )
    ...
    const _newApparentPowerIds = (entity.apparentPower || [])
      .map((a: any) => a.mrid)
      .filter((id: any) => id)
    ...
    const toAddApparentPower = (entity.apparentPower || []).filter(
      (a: any) => a.mrid && !oldApparentPowerIds.includes(a.mrid)
    )
    ...
    const _newTemperatureIds = (entity.temperature || [])
      .map((t: any) => t.mrid)
      .filter((id: any) => id)
    void _newTemperatureIds
    const oldTemperatureIds = (old_entity.temperature || [])
      .map((t: any) => t.mrid)
      .filter((id: any) => id)
    const toAddTemperature = (entity.temperature || []).filter(
      (t: any) => t.mrid && !oldTemperatureIds.includes(t.mrid)
    )
    ...
    const productAssetModelResult: any = await insertProductAssetModelTransaction(
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataCurrentTransformer.success) {
        ...
        const dataOldCurrentTransformerInfo: any = await getOldCurrentTransformerInfoById(
        ...
        const dataCtCoreInfo: any = await getCtCoreInfoByCurrentTransformerInfoId(
          entity.oldCurrentTransformerInfo.mrid
        )
