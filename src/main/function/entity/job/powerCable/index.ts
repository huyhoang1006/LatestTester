...
export const insertPowerCableJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for PowerCable Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.powercableTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.powercableTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.powercableTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.powercableTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.powercableTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getPowerCableTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.powercableTestingEquipmentTestType = entity.powercableTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'powercable job entity retrieved successfully' }
            } else {
...
export const deletePowerCableJobEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.powercableTestingEquipmentTestType && entity.powercableTestingEquipmentTestType.length > 0) {
