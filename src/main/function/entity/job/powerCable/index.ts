...
import BushingJobEntity from '@/views/Flatten/Job/PowerCable/index.js'
import {
...
import {
  insertPowerCableTestingEquipmentTestTypeTransaction,
  getPowerCableTestingEquipmentTestingEqId,
  deletePowerCableTestingEquipmentTestTypeByIdTransaction
} from '../../powerCableTestingEquipmentTestType/index.js'
import {
...
export const insertPowerCableJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for PowerCable Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.powercableTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.powercableTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.powercableTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.powercableTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.powercableTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      ...
      return { success: true, data: entity, message: 'PowerCable Job entity inserted successfully' }
    }
  } catch (error: any) {
...
export const getPowerCableJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getPowerCableTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.powercableTestingEquipmentTestType =
              entity.powercableTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'powercable job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deletePowerCableJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.powercableTestingEquipmentTestType &&
      entity.powercableTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.powercableTestingEquipmentTestType) {
        await deletePowerCableTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    return { success: true, message: 'PowerCable Job entity deleted successfully' }
  } catch (error: any) {
    await runAsync('ROLLBACK')
    console.error('Delete PowerCable Job Error:', error)
    return { success: false, error, message: 'Error deleting PowerCable Job entity' }
  }
}
