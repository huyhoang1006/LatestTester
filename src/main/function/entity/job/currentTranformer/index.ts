...
import {
  insertCurrentTransformerTestingEquipmentTestTypeTransaction,
  getCurrentTransformerTestingEquipmentTestingEqId,
  deleteCurrentTransformerTestingEquipmentTestTypeByIdTransaction
} from '../../currentTranformerTestingEquipmentTestType/index.js'
import {
...
export const insertCurrentTransformerJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      ...
      const newIdsSet = entity.currentTransformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.currentTransformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.currentTransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.currentTransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.currentTransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteCurrentTransformerTestingEquipmentTestTypeByIdTransaction(
      ...
      return {
        success: true,
        data: entity,
        message: 'Current Transformer Job entity inserted successfully'
      }
    }
  } catch (error: any) {
    ...
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getCurrentTransformerTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.currentTransformerTestingEquipmentTestType =
              entity.currentTransformerTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
...
export const deleteCurrentTransformerJobEntity: any = async (entity: any) => {
  try {
    ...
    if (
      entity.currentTransformerTestingEquipmentTestType &&
      entity.currentTransformerTestingEquipmentTestType.length > 0
    ) {
