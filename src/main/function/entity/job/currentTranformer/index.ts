...
import { uploadAttachmentTransaction, deleteAttachmentByIdTransaction, deleteBackupFiles, restoreFiles, syncFilesWithDeletion, getAttachmentByForeignIdAndType } from '@/function/entity/attachment'
...
export const insertCurrentTransformerJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Current Transformer Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.currentTransformerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.currentTransformerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.currentTransformerTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.currentTransformerTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.currentTransformerTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
            for (const equipmentTestType of toAddSet) { await insertCurrentTransformerTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            for (const equipmentTestType of toUpdateSet) { await insertCurrentTransformerTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            ...
            for(const equipmentTestType of toDeleteSet) { await deleteCurrentTransformerTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db); }
...
export const getCurrentTransformerJobEntity: any = async (id: string) => {
    try {
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getCurrentTransformerTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.currentTransformerTestingEquipmentTestType = entity.currentTransformerTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'Current Transformer job entity retrieved successfully' }
            } else {
...
export const deleteCurrentTransformerJobEntity: any = async (entity: any) => {
    try {
        await runAsync('BEGIN TRANSACTION');
        ...
        if (entity.currentTransformerTestingEquipmentTestType && entity.currentTransformerTestingEquipmentTestType.length > 0) {
            for (const item of entity.currentTransformerTestingEquipmentTestType) { await deleteCurrentTransformerTestingEquipmentTestTypeByIdTransaction(item.mrid, db); }
        }
