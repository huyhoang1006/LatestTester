...
import {
  insertVoltageTransformerTestingEquipmentTestTypeTransaction,
  getVoltageTransformerTestingEquipmentTestingEqId,
  deleteVoltageTransformerTestingEquipmentTestTypeByIdTransaction
} from '../../voltageTransformerTestingEquipmentTestType/index.js'
import {
...
export const insertVoltageTransformerJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for VoltageTransformer Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.voltagetransformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.voltagetransformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.voltagetransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.voltagetransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.voltagetransformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteVoltageTransformerTestingEquipmentTestTypeByIdTransaction(
      ...
      return {
        success: true,
        data: entity,
        message: 'VoltageTransformer Job entity inserted successfully'
      }
    }
  } catch (error: any) {
    ...
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getVoltageTransformerTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.voltagetransformerTestingEquipmentTestType =
              entity.voltagetransformerTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'voltagetransformer job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteVoltageTransformerJobEntity: any = async (entity: any) => {
  try {
    ...
    if (
      entity.voltagetransformerTestingEquipmentTestType &&
      entity.voltagetransformerTestingEquipmentTestType.length > 0
    ) {
