...
import { insertReactorTestingEquipmentTestTypeTransaction, getReactorTestingEquipmentTestingEqId, deleteReactorTestingEquipmentTestTypeByIdTransaction } from '../../reactorTestingEquipmentTestType/index.js'
...
export const insertReactorJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Reactor Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.reactorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.reactorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.reactorTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.reactorTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.reactorTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getReactorTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.reactorTestingEquipmentTestType = entity.reactorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
...
export const deleteReactorJobEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.reactorTestingEquipmentTestType && entity.reactorTestingEquipmentTestType.length > 0) {
