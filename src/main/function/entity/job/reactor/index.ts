...
import BushingJobEntity from '@/views/Flatten/Job/Reactor/index.js'
import {
...
import {
  insertReactorTestingEquipmentTestTypeTransaction,
  getReactorTestingEquipmentTestingEqId,
  deleteReactorTestingEquipmentTestTypeByIdTransaction
} from '../../reactorTestingEquipmentTestType/index.js'
import {
...
export const insertReactorJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Reactor Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.reactorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.reactorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.reactorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.reactorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.reactorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertReactorTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      return { success: true, data: entity, message: 'Reactor Job entity inserted successfully' }
    }
  } catch (error: any) {
...
export const getReactorJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getReactorTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.reactorTestingEquipmentTestType = entity.reactorTestingEquipmentTestType.concat(
        ...
        return { success: true, data: entity, message: 'reactor job entity retrieved successfully' }
      } else {
        return {
          success: false,
...
export const deleteReactorJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.reactorTestingEquipmentTestType &&
      entity.reactorTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.reactorTestingEquipmentTestType) {
        await deleteReactorTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    return { success: true, message: 'Reactor Job entity deleted successfully' }
  } catch (error: any) {
    await runAsync('ROLLBACK')
