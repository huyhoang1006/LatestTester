...
const getJobName = (entity, fallback) => {
  ...
  return (
    normaliseAuditValue(work.name || work.work_order_number || work.mrid) ||
    fallback ||
    'Unnamed job'
  )
}
...
const getTestName = (item, context) => {
  ...
  return (
    firstText(workTask, ['name', 'alias_name', 'description', 'task_kind', 'mrid']) ||
...
const getMeasurementName = (item, groupLabel) => {
  return firstText(
    item,
    ['name', 'alias_name', 'description', 'path_name', 'measurement_name', 'analog', 'mrid'],
    groupLabel
  )
}
...
const getValueChanges = (beforeEntity, afterEntity) => {
  ...
  for (const group of VALUE_GROUPS) {
    ...
    beforeItems.forEach((item, index) => {
      ...
      changes.push({
        field: formatMeasurementField(
          item,
          buildJobContext(beforeEntity),
          group.label,
          rowLookup,
          index,
          'value'
