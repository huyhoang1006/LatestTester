...
export const insertTransformerJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Transformer Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.transformerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.transformerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.transformerTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.transformerTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.transformerTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            return { success: true, data: entity, message: 'Transformer Job entity inserted successfully' }
        }
    } catch (error: any) {
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getTransformerTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.transformerTestingEquipmentTestType = entity.transformerTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'transformer job entity retrieved successfully' }
            } else {
...
export const deleteTransformerJobEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.transformerTestingEquipmentTestType && entity.transformerTestingEquipmentTestType.length > 0) {
