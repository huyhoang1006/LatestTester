...
import BushingJobEntity from '@/views/Flatten/Job/Transformer/index.js'
import {
...
import {
  insertTransformerTestingEquipmentTestTypeTransaction,
  getTransformerTestingEquipmentTestingEqId,
  deleteTransformerTestingEquipmentTestTypeByIdTransaction
} from '../../transformerTestingEquipmentTestType/index.js'
import {
...
export const insertTransformerJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Transformer Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.transformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.transformerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.transformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.transformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.transformerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertTransformerTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      return {
        success: true,
        data: entity,
        message: 'Transformer Job entity inserted successfully'
      }
    }
  } catch (error: any) {
...
export const getTransformerJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getTransformerTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.transformerTestingEquipmentTestType =
              entity.transformerTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'transformer job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteTransformerJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.transformerTestingEquipmentTestType &&
      entity.transformerTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.transformerTestingEquipmentTestType) {
        await deleteTransformerTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    return { success: true, message: 'Transformer Job entity deleted successfully' }
  } catch (error: any) {
    await runAsync('ROLLBACK')
    console.error('Delete Transformer Job Error:', error)
    return { success: false, error, message: 'Error deleting Transformer Job entity' }
  }
}
