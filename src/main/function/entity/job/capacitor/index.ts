...
import CapacitorJobEntity from '@/views/Flatten/Job/Capacitor/index.js'
import { insertWorkTaskTransaction, getWorkTaskByWork, deleteWorkTaskByIdTransaction } from '@/function/cim/workTask/index.js'
import { insertCapacitorTestingEquipmentTestTypeTransaction, getCapacitorTestingEquipmentTestingEqId, deleteCapacitorTestingEquipmentTestTypeByIdTransaction } from '../../capacitorTestingEquipmentTestType/index.js'
...
export const insertCapacitorJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Capacitor Job Entity"), message: '' }
        } else {
            ...
            for (const equipment of toAdd) { await insertTestingEquipmentTransaction(equipment, db); }
            for (const equipment of toUpdate) { await insertTestingEquipmentTransaction(equipment, db); }
            const newIdsSet = entity.capacitorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.capacitorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.capacitorTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.capacitorTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.capacitorTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
            for (const equipmentTestType of toAddSet) { await insertCapacitorTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            for (const equipmentTestType of toUpdateSet) { await insertCapacitorTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            ...
            for (const workTask of toAddWorkTask) { await insertWorkTaskTransaction(workTask, db); }
            for (const workTask of toUpdateWorkTask) { await insertWorkTaskTransaction(workTask, db); }
            if (entity.attachment.id && Array.isArray(JSON.parse(entity.attachment.path))) {
                const pathData = JSON.parse(entity.attachment.path);
                const newPath: any[] = []
                for(let i = 0; i < pathData.length; i++) {
            ...
            for (const testData of toAddTestDataSet) { await insertTestDataSetTransaction(testData, db); }
            for (const testData of toUpdateTestDataSet) { await insertTestDataSetTransaction(testData, db); }
            ...
            for (const analogValue of toAddAnalogValue) { await insertAnalogValueTransaction(analogValue, db); }
            for (const analogValue of toUpdateAnalogValue) { await insertAnalogValueTransaction(analogValue, db); }
            ...
            for (const stringMeasurementValue of toAddStringMeasurementValue) { await insertStringMeasurementValueTransaction(stringMeasurementValue, db); }
            for (const stringMeasurementValue of toUpdateStringMeasurementValue) { await insertStringMeasurementValueTransaction(stringMeasurementValue, db); }
            ...
            for (const discreteValue of toAddDiscreteValue) { await insertDiscreteValueTransaction(discreteValue, db); }
            for (const discreteValue of toUpdateDiscreteValue) { await insertDiscreteValueTransaction(discreteValue, db); }
            ...
            for(const analogValue of toDeleteAnalogValue) { await deleteAnalogValueByIdTransaction(analogValue.mrid, db); }
            for(const stringMeasurementValue of toDeleteStringMeasurementValue) { await deleteStringMeasurementValueByIdTransaction(stringMeasurementValue.mrid, db); }
            for(const discreteValue of toDeleteDiscreteValue) { await deleteDiscreteValueByIdTransaction(discreteValue.mrid, db); }
            for (const testData of toDeleteTestDataSet) { await deleteTestDataSetByIdTransaction(testData.mrid, db); }
            for(const equipmentTestType of toDeleteSet) { await deleteCapacitorTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db); }
            for (const equipment of toDelete) { await deleteTestingEquipmentByIdTransaction(equipment.mrid, db); }
            for (const workTask of toDeleteWorkTask) { await deleteWorkTaskByIdTransaction(workTask.mrid, db); }
            await runAsync('COMMIT');
            deleteBackupFiles(null, entity.oldWork.mrid);
            for(const attachment of entity.attachmentTest) {
                deleteBackupFiles(null, attachment.id_foreign);
            }
            return { success: true, data: entity, message: 'Capacitor Job entity inserted successfully' }
        }
    } catch (error: any) {
        await runAsync('ROLLBACK');
        console.error('Error retrieving capacitor entity:', error);
        restoreFiles(null, null, entity.oldWork.mrid);
        deleteBackupFiles(null, entity.oldWork.mrid);
        for(const attachment of entity.attachmentTest) {
            restoreFiles(null, null, attachment.id_foreign);
        ...
        return { success: false, error, message: 'Error retrieving capacitor entity' };
    }
}

export const getCapacitorJobEntity: any = async (id: string) => {
    try {
        if(id == null || id === '') {
            return { success: false, error: new Error('Invalid ID') };
        } else {
            const entity = new CapacitorJobEntity()
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getCapacitorTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.capacitorTestingEquipmentTestType = entity.capacitorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'Capacitor job entity retrieved successfully' }
            } else {
                return { success: false, error: dataOldWork.error, message: 'Error retrieving old work data' };
            }
        }
    } catch (error: any) {
        console.error('Error retrieving capacitor job entity:', error);
        return { success: false, error, message: 'Error retrieving capacitor job entity' };
    }
}

export const deleteCapacitorJobEntity: any = async (entity: any) => {
    try {
        await runAsync('BEGIN TRANSACTION');
        if (entity.analogValues && entity.analogValues.length > 0) {
            for (const item of entity.analogValues) { await deleteAnalogValueByIdTransaction(item.mrid, db); }
        }
        if (entity.stringMeasurementValues && entity.stringMeasurementValues.length > 0) {
            for (const item of entity.stringMeasurementValues) { await deleteStringMeasurementValueByIdTransaction(item.mrid, db); }
        }
        if (entity.discreteValues && entity.discreteValues.length > 0) {
            for (const item of entity.discreteValues) { await deleteDiscreteValueByIdTransaction(item.mrid, db); }
        }
        if (entity.testDataSet && entity.testDataSet.length > 0) {
            for (const item of entity.testDataSet) { await deleteTestDataSetByIdTransaction(item.mrid, db); }
        }
        if (entity.capacitorTestingEquipmentTestType && entity.capacitorTestingEquipmentTestType.length > 0) {
            for (const item of entity.capacitorTestingEquipmentTestType) { await deleteCapacitorTestingEquipmentTestTypeByIdTransaction(item.mrid, db); }
        }
        if (entity.testingEquipment && entity.testingEquipment.length > 0) {
            for (const item of entity.testingEquipment) { await deleteTestingEquipmentByIdTransaction(item.mrid, db); }
        }
        if (entity.workTasks && entity.workTasks.length > 0) {
            for (const item of entity.workTasks) { await deleteWorkTaskByIdTransaction(item.mrid, db); }
        }
        if (entity.attachment && entity.attachment.id) { await deleteAttachmentByIdTransaction(entity.attachment.id, db); }
        if (entity.attachmentTest && entity.attachmentTest.length > 0) {
            for (const attachment of entity.attachmentTest) {
                if (attachment.id) { await deleteAttachmentByIdTransaction(attachment.id, db) }
            }
        }
        if (entity.oldWork && entity.oldWork.mrid) { await deleteOldWorkByIdTransaction(entity.oldWork.mrid, db); }
        await runAsync('COMMIT');
        if (entity.attachment && entity.attachment.path) {
            const pathData = JSON.parse(entity.attachment.path || '[]');
            if (Array.isArray(pathData) && pathData.length > 0) { syncFilesWithDeletion(pathData, null, entity.oldWork.mrid); }
        }
        if (entity.attachmentTest && entity.attachmentTest.length > 0) {
            for (const attachment of entity.attachmentTest) {
                if (attachment.path) {
                    const pathData = JSON.parse(attachment.path || '[]');
                    if (Array.isArray(pathData) && pathData.length > 0) { syncFilesWithDeletion(pathData, null, attachment.id_foreign); }
                }
            }
        }
        return { success: true, message: 'Capacitor Job entity deleted successfully' };
    } catch (error: any) {
        await runAsync('ROLLBACK');
        console.error('Delete Capacitor Job Error:', error);
        return { success: false, error, message: 'Error deleting Capacitor Job entity' };
    }
}
