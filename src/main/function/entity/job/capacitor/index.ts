...
import {
  insertOldWorkTransaction,
  getOldWorkById,
  deleteOldWorkByIdTransaction
} from '@/function/cim/oldWork/index'
import {
  insertTestingEquipmentTransaction,
  getTestingEquipmentByWorkId,
  deleteTestingEquipmentByIdTransaction
} from '../../testingEquipment/index.js'
import CapacitorJobEntity from '@/views/Flatten/Job/Capacitor/index.js'
import {
  insertWorkTaskTransaction,
  getWorkTaskByWork,
  deleteWorkTaskByIdTransaction
} from '@/function/cim/workTask/index.js'
import {
  insertCapacitorTestingEquipmentTestTypeTransaction,
  getCapacitorTestingEquipmentTestingEqId,
  deleteCapacitorTestingEquipmentTestTypeByIdTransaction
} from '../../capacitorTestingEquipmentTestType/index.js'
import {
  insertTestDataSetTransaction,
  getTestDataSetByWorkTaskId,
  deleteTestDataSetByIdTransaction
} from '@/function/cim/testDataSet'
import {
  insertAnalogValueTransaction,
  getAnalogValueByTestDataSetMrids,
  deleteAnalogValueByIdTransaction
} from '@/function/cim/analogValue/index.js'
import {
  insertStringMeasurementValueTransaction,
  getStringMeasurementValueByTestDataSetMrids,
  deleteStringMeasurementValueByIdTransaction
} from '@/function/cim/stringMeasurementValue/index.js'
import {
  insertDiscreteValueTransaction,
  getDiscreteValueByTestDataSetMrids,
  deleteDiscreteValueByIdTransaction
} from '@/function/cim/discreteValue/index.js'
...
export const insertCapacitorJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Capacitor Job Entity'),
        message: ''
      }
    } else {
      ...
      const toDelete = old_entity.testingEquipment.filter(
      ...
      const newIdsSet = entity.capacitorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.capacitorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.capacitorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.capacitorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.capacitorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertCapacitorTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      const toAddWorkTask = entity.workTasks.filter(
        (v: any) => v.mrid && !oldIdsWorkTask.includes(v.mrid)
      )
      const toDeleteWorkTask = old_entity.workTasks.filter(
        (v: any) => v.mrid && !newIdsWorkTask.includes(v.mrid)
      )
      const toUpdateWorkTask = entity.workTasks.filter(
        (v: any) => v.mrid && oldIdsWorkTask.includes(v.mrid)
      )
      ...
      const oldIdsTestDataSet = old_entity.testDataSet
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddTestDataSet = entity.testDataSet.filter(
        (v: any) => v.mrid && !oldIdsTestDataSet.includes(v.mrid)
      )
      const toUpdateTestDataSet = entity.testDataSet.filter(
        (v: any) => v.mrid && oldIdsTestDataSet.includes(v.mrid)
      )
      const toDeleteTestDataSet = old_entity.testDataSet.filter(
        (v: any) => v.mrid && !newIdsTestDataSet.includes(v.mrid)
      )
      ...
      const oldIdsAnalogValue = old_entity.analogValues
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddAnalogValue = entity.analogValues.filter(
        (v: any) => v.mrid && !oldIdsAnalogValue.includes(v.mrid)
      )
      const toUpdateAnalogValue = entity.analogValues.filter(
        (v: any) => v.mrid && oldIdsAnalogValue.includes(v.mrid)
      )
      const toDeleteAnalogValue = old_entity.analogValues.filter(
        (v: any) => v.mrid && !newIdsAnalogValue.includes(v.mrid)
      )
      ...
      const newIdsStringMeasurementValue = entity.stringMeasurementValues
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsStringMeasurementValue = old_entity.stringMeasurementValues
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddStringMeasurementValue = entity.stringMeasurementValues.filter(
        (v: any) => v.mrid && !oldIdsStringMeasurementValue.includes(v.mrid)
      )
      const toUpdateStringMeasurementValue = entity.stringMeasurementValues.filter(
        (v: any) => v.mrid && oldIdsStringMeasurementValue.includes(v.mrid)
      )
      const toDeleteStringMeasurementValue = old_entity.stringMeasurementValues.filter(
        (v: any) => v.mrid && !newIdsStringMeasurementValue.includes(v.mrid)
      )
      ...
      const newIdsDiscreteValue = entity.discreteValues
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsDiscreteValue = old_entity.discreteValues
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddDiscreteValue = entity.discreteValues.filter(
        (v: any) => v.mrid && !oldIdsDiscreteValue.includes(v.mrid)
      )
      const toUpdateDiscreteValue = entity.discreteValues.filter(
        (v: any) => v.mrid && oldIdsDiscreteValue.includes(v.mrid)
      )
      const toDeleteDiscreteValue = old_entity.discreteValues.filter(
        (v: any) => v.mrid && !newIdsDiscreteValue.includes(v.mrid)
      )
      ...
      for (const procedureDataSetMeasurementValue of entity.procedureDataSetMeasurementValue) {
        await insertProcedureDataSetMeasurementValueTransaction(
          procedureDataSetMeasurementValue,
          db
        )
      }
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteCapacitorTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db)
      }
    ...
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getCapacitorTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.capacitorTestingEquipmentTestType =
              entity.capacitorTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        for (let i = 0; i < entity.workTasks.length; i++) {
          ...
          const dataAttachmentTest: any = await getAttachmentByForeignIdAndType(
            workTask.mrid,
            'test'
          )
        ...
        return {
          success: true,
          data: entity,
          message: 'Capacitor job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
          error: dataOldWork.error,
          message: 'Error retrieving old work data'
        }
      }
    }
  } catch (error: any) {
...
export const deleteCapacitorJobEntity: any = async (entity: any) => {
  try {
    ...
    if (
      entity.capacitorTestingEquipmentTestType &&
      entity.capacitorTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.capacitorTestingEquipmentTestType) {
        await deleteCapacitorTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
