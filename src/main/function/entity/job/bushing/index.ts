...
import BushingJobEntity from '@/views/Flatten/Job/Bushing/index.js'
import {
...
import {
  insertBushingTestingEquipmentTestTypeTransaction,
  getBushingTestingEquipmentTestingEqId,
  deleteBushingTestingEquipmentTestTypeByIdTransaction
} from '../../bushingTestingEquipmentTestType/index.js'
import {
...
export const insertBushingJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Bushing Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.bushingTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.bushingTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.bushingTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.bushingTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.bushingTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertBushingTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteBushingTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db)
      }
...
export const getBushingJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getBushingTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.bushingTestingEquipmentTestType = entity.bushingTestingEquipmentTestType.concat(
              dataEquipmentTestType.data
            )
        }
        ...
        return { success: true, data: entity, message: 'Bushing job entity retrieved successfully' }
      } else {
        return {
          success: false,
...
export const deleteBushingJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.bushingTestingEquipmentTestType &&
      entity.bushingTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.bushingTestingEquipmentTestType) {
        await deleteBushingTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    await runAsync('ROLLBACK')
    console.error('Delete Bushing Job Error:', error)
    return { success: false, error, message: 'Error deleting Bushing Job entity' }
  }
}
