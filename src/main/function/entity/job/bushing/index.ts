...
import { insertBushingTestingEquipmentTestTypeTransaction, getBushingTestingEquipmentTestingEqId, deleteBushingTestingEquipmentTestTypeByIdTransaction } from '../../bushingTestingEquipmentTestType/index.js'
...
export const insertBushingJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Bushing Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.bushingTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.bushingTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.bushingTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.bushingTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.bushingTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getBushingTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.bushingTestingEquipmentTestType = entity.bushingTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
...
export const deleteBushingJobEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.bushingTestingEquipmentTestType && entity.bushingTestingEquipmentTestType.length > 0) {
