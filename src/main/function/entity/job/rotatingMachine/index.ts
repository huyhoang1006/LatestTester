...
import BushingJobEntity from '@/views/Flatten/Job/RotatingMachine/index.js'
import {
...
import {
  insertRotatingMachineTestingEquipmentTestTypeTransaction,
  getRotatingMachineTestingEquipmentTestingEqId,
  deleteRotatingMachineTestingEquipmentTestTypeByIdTransaction
} from '../../rotatingMachineTestingEquipmentTestType/index.js'
import {
...
export const insertRotatingMachineJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for RotatingMachine Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.rotatingmachineTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.rotatingmachineTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.rotatingmachineTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.rotatingmachineTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.rotatingmachineTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertRotatingMachineTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteRotatingMachineTestingEquipmentTestTypeByIdTransaction(
          equipmentTestType.mrid,
          db
        )
      }
      ...
      return {
        success: true,
        data: entity,
        message: 'RotatingMachine Job entity inserted successfully'
      }
    }
  } catch (error: any) {
...
export const getRotatingMachineJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getRotatingMachineTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.rotatingmachineTestingEquipmentTestType =
              entity.rotatingmachineTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'rotatingmachine job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteRotatingMachineJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.rotatingmachineTestingEquipmentTestType &&
      entity.rotatingmachineTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.rotatingmachineTestingEquipmentTestType) {
        await deleteRotatingMachineTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    return { success: true, message: 'RotatingMachine Job entity deleted successfully' }
  } catch (error: any) {
    await runAsync('ROLLBACK')
    console.error('Delete RotatingMachine Job Error:', error)
    return { success: false, error, message: 'Error deleting RotatingMachine Job entity' }
  }
}
