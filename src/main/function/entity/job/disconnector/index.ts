...
import { insertDisconnectorTestingEquipmentTestTypeTransaction, getDisconnectorTestingEquipmentTestingEqId, deleteDisconnectorTestingEquipmentTestTypeByIdTransaction } from '../../disconnectorTestingEquipmentTestType/index.js'
...
export const insertDisconnectorJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Disconnector Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.disconnectorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.disconnectorTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.disconnectorTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.disconnectorTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.disconnectorTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getDisconnectorTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.disconnectorTestingEquipmentTestType = entity.disconnectorTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'disconnector job entity retrieved successfully' }
            } else {
...
export const deleteDisconnectorJobEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.disconnectorTestingEquipmentTestType && entity.disconnectorTestingEquipmentTestType.length > 0) {
