...
import BushingJobEntity from '@/views/Flatten/Job/Disconnector/index.js'
import {
...
import {
  insertDisconnectorTestingEquipmentTestTypeTransaction,
  getDisconnectorTestingEquipmentTestingEqId,
  deleteDisconnectorTestingEquipmentTestTypeByIdTransaction
} from '../../disconnectorTestingEquipmentTestType/index.js'
import {
...
export const insertDisconnectorJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Disconnector Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.disconnectorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.disconnectorTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.disconnectorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.disconnectorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.disconnectorTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      for (const equipmentTestType of toAddSet) {
        await insertDisconnectorTestingEquipmentTestTypeTransaction(equipmentTestType, db)
      }
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteDisconnectorTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db)
      }
      ...
      return {
        success: true,
        data: entity,
        message: 'Disconnector Job entity inserted successfully'
      }
    }
  } catch (error: any) {
...
export const getDisconnectorJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getDisconnectorTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.disconnectorTestingEquipmentTestType =
              entity.disconnectorTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'disconnector job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteDisconnectorJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.disconnectorTestingEquipmentTestType &&
      entity.disconnectorTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.disconnectorTestingEquipmentTestType) {
        await deleteDisconnectorTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
    ...
    await runAsync('ROLLBACK')
    console.error('Delete Disconnector Job Error:', error)
    return { success: false, error, message: 'Error deleting Disconnector Job entity' }
  }
}
