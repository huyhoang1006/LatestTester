...
import CircuitBreakerJobEntity from '@/views/Flatten/Job/CircuitBreaker/index.js'
import { insertWorkTaskTransaction, getWorkTaskByWork, deleteWorkTaskByIdTransaction } from '@/function/cim/workTask/index.js'
import { insertCircuitBreakerTestingEquipmentTestTypeTransaction, getCircuitBreakerTestingEquipmentTestingEqId, deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction } from '../../circuitBreakerTestingEquipmentTestType/index.js'
...
export const insertCircuitBreakerJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for Circuit Breaker Job Entity"), message: '' }
        } else {
            ...
            for(const procedureAsset of entity.procedureAsset) { await insertProcedureAssetTransaction(procedureAsset, db); }
            ...
            const newIdsSet = entity.circuitBreakerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.circuitBreakerTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.circuitBreakerTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.circuitBreakerTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.circuitBreakerTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
            for (const equipmentTestType of toAddSet) { await insertCircuitBreakerTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            for (const equipmentTestType of toUpdateSet) { await insertCircuitBreakerTestingEquipmentTestTypeTransaction(equipmentTestType, db); }
            ...
            for(const equipmentTestType of toDeleteSet) { await deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction(equipmentTestType.mrid, db); }
            ...
            deleteBackupFiles(null, entity.oldWork.mrid);
            for(const attachment of entity.attachmentTest) { deleteBackupFiles(null, attachment.id_foreign); }
        ...
        return { success: false, error, message: 'Error retrieving Circuit Breaker entity' };
    }
}

export const getCircuitBreakerJobEntity: any = async (id: string) => {
    try {
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getCircuitBreakerTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.circuitBreakerTestingEquipmentTestType = entity.circuitBreakerTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'Circuit Breaker job entity retrieved successfully' }
            } else {
                return { success: false, error: dataOldWork.error, message: 'Error retrieving old work data' };
            }
        }
    } catch (error: any) {
        console.error('Error retrieving Circuit Breaker job entity:', error);
        return { success: false, error, message: 'Error retrieving Circuit Breaker job entity' };
    }
}

export const deleteCircuitBreakerJobEntity: any = async (entity: any) => {
    try {
        await runAsync('BEGIN TRANSACTION');
        ...
        if (entity.circuitBreakerTestingEquipmentTestType && entity.circuitBreakerTestingEquipmentTestType.length > 0) {
            for (const item of entity.circuitBreakerTestingEquipmentTestType) { await deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction(item.mrid, db); }
        }
