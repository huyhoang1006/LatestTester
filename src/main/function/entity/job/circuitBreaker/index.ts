...
import {
  insertCircuitBreakerTestingEquipmentTestTypeTransaction,
  getCircuitBreakerTestingEquipmentTestingEqId,
  deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction
} from '../../circuitBreakerTestingEquipmentTestType/index.js'
import {
...
export const insertCircuitBreakerJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for Circuit Breaker Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.circuitBreakerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.circuitBreakerTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.circuitBreakerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.circuitBreakerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.circuitBreakerTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      ...
      for (const equipmentTestType of toDeleteSet) {
        await deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction(
      ...
      return {
        success: true,
        data: entity,
        message: 'Circuit Breaker Job entity inserted successfully'
      }
    }
  } catch (error: any) {
    ...
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getCircuitBreakerTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.circuitBreakerTestingEquipmentTestType =
              entity.circuitBreakerTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'Circuit Breaker job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteCircuitBreakerJobEntity: any = async (entity: any) => {
  try {
    ...
    if (
      entity.circuitBreakerTestingEquipmentTestType &&
      entity.circuitBreakerTestingEquipmentTestType.length > 0
    ) {
      for (const item of entity.circuitBreakerTestingEquipmentTestType) {
        await deleteCircuitBreakerTestingEquipmentTestTypeByIdTransaction(item.mrid, db)
      }
    }
