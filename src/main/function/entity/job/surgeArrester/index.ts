...
import BushingJobEntity from '@/views/Flatten/Job/SurgeArrester/index.js'
import {
...
import {
  insertSurgeArresterTestingEquipmentTestTypeTransaction,
  getSurgeArresterTestingEquipmentTestingEqId,
  deleteSurgeArresterTestingEquipmentTestTypeByIdTransaction
} from '../../surgeArresterTestingEquipmentTestType/index.js'
import {
...
export const insertSurgeArresterJobEntity: any = async (old_entity: any, entity: any) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      return {
        success: false,
        error: new Error('MRID is required for SurgeArrester Job Entity'),
        message: ''
      }
    } else {
      ...
      const newIdsSet = entity.surgearresterTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const oldIdsSet = old_entity.surgearresterTestingEquipmentTestType
        .map((v: any) => v.mrid)
        .filter((id: any) => id)
      const toAddSet = entity.surgearresterTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !oldIdsSet.includes(v.mrid)
      )
      const toDeleteSet = old_entity.surgearresterTestingEquipmentTestType.filter(
        (v: any) => v.mrid && !newIdsSet.includes(v.mrid)
      )
      const toUpdateSet = entity.surgearresterTestingEquipmentTestType.filter(
        (v: any) => v.mrid && oldIdsSet.includes(v.mrid)
      )
      ...
      return {
        success: true,
        data: entity,
        message: 'SurgeArrester Job entity inserted successfully'
      }
    }
  } catch (error: any) {
...
export const getSurgeArresterJobEntity: any = async (id: string) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType: any = await getSurgeArresterTestingEquipmentTestingEqId(
            equipment.mrid
          )
          if (dataEquipmentTestType.success)
            entity.surgearresterTestingEquipmentTestType =
              entity.surgearresterTestingEquipmentTestType.concat(dataEquipmentTestType.data)
        }
        ...
        return {
          success: true,
          data: entity,
          message: 'surgearrester job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteSurgeArresterJobEntity: any = async (entity: any) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    if (
      entity.surgearresterTestingEquipmentTestType &&
      entity.surgearresterTestingEquipmentTestType.length > 0
    ) {
    ...
    return { success: true, message: 'SurgeArrester Job entity deleted successfully' }
  } catch (error: any) {
    await runAsync('ROLLBACK')
    console.error('Delete SurgeArrester Job Error:', error)
    return { success: false, error, message: 'Error deleting SurgeArrester Job entity' }
  }
}
