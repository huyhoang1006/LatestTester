...
export const insertSurgeArresterJobEntity: any = async (old_entity: any, entity: any) => {
    try {
        if(entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
            return { success: false, error: new Error("MRID is required for SurgeArrester Job Entity"), message: '' }
        } else {
            ...
            const newIdsSet = entity.surgearresterTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const oldIdsSet = old_entity.surgearresterTestingEquipmentTestType.map((v: any) => v.mrid).filter((id: any) => id);
            const toAddSet = entity.surgearresterTestingEquipmentTestType.filter((v: any) => v.mrid && !oldIdsSet.includes(v.mrid));
            const toDeleteSet = old_entity.surgearresterTestingEquipmentTestType.filter((v: any) => v.mrid && !newIdsSet.includes(v.mrid));
            const toUpdateSet = entity.surgearresterTestingEquipmentTestType.filter((v: any) => v.mrid && oldIdsSet.includes(v.mrid));
            ...
            return { success: true, data: entity, message: 'SurgeArrester Job entity inserted successfully' }
        }
    } catch (error: any) {
        ...
        if(id == null || id === '') {
            ...
            if(dataOldWork.success) {
                ...
                for(const equipment of entity.testingEquipment) {
                    const dataEquipmentTestType: any = await getSurgeArresterTestingEquipmentTestingEqId(equipment.mrid);
                    if(dataEquipmentTestType.success) entity.surgearresterTestingEquipmentTestType = entity.surgearresterTestingEquipmentTestType.concat(dataEquipmentTestType.data);
                }
                ...
                return { success: true, data: entity, message: 'surgearrester job entity retrieved successfully' }
            } else {
