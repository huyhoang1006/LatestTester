...
export const getAllNotifications = async () => {
  return new Promise((resolve, reject) => {
    db.all(
      'SELECT * FROM notification ORDER BY mrid DESC',
      [],
...
export const getNotificationById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM notification WHERE mrid = ?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err) reject(err)
        else if (row)
          resolve({ success: true, data: row, message: 'Notification retrieved successfully' })
  ...
  if (versionMatch) {
    ...
    if (existing) {
      return {
        success: false,
        data: existing,
        message: 'Update notification already exists',
        duplicate: true
      }
    }
  }
    ...
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) reject(err)
          ...
          resolve({
            success: true,
            data: { ...entity, created_at: createdAt },
...
export const updateNotification = async (
  mrid: string,
  entity: { name: string | null; message: string | null; type: string | null; status?: string }
) => {
  return new Promise((resolve, reject) => {
    db.run(
...
export const markAsRead = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'UPDATE notification SET status = ? WHERE mrid = ?',
      ['read', mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) reject(err)
...
export const deleteNotification = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'DELETE FROM notification WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) reject(err)
