...
export const insertPowerCableEntity: any = async (old_entity: any, entity: any) => {
    try {
        if (entity.asset.mrid === null || entity.asset.mrid === '') {
            ...
            const syncUnits = (newList: any[], oldList: any[], insertFn: any, deleteFn: any, name: string) => {
            ...
            syncUnits(entity.area, old_entity.area, insertAreaTransaction, deleteAreaByIdTransaction, 'area');
            syncUnits(entity.currentFlow, old_entity.currentFlow, insertCurrentFlowTransaction, deleteCurrentFlowByIdTransaction, 'currentFlow');
            syncUnits(entity.second, old_entity.second, insertSecondsTransaction, deleteSecondsByIdTransaction, 'second');
            syncUnits(entity.frequency, old_entity.frequency, insertFrequencyTransaction, deleteFrequencyByIdTransaction, 'frequency');
            syncUnits(entity.length, old_entity.length, insertLengthTransaction, deleteLengthByIdTransaction, 'length');
            syncUnits(entity.voltage, old_entity.voltage, insertVoltageTransaction, deleteVoltageByIdTransaction, 'voltage');
            syncUnits(entity.temperature, old_entity.temperature, insertTemperatureTransaction, deleteTemperatureByIdTransaction, 'temperature');
    ...
    try {
        if (id == null || id === '') {
            ...
            if (dataPowerCable.success) {
                ...
                const dataCencentricNeutralInfo: any = await getConcentricNeutralCableInfoById(entity.asset.asset_info);
                ...
                const dataOldCableInfo: any = await getOldCableInfoByCableInfoId(entity.concentricNeutral.mrid);
                if (dataOldCableInfo.success) {
                    entity.oldCableInfo = dataOldCableInfo.data;
                }

                const dataJointInfo: any = await getJointCableInfoByCableInfoId(entity.concentricNeutral.mrid);
                if (dataJointInfo.success) {
                    entity.joint = dataJointInfo.data;
                }

                const terminalCableInfo: any = await getTerminalCableInfoByCableInfoId(entity.concentricNeutral.mrid);
                if (terminalCableInfo.success) {
                    entity.terminal = terminalCableInfo.data;
                }

                const sheathVoltageLimiter: any = await getSheathVoltageLimiterByCableInfoId(entity.concentricNeutral.mrid);
                ...
                const voltage: any[] = [], currentFlow: any[] = [], second: any[] = [], temperature: any[] = [], area: any[] = [], frequency: any[] = [], length: any[] = [];
...
export const deletePowerCableEntity: any = async (entity: any) => {
    try {
        ...
        if (entity.concentricNeutral && entity.concentricNeutral.mrid) {
            ...
            await new Promise((res, rej) => db.run('DELETE FROM old_cable_info WHERE cable_info_id=?', [cableInfoId], (err: any) => err ? rej(err) : res(undefined)))
            await new Promise((res, rej) => db.run('DELETE FROM joint_cable_info WHERE cable_info_id=?', [cableInfoId], (err: any) => err ? rej(err) : res(undefined)))
            await new Promise((res, rej) => db.run('DELETE FROM terminal_cable_info WHERE cable_info_id=?', [cableInfoId], (err: any) => err ? rej(err) : res(undefined)))
            await new Promise((res, rej) => db.run('DELETE FROM sheath_voltage_limiter WHERE cable_info_id=?', [cableInfoId], (err: any) => err ? rej(err) : res(undefined)))
        }
