...
import {
  insertSubstationTransaction,
  getSubstationById,
  deleteSubstationByIdTransaction
} from '@/function/cim/substation'
import {
  insertStreetDetailTransaction,
  getStreetDetailById,
  deleteStreetDetailByIdTransaction
} from '@/function/cim/streetDetail'
import {
  insertTownDetailTransaction,
  getTownDetailById,
  deleteTownDetailByIdTransaction
} from '@/function/cim/townDetail'
import {
  insertStreetAddressTransaction,
  getStreetAddressById,
  deleteStreetAddressByIdTransaction
} from '@/function/cim/streetAddress'
import {
  insertLocationTransaction,
  getLocationById,
  deleteLocationByIdTransaction
} from '@/function/cim/location'
import {
  insertElectronicAddressTransaction,
  getElectronicAddressById,
  deleteElectronicAddressByIdTransaction
} from '@/function/cim/electronicAddress'
import {
  insertTelephoneNumberTransaction,
  getTelephoneNumberById,
  deleteTelephoneNumberByIdTransaction
} from '@/function/cim/telephoneNumber'
import {
  insertPersonTransaction,
  getPersonById,
  deletePersonByIdTransaction
} from '@/function/cim/person'
import {
  insertPersonRoleTransaction,
  getPersonRoleByPersonId,
  deletePersonRoleByIdTransaction
} from '@/function/cim/personRole'
import { insertUserTransaction, getUserById } from '@/function/entity/user'
import {
  insertUserIdentifiedObjectTransaction,
  getUserIdentifiedObjectByUserIdAndIdentifiedObjectId
} from '@/function/entity/userIdentifiedObject'
import {
  insertPersonSubstationTransaction,
  getPersonSubstationBySubstationId
} from '@/function/entity/personSubstation'
import {
...
import {
  insertOrganisationLocationTransaction,
  getOrganisationLocationByOrganisationIdAndLocationId
} from '@/function/entity/organisationLocation'
import {
  insertPositionPointArrayTransaction,
  getPositionPointByLocationId
} from '@/function/cim/positionPoint'
import {
  insertPsrTypeTransaction,
  getPsrTypeById,
  deletePsrTypeByIdTransaction
} from '@/function/cim/psrType'
import {
  insertOrganisationPersonTransaction,
  getOrganisationPersonByOrganisationIdAndPersonId
} from '../organisationPerson'
import {
  insertOrganisationPsrTransaction,
  getOrganisationPsrByOrganisationIdAndPsrId
} from '../organisationPsr'
import {
  insertConfigurationEventArrayTransaction,
  insertConfigurationEventTransaction
} from '@/function/cim/configurationEvent/index'
...
export const insertSubstationEntity: any = async (entity: any) => {
          ...
          db.serialize(async () => {
            ...
            try {
              ...
              if (entity.streetDetail.mrid)
                await insertStreetDetailTransaction(entity.streetDetail, db)
              if (entity.townDetail.mrid) await insertTownDetailTransaction(entity.townDetail, db)
              if (entity.streetAddress.mrid)
                await insertStreetAddressTransaction(entity.streetAddress, db)
              ...
              if (entity.electronicAddress.mrid)
                await insertElectronicAddressTransaction(entity.electronicAddress, db)
              if (entity.telephoneNumber.mrid)
                await insertTelephoneNumberTransaction(entity.telephoneNumber, db)
              ...
              if (entity.userIdentifiedObject.mrid)
              ...
              if (Array.isArray(entity.positionPoint) && entity.positionPoint.length > 0)
                await insertPositionPointArrayTransaction(
                  entity.positionPoint,
                  entity.location.mrid,
                  db
                )
...
export const getSubstationEntityById: any = async (
  id: string,
  user_id: string,
  ...
  if (id == null || id === '') {
    ...
    try {
      const dataSubstation: any = await getSubstationById(id)
      if (dataSubstation.success) {
        entity.substation = dataSubstation.data
        const dataPrsType: any = await getPsrTypeById(entity.substation.psr_type_id)
        if (dataPrsType.success) {
          entity.psrType = dataPrsType.data
        }

        const dataLocation: any = await getLocationById(entity.substation.location)
        if (dataLocation.success) {
          entity.location = dataLocation.data
        }

        const dataStreetAddress: any = await getStreetAddressById(entity.location.main_address)
        if (dataStreetAddress.success) {
          entity.streetAddress = dataStreetAddress.data
        }

        const dataStreetDetail: any = await getStreetDetailById(entity.streetAddress.street_detail)
        if (dataStreetDetail.success) {
          entity.streetDetail = dataStreetDetail.data
        }

        const dataTownDetail: any = await getTownDetailById(entity.streetAddress.town_detail)
        if (dataTownDetail.success) {
          entity.townDetail = dataTownDetail.data
        }

        const dataPersonSubstation: any = await getPersonSubstationBySubstationId(
          entity.substation.mrid
        )
        ...
        const dataPerson: any = await getPersonById(entity.personSubstation.person_id)
        if (dataPerson.success) {
          entity.person = dataPerson.data
        }

        const dataPersonRole: any = await getPersonRoleByPersonId(entity.person.mrid)
        if (dataPersonRole.success) {
          entity.personRole = dataPersonRole.data
        }

        const dataElectronicAddress: any = await getElectronicAddressById(
          entity.person.electronic_address
        )
        if (dataElectronicAddress.success) {
          entity.electronicAddress = dataElectronicAddress.data
        }

        const dataTelephoneNumber: any = await getTelephoneNumberById(entity.person.mobile_phone)
        if (dataTelephoneNumber.success) {
          entity.telephoneNumber = dataTelephoneNumber.data
        }

        const dataPositionPoint: any = await getPositionPointByLocationId(entity.location.mrid)
        ...
        const userIdentifiedObject: any =
          await getUserIdentifiedObjectByUserIdAndIdentifiedObjectId(
            user_id,
            entity.substation.mrid
          )
        ...
        const dataUser: any = await getUserById(user_id)
        if (dataUser.success) {
          entity.user = dataUser.data
        }

        const organisationLocation: any =
          await getOrganisationLocationByOrganisationIdAndLocationId(
            organisation_id,
            entity.location.mrid
          )
        ...
        const organisationPerson: any = await getOrganisationPersonByOrganisationIdAndPersonId(
          organisation_id,
          entity.person.mrid
        )
        ...
        const organisationPsr: any = await getOrganisationPsrByOrganisationIdAndPsrId(
          organisation_id,
          entity.substation.mrid
        )
...
export const deleteSubstationEntityById: any = async (data: any) => {
  try {
    if (data.substation == null || data.substation.mrid == null || data.substation.mrid === '') {
      ...
      try {
        ...
        if (data.location && data.location.mrid) {
          const powerSystemResource: any = await getPowerSystemResourceByLocationIdTransaction(
            data.location.mrid,
            db
          )
...
const runSQL: any = (sql: string, params: any[] = []) => {
