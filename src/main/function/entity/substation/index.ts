...
export const getSubstationEntityById: any = async (id: string, user_id: string, organisation_id: string) => {
    ...
    if (id == null || id === '') {
        ...
        try {
            ...
            if (dataSubstation.success) {
                ...
                const dataPersonSubstation: any = await getPersonSubstationBySubstationId(entity.substation.mrid);
                ...
                const dataElectronicAddress: any = await getElectronicAddressById(entity.person.electronic_address);
                ...
                const dataAttachment: any = await getAttachmentByForeignIdAndType(entity.substation.mrid, 'substation');
                if (dataAttachment.success) {
                    entity.attachment = dataAttachment.data;
                }

                const userIdentifiedObject: any = await getUserIdentifiedObjectByUserIdAndIdentifiedObjectId(user_id, entity.substation.mrid);
                ...
                const organisationLocation: any = await getOrganisationLocationByOrganisationIdAndLocationId(organisation_id, entity.location.mrid);
                if (organisationLocation.success) {
                    entity.organisationLocation = organisationLocation.data;
                }

                const organisationPerson: any = await getOrganisationPersonByOrganisationIdAndPersonId(organisation_id, entity.person.mrid);
                if (organisationPerson.success) {
                    entity.organisationPerson = organisationPerson.data;
                }

                const organisationPsr: any = await getOrganisationPsrByOrganisationIdAndPsrId(organisation_id, entity.substation.mrid);
        ...
        if (data.substation == null || data.substation.mrid == null || data.substation.mrid === '') {
            ...
            try {
                ...
                if (data.location && data.location.mrid) {
                    const powerSystemResource: any = await getPowerSystemResourceByLocationIdTransaction(data.location.mrid, db);
