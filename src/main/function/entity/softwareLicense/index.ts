...
export const getSoftwareLicenseById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM software_license WHERE mrid=?`, [mrid], (err: any, row: any) => {
...
export const getAllSoftwareLicense = async () => {
  return new Promise((resolve, reject) => {
    db.all(`SELECT * FROM software_license`, [], (err: any, rows: any) => {
      if (err) return reject({ success: false, err, message: 'Get all softwareLicense failed' })
      if (!rows || rows.length === 0)
        return resolve({ success: false, data: null, message: 'SoftwareLicense not found' })
...
export const insertSoftwareLicenseTransaction = async (softwareLicense: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err: any) {
        ...
        return resolve({
          ...
          message: 'Insert softwareLicense completed'
        })
      }
    )
  })
...
export const updateSoftwareLicenseByIdTransaction = async (
  mrid: string,
  softwareLicense: any,
  dbsql: any
) => {
    ...
    dbsql.run(
      ...
      function (err: any) {
        ...
        return resolve({
          success: true,
          data: softwareLicense,
          message: 'Update softwareLicense completed'
        })
      }
    )
  })
...
// Xóa softwareLicense
export const deleteSoftwareLicenseByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
