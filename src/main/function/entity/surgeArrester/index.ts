...
export const insertSurgeArresterEntity: any = async (old_entity: any, entity: any) => {
    try {
        if (entity.surgeArrester.mrid === null || entity.surgeArrester.mrid === '') {
            ...
            syncUnit(entity.assetInfoUnit, old_entity.assetInfoUnit, insertOldSurgeArresterInfoTransaction, db);
...
export const insertSurgeArresterLiteEntity: any = async (entity: any, old_entity: any, dbsql: any) => {
    try {
        if (entity.surgeArrester.mrid === null || entity.surgeArrester.mrid === '') {
            return { success: false, error: new Error("MRID is required for Surge Arrester Entity"), message: '' }
        } else {
            ...
            syncUnit(entity.assetInfoUnit, old_entity.assetInfoUnit, insertOldSurgeArresterInfoTransaction, dbsql);
    ...
    try {
        if (id == null || id === '') {
            ...
            if (dataSurgeArrester.success) {
                ...
                const dataLifecycleDate: any = await getLifecycleDateById(entity.surgeArrester.lifecycle_date);
                if (dataLifecycleDate.success) {
                    entity.lifecycleDate = dataLifecycleDate.data;
                }
                const dataOldSurgeArresterInfo: any = await getOldSurgeArresterInfoBySurgeArresterId(entity.surgeArrester.mrid);
                ...
                const dataAssetPsr: any = await getAssetPsrByAssetIdAndPsrId(entity.surgeArrester.mrid, psrId);
                ...
                for (const assetUnit of entity.assetUnit) {
                    const dataAssetInfoUnit: any = await getOldSurgeArresterInfoBySurgeArresterId(assetUnit.mrid)
                ...
                if (entity.assetInfoUnit.length > 0) {
                    ...
                    for (let i = 0; i < entity.assetInfoUnit.length; i++) {
                        ...
                        for (let j = 0; j < currentFlowArr.length; j++) {
                            const currentFlow: any = await getCurrentFlowById(entity.assetInfoUnit[i][currentFlowArr[j]]);
                ...
                const dataAttachment: any = await getAttachmentByForeignIdAndType(entity.surgeArrester.mrid, 'asset');
