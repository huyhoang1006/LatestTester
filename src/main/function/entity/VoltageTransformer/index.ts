  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataVt.success) {
        ...
        const dataVoltage: any = await getVoltageById(
          entity.OldPotentialTransformerInfo.rated_voltage
        )
        if (dataVoltage.success) {
          entity.voltage.push(dataVoltage.data)
        }

        const dataFrequency: any = await getFrequencyById(
        ...
        const dataPotentialTransformerTable: any =
