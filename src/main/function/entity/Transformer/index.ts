...
import { insertZeroSequenceImpedanceTableTransaction, getZeroSequenceImpedanceTableByZeroSequenceImpedanceId, deleteZeroSequenceImpedanceTableTransaction } from '@/function/cim/zeroSequenceImpedanceTable'
...
import { insertOldTapChangerInfoTransaction, getOldTapChangerInfoByPowerTransformerInfoId, deleteOldTapChangerInfoTransaction } from '@/function/cim/oldTapChangerInfo'
    ...
    try {
        if (entity.asset.mrid === null || entity.asset.mrid === '') {
            ...
            for (const unitType of unitTypes) {
                ...
                toDeleteUnit[unitType] = old_entity[unitType].filter((v: any) => v.mrid && !newIds.includes(v.mrid));
            ...
            for (const tableType of tableTypes) {
                ...
                toDeleteTable[tableType] = old_entity[tableType].filter((v: any) => v.mrid && !newIds.includes(v.mrid));
            ...
            if (entity.tapChanger) {
                ...
                const oldIdsPoint = oldTc.tapChangerTablePoint.map((v: any) => v.mrid).filter((id: any) => id);

                const toAddPoint = tc.tapChangerTablePoint.filter((v: any) => v.mrid && !oldIdsPoint.includes(v.mrid));
                const toDeletePoint = oldTc.tapChangerTablePoint.filter((v: any) => v.mrid && !newIdsPoint.includes(v.mrid));
                const toUpdatePoint = tc.tapChangerTablePoint.filter((v: any) => v.mrid && oldIdsPoint.includes(v.mrid));
            ...
            const oldIdsBushing = old_entity.bushing.map((v: any) => v.bushing.mrid).filter((id: any) => id);

            const toAddBushing = entity.bushing.filter((v: any) => v.bushing.mrid && !oldIdsBushing.includes(v.bushing.mrid));
            const toDeleteBushing = old_entity.bushing.filter((v: any) => v.bushing.mrid && !newIdsBushing.includes(v.bushing.mrid));

            const toUpdateBushing = entity.bushing.filter((v: any) => v.bushing.mrid && oldIdsBushing.includes(v.bushing.mrid));
            ...
            const newIdsSurge = entity.surgeArrester.map((v: any) => v.surgeArrester.mrid).filter((id: any) => id);
            const oldIdsSurge = old_entity.surgeArrester.map((v: any) => v.surgeArrester.mrid).filter((id: any) => id);

            const toAddSurge = entity.surgeArrester.filter((v: any) => v.surgeArrester.mrid && !oldIdsSurge.includes(v.surgeArrester.mrid));
            const toDeleteSurge = old_entity.surgeArrester.filter((v: any) => v.surgeArrester.mrid && !newIdsSurge.includes(v.surgeArrester.mrid));

            const toUpdateSurge = entity.surgeArrester.filter((v: any) => v.surgeArrester.mrid && oldIdsSurge.includes(v.surgeArrester.mrid));
            ...
            for (const surge of toUpdateSurge) {
                const oldSurge = old_entity.surgeArrester.find((x: any) => x.surgeArrester.mrid === surge.surgeArrester.mrid)
    ...
    try {
        if (id == null || id === '') {
            ...
            if (dataTransformer.success) {
                ...
                const dataOldTransformerInfo: any = await getOldPowerTransformerInfoById(entity.asset.asset_info);
                ...
                const dataOldTransformerEndInfo: any = await getOldTransformerEndInfoByPowerTransformerInfoId(entity.asset.asset_info);
                if (dataOldTransformerEndInfo.success) {
                    entity.oldTransformerEndInfo = dataOldTransformerEndInfo.data;
                }

                const dataOther: any = await getOtherByPowerTransformerInfoId(entity.oldPowerTransformerInfo.mrid);
                ...
                const dataCoolingPowerRating: any = await getCoolingPowerRatingByPowerTransformerInfoId(entity.oldPowerTransformerInfo.mrid);
                ...
                for (const powerRating of entity.coolingPowerRating) {
                    const dataCurrentRating: any = await getCurrentRatingByRatedPower(powerRating.power_rating);
                ...
                const dataShortCircuitRating: any = await getShortCircuitRatingByPowerTransformerInfoId(entity.oldPowerTransformerInfo.mrid);
                ...
                for (const shortCircuitTest of entity.oldTransformerEndInfo) {
                    const dataShortCircuitTest: any = await getShortCircuitTestByTransformerEndInfoId(shortCircuitTest.mrid);
                ...
                for (const shortCircuitTest of entity.shortCircuitTest) {
                    const dataSCTTransformerEndInfo: any = await getSCTTransformerEndInfoByShortCircuitTestId(shortCircuitTest.mrid);
                ...
                const dataZeroSequenceImpedance: any = await getZeroSequenceImpedanceByTransformerInfoId(entity.oldPowerTransformerInfo.mrid);
                ...
                const dataZeroSequenceImpedanceTable: any = await getZeroSequenceImpedanceTableByZeroSequenceImpedanceId(entity.zeroSequenceImpedance.mrid);
                ...
                const dataOldTapChangerInfo: any = await getOldTapChangerInfoByPowerTransformerInfoId(entity.oldPowerTransformerInfo.mrid)
                if (dataOldTapChangerInfo.success) {
                    entity.tapChanger.oldTapChangerInfo = dataOldTapChangerInfo.data
                }

                const dataTapChanger: any = await getAssetByAssetInfoId(entity.tapChanger.oldTapChangerInfo.mrid)
                if (dataTapChanger.success) {
                    entity.tapChanger.asset = dataTapChanger.data
                }

                const dataProductAssetModelTapChanger: any = await getProductAssetModelById(entity.tapChanger.oldTapChangerInfo.product_asset_model)
                if (dataProductAssetModelTapChanger.success) {
                    entity.tapChanger.productAssetModel = dataProductAssetModelTapChanger.data
                }

                const dataTapChangerTablePoint: any = await getTapChangerTablePointByTapChangerInfoId(entity.tapChanger.oldTapChangerInfo.mrid)
                if (dataTapChangerTablePoint.success) {
                    entity.tapChanger.tapChangerTablePoint = dataTapChangerTablePoint.data
                }

                const dataTapChangerVoltage: any = await getVoltageByIds(entity.tapChanger.tapChangerTablePoint.map((x: any) => x.voltage))
                ...
                const dataMridOldBushingInfo: any = await getOldBushingInfoByTransformerEndInfoIds(idsTransformerEndInfo)
                if (dataMridOldBushingInfo.success) {
                    ...
                    for (const dataMridOldBushingInfoId of dataMridOldBushingInfoIds) {
                        ...
                        if (dataMridBushing.success) {
                            const dataBushing: any = await bushingFunc.getBushingEntityLiteById(dataMridBushing.data.mrid)
            ...
            try {
                ...
                if (data.tapChanger) {
                    ...
                    if (data.tapChanger.productAssetModel && data.tapChanger.productAssetModel.mrid) {
                        const isUsedAsset = await isUsedInTable('asset', 'product_asset_model', data.tapChanger.productAssetModel.mrid, db);
                        const isUsedInfo = await isUsedInTable('asset_info', 'product_asset_model', data.tapChanger.productAssetModel.mrid, db);

                        if (!isUsedInfo && !isUsedAsset) {
                            await deleteProductAssetModelByIdTransaction(data.tapChanger.productAssetModel.mrid, db);
                        }
                    }
                }
