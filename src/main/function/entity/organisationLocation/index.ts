  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Insert organisationLocation failed' })
        return resolve({
          success: true,
          data: organisationLocation,
          message: 'Insert organisationLocation completed'
        })
      }
    )
  })
...
export const getOrganisationLocationById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM organisation_location WHERE mrid = ?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err) return reject({ success: false, err, message: 'Get organisationLocation failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'OrganisationLocation not found' })
...
export const getOrganisationLocationByOrganisationIdAndLocationId = async (
  organisation_id: string,
  location_id: string
) => {
  return new Promise((resolve, reject) => {
...
export const updateOrganisationLocationById = async (
  mrid: string,
  organisationLocation: { organisation_id: string; location_id: string }
) => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Update organisationLocation failed' })
        if (this.changes === 0)
          return resolve({ success: false, message: 'OrganisationLocation not found' })
        return resolve({
          success: true,
          data: organisationLocation,
          message: 'Update organisationLocation completed'
        })
      }
    )
  })
