...
export const insertBayEntity = async (entity: any) => {
  try {
    if (entity.mrid) {
...
export const getBayEntity = async (id: string) => {
  try {
    const dataBay: any = await getBayById(id)
    if (dataBay.success) {
...
export const deleteBayEntityById = async (data: { mrid: string }) => {
  try {
    await runAsync('BEGIN TRANSACTION')
