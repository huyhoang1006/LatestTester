  ...
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: personSubstation,
          message: 'Insert personSubstation completed'
        })
      }
    )
  })
...
export const getPersonSubstationByPersonId = async (personId: string) => {
  return new Promise((resolve, reject) => {
    db.all(
      'SELECT * FROM person_substation WHERE person_id = ?',
      [personId],
      (err: Error | null, rows: unknown[]) => {
        if (err)
          return reject({ success: false, err, message: 'Get personSubstation by personId failed' })
        if (rows.length === 0)
          return resolve({ success: false, data: null, message: 'PersonSubstation not found' })
        return resolve({
          success: true,
          data: rows,
          message: 'Get personSubstation by personId completed'
        })
      }
    )
  })
...
export const getPersonSubstationBySubstationId = async (substationId: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM person_substation WHERE substation_id = ?',
      [substationId],
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get personSubstation by substationId failed'
          })
        if (!row)
          return resolve({ success: false, data: null, message: 'PersonSubstation not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get personSubstation by substationId completed'
        })
      }
    )
  })
...
export const getPersonSubstationById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM person_substation WHERE mrid = ?',
      [mrid],
      (err: Error | null, row: unknown) => {
...
export const updatePersonSubstationById = async (
  mrid: string,
  personSubstation: { person_id: string; substation_id: string }
) => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, message: 'PersonSubstation not found' })
        return resolve({
          success: true,
          data: personSubstation,
          message: 'Update personSubstation completed'
        })
      }
    )
  })
...
export const deletePersonSubstationById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'DELETE FROM person_substation WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
