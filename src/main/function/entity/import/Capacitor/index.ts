...
export const importCapacitor: any = async (
  dto: any,
  parentNode: any,
  ...
  try {
    ...
    if (dto.clone === true) {
      ...
      if (originalLocationId && originalLocationId !== parentMrid) {
        ...
        try {
          const existing: any = await electronAPI.getCapacitorEntityByMrid(
            entity.asset.mrid,
            parentMrid
          )
...
export const deleteCapacitor: any = async (mrid: string, psrId: string, { electronAPI }: any) => {
  try {
    ...
    if (!entityRes.success || !entityRes.data) {
      console.warn(
        `Capacitor with mrid ${mrid} not found for deletion, assuming it's already deleted.`
      )
