...
export const importVoltageTransformer: any = async (
  dto: any,
  parentNode: any,
  ...
  try {
    if (
      ...
      return {
        success: false,
        message:
          'Voltage transformer can only be imported under a Substation, Bay or Voltage Transformer'
      }
    }
    ...
    dto.ratings.standard =
      typeof dto.ratings.standard === 'object'
        ? JSON.stringify(dto.ratings.standard)
        : dto.ratings.standard || ''
...
const regenerateAllMrids = (entity: any) => {
  ...
  const updateInfoRefs = (info: any) => {
    ...
    info.rated_power_frequency_withstand_voltage =
      mridMap[info.rated_power_frequency_withstand_voltage] || null
    info.rated_lightning_impulse_withstand_voltage =
      mridMap[info.rated_lightning_impulse_withstand_voltage] || null
