...
export const importReactor: any = async (
  dto: any,
  parentNode: any,
  ...
  try {
    if (
      ...
      return {
        success: false,
        message: 'Reactor can only be imported under a Substation, Bay or Reactor'
      }
    }
    ...
    if (dto.clone === true) {
      ...
      if (originalLocationId && originalLocationId !== parentMrid) {
        ...
        try {
          const existing: any = await electronAPI.getReactorEntityByMrid(
            entity.asset.mrid,
            parentMrid
          )
    ...
    const result: any = await electronAPI.insertReactorEntity(emptyEntity, entity)
