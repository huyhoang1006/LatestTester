...
export const importBushing = async (
  dto: any,
  parentNode: any,
  ...
  try {
    if (!parentNode?.mode || !['substation', 'bay', 'bushing'].includes(parentNode.mode)) {
      return {
        success: false,
        message: 'Bushing can only be imported under Substation, Bay, or Bushing'
      }
    }
    ...
    if (!entity.attachment) {
      entity.attachment = {
        ...
        name: null,
        type: 'asset',
        id_foreign: entity.bushing.mrid
      }
    } else {
...
export const deleteBushing = async (
  mrid: string,
  psrId: string,
