...
export const importTransformer: any = async (
  dto: any,
  parentNode: any,
  ...
  try {
    ...
    const regenerateIdsAndRelink = (obj: any) => {
      const idMap = new Map()

      const collectAndGenerate = (current: any) => {
      ...
      const updateReferences = (current: any) => {
        if (!current || typeof current !== 'object') return

        Object.keys(current).forEach((key) => {
          if (key === 'mrid' || key === 'id') return
    ...
    if (dto.clone === true) {
      ...
      if (originalLocationId && originalLocationId !== parentNode?.mrid) {
        ...
        try {
          const existing: any = await electronAPI.getTransformerEntityByMrid(
            entity.asset.mrid,
            parentNode?.mrid
          )
    ...
    const result: any = await electronAPI.insertTransformerEntity(oldEntity, entity)
...
export const deleteTransformer: any = async (mrid: string, psrId: string, { electronAPI }: any) => {
  try {
    if (!mrid) {
      return { success: false, message: 'MRID is required for deletion' }
    }

    const entityRes: any = await electronAPI.getTransformerFullEntityByMrid(mrid, psrId)
    if (!entityRes.success || !entityRes.data) {
      console.warn(
        `Transformer with mrid ${mrid} not found for deletion, assuming it's already deleted.`
      )
      return { success: true, message: 'Transformer not found, assumed already deleted.' }
    }

    const result: any = await electronAPI.deleteTransformerEntity(entityRes.data)
    return result
  } catch (error: any) {
