...
export const importRotatingMachine = async (dto: any, parentNode: any, { electronAPI, mappings }: { electronAPI: any; mappings: any }) => {
    try {
        if (
            parentNode?.mode &&
        ...
        return {
            success: false,
            message: (error as Error).message || 'Import rotating machine failed'
        }
    }
}
