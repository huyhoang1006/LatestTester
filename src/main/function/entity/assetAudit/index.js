...
const IGNORED_TOP_LEVEL_KEYS = ['configurationEvent', 'user', 'assetPsr', 'userIdentifiedObject']
...
const shouldIgnoreField = (key) => {
  ...
  if (
    ...
    return true
  return (
    /(^|_)id$/.test(lowerKey) || /(^|_)mrid$/.test(lowerKey) || /Id$/.test(key) || /MRID$/.test(key)
  )
}
...
const shouldSkipOutputField = (key, value) => {
  ...
  return (
    ...
    USER_FIELD_WHITELIST.indexOf(key) === -1
  )
}
...
const getReferencedItemLabel = (entity, groupKey, item) => {
  ...
  for (const key of Object.keys(entity)) {
    ...
    if (Array.isArray(value)) {
      for (let index = 0; index < value.length; index += 1) {
        ...
        const matchedField = Object.keys(row).find(
          (field) => row[field] === itemId && !shouldIgnoreField(field)
        )
        if (matchedField) {
    ...
    if (value && typeof value === 'object') {
      const matchedField = Object.keys(value).find(
        (field) => value[field] === itemId && !shouldIgnoreField(field)
      )
      if (matchedField) {
  ...
  const setScalarFields = (prefix, value, groupKey = '') => {
    ...
    if (Array.isArray(value)) {
      value.forEach((item, index) => {
        const itemPrefix =
          getReferencedItemLabel(entity, groupKey, item) ||
          `${prefix} ${getRowLabel(groupKey, item, index)}`
