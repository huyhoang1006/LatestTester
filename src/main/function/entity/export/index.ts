    ...
    for (const node of nodesArray) {
        try {
            if (node.mode === 'substation') {
                const entity: any = await electronAPI.getSubstationEntityByMrid(node.mrid, userId, node.parentId)
                ...
                if (node.asset === 'Surge arrester') {
                    ...
                    const entity: any = await electronAPI.getDisconnectorEntityByMrid(node.mrid, node.parentId)
                    if (entity.success && entity.data) {
                    ...
                    const entity: any = await electronAPI.getRotatingMachineEntityByMrid(node.mrid, node.parentId)
                    if (entity.success && entity.data) {
                    ...
                    const entity: any = await electronAPI.getVoltageTransformerEntityByMrid(node.mrid, node.parentId)
                    if (entity.success && entity.data) {
                    ...
                    const entity: any = await electronAPI.getCurrentTransformerEntityByMrid(node.mrid, node.parentId)
                    if (entity.success && entity.data) {
