...
export const getSyncStateByMrid = async (nodeMrid) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM sync_state WHERE node_mrid = ?', [nodeMrid], (err, row) => {
...
export const getSyncStatesByMrids = async (nodeMrids = []) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM sync_state WHERE node_mrid IN (${placeholders})`, ids, (err, rows) => {
    ...
    db.all(
      ...
      (err, rows) => {
        ...
        return resolve({
          success: true,
          data: rows || [],
          message: 'Get sync states by status completed'
        })
      }
    )
  })
...
export const markSyncState = async (nodeMrid, patch) =>
  markSyncStateTransaction(nodeMrid, patch, db)
...
export const deleteSyncStateByMridTransaction = async (nodeMrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM sync_state WHERE node_mrid = ?', [nodeMrid], (err) => {
...
export const deleteSyncStateByMrid = async (nodeMrid) =>
  deleteSyncStateByMridTransaction(nodeMrid, db)
