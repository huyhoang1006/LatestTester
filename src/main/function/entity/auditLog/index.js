...
const buildChangeDescription = (changes) => {
  ...
  return changes
    .map(
      (change) =>
        `${change.field} changed from ${formatAuditValue(change.from)} to ${formatAuditValue(change.to)}`
    )
    .join('; ')
}
...
const deleteOldAuditLogs = async (ids, dbsql) => {
  ...
  await new Promise((resolve, reject) => {
    dbsql.run(`DELETE FROM identified_object WHERE mrid IN (${placeholders})`, ids, function (err) {
      if (err) reject(err)
      else resolve()
    })
  })
