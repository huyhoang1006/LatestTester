...
import {
  insertParentOrganizationTransaction,
  getParentOrganizationById,
  deleteParentOrganizationByIdTransaction
} from '@/function/cim/parentOrganization'
import { insertGeoMapTransaction, getGeoMapByOrganisationId } from '../geoMap'
import uuid from '@/utils/uuid'
import path from 'path'

export const insertOrganisationEntity: any = async (entity: any) => {
          ...
          db.serialize(async () => {
            ...
            try {
              ...
              if (entity.organisation.mrid)
                await insertParentOrganizationTransaction(entity.organisation, db)
              if (Array.isArray(entity.positionPoints) && entity.positionPoints.length > 0) {
                for (const pp of entity.positionPoints) await insertGeoMapTransaction(pp, db)
              }
              if (entity.attachment.id) {
                ...
                for (let i = 0; i < pathData.length; i++) {
                  ...
                  pathData[i].path = path.join(
                    attachmentContext.getAttachmentPath(entity.organisation.mrid),
                    namefile
                  )
...
export const getOrganisationEntityById: any = async (id: string) => {
  try {
    if (!id) {
      return { success: false, error: new Error('Invalid ID') }
    } else {
      const orgEntity = new OrganisationEntity()
      const dataParentOrganization: any = await getParentOrganizationById(id)
      if (dataParentOrganization.success) {
        orgEntity.organisation = dataParentOrganization.data
      }

      const dataStreetAddress: any = await getStreetAddressById(
        orgEntity.organisation.street_address
      )
      if (dataStreetAddress.success) {
        orgEntity.streetAddress = dataStreetAddress.data
      }

      const dataStreetDetail: any = await getStreetDetailById(orgEntity.streetAddress.street_detail)
      if (dataStreetDetail.success) {
        orgEntity.streetDetail = dataStreetDetail.data
      }

      const dataTownDetail: any = await getTownDetailById(orgEntity.streetAddress.town_detail)
      ...
      const dataElectronicAddress: any = await getElectronicAddressById(
        orgEntity.organisation.electronic_address
      )
      if (dataElectronicAddress.success) {
        orgEntity.electronicAddress = dataElectronicAddress.data
      }

      const dataTelephoneNumber: any = await getTelephoneNumberById(orgEntity.organisation.phone)
      ...
      const dataGeoMap: any = await getGeoMapByOrganisationId(orgEntity.organisation.mrid)
...
export const deleteOrganisationEntityById: any = async (data: any) => {
