...
export const getAttachmentByForeignIdAndType = async (id_foreign: string, type: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM attachment where id_foreign=? and type=?', [id_foreign, type], (err: Error | null, row: unknown) => {
...
export const updateAttachmentById = async (id: string, attachment: { path: string; name: unknown; type: string; id_foreign: string }) => {
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE attachment
...
export const uploadAttachment = async (attachment: { id?: string; id_foreign: string; type: string; name: unknown }) => {
    return new Promise((resolve, reject) => {
        db.run(
            `INSERT INTO attachment(id, id_foreign, type, name)
             ...
             ON CONFLICT(id) DO UPDATE SET
               ...
               type = excluded.type,
               name = excluded.name`,
            [attachment.id || newUuid(), attachment.id_foreign, attachment.type, JSON.stringify(attachment.name)],
            function (this: { lastID: number; changes: number }, err: Error | null) {
