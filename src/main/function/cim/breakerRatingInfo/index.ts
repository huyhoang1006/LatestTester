  ...
  return new Promise((resolve, reject) => {
    db.get(
      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err, message: 'Get breakerRatingInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'BreakerRatingInfo not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get breakerRatingInfo by id completed'
        })
      }
    )
  })
      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get breakerRatingInfo by breakerInfoId failed'
          })
        if (!row)
          return resolve({ success: false, data: null, message: 'BreakerRatingInfo not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get breakerRatingInfo by breakerInfoId completed'
        })
      }
    )
  })
...
export const deleteBreakerRatingInfoTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM breaker_rating_info WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
