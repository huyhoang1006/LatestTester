...
export const getTownDetailById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM town_detail WHERE mrid=?', [mrid], (err: any, row: any) => {
...
export const getTownDetailByLocationId = async (locationId: string) => {
  return new Promise((resolve, reject) => {
    const query = `
            SELECT td.*
            FROM location l
    ...
    db.get(query, [locationId], (err: any, row: any) => {
      if (err) {
        return reject({
          success: false,
          err: err,
          message: 'Get town detail by location id failed'
        })
      }
      if (!row) {
      ...
      return resolve({
        success: true,
        data: row,
        message: 'Get town detail by location id completed'
      })
    })
  })
}

export const insertTownDetail = async (townDetail: any) => {
  return new Promise((resolve, reject) => {
    db.run(
...
export const insertTownDetailTransaction = async (townDetail: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      `INSERT INTO town_detail(mrid, code, country, section, state_or_province,
             ward_or_commune, city, district_or_town)
...
export const updateTownDetailById = async (mrid: string, townDetail: any) => {
  return new Promise((resolve, reject) => {
    db.run(
      `UPDATE town_detail
...
export const updateTownDetailByIdTransaction = async (
  mrid: string,
  townDetail: any,
  dbsql: any
) => {
...
export const deleteTownDetailById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM town_detail WHERE mrid=?', [mrid], function (this: any, err: any) {
...
export const deleteTownDetailByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM town_detail WHERE mrid=?', [mrid], function (this: any, err: any) {
      if (err) return reject({ success: false, err, message: 'Delete town detail failed' })
      if (this.changes === 0)
        return resolve({ success: false, data: null, message: 'Town detail not found' })
      return resolve({ success: true, data: null, message: 'Delete town detail completed' })
