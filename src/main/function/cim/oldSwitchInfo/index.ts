...
import * as SwitchInfoFunc from '../switchInfo/index'

export const getOldSwitchInfoById = async (mrid: string) => {
  try {
    const switchInfoResult: any = await SwitchInfoFunc.getSwitchInfoById(mrid)
    if (!switchInfoResult.success) {
      return { success: false, data: null, message: 'SwitchInfo not found' }
    }
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM old_switch_info WHERE mrid=?', [mrid], (err: any, row: any) => {
...
export const insertOldSwitchInfoTransaction = async (info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const switchInfoResult: any = await SwitchInfoFunc.insertSwitchInfoTransaction(info, dbsql)
      if (!switchInfoResult.success) {
        return reject({
          success: false,
          message: 'Insert SwitchInfo failed',
...
export const updateOldSwitchInfoTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const switchInfoResult: any = await SwitchInfoFunc.updateSwitchInfoTransaction(
        mrid,
        info,
        dbsql
      )
      if (!switchInfoResult.success) {
        return reject({
          success: false,
          message: 'Update SwitchInfo failed',
          err: switchInfoResult.err
        })
      }
      dbsql.run(
...
export const deleteOldSwitchInfoByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    SwitchInfoFunc.deleteSwitchInfoByIdTransaction(mrid, dbsql)
      .then((result: any) => {
        ...
        dbsql.run('DELETE FROM old_switch_info WHERE mrid=?', [mrid], function (err: any) {
...
export const deleteOldSwitchInfoTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    SwitchInfoFunc.deleteSwitchInfoByIdTransaction(mrid, dbsql)
      .then((result: any) => {
        if (!result.success) {
