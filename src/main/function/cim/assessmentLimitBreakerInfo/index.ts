    ...
    return new Promise((resolve, reject) => {
        db.get(
            ...
            (err: Error | null, rows: unknown) => {
                if (err)
                    return reject({ success: false, err, message: 'Get assessmentLimitBreakerInfo by old_breaker_info_id failed' })
...
export const deleteAssessmentLimitBreakerInfoTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
