...
export const getElectronicAddressById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM electronic_address WHERE mrid=?', [mrid], (err: any, row: any) => {
      if (err)
        return reject({ success: false, err: err, message: 'Get electronic address by id failed' })
      if (!row)
        return resolve({ success: false, data: null, message: 'Electronic address not found' })
      return resolve({
        success: true,
        data: row,
        message: 'Get electronic address by id completed'
      })
    })
  })
}
    ...
    db.run(
      ...
      function (err: any) {
        ...
        return resolve({
          ...
          message: 'Insert electronic address completed'
        })
      }
    )
  })
    ...
    db.run(
      ...
      function (err: any) {
        ...
        return resolve({
          success: true,
          data: electronicAddress,
          message: 'Update electronic address completed'
        })
      }
    )
  })
...
export const updateElectronicAddressByIdTransaction = async (
  mrid: string,
...
export const deleteElectronicAddressByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM electronic_address WHERE mrid=?',
      [mrid],
      function (this: any, err: any) {
        ...
        return resolve({
          success: true,
          data: null,
          message: 'Delete electronic address completed'
        })
      }
    )
  })
