    ...
    return new Promise((resolve, reject) => {
        db.get(
            ...
            (err: Error | null, row: unknown) => {
                if (err)
                    return reject({ success: false, err, message: 'Get transformerObservationProcedureDataSet by id failed' })
                if (!row)
                    return resolve({ success: false, data: null, message: 'TransformerObservationProcedureDataSet not found' })
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
                if (err) {
                    return reject({ success: false, err, message: 'Insert transformerObservationProcedureDataSet failed' })
                }
...
export const updateTransformerObservationProcedureDataSetByIdTransaction = async (
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
                if (err) {
                    return reject({ success: false, err, message: 'Update transformerObservationProcedureDataSet failed' })
                }
...
export const deleteTransformerObservationProcedureDataSetByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM transformerObservationProcedureDataSet WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) {
                return reject({ success: false, err, message: 'Delete transformerObservationProcedureDataSet failed' })
            }
            if (this.changes === 0)
                return resolve({ success: false, data: null, message: 'TransformerObservationProcedureDataSet not found' })
