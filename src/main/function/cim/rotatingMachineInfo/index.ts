...
export const getRotatingMachineInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM rotating_machine_info WHERE mrid=?`, [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, err, message: 'Get rotatingMachineInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'RotatingMachineInfo not found' })
        return resolve({
          success: true,
          data: { ...assetInfoResult.data, ...row },
          message: 'Get rotatingMachineInfo by id completed'
        })
      })
    })
  } catch (err) {
...
export const insertRotatingMachineInfoTransaction = async (info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Insert rotatingMachineInfo completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Insert rotatingMachineInfo transaction failed'
      })
    }
  })
}

export const updateRotatingMachineInfoTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update rotatingMachineInfo completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Update rotatingMachineInfo transaction failed'
      })
    }
  })
}

export const deleteRotatingMachineInfoTransaction = async (mrid: string, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run('DELETE FROM rotating_machine_info WHERE mrid=?', [mrid], function (err: any) {
        if (err) {
        ...
        return resolve({
          success: true,
          data: mrid,
          message: 'Delete rotatingMachineInfo completed'
        })
      })
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Delete rotatingMachineInfo transaction failed'
      })
    }
  })
}
