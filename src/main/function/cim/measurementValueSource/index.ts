...
export const insertMeasurementValueSourceTransaction = async (measurementValueSource: { mrid: string }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.insertIdentifiedObjectTransaction(measurementValueSource, dbsql)
...
export const updateMeasurementValueSourceByIdTransaction = async (mrid: string, measurementValueSource: { mrid: string }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, measurementValueSource, dbsql)
            if (!idObjResult.success) {
...
export const deleteMeasurementValueSourceByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM measurement_value_source WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
