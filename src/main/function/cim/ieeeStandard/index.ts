...
export const getIeeeStandardById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM ieeeStandard WHERE mrid=?`, [mrid], (err: Error | null, row: unknown) => {
    ...
    dbsql.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: ieeeStandard,
          message: 'Insert ieeeStandard completed'
        })
      }
    )
  })
    ...
    dbsql.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: ieeeStandard,
          message: 'Update ieeeStandard completed'
        })
      }
    )
  })
...
export const deleteIeeeStandardByIdTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM ieeeStandard WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'IeeeStandard not found' })
