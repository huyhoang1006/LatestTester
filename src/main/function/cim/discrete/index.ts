...
export const getDiscreteById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM discrete WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          success: true,
          data: { ...measurementResult.data, ...row },
          message: 'Get discrete by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!measurementResult.success) {
        return reject({
          success: false,
          message: 'Insert discrete measurement failed',
          err: measurementResult.err
        })
      }
      dbsql.run(
        `INSERT INTO discrete(
                ...
                `,
        [info.mrid, info.max_value, info.min_value, info.normal_value, info.value_alias_set],
        function (err: any) {
          if (err) {
...
export const updateDiscreteByIdTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const measurementResult: any = await MeasurementFunc.updateMeasurementByIdTransaction(
        mrid,
        info,
        dbsql
      )
      if (!measurementResult.success) {
        return reject({
          success: false,
          message: 'Update measurement failed',
          err: measurementResult.err
        })
      }
      dbsql.run(
        `UPDATE discrete SET
                    ...
                    value_alias_set = ?
                WHERE mrid = ?`,
        [info.max_value, info.min_value, info.normal_value, info.value_alias_set, mrid],
        function (err: any) {
          if (err) {
        ...
        MeasurementFunc.deleteMeasurementByIdTransaction(mrid, dbsql)
          ...
          .catch((error: any) => {
            return reject({
              success: false,
              err: error,
              message: 'Delete measurement transaction failed'
            })
          })
      })
    } catch (err) {
