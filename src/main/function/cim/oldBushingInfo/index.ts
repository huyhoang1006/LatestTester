...
export const getOldBushingInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_bushing_info WHERE mrid=?`, [mrid], (err: any, row: any) => {
        if (err) return reject({ success: false, err, message: 'Get oldBushingInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'OldBushingInfo not found' })
        return resolve({
          success: true,
          data: { ...bushingInfoResult.data, ...row },
          message: 'Get oldBushingInfo by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!bushingInfoResult.success) {
        return reject({
          success: false,
          message: 'Insert bushingInfo failed',
          err: bushingInfoResult.err
        })
      }
      dbsql.run(
...
export const updateOldBushingInfoTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const bushingInfoResult: any = await BushingInfoFunc.updateBushingInfoTransaction(
        mrid,
        info,
        dbsql
      )
      if (!bushingInfoResult.success) {
        return reject({
          success: false,
          message: 'Update bushingInfo failed',
          err: bushingInfoResult.err
        })
      }
      dbsql.run(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!bushingInfoResult.success) {
        return reject({
          success: false,
          message: 'Delete bushingInfo failed',
          err: bushingInfoResult.err
        })
      }
