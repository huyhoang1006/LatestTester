...
export const getUkministryOfDefenceStandardById = async (mrid: string) => {
  return new Promise<any>((resolve, reject) => {
    db.get(`SELECT * FROM ukministry_of_defence_standard WHERE mrid=?`, [mrid], (err, row) => {
      if (err)
        return reject({
          success: false,
          err,
          message: 'Get ukministryOfDefenceStandard by id failed'
        })
      if (!row)
        return resolve({
          success: false,
          data: null,
          message: 'UkministryOfDefenceStandard not found'
        })
      return resolve({
        success: true,
        data: row,
        message: 'Get ukministryOfDefenceStandard by id completed'
      })
    })
  })
}
...
export const insertUkministryOfDefenceStandardTransaction = async (data: any, dbsql: any) => {
  // 1. insert standard
...
export const deleteUkministryOfDefenceStandardByIdTransaction = async (
  mrid: string,
  dbsql: any
) => {
  // optional → không cần check
  await runPromise(dbsql as any, 'DELETE FROM ukministry_of_defence_standard WHERE mrid=?', [mrid])
