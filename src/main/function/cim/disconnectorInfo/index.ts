    ...
    return new Promise((resolve, reject) => {
      db.get(
        ...
        (err: Error | null, row: any) => {
          if (err)
            return reject({ success: false, err, message: 'Get disconnectorInfo by id failed' })
          if (!row)
            return resolve({ success: false, data: null, message: 'DisconnectorInfo not found' })
          return resolve({
            success: true,
            data: { ...switchInfoResult.data, ...row },
            message: 'Get disconnectorInfo by id completed'
          })
        }
      )
    })
...
export const insertDisconnectorInfoTransaction = async (
  info: {
    mrid: string
    rated_duration_short_circuit: string | null
    withstand_voltage_earth_poles: string | null
    power_frequency_isolating_distance: string | null
  },
  dbsql: any
) => {
      ...
      dbsql.run(
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Insert disconnectorInfo completed'
          })
        }
      )
    } catch (err) {
      ...
      dbsql.run(
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update disconnectorInfo completed'
          })
        }
      )
    } catch (err) {
...
export const deleteDisconnectorInfoTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      dbsql.run(
        'DELETE FROM disconnector_info WHERE mrid=?',
        [mrid],
        function (this: { lastID: number; changes: number }, err: Error | null) {
          if (err) {
          ...
          return resolve({
            success: true,
            data: mrid,
            message: 'Delete disconnectorInfo completed'
          })
        }
      )
    } catch (err) {
