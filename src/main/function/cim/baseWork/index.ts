...
export const insertBaseWorkTransaction = async (
    baseWork: { mrid: string; kind: string | null; priority: string | null; status_kind: string | null; work_location: string | null },
    dbsql: typeof db
) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            dbsql.run(
                ...
                `,
                [baseWork.mrid, baseWork.kind, baseWork.priority, baseWork.status_kind, baseWork.work_location],
...
export const updateBaseWorkByIdTransaction = async (
    mrid: string,
    baseWork: { kind: string | null; priority: string | null; status_kind: string | null; work_location: string | null },
    dbsql: typeof db
) => {
    return new Promise(async (resolve, reject) => {
        try {
...
export const deleteBaseWorkByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM base_work WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
