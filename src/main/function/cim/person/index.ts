  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      identifiedObjectFunc
        .insertIdentifiedObjectTransaction(person, db)
...
export const insertPersonTransaction = async (person: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .insertIdentifiedObjectTransaction(person, dbsql)
      .then((identifiedResult: any) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
  ...
  return new Promise((resolve, reject) => {
    ...
    db.all(query, [organisationId], (err: any, rows: any) => {
      ...
      return resolve({
        ...
        data: rows,
        message: rows.length
          ? 'Get persons by organisation id completed'
          : 'No persons found for this organisation'
      })
    })
  })
}
  ...
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      ...
      .then((result: any) => {
        ...
        return resolve({
          success: true,
          message: 'Delete person (and identified object) completed'
        })
      })
      .catch((err: any) => {
...
export const updatePersonById = async (mrid: string, person: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      identifiedObjectFunc
        .updateIdentifiedObjectByIdTransaction(mrid, person, db)
        .then((identifiedResult: any) => {
          if (!identifiedResult.success) {
            db.run('ROLLBACK')
            return reject({
...
export const updatePersonByIdTransaction = async (mrid: string, person: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .updateIdentifiedObjectByIdTransaction(mrid, person, dbsql)
      .then((identifiedResult: any) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
