    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            `UPDATE close_operation SET
                ...
                t_dev = ?
            WHERE mrid = ?`,
            [info.auxiliary_contacts_breaker_info_id, info.parameter_name, info.t_min, info.t_max, info.t_ref, info.t_dev, mrid],
            function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteCloseOperationTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM close_operation WHERE mrid = ?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
