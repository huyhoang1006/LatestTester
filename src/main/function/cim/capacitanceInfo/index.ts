...
export const getCapacitanceCapacitorInfoById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM capacitance_capacitor_info WHERE mrid=?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get capacitanceCapacitorInfo by id failed'
          })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'CapacitanceCapacitorInfo not found'
          })
        return resolve({
          success: true,
          data: row,
          message: 'Get capacitanceCapacitorInfo by id completed'
        })
      }
    )
  })
      ...
      (err: Error | null, rows: unknown[]) => {
        if (err)
          return reject({
            success: false,
            err: err,
            message: 'Get capacitanceCapacitorInfo by ids failed'
          })
        ...
        return resolve({
          success: true,
          data: rows,
          message: 'Get capacitanceCapacitorInfo by ids completed'
        })
      }
    )
  })
...
export const updateCapacitanceCapacitorInfoTransaction = async (
  ...
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Update capacitanceCapacitorInfo failed' })
        return resolve({
          success: true,
          data: capacitanceCapacitorInfo,
          message: 'Update capacitanceCapacitorInfo completed'
        })
      }
    )
  })
...
export const deleteCapacitanceInfoTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM capacitance_capacitor_info WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Delete capacitanceCapacitorInfo failed' })
        if (this.changes === 0)
          return resolve({
            success: false,
            data: null,
        ...
        return resolve({
          success: true,
          data: null,
          message: 'Delete capacitanceCapacitorInfo completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Delete capacitance capacitor info failed'
          })
        if (this.changes === 0)
          return resolve({
            success: false,
            data: null,
            message: 'Capacitance Capacitor Info not found'
          })
        return resolve({
          success: true,
          data: null,
          message: 'Delete capacitance capacitor info completed'
        })
      }
    )
  })
