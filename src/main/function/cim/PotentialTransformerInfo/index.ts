        ...
        (err: any, row: any) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get potentialTransformerInfo by id failed'
            })
          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'PotentialTransformerInfo not found'
            })
          return resolve({
            success: true,
            data: { ...assetInfoResult.data, ...row },
            message: 'Get potentialTransformerInfo by id completed'
          })
        }
      )
    })
...
export const insertPotentialTransformerTransaction = async (
  potentialTransformer: any,
  dbsql: any
) => {
  return new Promise(async (resolve, reject) => {
    try {
      const assetInfoResult: any = await AssetInfoFunc.insertAssetInfoTransaction(
      ...
      dbsql.run(
        ...
        function (err: any) {
          if (err)
            return reject({ success: false, err, message: 'Insert potential transformer failed' })
          return resolve({
            success: true,
            data: potentialTransformer,
            message: 'Insert potential transformer completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Insert potential transformer transaction failed'
      })
    }
  })
}

export const updatePotentialTransformerInfoTransaction = async (
  mrid: string,
  info: any,
  dbsql: any
) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update potentialTransformerInfo completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Update potentialTransformerInfo transaction failed'
      })
    }
  })
}
...
export const deletePotentialTransformerInfoTransaction = async (mrid: string, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run(
        'DELETE FROM potential_transformer_info WHERE mrid=?',
        [mrid],
        async function (err: any) {
          ...
          try {
            const assetInfoResult: any = await AssetInfoFunc.deleteAssetInfoByIdTransaction(
              mrid,
              dbsql
            )
            if (!assetInfoResult.success) {
