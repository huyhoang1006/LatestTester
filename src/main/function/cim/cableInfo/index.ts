...
export const getCableInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM cable_info WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          success: true,
          data: { ...wireInfoResult.data, ...row },
          message: 'Get cableInfo by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!wireInfoResult.success) {
        return reject({
          success: false,
          message: 'Insert wireInfo failed',
          err: wireInfoResult.err
        })
      }
      dbsql.run(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!wireInfoResult.success) {
        return reject({
          success: false,
          message: 'Update wireInfo failed',
          err: wireInfoResult.err
        })
      }
      dbsql.run(
      ...
      dbsql.run('DELETE FROM cable_info WHERE mrid=?', [mrid], async function (err: any) {
        ...
        try {
          ...
          if (!wireInfoResult.success) {
            return reject({
              success: false,
              message: 'Delete wireInfo failed',
              err: wireInfoResult.err
            })
          }
          ...
          return reject({
            success: false,
            err: err2,
            message: 'Delete cableInfo transaction failed'
          })
        }
      })
    } catch (err) {
