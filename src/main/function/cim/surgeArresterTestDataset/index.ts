        ...
        (err: Error | null, row: any) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get surgeArresterTestDataSet by id failed'
            })
          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'SurgeArresterTestDataSet not found'
            })
          return resolve({
            success: true,
            data: { ...testDataSet.data, ...row },
            message: 'Get surgeArresterTestDataSet by id completed'
          })
        }
      )
    })
...
export const insertSurgeArresterTestDataSetTransaction = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const testResult: any = await testDataSetFunc.insertTestDataSetTransaction(
        surgeArresterTestDataSet,
        dbsql
      )
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Insert surgeArresterTestDataSet failed'
            })
          return resolve({
            success: true,
            data: surgeArresterTestDataSet,
            message: 'Insert surgeArresterTestDataSet completed'
          })
        }
      )
    } catch (err) {
...
export const updateSurgeArresterTestDataSetByIdTransaction = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const testResult: any = await testDataSetFunc.updateTestDataSetByIdTransaction(
        mrid,
        surgeArresterTestDataSet,
        dbsql
      )
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Update surgeArresterTestDataSet failed'
            })
          return resolve({
            success: true,
            data: surgeArresterTestDataSet,
            message: 'Update surgeArresterTestDataSet completed'
          })
        }
      )
    } catch (err) {
...
export const deleteSurgeArresterTestDataSetByIdTransaction = async (
  mrid: string,
  dbsql: typeof db
) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run(
        'DELETE FROM surge_arrester_test_dataset WHERE mrid=?',
        [mrid],
        function (this: { lastID: number; changes: number }, err: Error | null) {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Delete surgeArresterTestDataSet failed'
            })
          if (this.changes === 0)
            return resolve({
              success: false,
          ...
          return resolve({
            success: true,
            data: null,
            message: 'Delete surgeArresterTestDataSet completed'
          })
        }
      )
    } catch (err) {
