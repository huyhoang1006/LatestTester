...
export const getOldPotentialTransformerInfoById = async (mrid: string) => {
  try {
    const ptInfoResult: any =
      await PotentialTransformerInfoFunc.getPotentialTransformerInfoById(mrid)
        ...
        (err: any, row: any) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get oldPotentialTransformerInfo by id failed'
            })
          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'OldPotentialTransformerInfo not found'
            })
          return resolve({
            success: true,
            data: { ...ptInfoResult.data, ...row },
            message: 'Get oldPotentialTransformerInfo by id completed'
          })
        }
      )
    })
...
export const insertOldPotentialTransformerTransaction = async (info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const ptInfoResult: any =
        await PotentialTransformerInfoFunc.insertPotentialTransformerTransaction(info, dbsql)
      if (!ptInfoResult.success) {
        return reject({
          success: false,
          message: 'Insert potentialTransformerInfo failed',
        ...
        function (err: any) {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Insert oldPotentialTransformerInfo failed'
            })
          return resolve({
            success: true,
            data: info,
            message: 'Insert oldPotentialTransformerInfo completed'
          })
        }
      )
    } catch (error) {
      return reject({
        success: false,
        err: error,
        message: 'Insert oldPotentialTransformerInfo transaction failed'
      })
    }
  })
}

export const updateOldPotentialTransformerInfoTransaction = async (
  mrid: string,
  info: any,
  dbsql: any
) => {
  return new Promise(async (resolve, reject) => {
    try {
      const ptInfoResult: any =
        await PotentialTransformerInfoFunc.updatePotentialTransformerInfoTransaction(
          mrid,
          info,
          dbsql
        )
      if (!ptInfoResult.success) {
        return reject({
          success: false,
          message: 'Update potentialTransformerInfo failed',
          err: ptInfoResult.err
        })
      }
      dbsql.run(
        ...
        function (err: any) {
          if (err) {
            return reject({
              success: false,
              err,
              message: 'Update oldPotentialTransformerInfo failed'
            })
          }
          return resolve({
            success: true,
            data: info,
            message: 'Update oldPotentialTransformerInfo completed'
          })
        }
      )
    } catch (error) {
      return reject({
        success: false,
        err: error,
        message: 'Update oldPotentialTransformerInfo transaction failed'
      })
    }
  })
}
...
export const deleteOldPotentialTransformerInfoTransaction = async (mrid: string, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run(
        'DELETE FROM old_potential_transformer_info WHERE mrid=?',
        [mrid],
        async function (err: any) {
          ...
          try {
            const ptInfoResult: any =
              await PotentialTransformerInfoFunc.deletePotentialTransformerInfoTransaction(
                mrid,
                dbsql
              )
            if (!ptInfoResult.success) {
