...
export const updateProcedureDataSetByIdTransaction = async (
    ...
    return new Promise(async (resolve, reject) => {
        try {
            const docResult: any = await documentFunc.updateDocumentByIdTransaction(mrid, procedureDataSet, dbsql)
            if (!docResult.success) {
            ...
            dbsql.run(
                `UPDATE procedure_dataset SET
                    ...
                    procedure = ?
                WHERE mrid = ?`,
                [procedureDataSet.completed_date_time, procedureDataSet.work_task, procedureDataSet.asset, procedureDataSet.procedure, mrid],
                function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteProcedureDataSetByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM procedure_dataset WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
