...
export const getZeroSequenceImpedanceTableByTransformerEndIdAndZeroSequenceImpedance = async (
  transformerEndId: string,
  zeroSequenceImpedanceId: string
) => {
  return new Promise((resolve, reject) => {
    db.all(
      `SELECT *
...
export const getZeroSequenceImpedanceTableByZeroSequenceImpedanceId = async (
  zeroSequenceImpedanceId: string
) => {
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err: Error | null, rows: unknown[]) => {
        ...
        if (rows.length === 0) {
          return reject({
            success: false,
            err: null,
            message: 'Get by zero_sequence_impedance failed'
          })
        }
        return resolve({ success: true, data: rows, message: 'Completed' })
      }
...
export const insertZeroSequenceImpedanceTableTransaction = async (
  info: {
    mrid: string
    transformer_end_id: string
    zero: string | null
    zero_sequence_impedance: string
  },
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
...
export const deleteZeroSequenceImpedanceTableTransaction = async (
  mrid: string,
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
