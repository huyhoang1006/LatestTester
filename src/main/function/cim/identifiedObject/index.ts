...
export const getIdentifiedObjectById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM identified_object WHERE mrid=?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err: err, message: 'Get identified object by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'Identified object not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get identified object by id completed'
        })
      }
    )
  })
...
export const updateIdentifiedObjectById = async (
  mrid: string,
  identifiedObject: { [key: string]: any }
) => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: identifiedObject,
          message: 'Update identified object completed'
        })
      }
    )
  })
...
export const updateIdentifiedObjectByIdTransaction = async (
  mrid: string,
  identifiedObject: { [key: string]: any },
  dbsql: typeof db
) => {
...
export const deleteIdentifiedObjectById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'DELETE FROM identified_object WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
