...
export const getDocumentById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM document WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          ...
          message: 'Get document by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM document WHERE mrid=?', [mrid], function (this: any, err: any) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'Document not found' })
