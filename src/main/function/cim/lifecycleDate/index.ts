...
export const getLifecycleDateById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM lifecycle_date WHERE mrid=?', [mrid], (err: Error | null, row: unknown) => {
...
export const insertLifecycleDateTransaction = (
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
                if (err) return reject({ success: false, err: err, message: 'Insert lifecycle date transaction failed' })
...
export const updateLifecycleDate = async (mrid: string, data: {
    installation_date: string | null
    manufactured_date: string | null
    purchase_date: string | null
    received_date: string | null
...
export const updateLifecycleDateTransaction = (
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
                if (err) return reject({ success: false, err: err, message: 'Update lifecycle date transaction failed' })
...
export const deleteLifecycleDateById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM lifecycle_date WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteLifecycleDateByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM lifecycle_date WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete lifecycle date failed' })
            return resolve({ success: true, data: mrid, message: 'Delete lifecycle date completed' })
        })
    })
