...
export const getOldBreakerInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_breaker_info WHERE mrid=?`, [mrid], (err: any, row: any) => {
        if (err) return reject({ success: false, err, message: 'Get oldBreakerInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'OldBreakerInfo not found' })
        return resolve({
          success: true,
          data: { ...breakerInfoResult.data, ...row },
          message: 'Get oldBreakerInfo by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!breakerInfoResult.success) {
        return reject({
          success: false,
          message: 'Insert BreakerInfo failed',
          err: breakerInfoResult.err
        })
      }
      dbsql.run(
...
export const updateOldBreakerInfoTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const breakerInfoResult: any = await BreakerInfoFunc.updateBreakerInfoTransaction(
        mrid,
        info,
        dbsql
      )
      if (!breakerInfoResult.success) {
        return reject({
          success: false,
          message: 'Update BreakerInfo failed',
          err: breakerInfoResult.err
        })
      }
      dbsql.run(
        ...
        BreakerInfoFunc.deleteBreakerInfoTransaction(mrid, dbsql)
          .then((breakerRes: any) => {
            if (!breakerRes.success) {
              return reject({
                success: false,
                message: 'Delete BreakerInfo failed',
                err: breakerRes.err
              })
            }
            return resolve({
              success: true,
              data: mrid,
              message: 'Delete oldBreakerInfo completed'
            })
          })
          .catch((err2: any) => {
            return reject({
              success: false,
              err: err2,
              message: 'Delete BreakerInfo transaction failed'
            })
          })
      })
    } catch (err) {
