  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      equipmentContainerFunc
        .insertEquipmentContainerTransaction(voltageLevel, db)
        .then((result: any) => {
          if (!result.success) {
            db.run('ROLLBACK')
            return reject({
          ...
          db.run(
            ...
            function (err: any) {
              ...
              return resolve({
                success: true,
                data: voltageLevel,
                message: 'Insert VoltageLevel completed'
              })
            }
          )
        })
...
export const insertVoltageLevelTransaction = async (voltageLevel: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    equipmentContainerFunc
      .insertEquipmentContainerTransaction(voltageLevel, dbsql)
      .then((result: any) => {
        if (!result.success) {
          return reject({
            success: false,
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM voltage_level WHERE mrid = ?', [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, data: null, message: 'Get VoltageLevel failed', err })
...
export const updateVoltageLevelById = async (mrid: string, voltageLevel: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      equipmentContainerFunc
        .updateEquipmentContainerByIdTransaction(mrid, voltageLevel, db)
        .then((result: any) => {
          if (!result.success) {
            db.run('ROLLBACK')
            return reject({
          ...
          db.run(
            ...
            function (err: any) {
              ...
              return resolve({
                success: true,
                data: voltageLevel,
                message: 'Update VoltageLevel completed'
              })
            }
          )
        })
...
export const updateVoltageLevelByIdTransaction = async (
  mrid: string,
  voltageLevel: any,
  dbsql: any
) => {
  return new Promise((resolve, reject) => {
    equipmentContainerFunc
      .updateEquipmentContainerByIdTransaction(mrid, voltageLevel, dbsql)
      .then((result: any) => {
        if (!result.success) {
          return reject({
            success: false,
