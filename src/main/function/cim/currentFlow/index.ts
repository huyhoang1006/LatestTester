...
export const getCurrentFlowById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM current_flow WHERE mrid=?', [mrid], (err: Error | null, row: unknown) => {
...
export const getCurrentFlowByIds = async (mrids: string[]) => {
    return new Promise((resolve, reject) => {
...
export const insertCurrentFlow = async (currentFlow: { mrid: string; multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
            ...
            [currentFlow.mrid, currentFlow.multiplier, currentFlow.unit, currentFlow.value],
...
export const insertCurrentFlowTransaction = async (
    currentFlow: { mrid: string; multiplier: string | null; unit: string | null; value: number | null },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const updateCurrentFlowById = async (mrid: string, currentFlow: { multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE current_flow
...
export const updateCurrentFlowByIdTransaction = async (
    mrid: string,
    currentFlow: { multiplier: string | null; unit: string | null; value: number | null },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const deleteCurrentFlowById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM current_flow WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteCurrentFlowByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM current_flow WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete current flow failed' })
            if (this.changes === 0) return resolve({ success: false, data: null, message: 'Current flow not found' })
            return resolve({ success: true, data: null, message: 'Delete current flow completed' })
        })
