...
export const getActivityRecordByAssetId = async (assetId: string, type: string = 'Repair') => {
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err: any, rows: any) => {
        if (err)
          return reject({ success: false, err, message: 'Get activity record by asset failed' })
        if (!rows || rows.length === 0)
          return resolve({ success: false, data: null, message: 'Activity record not found' })
        return resolve({
          success: true,
          data: rows,
          message: 'Get activity record by asset completed'
        })
      }
    )
  })
      ...
      identifiedObjectFunc
          ...
          db.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
              ...
              return resolve({
                success: true,
                data: activity,
                message: 'Insert activity record completed'
              })
            }
          )
        })
        .catch((err: Error) => {
          ...
          return reject({
            success: false,
            err,
            message: 'Insert activity record transaction failed'
          })
        })
    })
  })
...
export const getActivityRecordById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM activity_record WHERE mrid = ?',
        [mrid],
      ...
      identifiedObjectFunc
          ...
          db.run(
            ...
            function (this: { lastID: number; changes: number }, err: Error | null) {
              ...
              return resolve({
                success: true,
                data: activity,
                message: 'Update activity record completed'
              })
            }
          )
        })
        .catch((err: Error) => {
          ...
          return reject({
            success: false,
            err,
            message: 'Update activity record transaction failed'
          })
        })
    })
  })
...
export const updateActivityRecordByIdTransaction = async (
  mrid: string,
  activity: any,
  dbsql: typeof db
) => {
  ...
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      ...
      .then((result: any) => {
        ...
        return resolve({
          success: true,
          message: 'Delete activity record (and identified object) completed'
        })
      })
      .catch((err: Error) => {
