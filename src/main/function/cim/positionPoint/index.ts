...
export const insertPositionPointArrayTransaction = async (positionPoints: any[], locationId: string, dbsql: any) => {
    return new Promise((resolve, reject) => {
        ...
        dbsql.all(
            ...
            (err: any, rows: any) => {
                ...
                const mridsToDelete: string[] = existingMrids.filter((mrid: string) => !incomingMrids.includes(mrid));
...
export const updatePositionPointArrayByIdTransaction = async (positionPoints: any[], dbsql: any) => {
    return new Promise((resolve, reject) => {
        if (!Array.isArray(positionPoints) || positionPoints.length === 0) {
            return reject({ success: false, message: 'Position points array is empty or invalid' })
        }
...
export const deletePositionPointByArrayMridTransaction = async (mridArray: string[], dbsql: any) => {
    return new Promise((resolve, reject) => {
        if (!Array.isArray(mridArray) || mridArray.length === 0) {
            return reject({ success: false, message: 'Mrid array is empty or invalid' })
        }
