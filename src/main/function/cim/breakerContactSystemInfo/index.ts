      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get breakerContactSystemInfo by id failed'
          })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'BreakerContactSystemInfo not found'
          })
        return resolve({
          success: true,
          data: row,
          message: 'Get breakerContactSystemInfo by id completed'
        })
      }
    )
  })
      ...
      (err: Error | null, rows: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get breakerContactSystemInfo by breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get breakerContactSystemInfo by breaker_info_id completed'
        })
      }
    )
  })
...
export const insertBreakerContactSystemInfoTransaction = async (
  info: {
    ...
    nominal_total_travel: string | null
    damping_time: string | null
    nozzle_length: string | null
  },
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Insert breakerContactSystemInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Insert breakerContactSystemInfo completed'
        })
      }
    )
  })
...
export const updateBreakerContactSystemInfoTransaction = async (
  ...
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Update breakerContactSystemInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Update breakerContactSystemInfo completed'
        })
      }
    )
  })
...
export const deleteBreakerContactSystemInfoTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM breaker_contact_system_info WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({ success: false, err, message: 'Delete breakerContactSystemInfo failed' })
        return resolve({
          success: true,
          data: mrid,
          message: 'Delete breakerContactSystemInfo completed'
        })
      }
    )
  })
