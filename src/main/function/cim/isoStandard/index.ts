...
export const getIsoStandardById = async (mrid: string) => {
  return new Promise<any>((resolve, reject) => {
    db.get(`SELECT * FROM iso_standard WHERE mrid=?`, [mrid], (err, row) => {
...
export const insertIsoStandardTransaction = async (data: any, dbsql: any) => {
  // 1. insert standard
...
// Xóa isoStandard
export const deleteIsoStandardByIdTransaction = async (mrid: string, dbsql: any) => {
  // optional → không cần check
  await runPromise(dbsql as any, 'DELETE FROM iso_standard WHERE mrid=?', [mrid])
