...
export const getStringMeasurementById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM string_measurement WHERE mrid=?`, [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, err, message: 'Get stringMeasurement by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'StringMeasurement not found' })
        return resolve({
          success: true,
          data: { ...measurement.data, ...row },
          message: 'Get stringMeasurement by id completed'
        })
      })
    })
  } catch (err) {
...
export const insertStringMeasurementTransaction = async (stringMeasurement: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const measResult: any = await measurementFunc.insertMeasurementTransaction(
        stringMeasurement,
        dbsql
      )
      if (!measResult.success) {
        return reject({
          success: false,
          message: 'Insert string measurement failed',
          err: measResult.err
        })
      }
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            ...
            message: 'Insert stringMeasurement completed'
          })
        }
      )
    } catch (err) {
      ...
      db.run('BEGIN TRANSACTION', async (err: any) => {
        ...
        try {
          ...
          return reject({
            success: false,
            err,
            message: 'Insert string measurement failed (exception)'
          })
        }
      })
    })
...
export const updateStringMeasurementByIdTransaction = async (
  mrid: string,
  stringMeasurement: any,
  dbsql: any
) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: stringMeasurement,
            message: 'Update stringMeasurement completed'
          })
        }
      )
    } catch (err) {
...
export const deleteStringMeasurementByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run(
        'DELETE FROM string_measurement WHERE mrid=?',
        [mrid],
        function (this: any, err: any) {
          ...
          return resolve({
            success: true,
            data: null,
            message: 'Delete stringMeasurement completed'
          })
        }
      )
    } catch (err) {
      ...
      db.run('BEGIN TRANSACTION', async (err: any) => {
        ...
        try {
            ...
            function (this: any, err: any) {
              ...
              if (this.changes === 0) {
                ...
                return resolve({
                  success: false,
                  data: null,
                  message: 'StringMeasurement not found'
                })
              }
          ...
          return reject({
            success: false,
            err,
            message: 'Delete string measurement transaction failed'
          })
        }
      })
    })
