...
export const getOldTransformerEndInfoById = async (mrid: string) => {
  try {
    ...
    const row: any = await new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_transformer_end_info WHERE mrid=?`, [mrid], (err: any, row: any) =>
...
export const getOldTransformerEndInfoByPowerTransformerInfoId = async (
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return {
        success: false,
        data: [],
        message: 'No OldTransformerEndInfo found for this powerTransformerInfoId'
      }
    }
    return {
      success: true,
    ...
    return {
      success: false,
      err,
      message: 'Get oldTransformerEndInfo by powerTransformerInfoId failed'
    }
  }
}
...
export const insertOldTransformerEndInfoTransaction = async (info: any, dbsql: any) => {
  try {
    const baseResult: any = await TransformerEndInfoFunc.insertTransformerEndInfoTransaction(
      info,
      dbsql
    )
    if (!baseResult.success) {
...
export const updateOldTransformerEndInfoTransaction = async (
  mrid: string,
  info: any,
  dbsql: any
) => {
  try {
    const baseResult: any = await TransformerEndInfoFunc.updateTransformerEndInfoTransaction(
      mrid,
      info,
      dbsql
    )
...
export const deleteOldTransformerEndInfoTransaction = async (mrid: string, dbsql: any) => {
  ...
  const baseResult: any = await TransformerEndInfoFunc.deleteTransformerEndInfoTransaction(
    mrid,
    dbsql
  )
  if (!baseResult.success) {
