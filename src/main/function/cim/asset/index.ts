  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM asset WHERE mrid=?', [mrid], (err: Error | null, row: any) => {
        ...
        if (!row) {
          console.warn(
            `[DEBUG] Asset Table row NOT found for MRID: ${mrid} (but IdentifiedObject existed)`
          )
    ...
    return new Promise((resolve, reject) => {
      db.get(
        ...
        async (err: Error | null, row: any) => {
          if (err) {
            console.error(
              `[DEBUG] SQLite Error in getAssetByAssetInfoId for AssetInfoId: ${assetInfoId}`,
              err
            )
          ...
          if (!row) {
            return resolve({
              success: false,
              data: null,
              message: 'Asset not found for this AssetInfo'
            })
          }
      ...
      dbsql.run(
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          ...
          return resolve({
            success: true,
            data: asset,
            message: 'Insert asset transaction completed'
          })
        }
      )
    } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const identifiedResult: any =
        await IdentifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, asset, dbsql)
      if (!identifiedResult.success) {
        return reject({
          success: false,
          message: 'Update identified object failed',
      ...
      dbsql.run(
        ...
        function (this: { lastID: number; changes: number }, err: Error | null) {
          ...
          return resolve({
            success: true,
            data: asset,
            message: 'Update asset transaction completed'
          })
        }
      )
    } catch (err) {
