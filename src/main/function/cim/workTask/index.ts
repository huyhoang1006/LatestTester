...
export const getWorkTaskById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM work_task WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          ...
          message: 'Get workTask by id completed'
        })
      })
    })
  } catch (err) {
...
export const updateWorkTaskByIdTransaction = async (mrid: string, workTask: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const baseResult: any = await baseWorkFunc.updateBaseWorkByIdTransaction(
        mrid,
        workTask,
        dbsql
      )
  ...
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM work_task WHERE mrid=?', [mrid], function (this: any, err: any) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'Work_task not found' })
