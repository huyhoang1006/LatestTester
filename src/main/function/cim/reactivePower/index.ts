...
export const getReactivePowerById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM reactive_power WHERE mrid=?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err: err, message: 'Get reactive power by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'Reactive power not found' })
    ...
    db.all(
      ...
      (err: Error | null, rows: unknown[]) => {
        ...
        return resolve({
          success: true,
          data: rows,
          message: 'Get reactive power by ids completed'
        })
      }
    )
  })
    ...
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: reactivePower,
          message: 'Insert reactive power completed'
        })
      }
    )
  })
...
export const updateReactivePowerById = async (
  mrid: string,
    ...
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: reactivePower,
          message: 'Update reactive power completed'
        })
      }
    )
  })
...
export const deleteReactivePowerById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'DELETE FROM reactive_power WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
