...
export const getCurrentRatingById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM current_rating WHERE mrid=?',
      [mrid],
      (err: Error | null, row: unknown) => {
        if (err) return reject({ success: false, err, message: 'Get current rating by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'Current rating not found' })
    ...
    db.all(
      ...
      (err: Error | null, rows: unknown[]) => {
        ...
        return resolve({
          success: true,
          data: rows,
          message: 'Get current ratings by ids completed'
        })
      }
    )
  })
...
export const deleteCurrentRatingByIdTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM current_rating WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
