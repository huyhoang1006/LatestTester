  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      equipmentContainerFunc
        .insertEquipmentContainerTransaction(substation, db)
        .then((result: any) => {
          if (!result.success) {
            db.run('ROLLBACK')
            return reject({
          ...
          db.run(
            ...
            function (err: any) {
              ...
              return resolve({
                success: true,
                data: substation,
                message: 'Insert Substation completed'
              })
            }
          )
        })
...
export const insertSubstationTransaction = async (substation: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    equipmentContainerFunc
      .insertEquipmentContainerTransaction(substation, dbsql)
      .then((result: any) => {
        if (!result.success) {
          return reject({
            success: false,
...
export const getSubstationsInOrganisationForUser = async (
      ...
      db.all(query, [organisationId, userId], (err: any, rows: any) => {
        ...
        if (!rows || rows.length === 0) {
          return resolve({
            success: false,
            data: [],
            message: 'No substations found for this user in organisation'
          })
        }
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM substation WHERE mrid = ?', [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, data: null, message: 'Get Substation failed', err })
...
export const updateSubstationById = async (mrid: string, substation: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      equipmentContainerFunc
        .updateEquipmentContainerByIdTransaction(mrid, substation, db)
        .then((result: any) => {
          if (!result.success) {
            db.run('ROLLBACK')
            return reject({
          ...
          db.run(
            ...
            function (err: any) {
              ...
              return resolve({
                success: true,
                data: substation,
                message: 'Update Substation completed'
              })
            }
          )
        })
...
export const updateSubstationByIdTransaction = async (
  mrid: string,
  substation: any,
  dbsql: any
) => {
  return new Promise((resolve, reject) => {
    equipmentContainerFunc
      .updateEquipmentContainerByIdTransaction(mrid, substation, dbsql)
      .then((result: any) => {
        if (!result.success) {
          return reject({
            success: false,
  ...
  return new Promise((resolve, reject) => {
    equipmentContainerFunc
      ...
      .then((result: any) => {
        ...
        return resolve({
          success: true,
          message: 'Delete Substation (and EquipmentContainer) completed'
        })
      })
      .catch((err: any) => {
