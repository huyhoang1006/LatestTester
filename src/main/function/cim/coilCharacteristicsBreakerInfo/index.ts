...
export const getCoilCharacteristicsBreakerInfoByAssessmentLimitId = async (assessmentLimitId: string) => {
    return new Promise((resolve, reject) => {
        db.all(
...
export const insertCoilCharacteristicsBreakerInfoTransaction = async (
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            `,
            [info.mrid, info.assessment_limit_breaker_info_id, info.parameter_name, info.min, info.max, info.ref, info.dev_negative, info.dev_positive],
    ...
    return new Promise((resolve, reject) => {
        dbsql.run(
            `UPDATE coil_characteristics_breaker_info SET
                ...
                dev_positive = ?
            WHERE mrid = ?`,
            [info.assessment_limit_breaker_info_id, info.parameter_name, info.min, info.max, info.ref, info.dev_negative, info.dev_positive, mrid],
            function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteCoilCharacteristicsBreakerInfoTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM coil_characteristics_breaker_info WHERE mrid = ?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
