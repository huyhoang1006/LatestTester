...
export const getInUseDateById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM in_use_date WHERE mrid=?', [mrid], (err: any, row: any) => {
...
export const insertInUseDateTransaction = (data: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      `INSERT INTO in_use_date(
            ...
            `,
      [data.mrid, data.in_use_date, data.not_ready_for_use_date, data.ready_for_use_date],
      function (err: any) {
        if (err)
          return reject({ success: false, err, message: 'Insert in-use date transaction failed' })
