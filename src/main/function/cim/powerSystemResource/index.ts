...
export const updatePowerSystemResourceByIdTransaction = async (mrid: string, psr: any, dbsql: any) => {
    return new Promise((resolve, reject) => {
...
export const getPowerSystemResourceByLocationIdTransaction = async (locationId: string, dbsql: any) => {
    try {
        return new Promise((resolve, reject) => {
            dbsql.all("SELECT * FROM power_system_resource WHERE location = ?", [locationId], (err: any, rows: any) => {
                if (err) return reject({ success: false, err, message: 'Get powerSystemResource failed' })
