...
export const getProcedureDataSetMeasurementValueById = async (procedure_dataset_id: string, measurement_value_id: string) => {
...
export const insertProcedureDataSetMeasurementValueTransaction = async (
    info: { procedure_dataset_id: string; measurement_value_id: string },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            ...
            [info.procedure_dataset_id, info.measurement_value_id],
...
export const updateProcedureDataSetMeasurementValueTransaction = async (
    old_procedure_dataset_id: string,
    old_measurement_value_id: string,
    info: { procedure_dataset_id: string; measurement_value_id: string },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            `UPDATE procedure_dataset_measurement_value SET
                ...
                measurement_value_id = ?
             WHERE procedure_dataset_id = ? AND measurement_value_id = ?`,
            [info.procedure_dataset_id, info.measurement_value_id, old_procedure_dataset_id, old_measurement_value_id],
            function (this: { lastID: number; changes: number }, err: Error | null) {
                if (err) {
...
export const deleteProcedureDataSetMeasurementValueTransaction = async (
    procedure_dataset_id: string,
    measurement_value_id: string,
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
