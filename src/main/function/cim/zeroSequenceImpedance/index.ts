  ...
  return new Promise((resolve, reject) => {
    db.get(
      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err, message: 'Get zero sequence impedance failed' })
...
export const insertZeroSequenceImpedanceTransaction = async (
  info: {
    mrid: string
    power_transformer_info_id: string
    base_power: string | null
    base_voltage: string | null
  },
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Insert/Update zero sequence impedance failed'
          })
...
export const getZeroSequenceImpedanceByTransformerInfoId = async (
