...
export const getAuxiliaryContactsBreakerInfoByAssessmentLimitId = async (assessmentLimitId: string) => {
    return new Promise((resolve, reject) => {
        db.all(
            ...
            (err: Error | null, rows: unknown[]) => {
                if (err)
                    return reject({ success: false, err, message: 'Get auxiliaryContactsBreakerInfo by assessment_limit_breaker_info_id failed' })
...
export const deleteAuxiliaryContactsBreakerInfoTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM auxiliary_contacts_breaker_info WHERE mrid = ?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
