...
export const getAstmStandardById = async (mrid: string) => {
  return new Promise<any>((resolve, reject) => {
    db.get(`SELECT * FROM astm_standard WHERE mrid=?`, [mrid], (err, row) => {
...
export const insertAstmStandardTransaction = async (data: any, dbsql: any) => {
  // 1. insert standard
...
// Xóa astmStandard
export const deleteAstmStandardByIdTransaction = async (mrid: string, dbsql: any) => {
  // optional → không cần check
  await runPromise(dbsql as any, 'DELETE FROM astm_standard WHERE mrid=?', [mrid])
