...
export const deleteCtCoreInfoByCurrentTransformerInfoIdTransaction = (
  current_transformer_info_id: string,
  _dbsql: any
) => {
  return new Promise((resolve, reject) => {
    ...
    dbsql.run(sql, params, function (this: { lastID: number; changes: number }, err: Error | null) {
      ...
      return resolve({
        success: true,
        data: current_transformer_info_id,
        message: 'Delete ctCoreInfo completed'
      })
    })
  })
}
