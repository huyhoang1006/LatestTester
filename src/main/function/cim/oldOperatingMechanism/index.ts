        ...
        (err: any, row: any) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get oldOperatingMechanism by id failed'
            })
          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'OldOperatingMechanism not found'
            })
          return resolve({
            success: true,
...
export const insertOldOperatingMechanismTransaction = async (info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes: any = await OperatingMechanismFunc.insertOperatingMechanismTransaction(
        info,
        dbsql
      )
      if (!parentRes.success)
...
export const updateOldOperatingMechanismTransaction = async (
  mrid: string,
  info: any,
  dbsql: any
) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes: any = await OperatingMechanismFunc.updateOperatingMechanismTransaction(
        mrid,
        info,
        dbsql
      )
