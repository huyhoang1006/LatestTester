...
export const getAssetInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM asset_info WHERE mrid=?', [mrid], (err: any, row: any) => {
...
export const insertAssetInfo = async (info: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      IdentifiedObjectFunc.insertIdentifiedObjectTransaction(info, db)
        .then((identifiedResult: any) => {
          ...
          db.run(
            `INSERT INTO asset_info(
                        ...
                        `,
            [info.mrid, info.manufacturer_type, info.product_asset_model],
            function (err: any) {
              if (err) {
                db.run('ROLLBACK')
        ...
        .catch((err: any) => {
          ...
          return reject({
            success: false,
            err: err,
            message: 'Insert assetInfo transaction failed'
          })
        })
    })
  })
...
export const insertAssetInfoTransaction = (info: any, dbsql: any) => {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Insert assetInfo transaction completed'
          })
        }
      )
    } catch (err) {
...
export const updateAssetInfo = async (mrid: string, info: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      IdentifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, info, db)
        .then((identifiedResult: any) => {
          ...
          db.run(
            `UPDATE asset_info SET
                            ...
                            product_asset_model = ?
                        WHERE mrid = ?`,
            [info.manufacturer_type, info.product_asset_model, mrid],
            function (err: any) {
              if (err) {
                db.run('ROLLBACK')
        ...
        .catch((err: any) => {
          ...
          return reject({
            success: false,
            err: err,
            message: 'Update assetInfo transaction failed'
          })
        })
    })
  })
...
export const updateAssetInfoTransaction = (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update assetInfo transaction completed'
          })
        }
      )
    } catch (err) {
...
export const deleteAssetInfoById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM asset_info WHERE mrid=?', [mrid], function (err: any) {
      if (err) {
...
export const deleteAssetInfoByIdTransaction = (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM asset_info WHERE mrid=?', [mrid], function (err: any) {
      if (err) {
      ...
      IdentifiedObjectFunc.deleteIdentifiedObjectByIdTransaction(mrid, dbsql)
        .then((result: any) => {
          ...
          return resolve({
            success: true,
            data: mrid,
            message: 'Delete assetInfo transaction completed'
          })
        })
        .catch((err: any) => {
          console.log(err)
