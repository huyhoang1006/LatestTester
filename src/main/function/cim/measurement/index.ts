...
export const getMeasurementById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM measurement WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          ...
          message: 'Get measurement by id completed'
        })
      })
    })
  } catch (err) {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: measurement,
            message: 'Insert measurement completed'
          })
        }
      )
    } catch (err) {
...
export const updateMeasurementByIdTransaction = async (
  mrid: string,
  measurement: any,
  dbsql: any
) => {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: measurement,
            message: 'Update measurement completed'
          })
        }
      )
    } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM measurement WHERE mrid=?', [mrid], function (this: any, err: any) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'Measurement not found' })
