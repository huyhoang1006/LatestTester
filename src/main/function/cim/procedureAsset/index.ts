  ...
  return new Promise((resolve, reject) => {
    db.get(
      ...
      (err: Error | null, row: unknown) => {
        ...
        if (!row)
          return resolve({ success: false, data: null, message: 'procedure asset not found' })
...
export const insertProcedureAssetTransaction = async (
  info: { procedure_id: string; asset_id: string },
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
...
export const updateProcedureAssetTransaction = async (
  mrid: string,
  info: { procedure_id: string; asset_id: string },
  dbsql: typeof db
) => {
...
export const deleteProcedureAssetTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM procedure_asset WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) {
