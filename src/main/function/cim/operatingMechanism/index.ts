...
export const insertOperatingMechanismTransaction = async (info: { mrid: string; asset_id?: string | null }, dbsql: any) => {
...
export const updateOperatingMechanismTransaction = async (mrid: string, info: { asset_id?: string | null }, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
...
export const deleteOperatingMechanismTransaction = async (mrid: string, dbsql: any) => {
    return new Promise((resolve, reject) => {
        try {
            dbsql.run('DELETE FROM operating_mechanism WHERE mrid = ?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
                ...
                AssetFunc.deleteAssetByIdTransaction(mrid, dbsql)
                    ...
                    .catch((err2: Error) => reject({ success: false, err: err2, message: 'Delete Asset transaction failed' }))
            })
        } catch (err) {
