  ...
  return new Promise((resolve, reject) => {
    db.get(
      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err, message: 'Get terminalCableInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'TerminalCableInfo not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get terminalCableInfo by id completed'
        })
      }
    )
  })
...
export const getTerminalCableInfoByCableInfoId = async (cableInfoId: string) => {
  try {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM terminal_cable_info WHERE cable_info_id=?',
        [cableInfoId],
        (err: Error | null, row: unknown) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get terminal cable info by cable info id failed'
            })
          if (!row)
            return resolve({ success: false, data: null, message: 'Terminal cable info not found' })
          return resolve({
            success: true,
            data: row,
            message: 'Get terminal cable info by cable info id completed'
          })
        }
      )
    })
