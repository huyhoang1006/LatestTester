...
export const getResistanceById = async (mrid: string) => {
  try {
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM resistance WHERE mrid=?`, [mrid], (err: Error | null, row: unknown) => {
...
export const deleteResistanceByIdTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM resistance WHERE mrid=?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) return reject({ success: false, err, message: 'Delete resistance failed' })
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'Resistance not found' })
