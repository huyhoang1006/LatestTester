  ...
  return new Promise((resolve, reject) => {
    db.get(
      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({ success: false, err, message: 'Get breakerOtherInfo by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'BreakerOtherInfo not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get breakerOtherInfo by id completed'
        })
      }
    )
  })
      ...
      (err: Error | null, rows: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get breakerOtherInfo by breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get breakerOtherInfo by breaker_info_id completed'
        })
      }
    )
  })
...
export const deleteBreakerOtherInfoTransaction = async (mrid: string, dbsql: typeof db) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM breaker_other_info WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
