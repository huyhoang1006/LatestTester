...
export const insertValueToAliasTransaction = async (valueToAlias: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.insertIdentifiedObjectTransaction(valueToAlias, dbsql)
...
export const updateValueToAliasByIdTransaction = async (mrid: string, valueToAlias: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, valueToAlias, dbsql)
            if (!idObjResult.success) {
