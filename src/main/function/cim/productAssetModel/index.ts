...
export const insertProductAssetModelTransaction = (model: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const identifiedResult: any = await IdentifiedObjectFunc.insertIdentifiedObjectTransaction(model, dbsql)
            if (!identifiedResult.success) {
                return reject({ success: false, message: 'Insert identified object failed', err: identifiedResult.err })
            }
            dbsql.run(
...
export const updateProductAssetModelTransaction = (mrid: string, model: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const identifiedResult: any = await IdentifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, model, dbsql)
            if (!identifiedResult.success) {
                return reject({ success: false, message: 'Update identified object failed', err: identifiedResult.err })
            }
            dbsql.run(
