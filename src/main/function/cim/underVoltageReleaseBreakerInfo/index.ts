...
export const getUnderVoltageReleaseBreakerInfoByAssessmentLimitId = async (assessmentLimitId: string) => {
    return new Promise((resolve, reject) => {
        db.all(
...
export const deleteUnderVoltageReleaseBreakerInfoTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM under_voltage_release_breaker_info WHERE mrid = ?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
