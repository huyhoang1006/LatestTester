...
export const insertIOPointTransaction = async (iopoint: { mrid: string; iopoint_source: string | null }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.insertIdentifiedObjectTransaction(iopoint, dbsql)
...
export const updateIOPointByIdTransaction = async (mrid: string, iopoint: { iopoint_source: string | null }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const idObjResult: any = await identifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, iopoint, dbsql)
            if (!idObjResult.success) {
...
export const deleteIOPointByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM iopoint WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
