...
export const insertSeconds = async (seconds: { mrid: string; multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
...
export const updateSecondsById = async (mrid: string, seconds: { multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE seconds
...
export const deleteSecondsById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM seconds WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteSecondsByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM seconds WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete seconds failed' })
            if (this.changes === 0) return resolve({ success: false, data: null, message: 'Seconds not found' })
            return resolve({ success: true, data: null, message: 'Delete seconds completed' })
        })
