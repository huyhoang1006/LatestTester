...
export const getAssessmentById = async (mrid: string) => {
  return new Promise<any>((resolve, reject) => {
    db.get(`SELECT * FROM assessment WHERE mrid=?`, [mrid], (err, row) => {
...
// Thêm mới assessment
export const insertAssessmentTransaction = async (data: any, dbsql: any) => {
  // 2. upsert assessment
  await runPromise(
    dbsql as any,
...
// Xóa assessment
export const deleteAssessmentByIdTransaction = async (mrid: string, dbsql: any) => {
  // optional → không cần check
  await runPromise(dbsql as any, 'DELETE FROM assessment WHERE mrid=?', [mrid])
