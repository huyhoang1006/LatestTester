...
export const getApparentPowerById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM apparent_power WHERE mrid=?', [mrid], (err: Error | null, row: unknown) => {
...
export const insertApparentPower = async (apparentPower: { mrid: string; multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
...
export const insertApparentPowerTransaction = async (
    apparentPower: { mrid: string; multiplier: string | null; unit: string | null; value: number | null },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const updateApparentPowerById = async (mrid: string, apparentPower: { multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
...
export const deleteApparentPowerById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM apparent_power WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteApparentPowerByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM apparent_power WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete apparent power failed' })
            if (this.changes === 0) return resolve({ success: false, data: null, message: 'Apparent power not found' })
            return resolve({ success: true, data: null, message: 'Delete apparent power completed' })
        })
