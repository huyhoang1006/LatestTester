...
export const getAnalogValueByTestDataSetMrids = async (mrids: string[]) => {
    return new Promise((resolve, reject) => {
        if (!mrids || mrids.length === 0) {
            return resolve({ success: true, data: [], message: 'Get analogValue by testDataSet mrids completed' })
        }
        ...
        db.all(sql, mrids, (err: Error | null, rows: unknown[]) => {
            if (err) return reject({ success: false, err, message: 'Get analogValue by testDataSet mrids failed' })
            return resolve({ success: true, data: rows, message: 'Get analogValue by testDataSet mrids completed' })
        })
    })
}
...
export const insertAnalogValueTransaction = async (analogValue: any, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const mvResult: any = await measurementValueFunc.insertMeasurementValueTransaction(analogValue, dbsql)
            if (!mvResult.success) {
...
export const updateAnalogValueByIdTransaction = async (mrid: string, analogValue: any, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const mvResult: any = await measurementValueFunc.updateMeasurementValueByIdTransaction(mrid, analogValue, dbsql)
            if (!mvResult.success) {
...
export const deleteAnalogValueByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM analog_value WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
