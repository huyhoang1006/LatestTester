      ...
      (err: Error | null, row: unknown) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get operatingMechanismComponent by id failed'
          })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'OperatingMechanismComponent not found'
          })
        return resolve({
          success: true,
          data: row,
          message: 'Get operatingMechanismComponent by id completed'
        })
      }
    )
  })
...
export const getOperatingMechanismComponentByOperatingMechanismId = async (
  operatingMechanismId: string
) => {
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err: Error | null, rows: unknown[]) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get operatingMechanismComponent by operating_mechanism_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get operatingMechanismComponent by operating_mechanism_id completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Insert operatingMechanismComponent failed'
          })
        return resolve({
          success: true,
          data: info,
          message: 'Insert operatingMechanismComponent completed'
        })
      }
    )
  })
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Update operatingMechanismComponent failed'
          })
        return resolve({
          success: true,
          data: info,
          message: 'Update operatingMechanismComponent completed'
        })
      }
    )
  })
...
export const deleteOperatingMechanismComponentTransaction = async (
  mrid: string,
  dbsql: typeof db
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      'DELETE FROM operating_mechanism_component WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Delete operatingMechanismComponent failed'
          })
        return resolve({
          success: true,
          data: mrid,
          message: 'Delete operatingMechanismComponent completed'
        })
      }
    )
  })
