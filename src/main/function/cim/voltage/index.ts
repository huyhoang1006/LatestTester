    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(
            ...
            (err: Error | null, rows: unknown[]) => {
                ...
                if (!rows || rows.length === 0) return resolve({ success: false, data: [], message: 'Voltages not found' })
...
export const insertVoltage = async (voltage: { mrid: string; multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
...
export const updateVoltageById = async (mrid: string, voltage: { multiplier: string | null; unit: string | null; value: number | null }) => {
    return new Promise((resolve, reject) => {
        db.run(
            `UPDATE voltage
...
export const deleteVoltageById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM voltage WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteVoltageByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM voltage WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete voltage failed' })
            if (this.changes === 0) return resolve({ success: false, data: null, message: 'Voltage not found' })
            return resolve({ success: true, data: null, message: 'Delete voltage completed' })
        })
