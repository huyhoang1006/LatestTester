...
export const updateMeasurementValueByIdTransaction = async (mrid: string, measurementValue: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const ioResult: any = await iopointFunc.updateIOPointByIdTransaction(mrid, measurementValue, dbsql)
            if (!ioResult.success) {
...
export const deleteMeasurementValueByIdTransaction = async (mrid: string, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run("DELETE FROM measurement_value WHERE mrid=?", [mrid], function (this: any, err: any) {
