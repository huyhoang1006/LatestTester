...
export const getActivePowerById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM active_power WHERE mrid = ?',
      [mrid],
    ...
    db.all(
      ...
      (err: Error | null, rows: unknown[]) => {
        ...
        if (!rows || rows.length === 0)
          return resolve({ success: false, data: [], message: 'ActivePower not found' })
    ...
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: activePower,
          message: 'Insert activePower completed'
        })
      }
    )
  })
...
export const updateActivePowerById = async (
  mrid: string,
    ...
    db.run(
      ...
      function (this: { lastID: number; changes: number }, err: Error | null) {
        ...
        return resolve({
          success: true,
          data: activePower,
          message: 'Update activePower completed'
        })
      }
    )
  })
...
export const deleteActivePowerById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.run(
      'DELETE FROM active_power WHERE mrid = ?',
      [mrid],
      function (this: { lastID: number; changes: number }, err: Error | null) {
        if (err) return reject({ success: false, err, message: 'Delete activePower failed' })
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'ActivePower not found' })
