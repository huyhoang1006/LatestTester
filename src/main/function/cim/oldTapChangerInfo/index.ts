...
export const getOldTapChangerInfoById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_tap_changer_info WHERE mrid=?`, [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, err, message: 'Get oldTapChangerInfo by id failed' })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'OldTapChangerInfo specific data not found'
          })

        return resolve({
          success: true,
          data: { ...parentResult.data, ...row },
          message: 'Get oldTapChangerInfo by id completed'
        })
      })
    })
  } catch (err) {
...
export const getOldTapChangerInfoByPowerTransformerInfoId = async (
  ...
  try {
    return new Promise((resolve, reject) => {
      db.get(
        ...
        async (err: any, row: any) => {
          if (err)
            return reject({ success: false, err, message: 'Find OldTapChangerInfo mrid failed' })

          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'OldTapChangerInfo not found for this transformer'
            })
    ...
    return {
      success: false,
      err,
      message: 'Get OldTapChangerInfo by PowerTransformerInfoId failed'
    }
  }
}
...
export const insertOldTapChangerInfoTransaction = async (info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentResult: any = await TapChangerInfoFunc.insertTapChangerInfoTransaction(
        info,
        dbsql
      )
      if (!parentResult.success) {
        return reject({
          success: false,
          message: 'Insert TapChangerInfo failed',
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Insert oldTapChangerInfo completed'
          })
        }
      )
    } catch (err) {
...
export const updateOldTapChangerInfoTransaction = async (mrid: string, info: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentResult: any = await TapChangerInfoFunc.updateTapChangerInfoTransaction(
        mrid,
        info,
        dbsql
      )
      if (!parentResult.success) {
        return reject({
          success: false,
          message: 'Update TapChangerInfo failed',
          err: parentResult.err
        })
      }
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update oldTapChangerInfo completed'
          })
        }
      )
    } catch (err) {
    ...
    try {
      await new Promise((res, rej) => {
        dbsql.run('DELETE FROM old_tap_changer_info WHERE mrid=?', [mrid], function (err: any) {
          if (err)
            return rej({ success: false, err, message: 'Delete oldTapChangerInfo table failed' })
          res(undefined)
        })
      })

      const parentResult: any = await TapChangerInfoFunc.deleteTapChangerInfoTransaction(
        mrid,
        dbsql
      )
      ...
      if (!parentResult.success) {
        return reject({
          success: false,
          message: 'Delete TapChangerInfo parent failed',
          err: parentResult.err
        })
      }
