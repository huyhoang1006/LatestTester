...
export const getDissipationFactorCapacitorInfoById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM dissipation_factor_capacitor_info WHERE mrid=?', [mrid], (err: Error | null, row: unknown) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.all(
            ...
            (err: Error | null, rows: unknown[]) => {
                if (err) return reject({ success: false, err: err, message: 'Get dissipationFactorCapacitorInfo by ids failed' })
                if (!rows || rows.length === 0) return resolve({ success: false, data: [], message: 'DissipationFactorCapacitorInfo not found' })
...
export const insertDissipationFactorCapacitorInfoTransaction = async (
    dissipationFactorCapacitorInfo: { mrid: string; phase: string | null; value: number | null; capacitor_info_id: string | null },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
...
export const updateDissipationFactorCapacitorInfoTransaction = async (
    mrid: string,
    dissipationFactorCapacitorInfo: { phase: string | null; value: number | null; capacitor_info_id: string | null },
    dbsql: typeof db
) => {
    return new Promise((resolve, reject) => {
        dbsql.run(
            `UPDATE dissipation_factor_capacitor_info SET
                ...
                capacitor_info_id = ?
             WHERE mrid = ?`,
            [dissipationFactorCapacitorInfo.phase, dissipationFactorCapacitorInfo.value, dissipationFactorCapacitorInfo.capacitor_info_id, mrid],
            function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteDissipationFactorCapacitorInfoTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM dissipation_factor_capacitor_info WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteDissipationFactorCapacitorInfoById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.run('DELETE FROM dissipation_factor_capacitor_info WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) return reject({ success: false, err, message: 'Delete dissipationFactorCapacitorInfo failed' })
            if (this.changes === 0) return resolve({ success: false, data: null, message: 'DissipationFactorCapacitorInfo not found' })
            return resolve({ success: true, data: null, message: 'Delete dissipationFactorCapacitorInfo completed' })
        })
