...
export const insertLocationTransaction = async (location: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const identifiedResult: any = await IdentifiedObjectFunc.insertIdentifiedObjectTransaction(location, dbsql)
            if (!identifiedResult.success) {
                return reject({ success: false, message: 'Insert identified object failed', err: identifiedResult.err })
            }
...
export const updateLocationTransaction = async (mrid: string, location: any, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
            const identifiedResult: any = await IdentifiedObjectFunc.updateIdentifiedObjectByIdTransaction(mrid, location, dbsql)
            if (!identifiedResult.success) {
                return reject({ success: false, message: 'Update identified object failed', err: identifiedResult.err })
            }
