...
export const insertShortCircuitRatingTransaction = async (scr: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      [scr.mrid, scr.power_transformer_info_id, scr.short_circuit_current, scr.duration_seconds],
      function (err: any) {
        if (err) {
          return reject({
            success: false,
...
export const updateShortCircuitRatingByIdTransaction = async (
  mrid: string,
  scr: any,
  dbsql: any
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      `UPDATE short_circuit_rating SET
                ...
                duration_seconds = ?
            WHERE mrid = ?`,
      [scr.power_transformer_info_id, scr.short_circuit_current, scr.duration_seconds, mrid],
      function (err: any) {
        if (err) {
          return reject({
            success: false,
...
export const getShortCircuitRatingById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM short_circuit_rating WHERE mrid = ?`, [mrid], (err: any, row: any) => {
      if (err) {
        return reject({
          success: false,
          err,
...
export const getShortCircuitRatingByPowerTransformerInfoId = async (
