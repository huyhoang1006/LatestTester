...
export const insertBreakerInfoTransaction = async (info: { mrid: string; phase_trip: string | null }, dbsql: any) => {
...
export const updateBreakerInfoTransaction = async (mrid: string, info: { phase_trip: string | null }, dbsql: any) => {
    return new Promise(async (resolve, reject) => {
        try {
...
export const deleteBreakerInfoTransaction = async (mrid: string, dbsql: any) => {
    return new Promise((resolve, reject) => {
        try {
            dbsql.run('DELETE FROM breaker_info WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
