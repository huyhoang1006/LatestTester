...
export const insertSurgeArresterWorkTaskTransaction = async (
    surgeArresterWorkTask: { mrid: string; test_type_surge_arrester_id: string | null; test_standard_id: string | null },
    dbsql: typeof db
) => {
    return new Promise(async (resolve, reject) => {
        try {
            const workTaskResult: any = await workTaskFunc.insertWorkTaskTransaction(surgeArresterWorkTask, dbsql)
            ...
            dbsql.run(
                ...
                `,
                [surgeArresterWorkTask.mrid, surgeArresterWorkTask.test_type_surge_arrester_id, surgeArresterWorkTask.test_standard_id],
...
export const updateSurgeArresterWorkTaskByIdTransaction = async (
    mrid: string,
    surgeArresterWorkTask: { test_type_surge_arrester_id: string | null; test_standard_id: string | null },
    dbsql: typeof db
) => {
    return new Promise(async (resolve, reject) => {
        try {
            const workTaskResult: any = await workTaskFunc.updateWorkTaskByIdTransaction(mrid, surgeArresterWorkTask, dbsql)
            if (!workTaskResult.success) {
            ...
            dbsql.run(
                `UPDATE surge_arrester_work_task SET
                    ...
                    test_standard_id = ?
                WHERE mrid = ?`,
                [surgeArresterWorkTask.test_type_surge_arrester_id, surgeArresterWorkTask.test_standard_id, mrid],
                function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteSurgeArresterWorkTaskByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM surge_arrester_work_task WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
