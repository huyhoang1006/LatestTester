...
export const initLicenseLimitation = () => {
    ...
    db.serialize(() => {
        defaultLicenses.forEach((item) => {
            ...
            db.get('SELECT number_of_liscense FROM limitation_license WHERE mrid = ?', [fixedMrid], (err: Error | null, row: any) => {
                if (err) return
...
export const checkLicenseLimitation = (name: string) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT number_of_liscense FROM limitation_license WHERE name = ?', [name], (err: Error | null, row: any) => {
