    ...
    return new Promise((resolve, reject) => {
        db.get(
            ...
            (err: Error | null, row: unknown) => {
                if (err)
                    return reject({ success: false, err, message: 'Get reconditioningCoolingPowerRating by id failed' })
                if (!row)
                    return resolve({ success: false, data: null, message: 'ReconditioningCoolingPowerRating not found' })
...
export const deleteReconditioningCoolingPowerRatingTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise((resolve, reject) => {
        dbsql.run('DELETE FROM reconditioning_cooling_power_rating WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
            if (err) {
