...
export const insertSCTTransformerEndInfoTransaction = async (sctEndInfo: any, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      [sctEndInfo.mrid, sctEndInfo.short_circuit_test_id, sctEndInfo.transformer_end_info_id],
      function (err: any) {
        if (err) {
          return reject({
            success: false,
...
export const updateSCTTransformerEndInfoByIdTransaction = async (
  mrid: string,
  sctEndInfo: any,
  dbsql: any
) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      `UPDATE short_circuit_test_transformer_end_info SET
                ...
                transformer_end_info_id = ?
            WHERE mrid = ?`,
      [sctEndInfo.short_circuit_test_id, sctEndInfo.transformer_end_info_id, mrid],
      function (err: any) {
        if (err) {
          return reject({
            success: false,
...
export const deleteSCTTransformerEndInfoByIdTransaction = async (mrid: string, dbsql: any) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      `DELETE FROM short_circuit_test_transformer_end_info WHERE mrid = ?`,
      [mrid],
      function (err: any) {
        if (err) {
          return reject({
