...
export const getPowerCableById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM power_cable WHERE mrid=?', [mrid], (err: any, row: any) => {
        if (err)
          return reject({ success: false, err: err, message: 'Get power cable by id failed' })
...
export const insertPowerCable = async (cable: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
          ...
          db.run(
            ...
            function (err: any) {
              ...
              return resolve({
                success: true,
                data: cable,
                message: 'Insert power cable completed'
              })
            }
          )
        })
...
export const insertPowerCableTransaction = async (cable: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const assetResult: any = await AssetFunc.insertAssetTransaction(cable, db)
      if (!assetResult.success) {
        return reject({ success: false, message: 'Insert asset failed', err: assetResult.err })
      }
      dbsql.run(
      ...
      return reject({
        success: false,
        err: error,
        message: 'Insert power cable transaction failed'
      })
    }
  })
}

export const updatePowerCable = async (mrid: string, cable: any) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
...
export const updatePowerCableTransaction = async (mrid: string, cable: any, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      const assetResult: any = await AssetFunc.updateAssetTransaction(mrid, cable, dbsql)
      if (!assetResult.success) {
        db.run('ROLLBACK')
        return reject({ success: false, message: 'Update asset failed', err: assetResult.err })
      }
      ...
      return reject({
        success: false,
        err: error,
        message: 'Update power cable transaction failed'
      })
    }
  })
}

export const deletePowerCableById = async (mrid: string) => {
  return new Promise((resolve, reject) => {
    AssetFunc.deleteAssetById(mrid)
      .then((result: any) => {
        ...
        db.run('DELETE FROM power_cable WHERE mrid=?', [mrid], function (err: any) {
          if (err) {
      ...
      .catch((err: any) => {
        return reject({
          success: false,
          err: err,
          message: 'Delete power cable transaction failed'
        })
      })
  })
}

export const deletePowerCableTransaction = async (mrid: string, dbsql: any) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM power_cable WHERE mrid=?', [mrid], function (err: any) {
        if (err) {
          return reject({ success: false, err: err, message: 'Delete power cable failed' })
        }
...
export const getPowerCableByPsrId = (psrId: string) => {
  return new Promise((resolve, reject) => {
    const query = `
            SELECT 
                pc.*, 
