...
export const getIOPointSourceById = async (mrid: string) => {
    try {
        const measurementValueSource: any = await measurementValueSourceFunc.getMeasurementValueSourceById(mrid)
...
export const insertIOPointSourceTransaction = async (iopointSource: { mrid: string }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const mvsResult: any = await measurementValueSourceFunc.insertMeasurementValueSourceTransaction(iopointSource, dbsql)
...
export const updateIOPointSourceByIdTransaction = async (mrid: string, iopointSource: { mrid: string }, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const mvsResult: any = await measurementValueSourceFunc.updateMeasurementValueSourceByIdTransaction(mrid, iopointSource, dbsql)
            if (!mvsResult.success) {
...
export const deleteIOPointSourceByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM iopoint_source WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
