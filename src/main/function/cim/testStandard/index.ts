...
export const getTestStandardById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM test_standard WHERE mrid=?`, [mrid], (err: any, row: any) => {
        ...
        return resolve({
          ...
          message: 'Get testStandard by id completed'
        })
      })
    })
  } catch (err) {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: testStandard,
            message: 'Insert testStandard completed'
          })
        }
      )
    } catch (err) {
...
export const updateTestStandardByIdTransaction = async (
  mrid: string,
  testStandard: any,
  dbsql: any
) => {
      ...
      dbsql.run(
        ...
        function (err: any) {
          ...
          return resolve({
            success: true,
            data: testStandard,
            message: 'Update testStandard completed'
          })
        }
      )
    } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM test_standard WHERE mrid=?', [mrid], function (this: any, err: any) {
        ...
        if (this.changes === 0)
          return resolve({ success: false, data: null, message: 'TestStandard not found' })
