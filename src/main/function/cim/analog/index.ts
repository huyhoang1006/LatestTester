...
export const getAnalogById = async (mrid: string) => {
    try {
...
export const getAllAnalogByProcedureIds = (procedureIds: string[]) => {
    try {
        return new Promise((resolve, reject) => {
            if (!procedureIds || procedureIds.length === 0) {
                return resolve({ success: true, data: [], message: 'No procedureIds provided' })
            }
            ...
            db.all(sql, procedureIds, (err: Error | null, rows: unknown[]) => {
                if (err) return reject({ success: false, err, message: 'Get all analog by procedures failed' })
                if (!rows || rows.length === 0) {
                    return resolve({ success: false, data: [], message: 'No analog found for these procedures' })
                }
                return resolve({ success: true, data: rows, message: 'Get all analog by procedures completed' })
            })
        })
    } catch (err) {
...
export const getAllAnalogByProcedure = (procedureId: string) => {
    try {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT a.*, m.*, io.*
            ...
            `
            db.all(sql, [procedureId], (err: Error | null, rows: unknown[]) => {
                if (err) return reject({ success: false, err, message: 'Get all analog by procedure failed' })
                if (!rows || rows.length === 0) {
                    return resolve({ success: false, data: [], message: 'No analog found' })
                }
                return resolve({ success: true, data: rows, message: 'Get all analog by procedure completed' })
            })
        })
    } catch (err) {
...
export const insertAnalogTransaction = async (analog: any, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const measResult: any = await measurementFunc.insertMeasurementTransaction(analog, dbsql)
...
export const insertAnalog = async (analog: any) => {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            db.run('BEGIN TRANSACTION', async (err: Error | null) => {
                ...
                try {
                    const measResult: any = await measurementFunc.insertMeasurementTransaction(analog, db)
                    if (!measResult.success) {
                        db.run('ROLLBACK')
                        return reject({ success: false, message: 'Insert analog measurement failed', err: measResult.err })
                    }
...
export const updateAnalogByIdTransaction = async (mrid: string, analog: any, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            const measResult: any = await measurementFunc.updateMeasurementByIdTransaction(mrid, analog, dbsql)
            if (!measResult.success) {
...
export const deleteAnalogByIdTransaction = async (mrid: string, dbsql: typeof db) => {
    return new Promise(async (resolve, reject) => {
        try {
            dbsql.run('DELETE FROM analog WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
...
export const deleteAnalogById = async (mrid: string) => {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            db.run('BEGIN TRANSACTION', async (err: Error | null) => {
                if (err) {
                ...
                try {
                    db.run('DELETE FROM analog WHERE mrid=?', [mrid], function (this: { lastID: number; changes: number }, err: Error | null) {
                        if (err) {
                            db.run('ROLLBACK')
                            return reject({ success: false, err, message: 'Delete analog failed' })
                        }
                        ...
                        measurementFunc
                            ...
                            .then(() => {
                                db.run('COMMIT', (commitErr: Error | null) => {
                                    ...
                                    return resolve({ success: true, data: null, message: 'Delete analog & measurement completed' })
                                })
                            })
                            .catch((measErr: Error) => {
                                db.run('ROLLBACK')
                                return reject({ success: false, err: measErr, message: 'Delete measurement failed' })
                            })
                    })
                } catch (err) {
                    db.run('ROLLBACK')
