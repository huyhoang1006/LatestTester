...
export const getAnalogById = async (mrid: string) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM analog WHERE mrid=?`, [mrid], (err: Error | null, row: any) => {
        ...
        return resolve({
          success: true,
          data: { ...measurement.data, ...row },
          message: 'Get analog by id completed'
        })
      })
    })
  } catch (err) {
  ...
  try {
    return new Promise((resolve, reject) => {
      ...
      db.all(sql, procedureIds, (err: Error | null, rows: unknown[]) => {
        if (err)
          return reject({ success: false, err, message: 'Get all analog by procedures failed' })
  ...
  try {
    return new Promise((resolve, reject) => {
      ...
      db.all(sql, [procedureId], (err: Error | null, rows: unknown[]) => {
        if (err)
          return reject({ success: false, err, message: 'Get all analog by procedure failed' })
        if (!rows || rows.length === 0) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!measResult.success) {
        return reject({
          success: false,
          message: 'Insert analog measurement failed',
          err: measResult.err
        })
      }
      dbsql.run(
...
export const updateAnalogByIdTransaction = async (mrid: string, analog: any, dbsql: typeof db) => {
  return new Promise(async (resolve, reject) => {
    try {
      const measResult: any = await measurementFunc.updateMeasurementByIdTransaction(
        mrid,
        analog,
        dbsql
      )
