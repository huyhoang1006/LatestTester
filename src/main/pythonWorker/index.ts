...
export function startPythonWorker(): void {
    ...
    console.log(`[PythonWorker] Starting Python worker from: ${exePath}`)
    ...
    rl.on('line', (line: string) => {
        if (line === 'PYTHON_READY') {
            console.log('[PythonWorker] Python engine ready!')
            return
        }
    ...
    importerProcess.on('exit', (code) => {
        console.log(`[PythonWorker] Process exited with code: ${code}`)
    })
}

export function stopPythonWorker(): void {
    if (importerProcess) {
        console.log('[PythonWorker] Stopping Python worker...')
...
export function convertFiles(filePaths: string[], fileType: string): Promise<unknown>[] {
    if (!importerProcess) {
        ...
        return filePaths.map(() => Promise.resolve({ success: false, message: 'Python worker not available' }))
    }
