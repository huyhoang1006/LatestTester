...
export const initializeDatabaseFromSQL = async (dbsql: SqlCipherDatabase = db as any): Promise<void> => {
    try {
        console.log('[DB Init] ⏳ Starting database initialization from SQL file...')
...
export const setDbVersion = async (dbsql: SqlCipherDatabase = db as any, version: number): Promise<void> => {
    return new Promise((resolve, reject) => {
...
export const updateDatabaseFromSQL = async (
    ...
    console.log(`[DB Init] updateDatabaseFromSQL: ${oldVersion} -> ${newVersion} (no migrations defined)`)
}
