...
export const createOrganisationRoot = async (dbsql: any = db): Promise<{ success: boolean; data?: any; message?: string; err?: any }> => {
    ...
    try {
        ...
        const result: any = await parentOrganisationFunc.insertParentOrganizationTransaction(parentOrganisation, dbsql)
        if (!result.success) {
            await runAsync('ROLLBACK')
            throw new Error(result.err && result.err.message ? result.err.message : 'Insert failed')
        }
        await runAsync('COMMIT')
        console.log('[OrgRoot] ✅ Create organisation root completed')
