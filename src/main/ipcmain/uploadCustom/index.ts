...
// ═══════════════════════════════════════════════════════════════════════════
// JSON IMPORT / EXPORT (template config)
// ═══════════════════════════════════════════════════════════════════════════

export const importJSON = () => {
  ipcMain.handle('importJSON', async function (_event) {
    const rs = await dialog.showOpenDialog({
      title: 'Select template config JSON',
      buttonLabel: 'Import',
      filters: [{ name: 'json', extensions: ['json'] }],
      properties: ['openFile']
    })
    if (rs.canceled || !rs.filePaths.length) return { success: false, canceled: true }
    try {
      const text = fs.readFileSync(rs.filePaths[0], 'utf8')
      const data = JSON.parse(text)
      return { success: true, data, filePath: rs.filePaths[0] }
    } catch (e: any) {
...
// exportJSON handler is registered in entity/export/index.ts

// ═══════════════════════════════════════════════════════════════════════════
// pickExcelFileForImport, pickWordFileForImport, readExcelForImport, readWordForImport
// handlers are registered in entity/template/index.ts
// readExcelForImport / readWordForImport handlers are registered in entity/template/index.ts

// ═══════════════════════════════════════════════════════════════════════════
// openFileTemplate handler is registered in entity/template/index.ts
// ═══════════════════════════════════════════════════════════════════════════

export const uploadCustom = () => {
  ipcMain.handle('uploadCustom', async function (_event, name) {
    const rs: any = await dialog.showOpenDialog({
...
export const getNameTemplate = () => {
  ipcMain.handle('getNameTemplate', async function (_event) {
    try {
...
export const uploadReport = () => {
  ipcMain.handle('uploadReport', async function (_event, name, asset, location, job, user_id) {
    const rs: any = await dialog.showOpenDialog({
      title: 'Select the file to be uploaded',
      buttonLabel: 'Import',
      properties: ['openFile']
...
export const deleteTempByName = () => {
  ipcMain.handle('deleteTempByName', async function (_event, name) {
    try {
...
export const updateTempByName = () => {
  ipcMain.handle('updateTempByName', async function (_event, data) {
    try {
...
export const getColumnByName = () => {
  ipcMain.handle('getColumnByName', async function (_event, name) {
    try {
...
export const saveTemplate = () => {
  ipcMain.handle('saveTemplate', async function (_event, data) {
    try {
      let pathParse = data.path.split('.')
      if (pathParse[pathParse.length - 1] == 'xlsx') {
        data.var = await locationUploadFunc.handleDataLocation(data.path, data.var)
...
export const checkNameExist = () => {
  ipcMain.handle('checkNameExist', async function (_event, name) {
    try {
...
export const getVariableFromJson = () => {
  ipcMain.handle('getVariableFromJson', async function (_event) {
    const rs: any = await dialog.showOpenDialog({
      title: 'Select the file to be uploaded',
      buttonLabel: 'Upload',
      filters: [
...
export const exportVariableToJon = () => {
  ipcMain.handle('exportVariableToJon', async function (_event, data) {
    const rs: any = await dialog.showSaveDialog({
      ...
      if (rs != '') {
        if (!rs.canceled) {
          if (file.typeExport == 'EXCEL') {
            ...
            file.var = variable
            await uploadFunc.exportReport(
              file,
              rs.filePath.toString(),
              location,
              assetType,
              asset,
              job,
