...
export const getComparableTestsHandler = () => {
  ipcMain.handle(
    'getComparableTests',
    async function (event, { assetMrid, testCode, excludeWorkMrid }) {
      try {
        ...
        return {
          success: false,
          data: [],
          message: (error && error.message) || 'Get comparable tests failed'
        }
      }
    }
  )
  ...
  ipcMain.handle('getTestSnapshot', async function (event, workTaskMrid) {
    try {
      ...
      return {
        success: false,
        data: null,
        message: (error && error.message) || 'Get test snapshot failed'
      }
    }
  })
}
