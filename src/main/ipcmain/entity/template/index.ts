    ...
    fs.readdirSync(bundledDir).forEach((file) => {
      ...
      if (!fs.existsSync(dest)) {
        try {
          ...
          console.warn('Cannot copy bundled template:', file, e.message)
        }
      }
    })
  }
...
function classifyVariables(variables: any[]): Record<string, any> {
  ...
  for (const v of variables) {
    ...
    const rows = [...new Set(coords.map((c: any) => c.rowIndex))]
    const cols = [...new Set(coords.map((c: any) => c.colIndex))]
    let type
...
function buildVerticalGroups(classified: Record<string, any>): any[] {
  const vertCodes: any[] = Object.entries(classified).filter(
    ([, v]: [string, any]) => v.type === 'vertical'
  )
  const groups: any[] = []
...
function normalizeSplitRuns(xml) {
  ...
  for (const r of (replacements as any[]).sort((a, b) => b.start - a.start)) {
...
function parseDocxContent(buffer: Buffer): { cells: any[]; paras: any[] } {
  const PizZip = require('pizzip')
  const zip = new PizZip(buffer)
  const xml = zip.file('word/document.xml').asText()

  // Nối tất cả <w:t> text trong fragment (kể cả text bị split nhiều run)
  function getText(fragment: string) {
  ...
  const cells: any[] = [] // [{ table, row, cell, text }]
  const paras: any[] = [] // [{ para, text }]
...
export const getAllTemplates = () => {
  ipcMain.handle('getAllTemplates', async function (_event) {
    try {
...
export const getAllTemplatesByType = () => {
  ipcMain.handle('getAllTemplatesByType', async function (_event, type, category) {
    try {
...
export const getTemplateByName = () => {
  ipcMain.handle('getTemplateByName', async function (_event, name) {
    try {
...
export const insertTemplate = () => {
  ipcMain.handle('insertTemplate', async function (_event, data) {
    try {
...
export const updateTemplate = () => {
  ipcMain.handle('updateTemplate', async function (_event, data) {
    try {
...
export const deleteTemplate = () => {
  ipcMain.handle('deleteTemplate', async function (_event, name) {
    try {
...
export const checkNameTemplateExist = () => {
  ipcMain.handle('checkNameTemplateExist', async function (_event, name) {
    try {
      const rs = await entityFunc.templateFunc.getTemplateByName(name)
...
export const uploadExcelTemplate = () => {
  ipcMain.handle('uploadExcelTemplate', async function (_event, templateName) {
    try {
...
export const uploadWordTemplate = () => {
  ipcMain.handle('uploadWordTemplate', async function (_event, templateName) {
    try {
      const result = await dialog.showOpenDialog({
        title: 'Select Excel Template',
...
export const scanTemplateCoordinates = () => {
  ipcMain.handle('scanTemplateCoordinates', async function (_event, { filePath, codes }) {
    try {
...
export const saveTemplateWithScan = () => {
  ipcMain.handle(
    ...
    async function (_event, { name, filePath, variables, type, category }) {
      try {
        if (!filePath || !fs.existsSync(filePath)) {
...
export const exportTemplateWithData = () => {
  ipcMain.handle('exportTemplateWithData', async function (_event, payload) {
    try {
      const {
        templatePath,
        variables,
      ...
      function fillWorkbookSheet(wb, vars, codeMap) {
        ...
        for (const sheetName in actionsBySheet) {
          ...
          let currentOffset = 0
          const originalRows = Object.keys(actionsBySheet[sheetName])
            .map(Number)
            .sort((a, b) => a - b)
      ...
      if (selections.length === 1) {
        ...
        if (s && sheetName && sheetName !== s.name()) {
          try {
            s.name(sanitizeSheetName(sheetName, 0))
          } catch (e: any) {}
        }
        fillWorkbookSheet(wb, variables, codeMap)

        const { canceled, filePath } = await new Promise<any>((resolve) =>
          require('electron')
      ...
      const firstSheetSafeName = filledBuffers[0].sheetName
        ...
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
      wbXml = wbXml.replace(/(<sheet[^>]*name=")[^"]*(")/, function (_match, p1, p2) {
        return p1 + firstSheetSafeName + p2
      })
      ...
      const combinedBuffer = await baseZip.generateAsync({
        type: 'nodebuffer',
        compression: 'DEFLATE',
...
export const exportWordWithData = () => {
  ipcMain.handle(
    'exportWordWithData',
    async function (_event, { templatePath, codeMap, variables }) {
      try {
        if (!templatePath || !fs.existsSync(templatePath)) {
          return { success: false, message: 'Template file not found' }
        }
        ...
        const _vertCodes = new Set((vertGroups as any[]).flatMap((g: any) => g.codes))
        void _vertCodes
        ...
        for (const [code, info] of Object.entries(classified)) {
          ...
          const _sortedCoords = [...info.coords].sort((a, b) => a.colIndex - b.colIndex)
          void _sortedCoords
          let idx = 0
          const ph = `{${code}}`
        ...
        for (const group of vertGroups as any[]) {
          ...
          if (maxLen === 0) {
            ...
            group.codes.forEach((code) => {
              emptyRow = emptyRow.split(`{${code}}`).join('')
            })
        ...
        const _dataKeys = Object.keys(dataForWord)
        void _dataKeys
...
export const pickExcelFileForImport = () => {
  ipcMain.handle('pickExcelFileForImport', async function (_event) {
    try {
      const result = await dialog.showOpenDialog({
...
export const pickWordFileForImport = () => {
  ipcMain.handle('pickWordFileForImport', async function (_event) {
    try {
      const result = await dialog.showOpenDialog({
...
export const readExcelForImport = () => {
  ipcMain.handle(
    'readExcelForImport',
    async function (_event, { filePath, templatePath, variables }) {
      try {
        if (!filePath || !fs.existsSync(filePath)) {
        ...
        if (isCsv) {
          ...
          if (templatePath && fs.existsSync(templatePath)) {
            try {
              ...
              console.warn('Cannot open CSV template:', e.message)
            }
          }
          ...
          if (templatePath && fs.existsSync(templatePath)) {
            try {
              tmpl = await XlsxPopulate.fromFileAsync(templatePath)
            } catch (e: any) {
              console.warn('Cannot open template for prefix detection:', e.message)
            }
          }
      ...
      try {
        ...
        if (templatePath && fs.existsSync(templatePath)) {
          try {
            ...
            console.warn('Cannot parse Word template:', e.message)
          }
        }
        ...
        for (const variable of variables) {
          ...
          for (const coord of coordinates) {
            ...
            if (pos.table !== undefined) {
              ...
              tmplText = tmplCells.length
                ? getCellText(tmplCells, pos.table, pos.row, pos.cell)
                : ''
            } else {
            ...
            if (tmplText && tmplText !== filledText) {
              try {
                extracted = extractValue(tmplText, filledText, code)
              } catch (e: any) {}
            }
            codeValueMap[code].push(extracted.trim())
          }
...
export const openFileTemplate = () => {
  ipcMain.handle('openFileTemplate', async (_event, filePath) => {
