...
export const initApp = () => {
    console.log('[Helper] initApp called')
    const state = store.state

    const serviceAddr = localStorage.getItem('SERVICE_ADDR')
    console.log('[Helper] SERVICE_ADDR from localStorage:', serviceAddr)
    if (serviceAddr) {
        try {
            new URL(serviceAddr)
    ...
    console.log('[Helper] userStr:', userStr ? 'exists' : 'null')
    console.log('[Helper] token:', token ? 'exists' : 'null')
    console.log('[Helper] role:', role)

    if (userStr && token) {
        const userData = JSON.parse(userStr)
        console.log('[Helper] Parsed userData:', userData)
        ...
        console.log('[Helper] Store state after init:', {
            isAuthenticated: store.state.isAuthenticated,
            user: store.state.user,
            token: store.state.token ? 'exists' : 'null'
        })

        setupInterceptor(token)
    } else {
        console.log('[Helper] No auth data found, setting isAuthenticated to false')
