...
const findCellByCode = (cells, code) =>
  Object.values(cells || {}).find((c) => c && c.aliasName === code)
 ...
 */
export const buildSnapshotFromForm = (
  tableData,
  columns,
  conditions,
  keyCodes = [],
...
export const compareSnapshots = (current, reference) => {
  ...
  for (const title of titles) {
    ...
    const valueColumns = columns.filter(
      (col) => !col.isKey && !isExcludedFromCompare(col.aliasName)
    )

    const rows = matchRows(currentRows, referenceRows, hasKey).map((pair, index) => ({
      label:
        (pair.current && pair.current.label) ||
      ...
      cells: valueColumns.map((col) => {
        ...
        return Object.assign(
          { measurementId: col.measurementId, name: col.name, unit: col.unit },
          cell
        )
      })
    }))
  ...
  const conditionDiff = conditionKeys
    .map((key) => {
  ...
  return {
    ...
    summary: {
      comparedCells,
      changedCells,
      onlyCurrent: tables.reduce(
        (n, t) => n + t.rows.filter((r) => r.status === 'onlyCurrent').length,
        0
      ),
      onlyReference: tables.reduce(
        (n, t) => n + t.rows.filter((r) => r.status === 'onlyReference').length,
        0
      )
    }
  }
