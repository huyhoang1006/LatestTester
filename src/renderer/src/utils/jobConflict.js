 ...
 */
const isKeyedList = (arr) => Array.isArray(arr) && arr.every((x) => isObject(x) && x.mrid)
 ...
 */
const isCell = (v) => isObject(v) && 'value' in v && typeof v.type === 'string'
...
function mergeKeyedList(base, client, server, crumbs, conflicts) {
  const byMrid = (list) => {
    ...
    if (Array.isArray(list))
      list.forEach((x) => {
        if (isObject(x) && x.mrid) map.set(x.mrid, x)
      })
    return map
  }
  ...
  server.forEach((x) => {
    if (!clientMap.has(x.mrid)) order.push(x.mrid)
  })
  ...
  for (const mrid of order) {
    ...
    const value = mergeNode(b, c, s, [...crumbs, crumbForRecord(mrid, c, s, out.length)], conflicts)
