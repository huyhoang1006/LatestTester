...
export const uploadAssetMediaFromAttachmentData = async (
  assetTypeLabel,
  assetId,
  attachmentData = [],
  ...
  if (attachmentCount > 0) {
    result.attachments = await AssetMediaAPI.uploadAssetMedia(
      assetType,
      assetId,
      attachmentsFormData
    )
  ...
  if (deleteAttachmentIds.length > 0) {
    result.deletedAttachments = await Promise.all(
      deleteAttachmentIds.map((mediaId) =>
        AssetMediaAPI.deleteAssetMedia(assetType, assetId, mediaId)
      )
    )
  }

  console.log('[asset-media] Uploaded media:', {
    assetTypeLabel,
    assetType,
    assetId,
    hasNameplate,
...
export const downloadAssetMediaToAttachmentData = async (assetTypeLabel, assetId) => {
  ...
  for (const item of items) {
    ...
    try {
      ...
      const canWriteLocal =
        window.electronAPI && typeof window.electronAPI.writeAttachmentFileData === 'function'
      const saved = canWriteLocal
        ? await window.electronAPI.writeAttachmentFileData(
            base64,
            buildLocalMediaName(assetType, assetId, mediaId, name)
          )
        : null
...
const shouldRemoveNameplate = (previousAttachmentData = [], attachmentData = []) => {
  const hadNameplate = (previousAttachmentData || []).some(
    (item) => item && item.role === 'nameplate' && item.serverMediaId
  )
  const hasNameplate = (attachmentData || []).some(
    (item) => item && item.role === 'nameplate' && item.path
  )
