  ...
  <div class="logbar">
    <div class="option-bar">
      ...
      <div class="log-actions">
        <input v-model="filterText" class="filter-input" type="text" placeholder="Filter..." />
        <button
          class="icon-btn"
        ...
        <button
          ...
          :disabled="!logData.length || isDeleting"
          @click="deleteAllLogs"
        >
        ...
        <i
          :class="['fa-solid', 'fa-rotate-right', { spin: isLoadingReload }]"
    ...
    <div class="table-content">
      ...
      <div class="content-log">
        ...
        >
          ...
          <div v-if="isSyncLog(item) && item.severity === 'failed'" class="icon">
            <i
              style="background-color: white; color: #d0021b"
              class="fa-solid fa-circle-xmark"
              :title="item.type + ' failed'"
            ></i>
          </div>
          <div v-else-if="isSyncLog(item)" class="icon">
            <i
              style="background-color: white; color: green"
              :class="
                item.type === 'DOWNLOAD' ? 'fa-solid fa-circle-down' : 'fa-solid fa-circle-up'
              "
              :title="item.type + ' succeeded'"
            ></i>
          </div>
          <div v-else-if="item.type === 'INSERT'" class="icon">
            <i style="color: green; background-color: white" class="fa-solid fa-square-check"></i>
          </div>
          <div v-else-if="item.type === 'UPDATE'" class="icon">
            <i
              style="background-color: white; color: #0b5cad"
              class="fa-solid fa-pen-to-square"
            ></i>
          </div>
          <div v-else-if="item.type === 'ERROR'" class="icon">
            <i style="background-color: white; color: red" class="fa-solid fa-bug"></i>
          </div>
          <div v-else-if="item.type === 'DELETE'" class="icon">
            <i style="background-color: white; color: #d0021b" class="fa-solid fa-trash"></i>
          </div>
      ...
      <div class="detail-panel">
        ...
        <div class="detail-body">
          <div class="detail-grid">
            <div class="detail-row">
              <span>Date & Time</span><strong>{{ activeLog.effective_date_time || '-' }}</strong>
            </div>
            <div class="detail-row">
              <span>Category</span><strong>{{ activeLog.type || '-' }}</strong>
            </div>
            <div class="detail-row">
              <span>User</span><strong>{{ activeLog.user_name || '-' }}</strong>
            </div>
            ...
            <div class="detail-row">
              <span>Object</span><strong>{{ activeLog.name || '-' }}</strong>
            </div>
          </div>
          <div class="detail-row full">
            <span>Message</span>
            <p>{{ formatMessage(activeLog.description) || '-' }}</p>
          </div>
          ...
          <div class="detail-row full" v-else-if="activeLog.remark">
            <span>Remark</span>
            <p>{{ activeLog.remark }}</p>
          </div>
        </div>
        <div class="detail-footer">
          <button
            class="delete-detail"
            :disabled="isDeleting"
            @click="deleteLogs([activeLog.mrid])"
          >
            <i class="fa-solid fa-trash"></i>
            Delete
          </button>
