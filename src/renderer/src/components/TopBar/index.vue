<template>
  <div id="top-bar">
    <div id="top-windows" :class="{ 'logged-in': user, 'not-logged-in': !user }">
      ...
      <div class="right-bar">
              ...
              <template #dropdown>
                <el-dropdown-menu class="notification-dropdown-menu">
                  ...
                  <div class="notification-list">
                    ...
                    >
                      ...
                      <el-dropdown trigger="click" placement="bottom-end" :append-to-body="false">
                        ...
                        <template #dropdown>
                          <el-dropdown-menu class="notification-action-menu">
                            <el-dropdown-item @click.stop="loadMoreNotifications">
                              <i class="fas fa-plus-circle"></i>
                              Show More Notifications
                            </el-dropdown-item>
                            <el-dropdown-item @click.stop="markAsRead(notification.mrid)">
                              <i class="fas fa-check"></i>
                              Mark as Read
                            </el-dropdown-item>
                            <el-dropdown-item @click.stop="deleteNotification(notification.mrid)">
                  ...
                  <div class="notification-footer">
                    <span class="notification-pagination"
        ...
        <el-tooltip content="Settings" placement="bottom">
          <span @click.stop="handleDropdown" class="dropdown-trigger-wrapper">
            <el-dropdown
              ref="dropdown"
        ...
        <el-tooltip content="Close" placement="bottom">
          <span @click="closeApp" class="close-icon">
    ...
    <!-- Server address -->
    <el-dialog
      class="app-dialog app-dialog-config"
    ...
    >
      ...
      >
        <el-form-item label="Login Domain" prop="loginDomain">
          <el-input
            type="text"
            v-model="formConfig.loginDomain"
            placeholder="https://login.domain.com"
          ></el-input>
        </el-form-item>
        <el-form-item label="Service Domain" prop="serviceDomain">
          <el-input
            type="text"
            v-model="formConfig.serviceDomain"
      ...
      <template #footer>
        <span class="dialog-footer custom-footer">
          <el-button class="footer-btn" size="small" @click="dialogConfig = false"
            >Cancel</el-button
          >
          <el-button class="footer-btn" size="small" type="primary" @click="setServerAddr"
            >Save</el-button
          >
        </span>
      </template>
    ...
    <el-dialog
      ...
      :modal="true"
      append-to-body
      :close-on-click-modal="!updateInfo.isDownloading"
      :show-close="!updateInfo.isDownloading"
    >
      <div class="update-wrapper">
        ...
        <div class="changelog">
          ...
          <div class="changelog-body" v-html="formatReleaseNotes(updateInfo.releaseNotes)"></div>
        </div>
        <div v-if="updateInfo.isDownloading" class="download-progress">
          <el-progress
            :percentage="updateInfo.downloadProgress"
            :status="updateInfo.downloadProgress === 100 ? 'success' : undefined"
          ></el-progress>
    ...
    <el-dialog
      ...
      v-model="dialogNotificationDetail"
      :modal="true"
      append-to-body
      width="500px"
    >
...
export default {
  ...
  computed: {
    ...
    serverAddr() {
      return this.$userStore().serverAddr
    },
    loginAddr() {
      return this.$userStore().loginAddr
    },
    serviceAddr() {
      return this.$userStore().serviceAddr
    },
    unreadCount() {
    ...
    displayedNotifications() {
      ...
      const actualStartIndex =
        this.currentPage === 1
          ? 0
          : this.currentPage === 2 && this.notificationLimit > this.itemsPerPage
  ...
  methods: {
        ...
        .then((result) => {
          if (result.success) {
            this.$message.success(
              'Download started! The app will restart automatically after download completes.'
            )
          } else {
      ...
      this.$refs.formConfig.validate((valid) => {
        if (valid) {
          ...
          this.$helper.setServerAddr({
            loginDomain: cleanLoginDomain,
            serviceDomain: cleanServiceDomain
          })
    ...
    handleDropdown() {
      // el-dropdown tự xử lý click trigger - không cần handleClick
    },
...
:global(.app-dialog.app-dialog-config) {
  max-width: 300px;
}

:global(.app-dialog .el-form-item) {
  margin-bottom: 26px;
}

:global(.app-dialog .el-form-item__error) {
  position: static;
  padding-top: 6px;
  line-height: 1.4;
  font-size: 12px;
...
:global(.app-dialog .el-dialog__headerbtn .el-dialog__close) {
...
:global(.app-dialog .el-dialog__headerbtn:hover) {
  background: rgba(255, 255, 255, 0.25);
}

:global(.app-dialog .el-dialog__headerbtn:hover .el-dialog__close) {
  color: #ffffff !important;
  text-shadow: 0 0 10px rgba(255, 255, 255, 0.4);
}

:global(.app-dialog .el-dialog__headerbtn i) {
  line-height: 1;
...
:global(.app-dialog .el-form-item__label) {
...
:global(.app-dialog .el-input) {
  height: auto;
}

:global(.app-dialog .el-input__wrapper) {
...
:global(.app-dialog .el-input__inner) {
  width: 100%;
...
:global(.app-dialog .el-input__inner:focus) {
...
:global(.custom-footer .footer-btn.el-button--default) {
...
:global(.custom-footer .footer-btn.el-button--default:hover) {
...
:global(.custom-footer .footer-btn.el-button--primary) {
  background: linear-gradient(180deg, #1e5bb8 0%, #0f3d80 100%) !important;
  color: #ffffff !important;
  box-shadow:
    0 4px 12px rgba(30, 91, 184, 0.4),
    0 2px 6px rgba(0, 0, 0, 0.2) !important;
}

:global(.custom-footer .footer-btn.el-button--primary:hover) {
  background: linear-gradient(180deg, #2869cc 0%, #1e5bb8 100%) !important;
  transform: translateY(-2px);
  box-shadow:
    0 6px 18px rgba(30, 91, 184, 0.5),
    0 4px 10px rgba(0, 0, 0, 0.25) !important;
}

:global(.custom-footer .footer-btn.el-button--danger) {
  background: linear-gradient(180deg, #e61525 0%, #cc0514 100%) !important;
  color: #ffffff !important;
  box-shadow:
    0 4px 12px rgba(204, 5, 20, 0.4),
    0 2px 6px rgba(0, 0, 0, 0.2) !important;
}

:global(.custom-footer .footer-btn.el-button--danger:hover) {
  background: linear-gradient(180deg, #ff1a2e 0%, #e61525 100%) !important;
  transform: translateY(-2px);
  box-shadow:
    0 6px 18px rgba(204, 5, 20, 0.5),
    0 4px 10px rgba(0, 0, 0, 0.25) !important;
}

:global(.custom-footer .footer-btn:active) {
  transform: translateY(0);
  filter: brightness(0.9);
}

:global(.custom-footer .footer-btn i) {
  font-size: 14px;
  color: inherit;
}
...
.dropdown-menu.el-dropdown-menu .el-dropdown-menu__item span {
  width: 16px;
  text-align: center;
  font-size: 13px;
  color: #ffffff !important;
...
.dropdown-menu.el-dropdown-menu .el-dropdown-menu__item:focus {
  background: rgba(255, 255, 255, 0.2) !important;
  color: #ffffff !important;
}
...
.dropdown-menu.el-dropdown-menu .danger-item span {
  color: #ffffff !important;
  font-size: 14px;
}
...
.notification-action-menu.el-dropdown-menu .el-dropdown-menu__item:focus {
  background: rgba(255, 255, 255, 0.2) !important;
  color: #ffffff !important;
}
...
:root {
  --el-dropdown-menuItem-hover-fill: rgba(255, 255, 255, 0.2);
