...
export const createProcedureVoltageTransformer = async (
  dbsql,
  procedureDataMap,
  testDataMap,
  testConditionMap,
  ...
  const voltageTransformerTestDefinitions = await getTestDefinitionInfo(
    voltageTransformerTestDefinitionInfo
  )
  const voltageTransformerTestingConditions = await getTestConditionInfo(
    voltageTransformerTestingConditionInfo
  )
  ...
  for (const measurementProcedure of voltageTransformerTestDefinitions.measurementProcedure) {
    await measurementProcedureFunc.insertMeasurementProcedureTransaction(
