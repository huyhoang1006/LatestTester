...
export const getPickupVoltageBreakerInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM pickup_voltage_breaker_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get pickupVoltageBreakerInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
          message: 'PickupVoltageBreakerInfo not found'
        })
      return resolve({
        success: true,
        data: row,
        message: 'Get pickupVoltageBreakerInfo by id completed'
      })
    })
  })
}
      ...
      (err, rows) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get pickupVoltageBreakerInfo by assessment_limit_breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get pickupVoltageBreakerInfo by assessment_limit_breaker_info_id completed'
        })
      }
    )
  })
...
export const insertPickupVoltageBreakerInfoTransaction = async (info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Insert pickupVoltageBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Insert pickupVoltageBreakerInfo completed'
        })
      }
    )
  })
...
export const updatePickupVoltageBreakerInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Update pickupVoltageBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Update pickupVoltageBreakerInfo completed'
        })
      }
    )
  })
...
export const deletePickupVoltageBreakerInfoTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM pickup_voltage_breaker_info WHERE mrid = ?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete pickupVoltageBreakerInfo failed' })
      return resolve({
        success: true,
        data: mrid,
        message: 'Delete pickupVoltageBreakerInfo completed'
      })
    })
  })
}
