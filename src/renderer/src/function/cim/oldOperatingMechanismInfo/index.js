...
export const getOldOperatingMechanismInfoById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_operating_mechanism_info WHERE mrid = ?`, [mrid], (err, row) => {
        if (err)
          return reject({
            success: false,
            err,
...
export const insertOldOperatingMechanismInfoTransaction = async (info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes = await OperatingMechanismInfoFunc.insertOperatingMechanismInfoTransaction(
        info,
        dbsql
      )
      if (!parentRes.success)
...
export const updateOldOperatingMechanismInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes = await OperatingMechanismInfoFunc.updateOperatingMechanismInfoTransaction(
        mrid,
        info,
        dbsql
      )
  ...
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run('DELETE FROM old_operating_mechanism_info WHERE mrid = ?', [mrid], function (err) {
        if (err)
          return reject({ success: false, err, message: 'Delete oldOperatingMechanismInfo failed' })
