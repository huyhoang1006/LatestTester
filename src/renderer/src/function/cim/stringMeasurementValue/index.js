...
export const getStringMeasurementValueById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM string_measurement_value WHERE mrid=?`, [mrid], (err, row) => {
        if (err)
          return reject({ success: false, err, message: 'Get stringMeasurementValue by id failed' })
        if (!row)
          return resolve({
            success: false,
            data: null,
...
export const insertStringMeasurementValueTransaction = async (stringMeasurementValue, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const mvResult = await measurementValueFunc.insertMeasurementValueTransaction(
...
export const updateStringMeasurementValueByIdTransaction = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const mvResult = await measurementValueFunc.updateMeasurementValueByIdTransaction(
        mrid,
        stringMeasurementValue,
        dbsql
      )
