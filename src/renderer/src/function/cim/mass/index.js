...
export const getMassByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM mass WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
        return reject({ success: false, err: err, message: 'Get mass by ids failed' })
      }
      if (!rows || rows.length === 0) {
        return resolve({ success: false, data: [], message: 'Mass not found' })
      }
...
export const deleteMassById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM mass WHERE mrid=?', [mrid], function (err) {
      ...
      if (this.changes === 0)
        return resolve({ success: false, data: null, message: 'Mass not found' })
