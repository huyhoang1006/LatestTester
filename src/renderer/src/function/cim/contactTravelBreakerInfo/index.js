...
export const getContactTravelBreakerInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM contact_travel_breaker_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get contactTravelBreakerInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
          message: 'ContactTravelBreakerInfo not found'
        })
      return resolve({
        success: true,
        data: row,
        message: 'Get contactTravelBreakerInfo by id completed'
      })
    })
  })
}
...
export const getContactTravelBreakerInfoByAssessmentLimitBreakerInfoId = async (
  assessmentLimitId
) => {
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err, rows) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get contactTravelBreakerInfo by assessment_limit_breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get contactTravelBreakerInfo by assessment_limit_breaker_info_id completed'
        })
      }
    )
  })
...
export const insertContactTravelBreakerInfoTransaction = async (info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Insert contactTravelBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Insert contactTravelBreakerInfo completed'
        })
      }
    )
  })
...
export const updateContactTravelBreakerInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Update contactTravelBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Update contactTravelBreakerInfo completed'
        })
      }
    )
  })
...
export const deleteContactTravelBreakerInfoTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM contact_travel_breaker_info WHERE mrid = ?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete contactTravelBreakerInfo failed' })
      return resolve({
        success: true,
        data: mrid,
        message: 'Delete contactTravelBreakerInfo completed'
      })
    })
  })
}
