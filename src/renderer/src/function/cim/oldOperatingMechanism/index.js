...
export const getOldOperatingMechanismById = async (mrid) => {
  try {
    ...
    if (!parentRes.success)
      return { success: false, data: null, message: 'OperatingMechanism not found' }

    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_operating_mechanism WHERE mrid = ?`, [mrid], (err, row) => {
        if (err)
          return reject({ success: false, err, message: 'Get oldOperatingMechanism by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'OldOperatingMechanism not found' })
        return resolve({
          success: true,
          data: { ...parentRes.data, ...row },
          message: 'Get oldOperatingMechanism by id completed'
        })
      })
    })
  } catch (err) {
...
export const insertOldOperatingMechanismTransaction = async (info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes = await OperatingMechanismFunc.insertOperatingMechanismTransaction(
        info,
        dbsql
      )
      if (!parentRes.success)
        return reject({
          success: false,
          message: 'Insert OperatingMechanism failed',
          err: parentRes.err
        })
      ...
      dbsql.run(
        `INSERT INTO old_operating_mechanism(
                ...
                `,
        [info.mrid, info.number_of_trip_coil, info.number_of_close_coil],
        function (err) {
          if (err)
            return reject({ success: false, err, message: 'Insert oldOperatingMechanism failed' })
          return resolve({
            success: true,
            data: info,
            message: 'Insert oldOperatingMechanism completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Insert oldOperatingMechanism transaction failed'
      })
    }
  })
}
...
export const updateOldOperatingMechanismTransaction = async (mrid, info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const parentRes = await OperatingMechanismFunc.updateOperatingMechanismTransaction(
        mrid,
        info,
        dbsql
      )
      if (!parentRes.success)
        return reject({
          success: false,
          message: 'Update OperatingMechanism failed',
          err: parentRes.err
        })
      ...
      dbsql.run(
        `UPDATE old_operating_mechanism SET
                ...
                WHERE mrid = ?`,
        [info.number_of_trip_coil, info.number_of_close_coil, mrid],
        function (err) {
          if (err)
            return reject({ success: false, err, message: 'Update oldOperatingMechanism failed' })
          return resolve({
            success: true,
            data: info,
            message: 'Update oldOperatingMechanism completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Update oldOperatingMechanism transaction failed'
      })
    }
  })
}
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run('DELETE FROM old_operating_mechanism WHERE mrid = ?', [mrid], function (err) {
        if (err)
          return reject({ success: false, err, message: 'Delete oldOperatingMechanism failed' })
        ...
        OperatingMechanismFunc.deleteOperatingMechanismTransaction(mrid, dbsql)
          .then((res) => {
            if (!res.success)
              return reject({
                success: false,
                message: 'Delete OperatingMechanism failed',
                err: res.err
              })
            return resolve({
              success: true,
              data: mrid,
              message: 'Delete oldOperatingMechanism completed'
            })
          })
          .catch((err2) =>
            reject({
      ...
      return reject({
        success: false,
        err,
        message: 'Delete oldOperatingMechanism transaction failed'
      })
    }
  })
}
