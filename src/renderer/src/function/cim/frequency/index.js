...
export const getFrequencyByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM frequency WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
...
export const insertFrequency = async (frequency) => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      [frequency.mrid, frequency.multiplier, frequency.unit, frequency.value],
      function (err) {
...
export const deleteFrequencyById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM frequency WHERE mrid=?', [mrid], function (err) {
      ...
      if (this.changes === 0)
        return resolve({ success: false, data: null, message: 'Frequency not found' })
