...
export const getOperatingMechanismComponentById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM operating_mechanism_component WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({
          success: false,
          err,
...
export const deleteOperatingMechanismComponentTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM operating_mechanism_component WHERE mrid = ?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete operatingMechanismComponent failed' })
      return resolve({
        success: true,
        data: mrid,
