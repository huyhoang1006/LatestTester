...
export const getTapChangerTablePointById = async (mrid) => {
  try {
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM tap_changer_table_point WHERE mrid=?`, [mrid], (err, row) => {
        if (err)
          return reject({ success: false, err, message: 'Get tapChangerTablePoint by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'TapChangerTablePoint not found' })
        return resolve({
          success: true,
          data: row,
          message: 'Get tapChangerTablePoint by id completed'
        })
      })
    })
  } catch (err) {
        ...
        (err, row) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get tapChangerTablePoint by tap changer info id failed'
            })
          if (!row || row.length == 0)
            return resolve({
              success: false,
              data: null,
              message: 'TapChangerTablePoint not found'
            })
          return resolve({
            success: true,
            data: row,
            message: 'Get tapChangerTablePoint by tap changer info id completed'
          })
        }
      )
    })
  ...
  return new Promise((resolve, reject) => {
    try {
      dbsql.run(
        ...
        function (err) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Insert tapChangerTablePoint completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Insert tapChangerTablePoint transaction failed'
      })
    }
  })
}
  ...
  return new Promise((resolve, reject) => {
    try {
      dbsql.run(
        ...
        function (err) {
          ...
          return resolve({
            success: true,
            data: info,
            message: 'Update tapChangerTablePoint completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Update tapChangerTablePoint transaction failed'
      })
    }
  })
}
    ...
    try {
      dbsql.run('DELETE FROM tap_changer_table_point WHERE mrid=?', [mrid], function (err) {
        ...
        return resolve({
          success: true,
          data: mrid,
          message: 'Delete tapChangerTablePoint completed'
        })
      })
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Delete tapChangerTablePoint transaction failed'
      })
    }
  })
}
