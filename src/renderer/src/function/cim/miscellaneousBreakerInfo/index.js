...
export const getMiscellaneousBreakerInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM miscellaneous_breaker_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get miscellaneousBreakerInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
          message: 'MiscellaneousBreakerInfo not found'
        })
      return resolve({
        success: true,
        data: row,
        message: 'Get miscellaneousBreakerInfo by id completed'
      })
    })
  })
}
      ...
      (err, rows) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get miscellaneousBreakerInfo by assessment_limit_breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get miscellaneousBreakerInfo by assessment_limit_breaker_info_id completed'
        })
      }
    )
  })
...
export const insertMiscellaneousBreakerInfoTransaction = async (info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Insert miscellaneousBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Insert miscellaneousBreakerInfo completed'
        })
      }
    )
  })
...
export const updateMiscellaneousBreakerInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Update miscellaneousBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Update miscellaneousBreakerInfo completed'
        })
      }
    )
  })
...
export const deleteMiscellaneousBreakerInfoTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM miscellaneous_breaker_info WHERE mrid = ?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete miscellaneousBreakerInfo failed' })
      return resolve({
        success: true,
        data: mrid,
        message: 'Delete miscellaneousBreakerInfo completed'
      })
    })
  })
}
