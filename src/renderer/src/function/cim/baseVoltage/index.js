  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      identifiedObjectFunc
        .insertIdentifiedObjectTransaction(baseVoltage, db)
        .then((identifiedResult) => {
          ...
          db.run(
            ...
            [baseVoltage.mrid, baseVoltage.nominal_voltage],
            function (err) {
              if (err) {
                db.run('ROLLBACK')
              ...
              return resolve({
                success: true,
                data: baseVoltage,
                message: 'Insert baseVoltage completed'
              })
            }
          )
        })
...
export const insertBaseVoltageTransaction = async (baseVoltage, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .insertIdentifiedObjectTransaction(baseVoltage, dbsql)
      .then((identifiedResult) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
...
export const getBaseVoltageByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM base_voltage WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
...
export const updateBaseVoltageById = async (mrid, baseVoltage) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      identifiedObjectFunc
        .updateIdentifiedObjectByIdTransaction(mrid, baseVoltage, db)
        .then((identifiedResult) => {
          if (!identifiedResult.success) {
            db.run('ROLLBACK')
            return reject({
          ...
          db.run(
            `UPDATE base_voltage SET
                            nominal_voltage = ?
                        WHERE mrid = ?`,
            [baseVoltage.nominal_voltage, mrid],
            function (err) {
              if (err) {
                db.run('ROLLBACK')
              ...
              return resolve({
                success: true,
                data: baseVoltage,
                message: 'Update baseVoltage completed'
              })
            }
          )
        })
...
export const updateBaseVoltageByIdTransaction = async (mrid, baseVoltage, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .updateIdentifiedObjectByIdTransaction(mrid, baseVoltage, dbsql)
      .then((identifiedResult) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
  ...
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      ...
      .then((result) => {
        ...
        return resolve({
          success: true,
          message: 'Delete baseVoltage (and identified object) completed'
        })
      })
      .catch((err) => {
  ...
  try {
    return new Promise((resolve, reject) => {
      dbsql.all('SELECT * FROM base_voltage WHERE location = ?', [locationId], (err, rows) => {
        if (err) return reject({ success: false, err, message: 'Get baseVoltage failed' })
        if (!rows || rows.length === 0)
          return resolve({ success: false, data: null, message: 'BaseVoltage not found' })
