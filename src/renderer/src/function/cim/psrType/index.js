  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      identifiedObjectFunc
        .insertIdentifiedObjectTransaction(psrType, db)
...
export const insertPsrTypeTransaction = async (psrType, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .insertIdentifiedObjectTransaction(psrType, dbsql)
      .then((identifiedResult) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
...
export const updatePsrTypeById = async (mrid, psrType) => {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      db.run('BEGIN TRANSACTION')
      identifiedObjectFunc
        .updateIdentifiedObjectByIdTransaction(mrid, psrType, db)
        .then((identifiedResult) => {
          if (!identifiedResult.success) {
            db.run('ROLLBACK')
            return reject({
...
export const updatePsrTypeByIdTransaction = async (mrid, psrType, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .updateIdentifiedObjectByIdTransaction(mrid, psrType, dbsql)
      .then((identifiedResult) => {
        if (!identifiedResult.success) {
          return reject({
            success: false,
  ...
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      ...
      .then((result) => {
        ...
        return resolve({
          success: true,
          message: 'Delete psrType (and identified object) completed'
        })
      })
      .catch((err) => {
