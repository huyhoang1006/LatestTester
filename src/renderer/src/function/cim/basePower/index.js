...
export const insertBasePowerTransaction = async (basePower, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .insertIdentifiedObjectTransaction(basePower, dbsql)
      .then((result) => {
        if (!result.success) {
          return reject({
            success: false,
...
export const updateBasePowerByIdTransaction = async (mrid, basePower, dbsql) => {
  return new Promise((resolve, reject) => {
    identifiedObjectFunc
      .updateIdentifiedObjectByIdTransaction(mrid, basePower, dbsql)
      .then((result) => {
        if (!result.success) {
          return reject({
            success: false,
...
export const getBasePowerByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM base_power WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
        return reject({ success: false, err: err, message: 'Get base power by ids failed' })
      }
      if (!rows || rows.length === 0) {
        return resolve({ success: false, data: [], message: 'Base power not found' })
      }
