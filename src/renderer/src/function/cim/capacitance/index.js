...
export const getCapacitanceById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM capacitance WHERE mrid=?', [mrid], (err, row) => {
...
export const getCapacitanceByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM capacitance WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
    ...
    dbsql.run(
      ...
      function (err) {
        ...
        return resolve({
          success: true,
          data: capacitance,
          message: 'Insert capacitance completed'
        })
      }
    )
  })
    ...
    dbsql.run(
      ...
      function (err) {
        ...
        return resolve({
          success: true,
          data: capacitance,
          message: 'Update capacitance completed'
        })
      }
    )
  })
...
export const deleteCapacitanceByIdTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM capacitance WHERE mrid=?', [mrid], function (err) {
      ...
      if (this.changes === 0)
        return resolve({ success: false, data: null, message: 'Capacitance not found' })
