...
export const getOldTransformerObservationById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_transformer_observation WHERE mrid=?`, [mrid], (err, row) => {
        if (err)
          return reject({
            success: false,
            err,
...
export const insertOldTransformerObservationTransaction = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const obsResult = await transformerObservationFunc.insertTransformerObservationTransaction(
        oldTransformerObservation,
        dbsql
      )
