...
export const getOldPowerTransformerInfoById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM old_power_transformer_info WHERE mrid=?`, [mrid], (err, row) => {
        if (err)
          return reject({
            success: false,
            err,
...
export const insertOldPowerTransformerInfoTransaction = async (info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const baseResult = await PowerTransformerInfoFunc.insertPowerTransformerInfoTransaction(
        info,
        dbsql
      )
      if (!baseResult.success) {
...
export const updateOldPowerTransformerInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const baseResult = await PowerTransformerInfoFunc.updatePowerTransformerInfoTransaction(
        mrid,
        info,
        dbsql
      )
...
export const deleteOldPowerTransformerInfoTransaction = async (mrid, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const baseResult = await PowerTransformerInfoFunc.deletePowerTransformerInfoTransaction(
        mrid,
        dbsql
      )
      if (!baseResult.success) {
