...
export const getSheathVoltageLimiterByCableInfoId = async (cableInfoId) => {
  try {
    return new Promise((resolve, reject) => {
      db.get(
        'SELECT * FROM sheath_voltage_limiter WHERE cable_info_id=?',
        [cableInfoId],
        (err, row) => {
          if (err)
            return reject({
              success: false,
              err,
              message: 'Get sheath voltage limiter by cable info id failed'
            })
          if (!row)
            return resolve({
              success: false,
              data: null,
              message: 'Sheath voltage limiter not found'
            })
          return resolve({
            success: true,
            data: row,
            message: 'Get sheath voltage limiter by cable info id completed'
          })
        }
      )
    })
...
export const getSheathVoltageLimiterById = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.get('SELECT * FROM sheath_voltage_limiter WHERE mrid = ?', [mrid], (err, row) => {
...
export const deleteSheathVoltageLimiterTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM sheath_voltage_limiter WHERE mrid = ?', [mrid], function (err) {
      if (err) {
