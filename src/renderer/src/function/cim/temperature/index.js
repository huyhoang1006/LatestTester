...
export const getTemperatureByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM temperature WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
...
export const insertTemperature = async (temperature) => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      [temperature.mrid, temperature.multiplier, temperature.unit, temperature.value],
      function (err) {
        if (err) return reject({ success: false, err, message: 'Insert temperature failed' })
        return resolve({
          success: true,
          data: temperature,
          message: 'Insert temperature completed'
        })
      }
    )
  })
    ...
    db.run(
      ...
      function (err) {
        ...
        return resolve({
          success: true,
          data: temperature,
          message: 'Update temperature completed'
        })
      }
    )
  })
...
export const deleteTemperatureById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM temperature WHERE mrid=?', [mrid], function (err) {
      ...
      if (this.changes === 0)
        return resolve({ success: false, data: null, message: 'Temperature not found' })
