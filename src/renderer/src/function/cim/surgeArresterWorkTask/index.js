...
export const getSurgeArresterWorkTaskById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM surge_arrester_work_task WHERE mrid=?`, [mrid], (err, row) => {
        if (err)
          return reject({ success: false, err, message: 'Get surgeArresterWorkTask by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'SurgeArresterWorkTask not found' })
        return resolve({
          success: true,
...
export const insertSurgeArresterWorkTaskTransaction = async (surgeArresterWorkTask, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      // Thêm workTask trước
      const workTaskResult = await workTaskFunc.insertWorkTaskTransaction(
...
export const updateSurgeArresterWorkTaskByIdTransaction = async (
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const workTaskResult = await workTaskFunc.updateWorkTaskByIdTransaction(
        mrid,
        surgeArresterWorkTask,
        dbsql
      )
