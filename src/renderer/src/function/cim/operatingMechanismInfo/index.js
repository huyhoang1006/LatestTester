...
export const getOperatingMechanismInfoById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM operating_mechanism_info WHERE mrid = ?`, [mrid], (err, row) => {
        if (err)
          return reject({ success: false, err, message: 'Get operatingMechanismInfo by id failed' })
        if (!row)
          return resolve({
            success: false,
            data: null,
            message: 'OperatingMechanismInfo not found'
          })
        return resolve({
          success: true,
          data: { ...parentRes.data, ...row },
          message: 'Get operatingMechanismInfo by id completed'
        })
      })
    })
  } catch (err) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!parentRes.success)
        return reject({ success: false, message: 'Insert AssetInfo failed', err: parentRes.err })
      ...
      dbsql.run(
        ...
        function (err) {
          if (err)
            return reject({ success: false, err, message: 'Insert operatingMechanismInfo failed' })
          return resolve({
            success: true,
            data: info,
            message: 'Insert operatingMechanismInfo completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Insert operatingMechanismInfo transaction failed'
      })
    }
  })
}
...
export const updateOperatingMechanismInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!parentRes.success)
        return reject({ success: false, message: 'Update AssetInfo failed', err: parentRes.err })
      ...
      dbsql.run(
        ...
        function (err) {
          if (err)
            return reject({ success: false, err, message: 'Update operatingMechanismInfo failed' })
          return resolve({
            success: true,
            data: info,
            message: 'Update operatingMechanismInfo completed'
          })
        }
      )
    } catch (err) {
      return reject({
        success: false,
        err,
        message: 'Update operatingMechanismInfo transaction failed'
      })
    }
  })
}
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      dbsql.run('DELETE FROM operating_mechanism_info WHERE mrid = ?', [mrid], function (err) {
        if (err)
          return reject({ success: false, err, message: 'Delete operatingMechanismInfo failed' })

        // then delete parent AssetInfo
        AssetInfoFunc.deleteAssetInfoByIdTransaction(mrid, dbsql)
          .then((res) => {
            if (!res.success)
              return reject({ success: false, message: 'Delete AssetInfo failed', err: res.err })
            return resolve({
              success: true,
              data: mrid,
              message: 'Delete operatingMechanismInfo completed'
            })
          })
          .catch((err2) =>
            reject({ success: false, err: err2, message: 'Delete AssetInfo transaction failed' })
          )
      })
    } catch (err) {
      return reject({
