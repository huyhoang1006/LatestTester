...
export const getOperatingTimeBreakerInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM operating_time_breaker_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get operatingTimeBreakerInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
          message: 'OperatingTimeBreakerInfo not found'
        })
      return resolve({
        success: true,
        data: row,
        message: 'Get operatingTimeBreakerInfo by id completed'
      })
    })
  })
}
...
export const getOperatingTimeBreakerInfoByAssessmentLimitBreakerInfoId = async (
  assessmentLimitId
) => {
  return new Promise((resolve, reject) => {
    db.all(
      ...
      (err, rows) => {
        if (err)
          return reject({
            success: false,
            err,
            message: 'Get operatingTimeBreakerInfo by assessment_limit_breaker_info_id failed'
          })
        return resolve({
          success: true,
          data: rows,
          message: 'Get operatingTimeBreakerInfo by assessment_limit_breaker_info_id completed'
        })
      }
    )
  })
...
export const insertOperatingTimeBreakerInfoTransaction = async (info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Insert operatingTimeBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Insert operatingTimeBreakerInfo completed'
        })
      }
    )
  })
...
export const updateOperatingTimeBreakerInfoTransaction = async (mrid, info, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(
      ...
      function (err) {
        if (err)
          return reject({ success: false, err, message: 'Update operatingTimeBreakerInfo failed' })
        return resolve({
          success: true,
          data: info,
          message: 'Update operatingTimeBreakerInfo completed'
        })
      }
    )
  })
...
export const deleteOperatingTimeBreakerInfoTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM operating_time_breaker_info WHERE mrid = ?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete operatingTimeBreakerInfo failed' })
      return resolve({
        success: true,
        data: mrid,
        message: 'Delete operatingTimeBreakerInfo completed'
      })
    })
  })
}
