...
export const getAssessmentLimitBreakerInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM assessment_limit_breaker_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({
          success: false,
          err,
...
export const deleteAssessmentLimitBreakerInfoTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(`DELETE FROM assessment_limit_breaker_info WHERE mrid = ?`, [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete assessmentLimitBreakerInfo failed' })
      return resolve({
        success: true,
        data: mrid,
