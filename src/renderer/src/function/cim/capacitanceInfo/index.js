...
export const getCapacitanceCapacitorInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM capacitance_capacitor_info WHERE mrid=?', [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get capacitanceCapacitorInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
...
export const deleteCapacitanceCapacitorInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.run('DELETE FROM capacitance_capacitor_info WHERE mrid=?', [mrid], function (err) {
      if (err)
        return reject({ success: false, err, message: 'Delete capacitance capacitor info failed' })
      if (this.changes === 0)
        return resolve({
          success: false,
          data: null,
