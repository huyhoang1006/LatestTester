...
export const getOperatingMechanismById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get('SELECT * FROM operating_mechanism WHERE mrid = ?', [mrid], (err, row) => {
        if (err)
...
export const getOperatingMechanismByAssetId = async (assetId) => {
  return new Promise((resolve, reject) => {
    db.get('SELECT * FROM operating_mechanism WHERE asset_id = ?', [assetId], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get operatingMechanism by asset_id failed' })
      if (!row)
        return resolve({ success: false, data: null, message: 'OperatingMechanism not found' })
      return resolve({
        success: true,
