...
export const getBreakerContactSystemInfoById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM breaker_contact_system_info WHERE mrid = ?`, [mrid], (err, row) => {
      if (err)
        return reject({ success: false, err, message: 'Get breakerContactSystemInfo by id failed' })
      if (!row)
        return resolve({
          success: false,
          data: null,
