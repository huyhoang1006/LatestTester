...
export const getValueAliasSetById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM value_alias_set WHERE mrid=?`, [mrid], (err, row) => {
      ...
      dbsql.run(
        ...
        function (err) {
          ...
          return resolve({
            success: true,
            data: valueAliasSet,
            message: 'Insert valueAliasSet completed'
          })
        }
      )
    } catch (err) {
