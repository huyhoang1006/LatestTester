...
export const getQuantityValueById = async (mrid) => {
  return new Promise((resolve, reject) => {
    db.get(`SELECT * FROM quantity_value WHERE mrid = ?`, [mrid], (err, row) => {
...
export const getQuantityValueByIds = async (mrids) => {
  return new Promise((resolve, reject) => {
    ...
    db.all(`SELECT * FROM quantity_value WHERE mrid IN (${placeholders})`, mrids, (err, rows) => {
      if (err) {
      ...
      return resolve({ success: true, data: rows, message: 'Get quantity value by ids completed' })
    })
  })
}
...
export const deleteQuantityValueTransaction = async (mrid, dbsql) => {
  return new Promise((resolve, reject) => {
    dbsql.run(`DELETE FROM quantity_value WHERE mrid = ?`, [mrid], function (err) {
