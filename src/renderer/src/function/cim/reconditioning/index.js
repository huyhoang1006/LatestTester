...
export const getReconditioningById = async (mrid) => {
  try {
    ...
    return new Promise((resolve, reject) => {
      db.get(`SELECT * FROM reconditioning WHERE mrid=?`, [mrid], (err, row) => {
        if (err) return reject({ success: false, err, message: 'Get reconditioning by id failed' })
        if (!row)
          return resolve({ success: false, data: null, message: 'Reconditioning not found' })
        return resolve({
          success: true,
          data: { ...identifiedObjectResult.data, ...row },
          message: 'Get reconditioning by id completed'
        })
      })
    })
  } catch (err) {
...
export const insertReconditioningTransaction = async (info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const identifiedObjectResult = await IdentifiedObjectFunc.insertIdentifiedObjectTransaction(
        info,
        dbsql
      )
      if (!identifiedObjectResult.success) {
...
export const updateReconditioningTransaction = async (mrid, info, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const identifiedObjectResult = await IdentifiedObjectFunc.updateIdentifiedObjectTransaction(
        mrid,
        info,
        dbsql
      )
...
export const deleteReconditioningTransaction = async (mrid, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      const identifiedObjectResult = await IdentifiedObjectFunc.deleteIdentifiedObjectTransaction(
        mrid,
        dbsql
      )
      if (!identifiedObjectResult.success) {
