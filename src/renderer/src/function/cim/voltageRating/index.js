...
export const getVoltageRatingById = async (mrid) => {
  try {
    const row = await new Promise((resolve, reject) => {
      db.get(`SELECT * FROM voltage_rating WHERE mrid = ?`, [mrid], (err, row) =>
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return {
        success: false,
        data: [],
        message: 'No VoltageRating found for this transformerEndId'
      }
    }
...
export const deleteVoltageRatingTransaction = async (mrid, dbsql) => {
  return new Promise(async (resolve, reject) => {
    try {
      dbsql.run(`DELETE FROM voltage_rating WHERE mrid = ?`, [mrid], function (err) {
        if (err) {
