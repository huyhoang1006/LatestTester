...
export const insertPowerCableEntity = async (old_entity, entity) => {
  try {
    if (entity.asset.mrid === null || entity.asset.mrid === '') {
      ...
      for (const area of toDeleteArea)
        await deleteUnitSafely(deleteAreaByIdTransaction, area.mrid, db)
      ...
      const toAddCurrentFlow = entity.currentFlow.filter(
        (s) => s.mrid && !oldCurrentFlowIds.includes(s.mrid)
      )
      const toDeleteCurrentFlow = old_entity.currentFlow.filter(
        (s) => s.mrid && !newCurrentFlowIds.includes(s.mrid)
      )
      ...
      for (const item of toDeleteCurrentFlow)
      ...
      const toDeleteSecond = old_entity.second.filter(
        (s) => s.mrid && !newSecondIds.includes(s.mrid)
      )
      ...
      for (const item of toDeleteSecond)
      ...
      const toAddFrequency = entity.frequency.filter(
        (s) => s.mrid && !oldFrequencyIds.includes(s.mrid)
      )
      const toDeleteFrequency = old_entity.frequency.filter(
        (s) => s.mrid && !newFrequencyIds.includes(s.mrid)
      )
      ...
      for (const item of toDeleteFrequency)
      ...
      const toDeleteLength = old_entity.length.filter(
        (s) => s.mrid && !newLengthIds.includes(s.mrid)
      )
      ...
      for (const item of toDeleteLength)
      ...
      const toAddTemperature = entity.temperature.filter(
        (v) => v.mrid && !oldTemperatureIds.includes(v.mrid)
      )
      const toDeleteTemperature = old_entity.temperature.filter(
        (v) => v.mrid && !newTemperatureIds.includes(v.mrid)
      )
      ...
      for (const item of toDeleteTemperature)
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataPowerCable.success) {
        ...
        const dataCencentricNeutralInfo = await getConcentricNeutralCableInfoById(
        ...
        const terminalCableInfo = await getTerminalCableInfoByCableInfoId(
        ...
        const sheathVoltageLimiter = await getSheathVoltageLimiterByCableInfoId(
          entity.concentricNeutral.mrid
        )
  ...
  try {
    ...
    if (entity.concentricNeutral && entity.concentricNeutral.mrid) {
      ...
      await new Promise((res, rej) =>
        db.run('DELETE FROM old_cable_info WHERE cable_info_id=?', [cableInfoId], (err) =>
          err ? rej(err) : res()
        )
      )
      await new Promise((res, rej) =>
        db.run('DELETE FROM joint_cable_info WHERE cable_info_id=?', [cableInfoId], (err) =>
          err ? rej(err) : res()
        )
      )
      await new Promise((res, rej) =>
        db.run('DELETE FROM terminal_cable_info WHERE cable_info_id=?', [cableInfoId], (err) =>
          err ? rej(err) : res()
        )
      )
      await new Promise((res, rej) =>
        db.run('DELETE FROM sheath_voltage_limiter WHERE cable_info_id=?', [cableInfoId], (err) =>
          err ? rej(err) : res()
        )
      )
    }
