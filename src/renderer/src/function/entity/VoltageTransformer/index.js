...
import {
  insertOldPotentialTransformerTransaction,
  getOldPotentialTransformerInfoById,
  deleteOldPotentialTransformerInfoTransaction
} from '@/function/cim/OldPotentialTransformerInfo/index.js'
import {
  insertPotentialTransformerTable,
  deletePotentialTransformerTableByPotentialTransformerInfoId,
  getPotentialTransformerTableByPotentialTransformerInfoId
} from '@/function/cim/PotentialTransformerTable/index.js'
import {
  insertAssetPsrTransaction,
  getAssetPsrById,
  getAssetPsrByAssetIdAndPsrId,
  deleteAssetPsrTransaction,
  deleteAssetPsrByIdTransaction
  ...
  try {
    if (entity.asset.mrid === null || entity.asset.mrid === '') {
      ...
      const syncResult = syncFilesWithDeletion(
        ...
        null,
        entity.OldPotentialTransformerInfo.mrid
      )
      ...
      await deletePotentialTransformerTableByPotentialTransformerInfoId(
        entity.OldPotentialTransformerInfo.mrid,
        db
      )
      ...
      return {
        success: true,
        data: entity,
        message: 'Voltage Transformer entity inserted successfully'
      }
    }
  } catch (error) {
    restoreFiles(null, null, entity.OldPotentialTransformerInfo.mrid)
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataVt.success) {
        ...
        const dataFrequency = await getFrequencyById(
          entity.OldPotentialTransformerInfo.rated_frequency
        )
        if (dataFrequency.success) {
          entity.frequency.push(dataFrequency.data)
        }

        const dataPotentialTransformerTable =
          await getPotentialTransformerTableByPotentialTransformerInfoId(
            entity.OldPotentialTransformerInfo.mrid
          )
...
export const deleteVoltageTransformerEntity = async (data) => {
  try {
    ...
    try {
      ...
      await deletePotentialTransformerTableByPotentialTransformerInfoId(
        data.OldPotentialTransformerInfo.mrid,
        db
      )

      // Xóa OldPotentialTransformerInfo
      if (data.OldPotentialTransformerInfo && data.OldPotentialTransformerInfo.mrid) {
        await deleteOldPotentialTransformerInfoTransaction(
          data.OldPotentialTransformerInfo.mrid,
          db
        )
      }
