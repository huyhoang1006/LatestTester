...
export const insertBushingJobEntity = async (old_entity, entity) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      ...
      //bushingTestingEquipmentTestType
      const newIdsSet = entity.bushingTestingEquipmentTestType.map((v) => v.mrid).filter((id) => id) // bỏ null/empty
...
export const getBushingJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getBushingTestingEquipmentTestingEqId(equipment.mrid)
        ...
        return {
          success: true,
          data: entity,
          message: 'Bushing job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
...
export const deleteBushingJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
