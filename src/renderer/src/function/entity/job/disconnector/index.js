...
import DisconnectorJobEntity from '@/views/Flatten/Job/Disconnector/index.js'
import {
...
export const insertDisconnectorJobEntity = async (old_entity, entity) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      ...
      //disconnectorTestingEquipmentTestType
...
export const getDisconnectorJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      return { success: false, error: new Error('Invalid ID') }
    } else {
      const entity = new DisconnectorJobEntity()
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getDisconnectorTestingEquipmentTestingEqId(
            equipment.mrid
          )
    ...
    console.error('Error retrieving disconnector job entity:', error)
    return { success: false, error, message: 'Error retrieving disconnector job entity' }
  }
}

export const deleteDisconnectorJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
