...
export const getCircuitBreakerJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getCircuitBreakerTestingEquipmentTestingEqId(
            equipment.mrid
          )
...
export const deleteCircuitBreakerJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
