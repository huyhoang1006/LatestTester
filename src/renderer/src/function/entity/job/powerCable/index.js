  ...
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      ...
      return {
        success: true,
        data: entity,
        message: 'Power Cable Job entity inserted successfully'
      }
    }
  } catch (error) {
    ...
    return {
      success: false,
      error,
      message: 'Error retrieving power cable entity: ' + error.message
    }
  }
}
    ...
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getPowerCableTestingEquipmentTestingEqId(
            equipment.mrid
          )
...
export const deletePowerCableJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
