...
export const getTransformerJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getTransformerTestingEquipmentTestingEqId(
            equipment.mrid
          )
    ...
    console.error('Error retrieving Transformer job entity:', error)
    return { success: false, error, message: 'Error retrieving Transformer job entity' }
  }
}

export const deleteTransformerJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
