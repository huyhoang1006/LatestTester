...
import ReactorJobEntity from '@/views/Flatten/Job/Reactor/index.js'
import {
...
export const insertReactorJobEntity = async (old_entity, entity) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      ...
      //reactorTestingEquipmentTestType
      const newIdsSet = entity.reactorTestingEquipmentTestType.map((v) => v.mrid).filter((id) => id) // bỏ null/empty
    ...
    console.error('Error retrieving reactor entity:', error)
    restoreFiles(null, null, entity.oldWork.mrid)
    deleteBackupFiles(null, entity.oldWork.mrid)
    for (const attachment of entity.attachmentTest) {
      restoreFiles(null, null, attachment.id_foreign)
    ...
    return { success: false, error, message: 'Error retrieving reactor entity' }
  }
}

export const getReactorJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      return { success: false, error: new Error('Invalid ID') }
    } else {
      const entity = new ReactorJobEntity()
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getReactorTestingEquipmentTestingEqId(equipment.mrid)
        ...
        return {
          success: true,
          data: entity,
          message: 'Reactor job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
    ...
    console.error('Error retrieving reactor job entity:', error)
    return { success: false, error, message: 'Error retrieving reactor job entity' }
  }
}

export const deleteReactorJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
