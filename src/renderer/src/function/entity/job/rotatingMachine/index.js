...
import RotatingMachineJobEntity from '@/views/Flatten/Job/RotatingMachine/index.js'
import {
...
export const insertRotatingMachineJobEntity = async (old_entity, entity) => {
  try {
    if (entity.oldWork.mrid === null || entity.oldWork.mrid === '') {
      const result = {
        success: false,
        error: new Error('MRID is required for Rotating Machine Job Entity'),
        message: ''
      }
      return result
    } else {
      ...
      //rotatingMachineTestingEquipmentTestType
      const newIdsSet = entity.rotatingMachineTestingEquipmentTestType
        .map((v) => v.mrid)
        .filter((id) => id) // bỏ null/empty
      ...
      const toAddWorkTask = entity.workTasks.filter(
        (v) => v.mrid && !oldIdsWorkTask.includes(v.mrid)
      )
      const toDeleteWorkTask = old_entity.workTasks.filter(
        (v) => v.mrid && !newIdsWorkTask.includes(v.mrid)
      )
      ...
      const toAddStringMeasurementValue = entity.stringMeasurementValues.filter(
        (v) => v.mrid && !oldIdsStringMeasurementValue.includes(v.mrid)
      )
      ...
      const toDeleteStringMeasurementValue = old_entity.stringMeasurementValues.filter(
        (v) => v.mrid && !newIdsStringMeasurementValue.includes(v.mrid)
      )
      ...
      return {
        success: true,
        data: entity,
        message: 'Rotating Machine Job entity inserted successfully'
      }
    }
  } catch (error) {
...
export const getRotatingMachineJobEntity = async (id) => {
  try {
    if (id == null || id === '') {
      ...
      if (dataOldWork.success) {
        ...
        for (const equipment of entity.testingEquipment) {
          const dataEquipmentTestType = await getRotatingMachineTestingEquipmentTestingEqId(
            equipment.mrid
          )
        ...
        return {
          success: true,
          data: entity,
          message: 'Rotating Machine job entity retrieved successfully'
        }
      } else {
        return {
          success: false,
    ...
    console.error('Error retrieving rotating machine job entity:', error)
    return { success: false, error, message: 'Error retrieving rotating machine job entity' }
  }
}

export const deleteRotatingMachineJobEntity = async (entity) => {
  try {
    await runAsync('BEGIN TRANSACTION')
    ...
    return { success: true, message: 'Rotating Machine Job entity deleted successfully' }
  } catch (error) {
    await runAsync('ROLLBACK')
    console.error('Delete Rotating Machine Job Error:', error)
    return { success: false, error, message: 'Error deleting Rotating Machine Job entity' }
  }
}
