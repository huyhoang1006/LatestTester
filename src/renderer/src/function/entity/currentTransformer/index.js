...
import {
  insertOldCurrentTransformerInfoTransaction,
  getOldCurrentTransformerInfoById
} from '@/function/cim/oldCurrentTransformerInfo'
import {
...
import {
  insertCtCoreInfoTransaction,
  deleteCtCoreInfoByCurrentTransformerInfoIdTransaction,
  getCtCoreInfoByCurrentTransformerInfoId
} from '@/function/cim/ctCoreInfo'
import {
  insertCtTapInfoTransaction,
  deleteCtTapInfoByCtCoreInfoIdTransaction,
  getCtTapInfoByCtCoreInfoId
} from '@/function/cim/ctTapInfo'
import {
...
import {
  getAssetInfoById,
  insertAssetInfoTransaction,
  deleteAssetInfoByIdTransaction
} from '@/function/cim/assetInfo'
import {
...
export const insertCurrentTransformerEntity = async (old_entity, entity) => {
  try {
    ...
    const toAddVoltage = (entity.voltage || []).filter(
      (v) => v.mrid && !oldVoltageIds.includes(v.mrid)
    )
    const toDeleteVoltage = (old_entity.voltage || []).filter(
      (v) => v.mrid && !newVoltageIds.includes(v.mrid)
    )
    const toUpdateVoltage = (entity.voltage || []).filter(
      (v) => v.mrid && oldVoltageIds.includes(v.mrid)
    )
    ...
    const toAddCurrentFlow = (entity.currentFlow || []).filter(
      (c) => c.mrid && !oldCurrentFlowIds.includes(c.mrid)
    )
    const toDeleteCurrentFlow = (old_entity.currentFlow || []).filter(
      (c) => c.mrid && !newCurrentFlowIds.includes(c.mrid)
    )
    const toUpdateCurrentFlow = (entity.currentFlow || []).filter(
      (c) => c.mrid && oldCurrentFlowIds.includes(c.mrid)
    )
    ...
    const toAddSeconds = (entity.seconds || []).filter(
      (s) => s.mrid && !oldSecondsIds.includes(s.mrid)
    )
    const toDeleteSeconds = (old_entity.seconds || []).filter(
      (s) => s.mrid && !newSecondsIds.includes(s.mrid)
    )
    const toUpdateSeconds = (entity.seconds || []).filter(
      (s) => s.mrid && oldSecondsIds.includes(s.mrid)
    )
    ...
    const toAddFrequency = (entity.frequency || []).filter(
      (f) => f.mrid && !oldFrequencyIds.includes(f.mrid)
    )
    const toDeleteFrequency = (old_entity.frequency || []).filter(
      (f) => f.mrid && !newFrequencyIds.includes(f.mrid)
    )
    const toUpdateFrequency = (entity.frequency || []).filter(
      (f) => f.mrid && oldFrequencyIds.includes(f.mrid)
    )
    ...
    const toAddResistance = (entity.resistance || []).filter(
      (r) => r.mrid && !oldResistanceIds.includes(r.mrid)
    )
    const toDeleteResistance = (old_entity.resistance || []).filter(
      (r) => r.mrid && !newResistanceIds.includes(r.mrid)
    )
    const toUpdateResistance = (entity.resistance || []).filter(
      (r) => r.mrid && oldResistanceIds.includes(r.mrid)
    )
    ...
    const toAddPercent = (entity.percent || []).filter(
      (p) => p.mrid && !oldPercentIds.includes(p.mrid)
    )
    const toDeletePercent = (old_entity.percent || []).filter(
      (p) => p.mrid && !newPercentIds.includes(p.mrid)
    )
    const toUpdatePercent = (entity.percent || []).filter(
      (p) => p.mrid && oldPercentIds.includes(p.mrid)
    )
    ...
    const oldApparentPowerIds = (old_entity.apparentPower || [])
      .map((a) => a.mrid)
      .filter((id) => id)
    const toAddApparentPower = (entity.apparentPower || []).filter(
      (a) => a.mrid && !oldApparentPowerIds.includes(a.mrid)
    )
    const toDeleteApparentPower = (old_entity.apparentPower || []).filter(
      (a) => a.mrid && !newApparentPowerIds.includes(a.mrid)
    )
    const toUpdateApparentPower = (entity.apparentPower || []).filter(
      (a) => a.mrid && oldApparentPowerIds.includes(a.mrid)
    )
    ...
    const toAddTemperature = (entity.temperature || []).filter(
      (t) => t.mrid && !oldTemperatureIds.includes(t.mrid)
    )
    const toDeleteTemperature = (old_entity.temperature || []).filter(
      (t) => t.mrid && !newTemperatureIds.includes(t.mrid)
    )
    const toUpdateTemperature = (entity.temperature || []).filter(
      (t) => t.mrid && oldTemperatureIds.includes(t.mrid)
    )
    ...
    //productAssetModel
    const productAssetModelResult = await insertProductAssetModelTransaction(
    ...
    return {
      success: true,
      data: entity,
      message: 'Current Transformer entity inserted successfully'
    }
  } catch (error) {
    restoreFiles(null, null, entity.oldCurrentTransformerInfo.mrid)
    deleteBackupFiles(null, entity.oldCurrentTransformerInfo.mrid)
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataCurrentTransformer.success) {
        ...
        const dataOldCurrentTransformerInfo = await getOldCurrentTransformerInfoById(
        ...
        const dataCtCoreInfo = await getCtCoreInfoByCurrentTransformerInfoId(
          entity.oldCurrentTransformerInfo.mrid
        )
        ...
        return {
          success: false,
          error: dataCurrentTransformer.error,
          message: dataCurrentTransformer.message
        }
      }
    }
  } catch (error) {
...
export const deleteCurrentTransformerEntity = async (data) => {
  try {
    if (
      data.oldCurrentTransformerInfo == null ||
      data.oldCurrentTransformerInfo.mrid == null ||
      data.oldCurrentTransformerInfo.mrid === ''
    ) {
      return { success: false, error: new Error('Invalid ID') }
    } else {
      try {
        ...
        if (data.oldCurrentTransformerInfo && data.oldCurrentTransformerInfo.mrid) {
          await deleteCtCoreInfoByCurrentTransformerInfoIdTransaction(
            data.oldCurrentTransformerInfo.mrid,
            db
          )
        }
