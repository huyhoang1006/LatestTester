...
export const getAssetPsrByAssetIdAndPsrId = async (assetId, psrId) => {
  return new Promise((resolve, reject) => {
    db.get(
      'SELECT * FROM asset_psr WHERE asset_id=? AND psr_id=?',
      [assetId, psrId],
      (err, row) => {
        if (err)
          return reject({
            success: false,
            err: err,
            message: 'Get assetPsr by assetId and psrId failed'
          })
        if (!row) return resolve({ success: false, data: null, message: 'AssetPsr not found' })
    ...
    db.serialize(() => {
      ...
      db.run(
        `INSERT INTO asset_psr(
                ...
                `,
        [data.mrid, data.asset_id, data.psr_id],
        function (err) {
          if (err) {
            db.run('ROLLBACK')
    ...
    dbsql.run(
      ...
      function (err) {
        ...
        return resolve({
          success: true,
          data: data,
          message: 'Insert assetPsr transaction completed'
        })
      }
    )
  })
    ...
    db.serialize(() => {
      ...
      db.run(
        `UPDATE asset_psr SET
                    ...
                    psr_id = ?
                WHERE mrid = ?`,
        [data.asset_id, data.psr_id, mrid],
        function (err) {
          if (err) {
            db.run('ROLLBACK')
    ...
    dbsql.run(
      ...
      function (err) {
        ...
        return resolve({
          success: true,
          data: data,
          message: 'Update assetPsr transaction completed'
        })
      }
    )
  })
  ...
  return new Promise((resolve, reject) => {
    dbsql.run('DELETE FROM asset_psr WHERE mrid=?', [mrid], function (err) {
      ...
      return resolve({
        success: true,
        data: mrid,
        message: 'Delete assetPsr transaction completed'
      })
    })
  })
}
