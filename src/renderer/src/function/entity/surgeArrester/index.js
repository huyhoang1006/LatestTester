...
export const insertSurgeArresterEntity = async (old_entity, entity) => {
  try {
    if (entity.surgeArrester.mrid === null || entity.surgeArrester.mrid === '') {
      ...
      const oldOldSurgeArresterInfoIds = old_entity.assetInfoUnit
        .map((o) => o.mrid)
        .filter((id) => id)

      const toAddOldSurgeArresterInfo = entity.assetInfoUnit.filter(
        (o) => o.mrid && !oldOldSurgeArresterInfoIds.includes(o.mrid)
      )
      const toDeleteOldSurgeArresterInfo = old_entity.assetInfoUnit.filter(
        (o) => o.mrid && !newOldSurgeArresterInfoIds.includes(o.mrid)
      )
      const toUpdateOldSurgeArresterInfo = entity.assetInfoUnit.filter(
        (o) => o.mrid && oldOldSurgeArresterInfoIds.includes(o.mrid)
      )
      ...
      const toAddSurgeArrester = entity.assetUnit.filter(
        (o) => o.mrid && !oldSurgeArresterIds.includes(o.mrid)
      )
      const toDeleteSurgeArrester = old_entity.assetUnit.filter(
        (o) => o.mrid && !newSurgeArresterIds.includes(o.mrid)
      )
      const toUpdateSurgeArrester = entity.assetUnit.filter(
        (o) => o.mrid && oldSurgeArresterIds.includes(o.mrid)
      )
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataSurgeArrester.success) {
        ...
        const dataOldSurgeArresterInfo = await getOldSurgeArresterInfoBySurgeArresterId(
          entity.surgeArrester.mrid
        )
        ...
        if (entity.assetInfoUnit.length > 0) {
          ...
          for (let i = 0; i < entity.assetInfoUnit.length; i++) {
            ...
            for (let j = 0; j < currentFlowArr.length; j++) {
              const currentFlow = await getCurrentFlowById(
