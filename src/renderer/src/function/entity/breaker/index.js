    ...
    if (id == null || id === '') {
      ...
      if (dataBreaker.success) {
        ...
        if (entity.oldBreakerInfo && entity.oldBreakerInfo.mrid) {
          const dataBreakerRatingInfo = await getBreakerRatingInfoByBreakerInfoId(
          ...
          const dataBreakerContactSystemInfo = await getBreakerContactSystemInfoByBreakerInfoId(
            entity.oldBreakerInfo.mrid
          )
          ...
          const dataBreakerOtherInfo = await getBreakerOtherInfoByBreakerInfoId(
            entity.oldBreakerInfo.mrid
          )
          ...
          const dataAssessmentLimitBreakerInfo = await getAssessmentLimitBreakerInfoByBreakerInfoId(
            entity.oldBreakerInfo.mrid
          )
          if (dataAssessmentLimitBreakerInfo.success) {
            ...
            // Auxiliary Contacts
            const dataAuxiliaryContactsBreakerInfo =
            ...
            if (dataAuxiliaryContactsBreakerInfo.success && dataAuxiliaryContactsBreakerInfo.data) {
              ...
              if (entity.auxiliaryContactsBreakerInfo) {
                // Trip & Close Ops
                const dataTripOperation = await getTripOperationByAuxiliaryContactsId(
                ...
                const dataCloseOperation = await getCloseOperationByAuxiliaryContactsId(
                  entity.auxiliaryContactsBreakerInfo.mrid
                )
            ...
            const dataMiscellaneous = await getMiscellaneousBreakerInfoByAssessmentLimitId(
              entity.assessmentLimitBreakerInfo.mrid
            )
            ...
            const dataPickupVoltage = await getPickupVoltageBreakerInfoByAssessmentLimitId(
              entity.assessmentLimitBreakerInfo.mrid
            )
            ...
            const dataMotorCharacteristics =
            ...
            const dataUnderVoltage = await getUnderVoltageReleaseBreakerInfoByAssessmentLimitId(
              entity.assessmentLimitBreakerInfo.mrid
            )
            ...
            const dataOvercurrent = await getOvercurrentReleaseBreakerInfoByAssessmentLimitId(
              entity.assessmentLimitBreakerInfo.mrid
            )
        ...
        const dataOldOperatingMechanism = await getOldOperatingMechanismByAssetIdTransaction(
          entity.asset.mrid
        )
        if (dataOldOperatingMechanism.success && dataOldOperatingMechanism.data) {
          ...
          if (entity.oldOperatingMechanism.asset_info) {
            const infoRes = await getOldOperatingMechanismInfoById(
          ...
          if (entity.oldOperatingMechanism.product_asset_model) {
            const modelRes = await getProductAssetModelById(
          ...
          const dataOperatingComponent = await getOperatingMechanismComponentByOperatingMechanismId(
        ...
        activePowerIds = activePowerIds.filter((id) => id)
        lengthIds = lengthIds.filter((id) => id)
        massIds = massIds.filter((id) => id)
        volumeIds = volumeIds.filter((id) => id)
        pressureIds = pressureIds.filter((id) => id)
        quantityIds = quantityIds.filter((id) => id)
        frequencyIds = frequencyIds.filter((id) => id)
        temperatureIds = temperatureIds.filter((id) => id)
        capacitanceIds = capacitanceIds.filter((id) => id)
        resistanceIds = resistanceIds.filter((id) => id)
