  ...
  if (id == null || id === '') {
    ...
    try {
      ...
      if (dataSubstation.success) {
        ...
        const dataElectronicAddress = await getElectronicAddressById(
        ...
        const dataAttachment = await getAttachmentByForeignIdAndType(
          entity.substation.mrid,
          'substation'
        )
        ...
        const userIdentifiedObject = await getUserIdentifiedObjectByUserIdAndIdentifiedObjectId(
          user_id,
          entity.substation.mrid
        )
        ...
        const organisationLocation = await getOrganisationLocationByOrganisationIdAndLocationId(
          organisation_id,
          entity.location.mrid
        )
        ...
        const organisationPerson = await getOrganisationPersonByOrganisationIdAndPersonId(
          organisation_id,
          entity.person.mrid
        )
        ...
        const organisationPsr = await getOrganisationPsrByOrganisationIdAndPsrId(
          organisation_id,
          entity.substation.mrid
        )
    ...
    if (data.substation == null || data.substation.mrid == null || data.substation.mrid === '') {
      ...
      try {
        ...
        if (data.location && data.location.mrid) {
          const powerSystemResource = await getPowerSystemResourceByLocationIdTransaction(
            data.location.mrid,
            db
          )
