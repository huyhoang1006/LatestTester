...
import {
  ...
  getZeroSequenceImpedanceTableByTransformerEndIdAndZeroSequenceImpedance
    ...
    if (entity.asset.mrid === null || entity.asset.mrid === '') {
      ...
      if (entity.tapChanger) {
        ...
        const toAddPoint = tc.tapChangerTablePoint.filter(
          (v) => v.mrid && !oldIdsPoint.includes(v.mrid)
        )
        const toDeletePoint = oldTc.tapChangerTablePoint.filter(
          (v) => v.mrid && !newIdsPoint.includes(v.mrid)
        )
      ...
      const toAddBushing = entity.bushing.filter(
        (v) => v.bushing.mrid && !oldIdsBushing.includes(v.bushing.mrid)
      )
      const toDeleteBushing = old_entity.bushing.filter(
        (v) => v.bushing.mrid && !newIdsBushing.includes(v.bushing.mrid)
      )
      ...
      const oldIdsSurge = old_entity.surgeArrester
        .map((v) => v.surgeArrester.mrid)
        .filter((id) => id)

      const toAddSurge = entity.surgeArrester.filter(
        (v) => v.surgeArrester.mrid && !oldIdsSurge.includes(v.surgeArrester.mrid)
      )
      const toDeleteSurge = old_entity.surgeArrester.filter(
        (v) => v.surgeArrester.mrid && !newIdsSurge.includes(v.surgeArrester.mrid)
      )
      ...
      for (const surge of toUpdateSurge) {
        const oldSurge = old_entity.surgeArrester.find(
          (x) => x.surgeArrester.mrid === surge.surgeArrester.mrid
        )
  ...
  try {
    if (id == null || id === '') {
      ...
      if (dataTransformer.success) {
        ...
        const dataProductAssetModel = await getProductAssetModelById(
        ...
        const dataOldTransformerEndInfo = await getOldTransformerEndInfoByPowerTransformerInfoId(
        ...
        // Load Other
        const dataOther = await getOtherByPowerTransformerInfoId(
        ...
        const dataCoolingPowerRating = await getCoolingPowerRatingByPowerTransformerInfoId(
          entity.oldPowerTransformerInfo.mrid
        )
        ...
        const dataShortCircuitRating = await getShortCircuitRatingByPowerTransformerInfoId(
          entity.oldPowerTransformerInfo.mrid
        )
        ...
        for (const shortCircuitTest of entity.oldTransformerEndInfo) {
          const dataShortCircuitTest = await getShortCircuitTestByTransformerEndInfoId(
        ...
        for (const shortCircuitTest of entity.shortCircuitTest) {
          const dataSCTTransformerEndInfo = await getSCTTransformerEndInfoByShortCircuitTestId(
            shortCircuitTest.mrid
          )
        ...
        const dataZeroSequenceImpedance = await getZeroSequenceImpedanceByTransformerInfoId(
          entity.oldPowerTransformerInfo.mrid
        )
        ...
        const dataZeroSequenceImpedanceTable =
        ...
        const dataOldTapChangerInfo = await getOldTapChangerInfoByPowerTransformerInfoId(
          entity.oldPowerTransformerInfo.mrid
        )
        ...
        const dataProductAssetModelTapChanger = await getProductAssetModelById(
        ...
        const dataTapChangerTablePoint = await getTapChangerTablePointByTapChangerInfoId(
        ...
        const dataTapChangerVoltage = await getVoltageByIds(
          entity.tapChanger.tapChangerTablePoint.map((x) => x.voltage)
        )
        ...
        if (dataMridOldBushingInfo.success) {
          ...
          for (const dataMridOldBushingInfoId of dataMridOldBushingInfoIds) {
            ...
            if (dataMridBushing.success) {
              const dataBushing = await bushingFunc.getBushingEntityLiteById(
    ...
    if (data.asset == null || data.asset.mrid == null || data.asset.mrid === '') {
      ...
      try {
        ...
        for (const surge of data.surgeArrester) {
          // FIX: Handle FK constraint by clearing voltage array in object passed to delete function
          // This prevents internal deletion of voltage before Info is deleted
          const surgeVoltages = surge.voltage ? [...surge.voltage] : []
          surge.voltage = []

          // Delete the Surge Arrester (Asset + Info + Table Points etc)
          await deleteSurgeArresterLiteEntity(surge, db)

          // Safely delete voltages using tryDelete (FK constraint is now gone as Info is deleted)
        ...
        if (data.tapChanger) {
          // 1. Xóa các điểm Table Points (Con của Info)
          ...
          // 2. Xóa Asset (Tap Changer) - QUAN TRỌNG: Phải xóa Asset trước để gỡ bỏ tham chiếu tới Info
          if (data.tapChanger.asset && data.tapChanger.asset.mrid) {
            await deleteAssetByIdTransaction(data.tapChanger.asset.mrid, db)
          }

          // 3. Xóa Info (OldTapChangerInfo) - Sau khi Asset đã bị xóa
          ...
          // 4. Xóa Voltage (Dữ liệu tham chiếu) - Dùng tryDelete để tránh lỗi nếu đang được dùng chỗ khác
          ...
          // 5. Xóa Product Asset Model của Tap Changer (nếu có và không dùng chung)
          if (data.tapChanger.productAssetModel && data.tapChanger.productAssetModel.mrid) {
            // Kiểm tra xem có Asset nào khác dùng Model này không
            ...
            // Kiểm tra xem có Info nào khác dùng Model này không
