 ...
 *
 * Đặt trên <el-radio-group>. Element Plus không hỗ trợ sẵn: bấm vào radio đã chọn
 ...
 *
 * Vue 3: hook đổi thành mounted/unmounted; ElRadioGroup dùng v-model
 * (modelValue / update:modelValue) thay cho `input` của Vue 2 + Element UI.
 */
...
const findRadioLabel = (target) => {
  ...
  while (node && node !== document) {
    if (
      node.classList &&
      (node.classList.contains('el-radio') || node.classList.contains('el-radio-button'))
    ) {
      return node
    }
...
const mounted = (el, binding, vnode) => {
  ...
  if (DEBUG) console.log('[radio-clearable] mounted vao', el.className)

  const handler = (event) => {
    const label = findRadioLabel(event.target)
    if (DEBUG)
      console.log(
        '[radio-clearable] click, tim thay label:',
        !!label,
        '| is-checked:',
        label && label.classList.contains('is-checked')
      )
    ...
    if (DEBUG)
      console.log(
        '[radio-clearable] group:',
        !!group,
        '| value hien tai:',
        group && group.modelValue
      )
    if (!group) return
    ...
    group.$emit('update:modelValue', emptyValue)
    group.$emit('change', emptyValue)

    if (DEBUG) {
      group.$nextTick(() => {
        console.log(
          '[radio-clearable] sau nextTick — group.modelValue:',
          group.modelValue,
          '| label con is-checked:',
