...
const getters = {
  ...
  percent: (state) =>
    state.total > 0 ? Math.min(100, Math.round((state.done / state.total) * 100)) : null
}
...
const actions = {
  ...
  start(
    ...
    { action = 'default', text = null, total = 0, type = 'default' } = {}
  ) {
    if (state.active) {
    ...
    commit('BEGIN', {
      ...
      text,
      total,
      timeoutMs: HEARTBEAT_TIMEOUT[type] || HEARTBEAT_TIMEOUT.default
    })
