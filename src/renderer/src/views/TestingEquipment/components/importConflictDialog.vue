<template>
  ...
  >
    <div class="cf-hint">
      {{ rows.length }} imported equipment already exist in this database. Choose what to do with
      each one — the rest of the file will be imported normally.
    </div>
        ...
        <tbody>
          <tr v-for="(row, i) in rows" :key="i">
            <td>
              ...
              <div class="cf-sub">
                {{ [row.manufacturer, row.type, row.model].filter(Boolean).join(' · ') }}
              </div>
            </td>
...
export default {
  ...
  methods: {
    ...
    confirm() {
      this.$emit(
        'confirm',
        this.rows.map((r) => ({ index: r.index, choice: r.choice }))
      )
    }
  }
}
