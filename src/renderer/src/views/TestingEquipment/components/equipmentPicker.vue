<template>
  <el-dialog
    title="Select Testing Equipment"
    :model-value="visible"
  ...
  >
    <div style="margin-bottom: 8px; display: flex; align-items: center; gap: 8px">
      <el-input
        ...
        placeholder="Search by name, model, serial no., manufacturer"
        style="flex: 1"
      />
      <el-button
        size="mini"
    ...
    >
      ...
      <el-table-column label="Equipment name" prop="name" min-width="150" show-overflow-tooltip>
        <template #default="{ row }">
          {{ row.name || '—' }}
          <el-tag v-if="row.is_accessory" size="mini" type="warning" style="font-size: 10px"
            >Accessory</el-tag
          >
        </template>
      </el-table-column>
    ...
    <template #footer>
      <span style="display: flex; justify-content: space-between; align-items: center">
        <span style="font-size: 11px; color: #909399"
          >{{ selection.length }} selected · {{ filtered.length }} /
          {{ items.length }} equipment</span
        >
        <span>
          <el-button size="small" @click="$emit('update:visible', false)">Cancel</el-button>
          <el-button size="small" type="primary" :disabled="!selection.length" @click="confirm">{{
            confirmLabel
          }}</el-button>
        </span>
      </span>
  ...
  methods: {
    async load() {
      ...
      try {
        ...
        const rs = await window.electronAPI.getAllTestingEquipmentList(
