  ...
  <div class="lic">
    ...
    <div class="lic-grid">
      <div v-for="(l, i) in licenses" :key="i" class="lic-card" :class="{ enabled: l.enabled }">
        <div class="lic-top">
          ...
          <label class="switch">
            <input
              type="checkbox"
              :checked="l.enabled"
              @change="l.enabled = $event.target.checked ? 1 : 0"
            />
        ...
        <div class="lic-foot">
          <span class="tag" :class="l.enabled ? 'green' : 'gray'">{{
            l.enabled ? 'Enabled' : 'Disabled'
          }}</span>
          <span v-if="expiryTag(l)" class="tag" :class="expiryTag(l).cls">{{
            expiryTag(l).label
          }}</span>
    ...
    <div v-if="pickerOpen" class="lp-overlay" @click.self="closePicker">
      <div class="lp-modal">
        ...
        <div class="lp-list">
          <label
            v-for="opt in filteredCatalog"
            :key="opt"
            class="lp-item"
            :class="{ disabled: isAdded(opt) }"
          >
            <input
              type="checkbox"
              :disabled="isAdded(opt)"
              :checked="isAdded(opt) || picked.includes(opt)"
              @change="togglePick(opt)"
            />
        ...
        <div class="lp-custom">
          <input
            class="lp-custom-inp"
            v-model="customName"
            placeholder="Or type a new license name…"
            @keyup.enter="addCustom"
          />
          <button class="btn sm" :disabled="!customName.trim()" @click="addCustom">
            Add custom
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    isAdded(opt) {
      return this.addedNames.has(String(opt).trim().toLowerCase())
    },
    togglePick(opt) {
