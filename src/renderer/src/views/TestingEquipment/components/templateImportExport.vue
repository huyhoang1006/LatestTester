<template>
  ...
  >
    ...
    >
      <template #actions>
        <el-button type="primary" size="mini" :disabled="!selectedTemplateName" @click="addRow()"
          >Add Row</el-button
        >
        <el-button
          type="success"
          ...
          @click="handleUploadTemplateFile"
        >
          {{ fileType === 'word' ? 'Word File' : 'Excel File' }}
        </el-button>
        <el-button type="danger" size="mini" :disabled="!selectedTemplateName" @click="handleDelete"
          >Delete</el-button
        >
        <el-button type="warning" size="mini" :disabled="!selectedTemplateName" @click="handleSave"
          >Save</el-button
        >
        <el-button
          type="info"
    ...
    <div v-if="currentFilePath" style="margin: 4px 0 8px; font-size: 12px; color: #909399">
      <span
        ...
        onmouseout="this.style.color = '#909399'"
      >
        <i
          :class="fileType === 'word' ? 'fa-solid fa-file-word' : 'fa-solid fa-file-excel'"
          :style="{ color: fileType === 'word' ? '#2B579A' : '#67C23A' }"
        ></i>
    ...
    >
      ...
      >
        <el-form-item label="Template name" prop="name"
          ><el-input v-model="addForm.name" placeholder="e.g. Equipment list" clearable
        /></el-form-item>
    ...
    <el-dialog
      title="Manufacturer by sheet"
      v-model="manufacturerSheetVisible"
      width="560px"
      append-to-body
    >
      <div class="mfr-sheet-note">
        Select the manufacturer for each sheet. These values are used when the imported file does
        not provide a manufacturer column.
      </div>
      ...
      <template #footer>
        <div>
          ...
          <el-button size="small" type="primary" @click="confirmManufacturerSheet"
            >Continue Import</el-button
          >
        </div>
      </template>
    </el-dialog>
...
export default {
  ...
  computed: {
    dialogTitle() {
      return (
        (this.mode === 'import' ? 'Import' : 'Export') +
        ' testing equipment — ' +
        (this.fileType === 'word' ? 'Word' : 'Excel') +
        ' template'
      )
    },
  ...
  methods: {
    ...
    async loadTemplates() {
      try {
        ...
        if (rs && rs.success) {
          this.templateList = rs.data.map((t) => ({
            ...t,
    ...
    resetAddDialog() {
      this.addForm = { name: '', filePath: '' }
      this.addLoading = false
    },
    async uploadTemplateForNew() {
      if (!this.addForm.name) {
        this.$message.warning('Please enter template name first')
        return
      }
      try {
        let rs
        ...
        if (rs && rs.success) {
          this.addForm.filePath = rs.filePath
    ...
    async confirmAddTemplate() {
      this.$refs.addForm.validate(async (valid) => {
        ...
        try {
          ...
          if (exists && exists.data === true) {
            this.$message.error(`Template "${this.addForm.name}" already exists`)
            return
          }
          const rs = await window.electronAPI.insertTemplate({
            name: this.addForm.name,
            path: this.addForm.filePath || '',
            variable: JSON.stringify([]),
            type: this.fileType,
            category: this.storeCategory
          })
          if (rs && rs.success) {
            this.$message.success(`Created "${this.addForm.name}"`)
    ...
    async handleUploadTemplateFile() {
      ...
      try {
        let rs
        if (this.fileType === 'word')
          rs = await window.electronAPI.uploadWordTemplate(this.selectedTemplateName)
        else rs = await window.electronAPI.uploadExcelTemplate(this.selectedTemplateName)
        if (!rs || rs.canceled) return
        if (rs.success) {
          this.currentFilePath = rs.filePath
          await this.saveWithScan()
        }
      } catch (e) {
        this.$message.error(e.message)
      }
    ...
    async saveWithScan() {
      if (!this.selectedTemplateName || !this.currentFilePath) {
        this.$message.warning('Please upload a Template file first')
        return
      }
    ...
    async handleImportJson() {
      try {
        ...
        if (!Array.isArray(variables)) {
          this.$message.error('Invalid template config file')
          return
        }
    ...
    async handleExportJson() {
      try {
        ...
        const rs = await window.electronAPI.exportJSON([variables], {
          defaultFileName,
          title: 'Save template config as JSON'
        })
        if (rs && rs.success) this.$message.success('Exported: ' + rs.filePath)
        else if (rs && rs.message !== 'Export cancelled')
    ...
    handleExportClick() {
      if (!this.tableData.some((r) => r.code && r.category)) {
        this.$message.warning('Template has no rows — add rows and Save first')
        return
      }
    ...
    async onEquipmentSelected(rows) {
      ...
      try {
        ...
        const codeMap = await templateExport.buildCodeMap(
          rows.map((r) => r.mrid),
          this.tableData
        )
        if (!Object.keys(codeMap).length) {
          this.$message.warning('No matching data — check template rows and Save')
          return
        }
        ...
        if (rs && rs.canceled) return
        if (rs && rs.success)
          this.$message.success('Exported ' + rows.length + ' equipment: ' + rs.filePath)
    ...
    async handleImportFile() {
      ...
      try {
        ...
        this.startImportProgress(
          1,
          this.fileType === 'word' ? 'Reading Word file...' : 'Reading Excel file...'
        )
        const read =
          this.fileType === 'word'
        ...
        if (!read || !read.success) {
          this.$message.error((read && read.message) || 'Failed to read file')
          return
        }
    ...
    async prepareImport(read, manufacturerBySheet) {
      ...
      const hasTemplateRows = this.tableData.some(
        (row) =>
          row.code &&
          templateImport.getLeafValue(row.featureLevels, row.category || 'TestingEquipmentDto')
      )
      if (!hasTemplateRows) {
        this.$message.warning(
          'Template has no valid testing equipment fields. Add rows, select features, then Save.'
        )
        return
      }
      if (!hasReadValues) {
        this.$message.warning(
          'No values were read from the selected file. Check template coordinates and make sure the data file uses the same layout.'
        )
        return
      }
      ...
      if (!this.pending) {
        this.$message.info('No testing equipment data found in file')
        return
      }
    ...
    async confirmManufacturerSheet() {
      ...
      this.manufacturerSheetRows.forEach((row) => {
        manufacturerBySheet[row.sheet] = row.manufacturer
      })
    ...
    onConflictConfirm(choices) {
      ...
      const pending = this.pending
      if (!pending) return
      const overwriteIdx = new Set(
        choices.filter((c) => c.choice === 'overwrite').map((c) => c.index)
      )
    ...
    addRow(index) {
      ...
      const r = {
        code: '',
        category: 'TestingEquipmentDto',
        featureLevels: [{ key: '' }],
        coordinates: []
      }
      if (typeof index === 'number') this.tableData.splice(index + 1, 0, r)
      else this.tableData.push(r)
    },
    removeRow(index) {
    ...
    getNodeByLevel(row, levelIndex) {
      let node = FEATURE_TREE[row.category]
      if (!node) return null
