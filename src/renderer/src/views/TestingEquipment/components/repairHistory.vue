<template>
  <div class="rep">
    <div class="sec-title">
      ...
      <button class="btn primary sm" type="button" @click="openTicketDialog">
      ...
      <div class="ticket-dialog" @click.stop>
        <div class="ticket-page-head">
          ...
          <div class="ticket-actions">
            <button class="btn cancel-button" type="button" @click.stop.prevent="closeTicketDialog">
              Cancel
            </button>
        ...
        <div class="ticket-grid">
          <section class="ticket-panel main">
            ...
            <div class="ticket-row">
              ...
              <div class="component-line">
                <div class="component-chip-list">
                  <span v-if="!ticketDraft.components.length" class="component-empty"
                    >No component selected</span
                  >
                  <span
                    v-for="component in ticketDraft.components"
                    :key="component.id"
                ...
                <button class="btn ghost" type="button" @click.stop="openComponentDialog">
                  + Add Component
                </button>
              </div>
            </div>
            ...
            <label class="ticket-row">
              ...
              <input
                class="ticket-field ticket-control"
                type="date"
                v-model="ticketDraft.created_date_time"
              />
            </label>
            ...
            <label class="ticket-row">
              ...
              <div class="status-stack ticket-control">
                <select class="ticket-field" v-model="ticketDraft.ticket_status">
                  <option v-for="s in ticketStatuses" :key="s.label" :value="s.label">
                    {{ s.label }}
                  </option>
                </select>
                <div class="status-options">
                  <button
                    ...
                    :class="[
                      'status-choice',
                      s.tone,
                      { active: ticketDraft.ticket_status === s.label }
                    ]"
          ...
          <aside class="ticket-side">
            <section class="ticket-panel">
              ...
              <div class="summary-row">
                <span>Ticket ID</span><b>{{ draftTicketId }}</b>
              </div>
              <div class="summary-row">
                <span>Created date</span><b>{{ ticketDraft.created_date_time || '-' }}</b>
              </div>
              <div class="summary-row">
                <span>Current status</span><b>{{ ticketStatusLabel }}</b>
              </div>
              <div class="summary-row">
                <span>Severity</span><b>{{ ticketSeverity }}</b>
              </div>
              <div class="summary-row">
                <span>Vendor</span><b>{{ ticketDraft.provider || '-' }}</b>
              </div>
              <div class="summary-row">
                <span>Location</span><b>{{ ticketDraft.location || '-' }}</b>
              </div>
            </section>
            ...
            <section class="ticket-panel legend">
              ...
              <div class="legend-row">
                <span class="legend-dot red"></span>Critical - Requires immediate attention
              </div>
              <div class="legend-row"><span class="legend-dot orange"></span>High - Act soon</div>
              <div class="legend-row">
                <span class="legend-dot yellow"></span>Medium - Moderate priority
              </div>
        ...
        <div
          v-if="componentDialogVisible"
          class="component-modal-mask"
          @click.self="closeComponentDialog"
        >
  ...
  methods: {
    ...
    editTicket(index) {
      ...
      this.ticketDraft = Object.assign(blankTicket(), {
        ...
        ticket_status:
          record.ticket_status || (record.status === 'Completed' ? 'Closed' : 'In Progress'),
    ...
    saveTicket() {
      ...
      const reason = [faultDescription, note]
        ...
        .join('\n')
      const currentRecord =
        this.editingRecordIndex >= 0 ? this.records[this.editingRecordIndex] : {}
      ...
      if (this.editingRecordIndex >= 0) this.records[this.editingRecordIndex] = record
    ...
    maintenanceCardClass(record) {
      return this.toRepairStatus(this.repairStatusLabel(record)) === 'Completed'
        ? 'closed'
        : 'active'
    },
    repairTitle(record) {
...
.maintenance-badge.vendor {
  background: #eef4ff;
  color: var(--blue-900);
}
.maintenance-badge.severity-critical {
  background: #fee2e2;
  color: #b91c1c;
}
.maintenance-badge.severity-high {
  background: #ffedd5;
  color: #c2410c;
}
.maintenance-badge.severity-medium {
  background: #fef3c7;
  color: #a16207;
}
.maintenance-badge.severity-low {
  background: #dcfce7;
  color: #15803d;
}
.empty-card {
...
.status-pill.severity-critical {
  background: #fff1f2;
  color: #dc2626;
  border: 1px solid #fecdd3;
}
.status-pill.severity-high {
  background: #fff7ed;
  color: #ea580c;
  border: 1px solid #fed7aa;
}
.status-pill.severity-medium {
  background: #fffbeb;
  color: #d97706;
