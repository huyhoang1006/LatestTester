    ...
    <table>
      ...
      <tbody>
        <tr v-if="!items.length">
          <td colspan="6" class="empty">No accessories</td>
        </tr>
    ...
    <div v-if="pickerOpen" class="ap-overlay" @click.self="closePicker">
      <div class="ap-modal">
        ...
        <input
          ...
          placeholder="Search existing accessories…"
        />

        <div class="ap-list">
          <div v-if="loading" class="ap-empty">Loading…</div>
          <label
            v-for="opt in filteredPool"
            :key="opt.mrid"
            class="ap-item"
            :class="{ disabled: isAdded(opt.mrid) }"
          >
            <input
              type="checkbox"
              :disabled="isAdded(opt.mrid)"
              :checked="isAdded(opt.mrid) || picked.includes(opt.mrid)"
              @change="togglePick(opt.mrid)"
            />
            <span class="ap-name">{{ opt.name || '(no name)' }}</span>
            <span class="ap-sub"
              >{{ opt.model }}<span v-if="opt.serial_no"> · S/N {{ opt.serial_no }}</span></span
            >
            <span v-if="isAdded(opt.mrid)" class="ap-added">added</span>
          </label>
          <div v-if="!loading && !filteredPool.length" class="ap-empty">
            No existing accessory in the pool
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    isAdded(mrid) {
      return this.addedMrids.has(mrid)
    },
    togglePick(mrid) {
