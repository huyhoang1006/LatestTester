  ...
  >
    <div class="cm-shell">
      <div class="cm-side">
        ...
        <el-checkbox v-model="useFirstRowAsHeader" class="cm-check"
          >Use first row as header</el-checkbox
        >
        ...
        <div class="cm-format-grid">
          ...
          <label>
            ...
            <el-input
              v-model="formatOptions.textQualifier"
              size="small"
              maxlength="1"
              placeholder='"'
            />
          ...
          <label>
            ...
            <el-input
              v-model="formatOptions.datetimeFormat"
              size="small"
        ...
        <div class="cm-hint">
          Default rule: each source row is one equipment. Rows with the same manufacturer, serial
          no. and type are merged before duplicate checking.
        </div>
      </div>
        ...
        <div class="cm-actions">
          <div>
            ...
            <div class="cm-subtitle">
              {{ sourceColumns.length }} columns, {{ dataRows.length }} of
          ...
          <table class="cm-table">
            ...
            <tbody>
              <tr v-for="col in sourceColumns" :key="col.key">
                ...
                <td>
                  <span v-if="duplicateMappedFields[mapping[col.key]]" class="cm-warning"
                    >Mapped more than once</span
                  >
  ...
  computed: {
    ...
    sheetOptions() {
      return this.fileData && Array.isArray(this.fileData.sheets)
        ? this.fileData.sheets.map((s) => s.name)
        : []
    },
    selectedSheet() {
      return this.fileData && Array.isArray(this.fileData.sheets)
    ...
    rawRows() {
      return this.selectedSheet && Array.isArray(this.selectedSheet.rows)
        ? this.selectedSheet.rows
        : []
    },
    headerRow() {
    ...
    allDataRows() {
      ...
      return rows.filter((row) =>
        (row || []).some((value) => String(value == null ? '' : value).trim() !== '')
      )
    },
    dataRows() {
    ...
    sourceColumns() {
      ...
      return Array.from({ length: maxCol }, (_, index) => {
        ...
        const sampleRow = this.dataRows.find(
          (row) => String(row[index] == null ? '' : row[index]).trim() !== ''
        )
        return {
          key: `col_${index}`,
          index,
    ...
    canImport() {
      return (
        !!this.sourceColumns.length &&
        !!this.dataRows.length &&
        Object.values(this.mapping).includes('serial_no')
      )
    },
    userId() {
  ...
  methods: {
      ...
      this.presets =
        rs && rs.success
          ? rs.data.map((row) => {
              let variable = {}
              try {
                variable = row.variable ? JSON.parse(row.variable) : {}
              } catch (e) {
                variable = {}
              }
              return { ...row, variable }
    ...
    fileReadOptions() {
      return {
        ...
        textQualifier:
          this.formatOptions.textQualifier == null ? '"' : this.formatOptions.textQualifier,
        dateFormat: this.formatOptions.dateFormat || '',
        datetimeFormat: this.formatOptions.datetimeFormat || '',
        thousandsSeparator:
          this.formatOptions.thousandsSeparator == null
            ? ','
            : this.formatOptions.thousandsSeparator,
    ...
    async pickFile() {
      const rs = await window.electronAPI.pickTestingEquipmentColumnImportFile(
    ...
    async reloadFile() {
      ...
      if (!rs || !rs.success)
        return this.$message.error((rs && rs.message) || 'Failed to reload file')
      this.fileData = rs
      this.fileName = rs.fileName || ''
      this.selectedSheetName =
        rs.sheets && rs.sheets.some((sheet) => sheet.name === currentSheet)
          ? currentSheet
    ...
    applyPreset(name) {
      ...
      if (config.formatOptions)
        this.formatOptions = { ...this.formatOptions, ...config.formatOptions }
      if (config.batchOptions) this.batchOptions = { ...this.batchOptions, ...config.batchOptions }
      const next = {}
      config.mappings.forEach((item) => {
        let col = null
        if (item.header)
          col = this.sourceColumns.find(
            (c) => this.normalizeLabel(c.header) === this.normalizeLabel(item.header)
          )
    ...
    async savePreset() {
      try {
        const name = await this.$prompt('Enter mapping preset name', 'Save mapping', {
          ...
          inputValue:
            this.selectedPresetName || (this.fileName ? this.fileName.replace(/\.[^.]+$/, '') : '')
        })
        ...
        const rs = existing
          ? await window.electronAPI.updateTemplate(payload)
          : await window.electronAPI.insertTemplate(payload)
        if (rs && rs.success) {
    ...
    async runImport(newcomers, overwrites) {
      ...
      try {
        ...
        this.$message.success(
          `Imported ${rs.ok} equipment` + (rs.fail ? `, ${rs.fail} failed` : '')
        )
        this.$emit('imported')
...
.cm-section-title.preset {
  margin-top: 18px;
}
.cm-section-title.format,
.cm-section-title.batch {
  margin-top: 14px;
}
.cm-reload-file {
  margin-left: 6px;
