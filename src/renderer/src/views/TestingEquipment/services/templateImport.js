...
const SERIAL_AWARE_PROP_FIELDS = [
  'name',
  'type',
  'manufacturer',
  'model',
...
const normalizeDate = (v) => {
  ...
  const namedDayFirst = text.match(
    /^(\d{1,2})(?:st|nd|rd|th)?[-\s,/]+([A-Za-z]{3,9})[-\s,/]+(\d{2}|\d{4})$/i
  )
  if (namedDayFirst)
    return toIsoDate(
      normalizeYear(namedDayFirst[3]),
      MONTHS[namedDayFirst[2].toLowerCase()],
      namedDayFirst[1]
    )
  const namedMonthFirst = text.match(
    /^([A-Za-z]{3,9})[-\s,/]+(\d{1,2})(?:st|nd|rd|th)?(?:,)?[-\s,/]+(\d{2}|\d{4})$/i
  )
  if (namedMonthFirst)
    return toIsoDate(
      normalizeYear(namedMonthFirst[3]),
      MONTHS[namedMonthFirst[1].toLowerCase()],
      namedMonthFirst[2]
    )
...
export const templateImport = {
  ...
  buildDtos(lvm, userId, manufacturerBySheet) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const serialValues = (serials.length ? serials : [rawSerial]).filter(
        (serial) => !isBlankSerial(serial)
      )
      ...
      serialValues.forEach((serial) => {
        ...
        PROP_FIELDS.forEach((f) => {
          ...
          if (
            f === 'manufacturer' &&
            (!arr || arr[i] === undefined || String(arr[i]).trim() === '')
          ) {
        ...
        dto.calibration = this.resolveNestedForSerial(
          this._buildNested(lvm, 'cal_', CAL_FIELDS, i, count),
          'cal_',
          serial,
          serialValues
        )
        dto.licenses = this.resolveNestedForSerial(
          this._buildNested(lvm, 'lic_', LIC_FIELDS, i, count),
          'lic_',
          serial,
          serialValues
        )
        dto.repairs = this.resolveNestedForSerial(
          this._buildNested(lvm, 'rep_', REP_FIELDS, i, count),
          'rep_',
          serial,
          serialValues
        )
  ...
  normalizeDtoDates(dto) {
    ...
    if (dto.properties && isYearOnlyDate(dto.properties.in_use_date)) {
      ...
      if (
        ...
        dateYear(dto.properties.manufacturer_year) === inUseYear
      ) {
    ...
    ;(dto.licenses || []).forEach((record) =>
      DATE_NESTED_FIELDS.lic_.forEach((field) => normalizeDateField(record, field))
    )
    ...
    const hasInProgressRepair = (dto.repairs || []).some(
      (record) => record && record.status === 'InProgress'
    )
    if (
      hasInProgressRepair &&
      dto.properties &&
      (!dto.properties.status || dto.properties.status === 'Available')
    ) {
  ...
  getSheetForIndex(lvm, index) {
    ...
    const first = Object.keys(sheetsByLeaf).find(
      (key) => sheetsByLeaf[key] && sheetsByLeaf[key][index]
    )
  ...
  splitRepairReason(value) {
    ...
    const protectedText = text
      ...
      .replace(/\b\d{1,2}\/\d{1,2}(?:\/\d{2,4})?\b/g, (match) =>
        match.replace(/\//g, '__DATE_SLASH__')
      )
      ...
      .replace(/\b\d{1,2}\\\d{1,2}(?:\\\d{2,4})?\b/g, (match) =>
        match.replace(/\\/g, '__DATE_BACKSLASH__')
      )
    ...
    const ascii = (part) =>
      String(part || '')
        ...
        .replace(/đ/g, 'd')
        .replace(/Đ/g, 'D')
    const startsEvent = (part) =>
      /^['"`\s]*(?:[-•*]+\s*|\d+[.)]\s*|[A-Z0-9]+:\s*|(?:bi hong|loi|sua|thay|gui|chuyen|ngay)\b)/i.test(
        ascii(part)
      )
    const events = []
    ...
    const resolved = records
      .flatMap((record) => {
        ...
        serialAwareFields.forEach((field) => {
          if (base[field])
            base[field] = this.resolveSerialAwareText(base[field], serial, allSerials)
        })
      ...
      .filter((record) =>
        Object.keys(record).some(
          (key) => key !== 'mrid' && record[key] !== '' && record[key] != null
        )
      )
    ...
    return String(value || '')
      ...
      .map((line) =>
        line
          .replace(/^[\s'"`•\-]+/, '')
          .replace(
            new RegExp(`^(?:(?:${serialPattern})\\s*(?:,|;|/|\\+|&|and)?\\s*)+[:：\\-]?\\s*`, 'i'),
            ''
          )
          .trim()
      )
  ...
  async prepare(codeValueMap, tableData, userId, options = {}) {
    ...
    const entities = this.buildDtos(lvm, userId, options.manufacturerBySheet || {}).map(
      mapDtoToEntity
    )
  ...
  async run(newcomers, overwrites, options = {}) {
    ...
    for (const n of newcomers) {
      try {
        await doInsert(n.entity)
      } catch (e) {
        console.error('TE import item failed:', e)
        fail++
      }
    }
    for (const c of overwrites) {
      try {
        ...
        console.error('TE overwrite item failed:', e)
        fail++
      }
    }
    return { ok, fail }
