...
export const templateExport = {
  ...
  async buildMaps(mrid) {
    ...
    const flatMap = {
      name: s(p.name),
      type: s(p.type),
      serial_no: s(p.serial_no),
      manufacturer: s(p.manufacturer),
      model: s(p.model),
      manufacturer_year: s(p.manufacturer_year),
      asset_tag: s(p.asset_tag),
      status: s(p.status),
      country_of_origin: s(p.country_of_origin),
      in_use_date: s(p.in_use_date),
      comment: s(p.comment),
    ...
    try {
      ...
      console.error('get usage history failed:', e)
    }
    const arrayMap = {
