...
export const entityKeyFields = (entity) => ({
  mrid:
    (entity.testingEquipment && entity.testingEquipment.mrid) ||
    (entity.asset && entity.asset.mrid) ||
    null,
  name:
    (entity.identifiedObject && entity.identifiedObject.name) ||
    (entity.asset && entity.asset.name) ||
    '',
...
export const findExisting = (fields, existingList) => {
  ...
  if (norm(fields.serial)) {
    hit = existingList.find(
      (e) =>
        norm(e.manufacturer) === norm(fields.manufacturer) && norm(e.serial) === norm(fields.serial)
    )
