...
const NUMBER_FIELDS = new Set(['cal_interval_months', 'rep_cost'])
...
const normalizeMappedValue = (value, fieldValue, options = {}) => {
  if (DATE_FIELDS.has(fieldValue)) {
    const format =
      fieldValue === 'rep_created_date_time'
...
const compactObjectValues = (obj, ignoreKeys = []) =>
  Object.keys(obj || {})
...
const mergeEntityInto = (target, source) => {
  const mrid =
    (target.testingEquipment && target.testingEquipment.mrid) || (target.asset && target.asset.mrid)
  if (target.asset && source.asset) {
    fillBlank(target.asset, source.asset, [
      'name',
      'description',
      'serial_number',
      'type',
  ...
  if (target.productAssetModel && source.productAssetModel) {
    fillBlank(target.productAssetModel, source.productAssetModel, [
      'name',
      'model_number',
      'manufacturer'
    ])
  ...
  target.calibrations = appendDistinct(
    target.calibrations,
    source.calibrations,
    mrid,
    'testing_equipment'
  ...
  const hasInProgressRepair = (target.repairs || []).some(
    (record) => record && record.severity === 'InProgress'
  )
  if (
    hasInProgressRepair &&
    target.asset &&
    (!target.asset.in_use_state || target.asset.in_use_state === 'Available')
  ) {
...
export const prepareColumnMappingImport = async ({
  rows,
  mappings,
  sheetName,
  userId,
  ...
  const selectedMappings = (mappings || []).filter(
    (m) => m && m.field && m.field.featureLevels && m.field.featureLevels.length
  )
  ...
  const tableData = selectedMappings.map((mapping) => {
    codeValueMap[mapping.key] = dataRows.map((row) =>
