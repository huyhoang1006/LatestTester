  ...
  <div class="te-root">
    ...
    <div class="te-header">
      ...
      <div class="te-header-side">
        ...
        <span v-if="calibrationTag" class="tag" :class="calibrationTag.cls">{{
          calibrationTag.label
        }}</span>
      </div>
    </div>
    ...
    <div class="te-kpi-row">
      ...
      <div class="te-kpi">
        ...
        <div class="te-kpi-value">
          {{ enabledLicenses
          }}<span class="te-kpi-sub">/{{ testingEquipmentDto.licenses.length }}</span>
        </div>
    ...
    <div class="te-tabs-shell">
      ...
      <div class="te-tab-body">
        ...
        <attachment-common
          ...
          :attachment_="testingEquipmentDto.attachments"
          height="240px"
...
export default {
  ...
  data() {
    return {
      activeTab: 'overview',
      // Calibration / Software License / Repair History / Usage History / Accessories
      // tabs are deferred to a later phase — the underlying entity/DB layer already
      // supports them (testingEquipmentDto.calibration/licenses/repairs/usage/accessories),
      // only the tab UI components still need to be built.
      tabs: [
  ...
  computed: {
    ...
    enabledLicenses() {
      return this.testingEquipmentDto.licenses.filter((l) => l.enabled).length
    },
    nextDue() {
      const dues = this.testingEquipmentDto.calibration
        .map((c) => c.due_date)
        .filter(Boolean)
        .sort()
  ...
  methods: {
    // Nạp form: ưu tiên mrid (đọc DB) -> bản ghi được chọn -> form trống (create)
    initForm() {
