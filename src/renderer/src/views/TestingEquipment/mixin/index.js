  ...
  methods: {
      ...
      try {
        ...
        if (rs && rs.success && rs.data) {
          ...
          try {
            ...
            dto.usage =
              usageRs && usageRs.success && Array.isArray(usageRs.data) ? usageRs.data : []
          } catch (e) {
    ...
    validateBeforeSave(dto) {
      ...
      ;(dto.calibration || []).forEach((c, i) => {
        ...
        if (c.calibration_date && !normalizeDateField(c, 'calibration_date'))
          errors.push(`${n}: Calibration date không hợp lệ`)
        if (c.due_date && !normalizeDateField(c, 'due_date'))
          errors.push(`${n}: Due date không hợp lệ`)
        if (
          c.interval_months !== '' &&
          c.interval_months != null &&
          !isIntNonNeg(c.interval_months)
        )
          errors.push(`${n}: Interval (months) phải là số nguyên ≥ 0`)
        if (
          isDate(c.calibration_date) &&
          isDate(c.due_date) &&
          Date.parse(c.due_date) < Date.parse(c.calibration_date)
        )
      ...
      ;(dto.licenses || []).forEach((l, i) => {
        const n = `License #${i + 1}`
        if (l.activation_date && !normalizeDateField(l, 'activation_date'))
          errors.push(`${n}: Activation date không hợp lệ`)
        if (l.expiry_date && !normalizeDateField(l, 'expiry_date'))
          errors.push(`${n}: Expiry date không hợp lệ`)
        if (
          isDate(l.activation_date) &&
          isDate(l.expiry_date) &&
          Date.parse(l.expiry_date) < Date.parse(l.activation_date)
        )
    ...
    async saveTestingEquipment() {
      try {
        ...
        if (errors.length) {
          const msg =
            errors.length === 1 ? errors[0] : `${errors.length} lỗi cần sửa — ${errors[0]}`
        ...
        const oldEntity = Mapping.mapDtoToEntity(
          JSON.parse(JSON.stringify(this.oldTestingEquipmentDto))
        )
        ...
        if (rs && rs.success) {
          ...
          if (this.$message)
            this.$message.error('Save failed: ' + ((rs && rs.message) || 'unknown'))
          return { success: false }
        }
      } catch (error) {
    ...
    loadData(equipment) {
      ...
      if (equipment) {
        ...
        p.status = 'Available'
        p.country_of_origin =
          e.manufacturer === 'OMICRON' ? 'Austria' : e.manufacturer === 'HVI' ? 'USA' : ''
