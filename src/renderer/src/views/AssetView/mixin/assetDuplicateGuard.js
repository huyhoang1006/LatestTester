...
const duplicateKeyField = (h, label, value, onInput) =>
  h(
    'div',
    {
      style: {
  ...
  try {
    await vm.$msgbox({
      ...
      message: h('div', { style: { width: '420px', maxWidth: '100%' } }, [
        ...
        duplicateKeyField(h, 'Serial number', draft.serialNumber, (value) => {
          draft.serialNumber = value
        }),
        duplicateKeyField(h, 'Manufacturer', draft.manufacturer, (value) => {
          draft.manufacturer = value
        }),
        duplicateKeyField(h, 'Asset type', draft.assetType, (value) => {
...
export const ensureUniqueAssetBeforeSave = async (vm, dto) => {
  ...
  keys.userId =
    vm && vm.$store && vm.$store.state && vm.$store.state.user ? vm.$store.state.user.user_id : null
  ...
  while (keys.serialNumber) {
    ...
    keys = updatedKeys
    keys.userId =
      vm && vm.$store && vm.$store.state && vm.$store.state.user
