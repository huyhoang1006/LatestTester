...
export default {
      ...
      if (mode === 'edit' || mode === 'dup') {
        if (assetId) {
          ...
          if (rs && rs.success && rs.data && rs.data.length) {
            ...
            // Bind back to form
            this.disconnector.properties = Object.assign(
              {},
              this.disconnector.properties,
              properties
            )
  ...
  methods: {
    async deleteAsset() {
      try {
        const assetMrid =
          this.disconnector && this.disconnector.properties && this.disconnector.properties.mrid
    ...
    async saveAsset() {
      try {
        if (
          this.disconnector.properties.serial_no !== null &&
          this.disconnector.properties.serial_no !== ''
        ) {
