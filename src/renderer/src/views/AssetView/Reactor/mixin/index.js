...
export default {
  ...
  methods: {
    async saveAsset() {
      try {
        if (
          this.reactor.properties.serial_no !== null &&
          this.reactor.properties.serial_no !== ''
        ) {
    ...
    resetForm() {
      this.reactor = new ReactorDTO()
      this.reactorOld = new ReactorDTO()
      this.attachmentData = []
    },

    loadData(data) {
      this.reactorOld = JSON.parse(JSON.stringify(data)) // Lưu bản sao của dữ liệu cũ
      this.reactor = data
