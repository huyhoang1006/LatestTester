  ...
  >
    ...
    <el-form :model="winding_config" label-position="top" class="vector-form">
      <el-row :gutter="12" align="top">
        <el-col :xs="24" :lg="8">
          ...
          <el-form-item label="Primary (Prim)">
            <el-select
              size="small"
              @change="changePrim"
              v-model="winding_config.prim"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handlePrimArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <el-col
          ...
          :lg="8"
          v-if="
            this.winding_config.prim !== '' &&
            this.asset_type !== $constant.WITHOUT_TERT &&
            this.asset_type !== $constant.WITH_TERT
          "
          class="vertical-col"
        >
          <!-- secondary I -->
          <el-form-item label="Secondary I (Sec I)">
            <el-select
              size="small"
              @change="changeSecI"
              v-model="winding_config.sec.i"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handleSecIArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item v-if="this.winding_config.sec.i !== ''" label="Secondary value (Sec value)">
            <el-select
              size="small"
              v-model="winding_config.sec.value"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handleSecValueArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
        ...
        <el-col
          ...
          :lg="8"
          v-if="
            (this.asset_type === 'Three-winding' && this.winding_config.prim !== '') ||
            (this.asset_type === 'Auto w/ tert' && this.winding_config.prim !== '')
          "
        >
          <!-- tert I -->
          <el-form-item label="Tertiary I (Tert I)">
            <el-select
              size="small"
              @change="changeTertI"
              v-model="winding_config.tert.i"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handleTertIArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
          ...
          <el-form-item
            v-if="
              (this.asset_type === 'Three-winding' || this.asset_type === 'Auto w/ tert') &&
          ...
          >
            <el-select
              size="small"
              @change="winding_config.tert.accessible = ''"
              v-model="winding_config.tert.value"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handleTertValueArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
          ...
          <!-- tert assessibility -->
          <el-form-item
            v-if="this.winding_config.tert.i === 'D' && this.winding_config.tert.value !== ''"
            label="Tertiary Accessibility"
          >
            <el-select
              size="small"
              v-model="winding_config.tert.accessible"
              placeholder="Select"
              style="width: 100%"
            >
              <el-option
                v-for="item in handleTertAssessibleArray()"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              />
    ...
    <template #footer>
      <span class="dialog-footer custom-footer">
        <el-button class="footer-btn" type="danger" size="small" @click="handleCancel"
          >Cancel</el-button
        >
        <el-button class="footer-btn" type="primary" size="small" @click="handleClose"
          >Confirm</el-button
        >
      </span>
    </template>
...
export default {
  ...
  methods: {
    ...
    handlePrimArray() {
      if (
        (this.asset_type === 'Two-winding' || this.asset_type === 'Three-winding') &&
        this.asset_phase === '1'
      ) {
        ...
        (this.asset_type === 'Auto w/o tert' || this.asset_type === 'Auto w/ tert') &&
        this.asset_phase === '3'
      ) {
        return [
          {
    ...
    handleSecIArray() {
      if (this.asset_type === 'Two-winding' && this.asset_phase === '1') {
        ...
        if (this.winding_config.prim === WindingConnection.Yn) {
          return two_winding_3phase_secondary_i.filter(
            (item) =>
              item.value === WindingConnection.D ||
              item.value === WindingConnection.Y ||
              item.value === WindingConnection.Yn
          )
        } else {
    ...
    handleSecValueArray() {
      if (
        ...
        if (
          ...
          (this.winding_config.sec.i === WindingConnection.D ||
            this.winding_config.sec.i === WindingConnection.Z ||
            this.winding_config.sec.i === WindingConnection.Zn)
        ) {
          ...
          this.winding_config.prim === WindingConnection.Y &&
          this.winding_config.sec.i === WindingConnection.D
        ) {
          return two_winding_3phase_secondary_value_odd
        } else if (
          this.winding_config.prim === WindingConnection.Y &&
          (this.winding_config.sec.i === WindingConnection.Y ||
            this.winding_config.sec.i === WindingConnection.Yn)
        ) {
          return two_winding_1phase_secondary_value
        } else if (
          this.winding_config.prim === WindingConnection.Y &&
          ...
          return two_winding_3phase_secondary_value_odd.filter(
            (item) => item.value !== '3' && item.value !== '9'
          )
        } else if (
          this.winding_config.prim === WindingConnection.Yn &&
          this.winding_config.sec.i === WindingConnection.D
        ) {
          return two_winding_3phase_secondary_value_odd
        } else {
    ...
    handleTertIArray() {
      if (
        (this.asset_type === 'Two-winding' && this.asset_phase === '1') ||
        (this.asset_type === 'Auto w/ tert' && this.asset_phase === '1')
      ) {
        return two_winding_1phase_secondary_i
      } else if (this.asset_type === 'Two-winding' && this.asset_phase === '3') {
        if (this.winding_config.prim === WindingConnection.Yn) {
        ...
        if (this.winding_config.prim === 'YyNa') {
          return two_winding_3phase_secondary_i.filter(
            (item) => !item.value.includes(WindingConnection.Z)
          )
        }
      } else {
        if (this.winding_config.prim === WindingConnection.Yn) {
    ...
    handleTertValueArray() {
      if (
        (this.asset_type === 'Two-winding' ||
          this.asset_type === 'Three-winding' ||
          this.asset_type === 'Auto w/ tert') &&
        this.asset_phase === '1'
      ) {
        return two_winding_1phase_secondary_value
      } else if (
        ...
        if (
          this.winding_config.prim === WindingConnection.D &&
          (this.winding_config.tert.i === WindingConnection.D ||
            this.winding_config.tert.i === WindingConnection.Z ||
            this.winding_config.tert.i === WindingConnection.Zn)
        ) {
          return two_winding_3phase_secondary_value_even
        } else if (this.winding_config.prim === WindingConnection.D) {
          return two_winding_3phase_secondary_value_odd
          ...
          this.winding_config.prim === WindingConnection.Y &&
          (this.winding_config.tert.i === WindingConnection.Y ||
            this.winding_config.tert.i === WindingConnection.Yn)
        ) {
          return two_winding_1phase_secondary_value
        } else if (
          this.winding_config.prim === WindingConnection.Y &&
...
:global(.vector-form .el-form-item) {
  margin-bottom: 10px;
}

:global(.vector-form .el-form-item__label) {
  white-space: normal;
  word-break: keep-all;
