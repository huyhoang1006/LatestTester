        ...
        <div>
            <el-form label-position="left" :label-width="labelWidth" style="margin-top: 5px">
                <el-form-item label="Units in stack" label-position="left">
                    <el-select @change="onUnitChange" v-model="ratingsData.unit" size="small" placeholder="Select unit">
            ...
            <table class="table-strip-input-data fixed-table" style="background-color: white">
                ...
                <tbody>
                    <tr v-for="(item, index) in ratingsData.table" :key="index">
                        <td>{{ index + 1 }}</td>
                        <td>
                            <el-input size="small" v-model="item.serial">
                            </el-input>
                        </td>
                        <td>
                            <el-input size="small" type="text" number="positive" v-model="item.voltageLl.value">
                                <template #append>
                        ...
                        <td>
                            <el-input size="small" type="text" number="positive" v-model="item.voltageLn.value">
                                <template #append>
                                    <span>{{ unitMultiplier.k + unitSymbol.V }}</span>
                                </template>
                            </el-input>
                        ...
                        <td>
                            <el-input size="small" type="text" number="positive" v-model="item.mcovRating.value">
                                <template #append>
                                    <span>{{ unitMultiplier.k + unitSymbol.V }}</span>
                                </template>
                            </el-input>
