<template>
  <div style="margin-top: 20px">
    <div
      ...
      v-for="(item, index) in surgeArrester.prim"
      :key="'prim-' + index"
    >
      <div style="margin-bottom: 10px">
        <el-checkbox
          @change="(val) => onCheckboxClick(val, 'prim', index)"
          v-model="surgeArrester.prim[index].sign"
          :label="'Surge arrester - ' + item.ratings.pos"
          size="small"
        />
    ...
    <div
      style="margin-bottom: 20px"
      v-for="(item, index) in surgeArrester.sec"
      :key="'sec-' + index"
    >
      <div style="margin-bottom: 10px">
        <el-checkbox
          @change="(val) => onCheckboxClick(val, 'sec', index)"
          v-model="surgeArrester.sec[index].sign"
          :label="'Surge arrester - ' + item.ratings.pos"
          size="small"
        />
        <el-divider></el-divider>
    ...
    <div
      style="margin-bottom: 20px"
      v-for="(item, index) in surgeArrester.tert"
      :key="'tert-' + index"
    >
      <div style="margin-bottom: 10px">
        <el-checkbox
          @change="(val) => onCheckboxClick(val, 'tert', index)"
          v-model="surgeArrester.tert[index].sign"
          :label="'Surge arrester - ' + item.ratings.pos"
          size="small"
        />
        <el-divider></el-divider>
