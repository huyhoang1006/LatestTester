...
export default {
  ...
  methods: {
    async saveAsset() {
      try {
        if (
          this.surge_arrester_data.properties.serial_no !== null &&
          this.surge_arrester_data.properties.serial_no !== ''
        ) {
    ...
    checkTableRating(data) {
      for (let i = 0; i < data.ratings.tableRating.length; i++) {
        ...
        if (
          data.ratings.tableRating[i].assetInfoId === null ||
          data.ratings.tableRating[i].assetInfoId === ''
        ) {
          data.ratings.tableRating[i].assetInfoId = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].continousVoltage.mrid === null ||
          data.ratings.tableRating[i].continousVoltage.mrid === ''
        ) {
          data.ratings.tableRating[i].continousVoltage.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].isoVoltage.mrid === null ||
          data.ratings.tableRating[i].isoVoltage.mrid === ''
        ) {
          data.ratings.tableRating[i].isoVoltage.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].maximumVoltage.mrid === null ||
          data.ratings.tableRating[i].maximumVoltage.mrid === ''
        ) {
          data.ratings.tableRating[i].maximumVoltage.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].polesVoltage.mrid === null ||
          data.ratings.tableRating[i].polesVoltage.mrid === ''
        ) {
          data.ratings.tableRating[i].polesVoltage.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].ratedCircuit.mrid === null ||
          data.ratings.tableRating[i].ratedCircuit.mrid === ''
        ) {
          data.ratings.tableRating[i].ratedCircuit.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].ratedVoltage.mrid === null ||
          data.ratings.tableRating[i].ratedVoltage.mrid === ''
        ) {
          data.ratings.tableRating[i].ratedVoltage.mrid = uuid.newUuid()
        }
        if (
          data.ratings.tableRating[i].shortCurrent.mrid === null ||
          data.ratings.tableRating[i].shortCurrent.mrid === ''
        ) {
