...
import BushingJobDto from '@/views/Dto/Job/Bushing/index'
...
export default {
  ...
  data() {
    return {
      bushingJobDto: new BushingJobDto(),
      bushingJobDtoOld: new BushingJobDto()
    }
  },
  methods: {
    async saveJob() {
      try {
        if (!this.bushingJobDto.properties.name || this.bushingJobDto.properties.name === '') {
          this.$message.error('Name is required')
        } else {
          const dto = JSON.parse(JSON.stringify(this.bushingJobDto))
          const resultDto = await this.checkJob(dto)
          const entity = bushingJobMapping.jobDtoToEntity(resultDto)
          const old_entity = bushingJobMapping.jobDtoToEntity(this.bushingJobDtoOld)
          const rs = await window.electronAPI.insertBushingJob(old_entity, entity)
          if (rs.success) {
            return {
              success: true,
              data: rs.data,
    ...
    async saveCtrS() {
      ...
      if (result.success) {
        const dto = bushingJobMapping.JobEntityToDto(result.data)
        this.loadData(dto)
    ...
    async resetForm() {
      this.bushingJobDto = new BushingJobDto()
    },

    async loadData(data) {
      this.bushingJobDto = data
      this.bushingJobDtoOld = JSON.parse(JSON.stringify(data))
    },
    ...
    checkTestingEquipment(data) {
      ...
      for (const item of data.testingEquipmentData) {
        ...
        for (const test_type_id of item.test_type_bushing_id) {
          arr.push({
            mrid: uuid.newUuid(),
      ...
      // Thêm các phần tử mới vào data.bushingTestingEquipmentTestType nếu chưa có
      for (const current of arr) {
        const existed = data.bushingTestingEquipmentTestType.some(
          (old) =>
            old.testing_equipment_id === current.testing_equipment_id &&
            old.test_type_id === current.test_type_id
        )
        if (!existed) {
          data.bushingTestingEquipmentTestType.push(current)
        }
      }

      // Xóa các phần tử quá khứ không còn trong hiện tại
      data.bushingTestingEquipmentTestType = data.bushingTestingEquipmentTestType.filter((old) =>
        arr.some(
          (current) =>
            old.testing_equipment_id === current.testing_equipment_id &&
            old.test_type_id === current.test_type_id
