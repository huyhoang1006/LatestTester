...
export default {
  ...
  methods: {
    async saveJob() {
      try {
        if (
          !this.voltageTransformerJobDto.properties.name ||
          this.voltageTransformerJobDto.properties.name === ''
        ) {
          ...
          const old_entity = voltageTransformerJobMapping.jobDtoToEntity(
            this.voltageTransformerJobDtoOld
          )
    ...
    checkAssetId(data) {
      if (data.properties.asset_id === '' || data.properties.asset_id === null) {
        ...
        data.properties.asset_id =
          this.assetData.properties?.mrid || this.assetData.mrid || this.assetData.asset.mrid
      }
    },
    ...
    checkTestingEquipment(data) {
      ...
      data.voltageTransformerTestingEquipmentTestType =
        data.voltageTransformerTestingEquipmentTestType.filter((old) =>
          arr.some(
            (current) =>
              old.testing_equipment_id === current.testing_equipment_id &&
              old.test_type_id === current.test_type_id
      ...
      for (const test of data.testList) {
        ...
        if (data.procedureAsset.map((x) => x.procedure_id).indexOf(test.testTypeId) === -1) {
          data.procedureAsset.push({
            ...
            asset_id:
              this.assetData.properties?.mrid || this.assetData.asset?.mrid || this.assetData.mrid
          })
        }
      }
    }
