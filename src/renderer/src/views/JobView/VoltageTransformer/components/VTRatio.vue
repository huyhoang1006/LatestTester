        ...
        <tr v-for="(item, index) in testData.table.table1" :key="index">
          ...
          <td>
            <el-input
              ...
              type="text"
              number="positive"
              v-model="item.ratio_dev.value"
            ></el-input>
          </td>
          <td>
            <el-select size="small" v-model="item.polarity.value">
          ...
          if (!isNaN(parseFloat(item.ratio_meas.value)) && item.ratio_meas.value != 0) {
            if (!isNaN(item.upr.value) && !isNaN(item.usr.value) && item.usr.value != 0) {
              item.ratio_dev.value = (
                (100 *
                  (parseFloat(item.ratio_meas.value) -
                    parseFloat(item.upr.value) / parseFloat(item.usr.value))) /
                (parseFloat(item.upr.value) / parseFloat(item.usr.value))
              ).toFixed(4)
            } else if (
              !isNaN(parseFloat(item.upr.value)) &&
              !isNaN(parseFloat(item.usr.value)) &&
              item.usr.value != 0
            ) {
              item.ratio_dev.value = (
                (100 *
        ...
        if (!isNaN(parseFloat(element.ratio_meas.value)) && element.ratio_meas.value != 0) {
          if (!isNaN(element.upr.value) && !isNaN(element.usr.value) && element.usr.value != 0) {
            element.ratio_dev.value = (
              (100 *
                (parseFloat(element.ratio_meas.value) -
                  parseFloat(element.upr.value) / parseFloat(element.usr.value))) /
              (parseFloat(element.upr.value) / parseFloat(element.usr.value))
            ).toFixed(4)
          } else if (
            !isNaN(parseFloat(element.upr.value)) &&
            !isNaN(parseFloat(element.usr.value)) &&
            element.usr.value != 0
          ) {
            element.ratio_dev.value = (
              (100 *
