  ...
  <div class="ov-wrap">
      ...
      <section class="ov-card ov-half">
        ...
        <div class="ov-body">
          <el-input
            ...
            type="textarea"
            :rows="6"
            placeholder="Notes, remarks, conclusions..."
          ></el-input>
          <div class="ov-attachment">
            <Attachment
              :attachment_="attachmentData"
              title="Overview"
              height="120px"
    ...
    <div class="ov-row">
      ...
      <section class="ov-card ov-half">
        ...
        <div class="ov-body ov-info">
          <div class="ov-info-row">
            <span>Name</span><b>{{ locationData.name || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>Address</span><b>{{ locationData.address_general || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>City</span><b>{{ locationData.city || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>State/Province</span><b>{{ locationData.state_or_province || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>Country</span><b>{{ locationData.country || '—' }}</b>
          </div>
        </div>
      </section>
      ...
      <section class="ov-card ov-half">
        ...
        <div class="ov-body ov-info">
          <div class="ov-info-row">
            <span>Asset</span><b>{{ assetInfo.kind || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>Asset type</span><b>{{ assetInfo.type || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>Serial number</span><b>{{ assetInfo.serial || '—' }}</b>
          </div>
          <div class="ov-info-row">
            <span>Manufacturer</span><b>{{ assetInfo.manufacturer || '—' }}</b>
          </div>
        </div>
      </section>
    </div>
...
export default {
  ...
  computed: {
    ...
    assetInfo() {
      const p =
        this.assetData && this.assetData.properties
          ? this.assetData.properties
          : this.assetData || {}
