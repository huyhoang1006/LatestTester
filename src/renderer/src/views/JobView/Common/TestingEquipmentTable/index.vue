        ...
        <tr>
          ...
          <th class="col-act">
            <el-tooltip content="Add equipment" placement="top">
              <button type="button" class="act-btn add" @click="addRow()">
          ...
          <th class="col-act">
            <el-tooltip content="Remove all" placement="top">
              <button type="button" class="act-btn danger" @click="removeAll()">
      ...
      <tbody>
        <tr v-if="rows.length === 0">
          <td colspan="7" class="te-empty">
            No testing equipment — click <i class="fa-solid fa-plus"></i> to add one from the
            equipment manager.
          </td>
        </tr>
        <tr v-for="(item, index) in rows" :key="index">
          ...
          <td class="col-eq">
            <div class="eq-cell">
              ...
              >
                <div class="eq-info">
                  <div class="eq-info-title">
                    {{ detailsOf(item).model || detailsOf(item).name || 'Equipment' }}
                  </div>
                  <div class="eq-info-row">
                    <span>Name</span><b>{{ detailsOf(item).name || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Manufacturer</span><b>{{ detailsOf(item).manufacturer || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Model</span><b>{{ detailsOf(item).model || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Serial number</span><b>{{ detailsOf(item).serial || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Asset tag</span><b>{{ detailsOf(item).asset_tag || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Last calibration</span
                    ><b>{{ detailsOf(item).calibration_date || '—' }}</b>
                  </div>
                  <div class="eq-info-row">
                    <span>Repairs</span><b>{{ detailsOf(item).repair_count || 0 }}</b>
                  </div>
                </div>
                <template #reference>
                  <button type="button" class="act-btn info" title="Equipment info">
          ...
          <td class="col-serial">
            <span class="ro" :class="{ empty: !item.serial_number }">{{
              item.serial_number || '—'
            }}</span>
          </td>
          <td class="col-cal">
            <span class="ro" :class="{ empty: !item.calibration_date }">{{
          ...
          <td class="col-test">
            ...
            <div v-if="testList.length === 0" class="te-hint">
              No tests have been selected in the Test settings tab.
            </div>
          </td>
          <td class="col-act">
            <el-tooltip content="Add row below" placement="top">
              <button type="button" class="act-btn add" @click="addRow(index)">
                <i class="fa-solid fa-plus"></i>
              </button>
            </el-tooltip>
          </td>
          <td class="col-act">
            <el-tooltip content="Remove" placement="top">
              <button type="button" class="act-btn danger" @click="removeRow(index)">
                <i class="fa-solid fa-trash"></i>
              </button>
            </el-tooltip>
          </td>
...
export default {
  ...
  methods: {
    ...
    onSelectEquipment(item, mrid) {
      ...
      item.mrid = mrid || ''
      item.model = eq ? eq.model || eq.name || '' : ''
      item.serial_number = eq ? eq.serial || '' : ''
      item.calibration_date = eq ? eq.calibration_date || '' : ''
    },
    newRow() {
      const row = {
        mrid: '',
