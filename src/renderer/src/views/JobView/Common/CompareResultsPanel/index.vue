  ...
  <section class="ti-card cmp-card">
    <div class="ti-header">
      ...
      <div class="cmp-actions">
        <i
          class="fa-solid fa-rotate-right cmp-act"
    ...
    <div class="ti-body cmp-body" v-loading="loading">
      ...
      <div class="cmp-scroll">
        ...
        <div v-if="!options.length" class="cmp-state">
          ...
          <p>
            No other <b>{{ testName || testCode }}</b> test for this asset on this machine.
          </p>
        </div>
        ...
        <template v-else-if="result">
            ...
            <table class="cmp-table">
              ...
              <thead>
                <tr>
                  <th>Condition</th>
                  <th>This Test</th>
                  <th>Reference Test</th>
                </tr>
              </thead>
              <tbody>
                <tr
                  v-for="c in result.conditionDiff"
                  :key="c.key"
                  :class="{ 'is-diff': c.differs }"
                >
          ...
          <div v-for="table in flatTables" :key="table.title" class="cmp-block">
            ...
            <div v-if="flatTables.length > 1" class="cmp-block-title">
              {{ table.displayTitle || table.title }}
            </div>
            <table class="cmp-table">
              <colgroup>
                <col style="width: 27%" />
      ...
      <div v-if="result && hasReferenceData" class="cmp-foot">
        <div class="cmp-stats">
          ...
          <span v-if="result.summary.onlyReference"
            >{{ result.summary.onlyReference }} reference rows</span
          >
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    hasReferenceData() {
      return this.referenceRowCount > 0
    },
    /**
    ...
    flatTables() {
      ...
      return this.result.tables.map((table) => ({
        ...
        rows: table.rows.map((row) => ({
          ...
          status: row.status,
          statusClass:
            row.status === 'onlyCurrent'
              ? 'is-only-current'
              : row.status === 'onlyReference'
                ? 'is-only-reference'
                : '',
  ...
  methods: {
    ...
    async loadOptions(preferredWorkTaskMrid = '') {
      ...
      try {
        ...
        this.options = (rs.data || []).map((item) => ({
          ...
          label:
            [item.jobType, item.workName, item.executionDate].filter(Boolean).join(' · ') ||
            'Untitled test'
        }))
    ...
    async loadReference() {
      ...
      try {
        ...
        this.referenceRowCount = (reference.tables || []).reduce(
...
.cell-delta.is-symbol:not(.lv-warn) {
  color: #b0b4bb;
  font-weight: 400;
}
.muted {
