  ...
  >
    ...
    <div v-else v-loading="loading">
      ...
      <div v-if="result && result.conditionDiff.length" class="cmp-card">
        ...
        <div class="cmp-hint">
          Điều kiện đo khác nhau có thể làm sai lệch so sánh — nhất là các phép đo phụ thuộc nhiệt
          độ.
        </div>
      </div>
        ...
        <table class="cmp-table">
          ...
          <tbody>
            <tr v-for="(row, idx) in table.rows" :key="idx" :class="rowClass(row)">
              <td class="col-label">
                {{ row.label || '—' }}
                <span v-if="row.status === 'onlyCurrent'" class="tag tag-cur"
                  >chỉ có ở bản hiện tại</span
                >
                <span v-if="row.status === 'onlyReference'" class="tag tag-ref"
                  >chỉ có ở bản tham chiếu</span
                >
              </td>
      ...
      <div v-if="result" class="cmp-summary">
        So sánh {{ result.summary.comparedCells }} ô · {{ result.summary.changedCells }} ô thay đổi
    ...
    async loadOptions() {
      ...
      try {
        ...
        this.options = (rs.data || []).map((item) => ({
          ...
          label:
            [item.jobType, item.executionDate, item.workName].filter(Boolean).join(' — ') ||
            item.workTaskMrid
        }))
