  ...
  methods: {
    ...
    validateIKnee(item, index) {
      ...
      if (isnValue && !isNaN(parseFloat(isnValue))) {
        ...
        if (iKneeValue > isn) {
          this.$message.error(
            `Row ${index + 1}: I knee (${iKneeValue} A) cannot be greater than Isn (${isn} A)`
          )
          item.i_knee.value = ''
          return
        }
    ...
    getIsnForRow(rowName) {
      ...
      if (
        this.assetData &&
        this.assetData.ctConfiguration &&
        this.assetData.ctConfiguration.dataCT
      ) {
