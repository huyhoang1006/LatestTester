    ...
    <table class="table-strip-input-data" style="width: 100%; font-size: 12px">
      ...
      <tbody>
        <tr v-for="(item, index) in testData.table.table1" :key="index">
          ...
          <td>
            <el-input size="small" type="text" v-model="item.ipr.value"></el-input>
          </td>
          <td>
            <el-input size="small" type="text" v-model="item.isr.value"></el-input>
          </td>
          <td>
            <el-input
              size="small"
          ...
          <td>
            <el-input size="small" type="text" v-model="item.ratio_dev.value"></el-input>
          </td>
          <td>
    ...
    calcRdev() {
      ...
      this.testData.table.table1.forEach((element) => {
        if (!isNaN(parseFloat(element.ratio_meas.value)) && element.ratio_meas.value != 0) {
          if (
            !isNaN(parseFloat(element.ipr.value)) &&
            !isNaN(parseFloat(element.isr.value)) &&
            element.isr.value != 0
          ) {
            element.ratio_dev.value = (
              (100 *
                (parseFloat(element.ratio_meas.value) -
                  parseFloat(element.ipr.value) / parseFloat(element.isr.value))) /
              (parseFloat(element.ipr.value) / parseFloat(element.isr.value))
            ).toFixed(4)
          }
        }
