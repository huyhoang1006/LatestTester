    ...
    <table class="table-strip-input-data" style="width: 100%; font-size: 12px">
      ...
      <tbody>
        <tr v-for="(item, index) in testData.table.table1" :key="index">
          ...
          <td>
            <el-input size="small" type="text" v-model="item.r_meas.value"></el-input>
          </td>
          <td>
            <el-input size="small" type="text" v-model="item.r_ref.value"></el-input>
          </td>
          <td>
            <el-input size="small" type="text" v-model="item.r_corr.value"></el-input>
          </td>
          <td>
            <el-input size="small" type="text" v-model="item.r_dev.value"></el-input>
          </td>
          <td>
  ...
  methods: {
    ...
    calcRcorr() {
      ...
      this.testData.table.table1.forEach((item) => {
        ...
        if (
          !isNaN(parseFloat(item.r_meas.value)) &&
          (!item.r_corr.value || item.r_corr.value === '')
        ) {
          // Check if testCondition and condition exist
          if (
            this.testConditionData &&
            this.testConditionData.condition &&
            ...
            item.r_corr.value = parseFloat(
              (parseFloat(item.r_meas.value) *
                (235 + parseFloat(this.testConditionData.condition.reference_temp.value))) /
                (235 + parseFloat(this.testConditionData.condition.winding_temp.value))
            ).toFixed(4)
          } else {
    ...
    calcRdev() {
      ...
      this.testData.table.table1.forEach((item) => {
        ...
        if (
          !isNaN(parseFloat(item.r_corr.value)) &&
          !isNaN(parseFloat(item.r_ref.value)) &&
          item.r_ref.value != 0
        ) {
          item.r_dev.value = (
            (100 * (parseFloat(item.r_corr.value) - parseFloat(item.r_ref.value))) /
            parseFloat(item.r_ref.value)
          ).toFixed(4)
        }
      })
    },
