...
export default {
  ...
  data() {
    return {
      rotatingMachineJobDto: new RotatingMachineJobDto(),
      rotatingMachineJobDtoOld: new RotatingMachineJobDto()
    }
  },
  methods: {
    async saveJob() {
      try {
        if (
          !this.rotatingMachineJobDto.properties.name ||
          this.rotatingMachineJobDto.properties.name === ''
        ) {
    ...
    async resetForm() {
      this.rotatingMachineJobDto = new RotatingMachineJobDto()
    },

    async loadData(data) {
      this.rotatingMachineJobDto = data
      this.rotatingMachineJobDtoOld = JSON.parse(JSON.stringify(data))
    },
    ...
    checkTestingEquipment(data) {
      ...
      for (const item of data.testingEquipmentData) {
        ...
        for (const test_type_id of item.test_type_rotating_machine_id) {
          arr.push({
            mrid: uuid.newUuid(),
      ...
      data.rotatingMachineTestingEquipmentTestType =
        data.rotatingMachineTestingEquipmentTestType.filter((old) =>
          arr.some(
            (current) =>
              old.testing_equipment_id === current.testing_equipment_id &&
              old.test_type_id === current.test_type_id
