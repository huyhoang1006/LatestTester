    ...
    <div class="display-flex mgb-10">
      <table class="table-strip-input-data mgr-10">
        ...
        <tbody v-for="(item, index) in data_" :key="index">
          <tr
            v-if="
              ['BushingPrimC1', 'BushingPrimC2', 'WindingDfCap'].includes(item.testTypeCode) &&
              item.name.includes('(DF)')
            "
          >
            <td>{{ index + 1 }}</td>
            <td>{{ item.name }}</td>
            <td v-if="item.average_score_df != null">
              {{ parseFloat(item.average_score_df).toFixed(4) }}
            </td>
            <td v-if="item.weighting_factor_df != null">
              {{ parseFloat(item.weighting_factor_df).toFixed(4) }}
            </td>
          ...
          >
            <td>{{ index + 1 }}</td>
            <td>{{ item.name }}</td>
            <td v-if="item.average_score_c != null">
              {{ parseFloat(item.average_score_c).toFixed(4) }}
            </td>
            <td v-if="item.weighting_factor_c != null">
              {{ parseFloat(item.weighting_factor_c).toFixed(4) }}
            </td>
          ...
          <tr v-else>
            <td>{{ index + 1 }}</td>
            <td>{{ item.name }}</td>
            <td v-if="item.average_score != null">
              {{ parseFloat(item.average_score).toFixed(4) }}
            </td>
            <td v-if="item.weighting_factor != null">
              {{ parseFloat(item.weighting_factor).toFixed(4) }}
            </td>
      ...
      <table class="" style="width: 340px">
        <tbody>
          <tr>
            ...
            <td
              v-if="
                properties.average_health_index != null &&
                properties.average_health_index != undefined &&
                properties.average_health_index != ''
              "
            >
              {{ properties.average_health_index.toFixed(4) }}
            </td>
          </tr>
          <tr>
            <th>Assessment</th>
            <td
              ...
              "
              :class="
                nameColor(judge_average(parseFloat(properties.average_health_index).toFixed(4)))
              "
            >
              {{ judge_average(properties.average_health_index.toFixed(4)) }}
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="" style="width: 340px">
        <tbody>
          ...
          <tr>
            <th>Assessment</th>
            <td
              :class="nameColor(judge_worst(parseFloat(properties.worst_health_index).toFixed(4)))"
            >
              {{ judge_worst(parseFloat(properties.worst_health_index).toFixed(4)) }}
            </td>
          </tr>
        </tbody>
      </table>
