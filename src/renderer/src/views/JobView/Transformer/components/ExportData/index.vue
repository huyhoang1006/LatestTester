<template>
  <div id="select-test">
    <table class="mgt-5 w-100 table-strip-input-data" style="width: 70%">
      ...
      <tbody>
        <tr v-for="(item, index) in testListData" :key="index">
          ...
          <td>
            <el-select class="assessment" size="small" v-model="assessment_[index]">
              <el-option value="Pass"><i class="fa-solid fa-square-check pass"></i> Pass</el-option>
              <el-option value="Fail"><i class="fa-solid fa-xmark fail"></i> Fail</el-option>
            </el-select>
            <span
              v-if="assessment_[index] === 'Pass'"
              class="fa-solid fa-square-check pass icon-status"
            ></span>
            <span
              v-else-if="assessment_[index] === 'Fail'"
    ...
    <br /><br /><br />
    <el-button type="primary" @click="show()" size="small">Export</el-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    async show() {
      await window.electronAPI.exportEtrc(this.$selectedAsset()[0], '')
    },
