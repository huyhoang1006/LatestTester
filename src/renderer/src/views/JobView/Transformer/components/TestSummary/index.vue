    ...
    <table class="w-100 mgt-10">
      ...
      <tbody>
        ...
        <tr v-for="(item, index) in testListData" :key="index">
          ...
          <td>{{ item.name }}</td>
          <template
            v-if="['BushingPrimC1', 'BushingPrimC2', 'WindingDfCap'].includes(item.testTypeCode)"
          >
            <td style="padding: 0">
              <div class="flex-container">
                <template>
        ...
        <tr v-if="this.isMonitor">
          ...
          <td style="padding: 0">
            <div class="flex-container">
              <template>
                <div
                  v-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_df_worst == 3"
                  class="good"
                >
                  Good (DF)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_df_worst == 2"
                  class="fair"
                >
                  Fair (DF)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_df_worst == 1"
                  class="poor"
                >
                  Poor (DF)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_df_worst == 0"
                  class="bad"
                >
                  Bad (DF)
                </div>
              ...
              <template>
                <div
                  v-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_c_worst == 3"
                  class="good"
                >
                  Good (C)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_c_worst == 2"
                  class="fair"
                >
                  Fair (C)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_c_worst == 1"
                  class="poor"
                >
                  Poor (C)
                </div>
                <div
                  v-else-if="dataOnlineMonitoring && dataOnlineMonitoring.bushing_c_worst == 0"
                  class="bad"
                >
                  Bad (C)
                </div>
...
export default {
  ...
  async beforeMount() {
    if (
      this.selectedAsset &&
      Array.isArray(this.selectedAsset) &&
      this.selectedAsset.length > 0 &&
      this.selectedAsset[0] &&
      this.selectedAsset[0].id
    ) {
