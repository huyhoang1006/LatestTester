  ...
  methods: {
    ...
    async initGeneralInspection(testTypeCode) {
      ...
      const rowDataExampleCondition = common.buildEmptyTestCondition(
        transformerConditionMap[testTypeCode].columns
      )
    ...
    async initRatioPrimSec(testTypeCode, assetData) {
      ...
      if (
        !tapChangers.mode ||
        !tapChangers.winding ||
        !tapChangers.tap_scheme ||
        !tapChangers.no_of_taps
      ) {
        return {
          rowDataExampleCondition,
          table: {
    ...
    async initDCWindingPrim(testTypeCode, assetData) {
      ...
      if (
        tapChangers.mode &&
        tapChangers.winding === this.$constant.PRIM &&
        tapChangers.tap_scheme &&
        tapChangers.no_of_taps
      ) {
        const voltage_table = tapChangers.voltage_table || []
    ...
    async initDCWindingSec(testTypeCode, assetData) {
      ...
      if (
        tapChangers.mode &&
        tapChangers.winding === this.$constant.SEC &&
        tapChangers.tap_scheme &&
        tapChangers.no_of_taps
      ) {
        const voltage_table = tapChangers.voltage_table || []
    ...
    async initDCWindingTert(testTypeCode, assetData) {
      ...
      if (
        tapChangers.mode &&
        tapChangers.winding === this.$constant.TERT &&
        tapChangers.tap_scheme &&
        tapChangers.no_of_taps
      ) {
        const voltage_table = tapChangers.voltage_table || []
