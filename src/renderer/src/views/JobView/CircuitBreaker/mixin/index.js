...
export default {
  ...
  data() {
    return {
      circuitBreakerJobDto: new CircuitBreakerJobDto(),
      circuitBreakerJobDtoOld: new CircuitBreakerJobDto()
    }
  },
  methods: {
    async saveJob() {
      try {
        if (
          !this.circuitBreakerJobDto.properties.name ||
          this.circuitBreakerJobDto.properties.name === ''
        ) {
    ...
    checkTestingEquipment(data) {
      ...
      data.circuitBreakerTestingEquipmentTestType =
        data.circuitBreakerTestingEquipmentTestType.filter((old) =>
          arr.some(
            (current) =>
              old.testing_equipment_id === current.testing_equipment_id &&
              old.test_type_id === current.test_type_id
