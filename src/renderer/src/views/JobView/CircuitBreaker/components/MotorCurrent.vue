<template>
  <div id="dc-winding-resistance-prim">
        ...
        <tr v-for="(item, index) in testData.table.table1" :key="index">
          <td>
            <el-input
              ...
              type="text"
              number="positive"
              v-model="item.inrush_current.value"
            ></el-input>
          </td>
          <td>
            <el-input
          ...
          <td>
            <el-input
              ...
              type="text"
              number="positive"
              v-model="item.charging_current.value"
            ></el-input>
          </td>
          <td>
            <el-input
              ...
              type="text"
              number="positive"
              v-model="item.mini_voltage.value"
            ></el-input>
          </td>
          <td>
    ...
    >
      ...
      <table
        v-else-if="assetData.assessmentLimits.limits === 'Relative'"
  ...
  methods: {
      ...
      this.testData.table.forEach((item) => {
        ...
        if (this.testData.limits === 'Relative') {
          if (
            //first item
            Math.abs(
              parseFloat(item.inrush_current) - parseFloat(this.asset_.motorChar.rel[0].ref)
            ) <= parseFloat(this.asset_.motorChar.rel[0].dev) &&
            //second item
            Math.abs(parseFloat(item.charging) - parseFloat(this.asset_.motorChar.rel[1].ref)) <=
              parseFloat(this.asset_.motorChar.rel[1].dev) &&
            //third item
            Math.abs(
              parseFloat(item.charging_current) - parseFloat(this.asset_.motorChar.rel[2].ref)
            ) <= parseFloat(this.asset_.motorChar.rel[2].dev) &&
            //Fourth item
            Math.abs(
              parseFloat(item.mini_voltage) - parseFloat(this.asset_.motorChar.rel[3].ref)
            ) <= parseFloat(this.asset_.motorChar.rel[3].dev)
          ) {
            item.assessment = 'Pass'
          } else {
            item.assessment = 'Fail'
    ...
    normalizeMotorAssessmentLimits(data) {
      ...
      if (!normalized.motorChar.abs || normalized.motorChar.abs.length < 4) {
        normalized.motorChar.abs = Array(4)
          .fill(null)
          .map((_, index) =>
      ...
      if (!normalized.motorChar.rel || normalized.motorChar.rel.length < 4) {
        normalized.motorChar.rel = Array(4)
          .fill(null)
          .map((_, index) =>
