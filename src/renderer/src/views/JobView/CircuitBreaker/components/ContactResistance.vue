<template>
  <div id="dc-winding-resistance-prim">
    ...
    <div
      v-if="
        assetData.circuitBreaker.interruptersPerPhase === 1 ||
        assetData.circuitBreaker.interruptersPerPhase === ''
      "
    >
      <br />
      <table class="table-strip-input-data" style="width: 100%; font-size: 12px">
        <thead class="test">
          <tr>
            ...
            <th @click="removeAll()" class="action-col">
          ...
          <tr v-for="(item, index) in testData.table.table1" :key="index">
            ...
            <td>
              <el-input
                ...
                type="text"
                number="positive"
                v-model="item.contact_resistance.value"
              ></el-input>
            </td>
            <td>
    ...
    <div v-if="assetData.circuitBreaker.interruptersPerPhase > 1">
      ...
      <table class="table-strip-input-data" style="width: 100%; font-size: 12px">
        ...
        <tbody>
          <tr v-for="(item, index) in testData.table.table1" :key="index">
            <td
              v-if="index % assetData.circuitBreaker.interruptersPerPhase === 0"
            ...
            <td>
              <el-input size="small" v-model="item.interrupt_no.value"></el-input>
            </td>
            <td>
              <el-input size="small" v-model="item.i_test.value"></el-input>
            </td>
            <td>
              <el-input size="small" v-model="item.contact_resistance.value"></el-input>
            </td>
            <td>
              <el-select class="assessment" size="small" v-model="item.assessment.value">
                <el-option value="Pass"
    ...
    >
      <!-- <el-radio-group v-model="assetData.assessmentLimits.limits" style="margin-bottom: 20px">
                ...
                <table class="table-strip-input-data" v-else-if="assetData.assessmentLimits.limits === 'Relative'">
                    <thead>
                        <tr>
                            <th></th>
                            <th>R ref</th>
  ...
  methods: {
    normalizeAssessmentLimits(data) {
      ...
      if (data.contact_resistance) {
        ...
        if (contactRes.abs) {
          normalized.contactSys.abs.rmin =
            getValue(contactRes.abs.r_min) || getValue(contactRes.abs.rmin) || ''
          normalized.contactSys.abs.rmax =
            getValue(contactRes.abs.r_max) || getValue(contactRes.abs.rmax) || ''
          normalized.contactSys.abs.mrid = contactRes.abs.mrid || contactRes.mrid || ''
        }

        if (contactRes.rel) {
          normalized.contactSys.rel.rref =
            getValue(contactRes.rel.r_ref) || getValue(contactRes.rel.rref) || ''
          normalized.contactSys.rel.rdev =
            getValue(contactRes.rel.r_dev) || getValue(contactRes.rel.rdev) || ''
      ...
      // Normalize from contact_system structure if exists
        ...
        if (contactSys.abs) {
          normalized.contactSys.abs.rmin =
            getValue(contactSys.abs.r_min) || getValue(contactSys.abs.rmin) || ''
          normalized.contactSys.abs.rmax =
            getValue(contactSys.abs.r_max) || getValue(contactSys.abs.rmax) || ''
          normalized.contactSys.abs.mrid = contactSys.abs.mrid || ''
        }

        if (contactSys.rel) {
          normalized.contactSys.rel.rref =
            getValue(contactSys.rel.r_ref) || getValue(contactSys.rel.rref) || ''
          normalized.contactSys.rel.rdev =
            getValue(contactSys.rel.r_dev) || getValue(contactSys.rel.rdev) || ''
    ...
    calculator() {
      ...
      this.testData.table.forEach((item) => {
        if (this.testData.limits === 'Absolute') {
          if (
            parseFloat(item.contactResistance) >= parseFloat(this.asset_.contactSys.abs.rmin) &&
        ...
        if (this.testData.limits === 'Relative') {
          if (
            parseFloat(item.contactResistance) >=
              parseFloat(this.asset_.contactSys.rel.rref) -
                parseFloat(this.asset_.contactSys.rel.rdev) &&
            parseFloat(item.contactResistance) <=
              parseFloat(this.asset_.contactSys.rel.rref) +
                parseFloat(this.asset_.contactSys.rel.rdev)
          ) {
