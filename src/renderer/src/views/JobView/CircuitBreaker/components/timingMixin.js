  ...
  methods: {
    ...
    getTableEntries() {
      ...
      if (Array.isArray(table)) {
        return table.map(function (rows, i) {
          return { key: 'table' + (i + 1), rows: rows }
        })
      }
      return Object.keys(table).map(function (key) {
        return { key: key, rows: table[key] }
      })
    },
    ...
    reverseNormalizeToAssessmentLimits() {
      var al =
        this.assetData && this.assetData.assessmentLimits ? this.assetData.assessmentLimits : null
      ...
      if (asset_.openTime && al.operating_time) {
        absMapping.forEach(function (key, i) {
          ...
          if (al.operating_time.abs[key]) {
            if (al.operating_time.abs[key].t_min)
              al.operating_time.abs[key].t_min.value = abs.tmin || ''
            if (al.operating_time.abs[key].t_max)
              al.operating_time.abs[key].t_max.value = abs.tmax || ''
          }
          if (al.operating_time.rel[key]) {
            if (al.operating_time.rel[key].t_ref)
              al.operating_time.rel[key].t_ref.value = rel.rref || ''
            if (al.operating_time.rel[key].minus_t_dev)
              al.operating_time.rel[key].minus_t_dev.value = rel.tdevZ || ''
            if (al.operating_time.rel[key].plus_t_dev)
              al.operating_time.rel[key].plus_t_dev.value = rel.tdevN || ''
          }
        })
      }
      ...
      if (asset_.auxContact && al.auxiliary_contacts) {
        tripMapping.forEach(function (key, i) {
          var absT =
            asset_.auxContact.abs && asset_.auxContact.abs.trip
              ? asset_.auxContact.abs.trip[i] || {}
              : {}
          var relT =
            asset_.auxContact.rel && asset_.auxContact.rel.trip
              ? asset_.auxContact.rel.trip[i] || {}
              : {}
          var absC =
            asset_.auxContact.abs && asset_.auxContact.abs.close
              ? asset_.auxContact.abs.close[i] || {}
              : {}
          var relC =
            asset_.auxContact.rel && asset_.auxContact.rel.close
              ? asset_.auxContact.rel.close[i] || {}
              : {}
          if (al.auxiliary_contacts.trip_operation) {
            if (al.auxiliary_contacts.trip_operation.abs[key]) {
              if (al.auxiliary_contacts.trip_operation.abs[key].t_min)
                al.auxiliary_contacts.trip_operation.abs[key].t_min.value = absT.tmin || ''
              if (al.auxiliary_contacts.trip_operation.abs[key].t_max)
                al.auxiliary_contacts.trip_operation.abs[key].t_max.value = absT.tmax || ''
            }
            if (al.auxiliary_contacts.trip_operation.rel[key]) {
              if (al.auxiliary_contacts.trip_operation.rel[key].t_ref)
                al.auxiliary_contacts.trip_operation.rel[key].t_ref.value = relT.tref || ''
              if (al.auxiliary_contacts.trip_operation.rel[key].t_dev)
                al.auxiliary_contacts.trip_operation.rel[key].t_dev.value = relT.tdef || ''
            }
          }
          if (al.auxiliary_contacts.close_operation) {
            if (al.auxiliary_contacts.close_operation.abs[key]) {
              if (al.auxiliary_contacts.close_operation.abs[key].t_min)
                al.auxiliary_contacts.close_operation.abs[key].t_min.value = absC.tmin || ''
              if (al.auxiliary_contacts.close_operation.abs[key].t_max)
                al.auxiliary_contacts.close_operation.abs[key].t_max.value = absC.tmax || ''
            }
            if (al.auxiliary_contacts.close_operation.rel[key]) {
              if (al.auxiliary_contacts.close_operation.rel[key].t_ref)
                al.auxiliary_contacts.close_operation.rel[key].t_ref.value = relC.tref || ''
              if (al.auxiliary_contacts.close_operation.rel[key].t_dev)
                al.auxiliary_contacts.close_operation.rel[key].t_dev.value = relC.tdef || ''
            }
          }
        })
      }
      ...
      if (asset_.coilCharacter && al.coil_characteristics) {
        coilMapping.forEach(function (key, i) {
          ...
          if (al.coil_characteristics.abs[key]) {
            if (al.coil_characteristics.abs[key].min)
              al.coil_characteristics.abs[key].min.value = abs.min || ''
            if (al.coil_characteristics.abs[key].max)
              al.coil_characteristics.abs[key].max.value = abs.max || ''
          }
          if (al.coil_characteristics.rel[key]) {
            if (al.coil_characteristics.rel[key].ref)
              al.coil_characteristics.rel[key].ref.value = rel.ref || ''
            if (al.coil_characteristics.rel[key].minus_dev)
              al.coil_characteristics.rel[key].minus_dev.value = rel.devZ || ''
            if (al.coil_characteristics.rel[key].plus_dev)
              al.coil_characteristics.rel[key].plus_dev.value = rel.devN || ''
          }
        })
      }
    },
    ...
    async updateAssessment() {
      ...
      // 3. Save via IPC
      var assetId =
        this.assetData && this.assetData.properties ? this.assetData.properties.mrid : null
      if (!assetId) {
