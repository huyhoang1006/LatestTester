    ...
    calculator() {
      this.testData.table.forEach((item) => {
        ...
        if (this.testData.limits === 'Absolute') {
          if (
            parseFloat(item.rmeas) >= parseFloat(this.asset_.coilCharacter.abs[6].min) &&
            parseFloat(item.rmeas) <= parseFloat(this.asset_.coilCharacter.abs[6].max)
          ) {
        ...
        if (this.testData.limits === 'Relative') {
          if (parseFloat(item.rmeas) <= parseFloat(this.asset_.coilCharacter.rel[6].ref)) {
            if (
              parseFloat(item.rmeas) >=
              parseFloat(this.asset_.coilCharacter.rel[6].ref) -
                parseFloat(this.asset_.coilCharacter.rel[6].devZ)
            ) {
              item.assessment = 'Pass'
            } else {
              item.assessment = 'Fail'
            ...
            if (
              parseFloat(item.rmeas) <=
              parseFloat(this.asset_.coilCharacter.rel[6].ref) +
                parseFloat(this.asset_.coilCharacter.rel[6].devN)
            ) {
              item.assessment = 'Pass'
            } else {
              item.assessment = 'Fail'
