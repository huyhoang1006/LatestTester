  ...
  methods: {
    ...
    async initMotorCurrent(testTypeCode) {
      ...
      const rowDataExampleCondition = common.buildEmptyTestCondition(
        circuitBreakerConditionMap[testTypeCode].columns
      )
    ...
    async initCTiming(testTypeCode, assetData) {
      ...
      const closeCoilsCount = parseInt(
        operating?.numberCloseCoil || operating?.number_of_close_coil || 1
      )
      const phaseCount = parseInt(
        circuitBreaker?.numberOfPhase || circuitBreaker?.numberOfPhases || 3
      )
      const interruptCount = parseInt(
        circuitBreaker?.numberOfInterruptPhase || circuitBreaker?.interruptersPerPhase || 1
      )
    ...
    async initOCOTiming(testTypeCode, assetData) {
      ...
      const tripCoilsCount = parseInt(
        operating?.numberTripCoil || operating?.number_of_trip_coil || 1
      )
      const phaseCount = parseInt(
        circuitBreaker?.numberOfPhase || circuitBreaker?.numberOfPhases || 3
      )
