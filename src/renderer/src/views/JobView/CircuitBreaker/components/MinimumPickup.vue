    ...
    <table class="table-strip-input-data" style="width: 100%; font-size: 12px">
      ...
      <tbody>
        <tr v-for="(item, index) in testData.table.table1" :key="index">
          ...
          <td>
            <el-select size="small" v-model="item.operation.value">
          ...
          <td>
            <el-input
              ...
              type="text"
              number="positive"
              v-model="item.close_coil_no.value"
            ></el-input>
          </td>
          <td>
            <el-input
                ...
                <table v-if="testData.limits === 'Absolute'" class="table-strip-input-data">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in pickupVoltage" :key="index">
                            <td>{{ item }}</td>
                            <td>
                                <el-input size="small" v-model="asset_.pickupVol.abs[index].vmin">
                                    <template #append>V</template>
</el-input>
</td>
<td>
    <el-input size="small" v-model="asset_.pickupVol.abs[index].vmax">
        <template #append>V</template>
    </el-input>
</td>
</tr>
...
<table v-else-if="testData.limits === 'Relative'" class="table-strip-input-data">
    <thead>
        <tr>
            <th></th>
            <th>V ref</th>
    ...
    <tbody>
        <tr v-for="(item, index) in pickupVoltage" :key="index">
            <td>{{ item }}</td>
            <td>
                <el-input size="small" v-model="asset_.pickupVol.rel[index].vref">
                    <template #append>V</template>
                </el-input>
            </td>
            <td>
                <el-input size="small" v-model="asset_.pickupVol.rel[index].vdev">
                    <template #append>V</template>
                </el-input>
            </td>
        </tr>
      ...
      this.testData.table.forEach((item) => {
        if (this.testData.limits === 'Absolute') {
          if (item.operation === 'Trip') {
            if (
              this.asset_.pickupVol.abs &&
              this.asset_.pickupVol.abs[1] &&
        ...
        if (this.testData.limits === 'Relative') {
          if (item.operation === 'Trip') {
            if (
              this.asset_.pickupVol.rel &&
              this.asset_.pickupVol.rel[1] &&
              Math.abs(parseFloat(this.asset_.pickupVol.rel[1].vref) - parseFloat(item.vPickup)) <=
                parseFloat(this.asset_.pickupVol.rel[1].vdev)
            ) {
              item.assessment = 'Pass'
            } else item.assessment = 'Fail'
          }
          if (item.operation === 'Close') {
            if (
              this.asset_.pickupVol.rel &&
              this.asset_.pickupVol.rel[0] &&
              Math.abs(parseFloat(this.asset_.pickupVol.rel[0].vref) - parseFloat(item.vPickup)) <=
                parseFloat(this.asset_.pickupVol.rel[0].vdev)
            ) {
              item.assessment = 'Pass'
            } else item.assessment = 'Fail'
          }
