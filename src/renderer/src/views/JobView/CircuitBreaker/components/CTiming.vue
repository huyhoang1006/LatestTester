  ...
  <div id="CTiming">
    ...
    <div
      v-if="
        testData &&
        testData.table &&
        Object.keys(testData.table).length > 0 &&
        getInterruptersPerPhase() === 1
      "
    >
      <div v-for="(tableData, tableKey) in testData.table" :key="tableKey" style="margin-top: 2%">
        <div
          v-if="Object.keys(testData.table).length > 1"
          style="font-weight: bold; font-size: 12px"
        >
          Close coil no. {{ tableKey.replace('table', '') }}
        </div>
        <br v-if="Object.keys(testData.table).length > 1" />
        <div class="table-scroll">
          <table
            v-if="tableData && Array.isArray(tableData)"
            class="table-strip-input-data test-table"
              ...
              <tr v-for="(item, index) in tableData" :key="index">
                <td>
                  <div style="display: flex; width: 100%">
                    <el-input size="small" v-model="item.phase.value"></el-input>
                    <div
                      :class="{
                        colorTableRed: item.phase.value == 'A',
                        colorTableYellow: item.phase.value == 'B',
                        colorTableBlue: item.phase.value == 'C'
                      }"
                    ></div>
                  </div>
                </td>
                <td>
                  <el-input size="small" v-model="item.closing_time.value"></el-input>
                </td>
                <td
                  v-if="index % (getInterruptersPerPhase() * getNumberOfPhases()) === 0"
                ...
                >
                  <el-input
                    ...
                    type="textarea"
                    size="small"
                    v-model="item.closing_sync_between_phase.value"
                  ></el-input>
                </td>
                <td>
        ...
        <div class="table-scroll">
          ...
          >
            ...
            <tbody>
              <tr v-for="(item, index) in tableData" :key="index">
                <td
                  v-if="index % getInterruptersPerPhase() === 0"
                ...
                <td>
                  <el-input size="small" v-model="item.interrupter.value"></el-input>
                </td>
                <td>
                  <el-input
                    type="text"
                    number="positive"
                    size="small"
                    v-model="item.closing_time.value"
                  ></el-input>
                </td>
                <td
                  v-if="index % getInterruptersPerPhase() === 0"
                ...
                >
                  <el-input
                    ...
                    type="textarea"
                    number="positive"
                    v-model="item.closing_sync_between_interrupter.value"
                  ></el-input>
                </td>
                <td
                  v-if="index % (getInterruptersPerPhase() * getNumberOfPhases()) === 0"
    ...
    >
      ...
      <!--
                <table class="table-strip-input-data" v-if="testData.limits === 'Absolute'">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in opening_times" :key="index">
                            <td>{{ item }}</td>
                            <td>
                                <el-input size="small" v-model="asset_.openTime.abs[index].tmin">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input size="small" v-model="asset_.openTime.abs[index].tmax">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                        </tr>
                ...
                <table class="table-strip-input-data" v-else-if="testData.limits === 'Relative'">
                    <thead>
                        <tr>
                            <th></th>
                            <th>t ref</th>
                    ...
                    <tbody>
                        <tr v-for="(item, index) in opening_times" :key="index">
                            <td>{{ item }}</td>
                            <td>
                                <el-input size="small" v-model="asset_.openTime.rel[index].rref">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input size="small" v-model="asset_.openTime.rel[index].tdevZ">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input size="small" v-model="asset_.openTime.rel[index].tdevN">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                        </tr>
                ...
                <table class="table-strip-input-data" v-if="testData.limits === 'Absolute'">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in Auxiliary_contact" :key="index">
                            ...
                            <td>
                                <el-input size="small" v-model="asset_.auxContact.abs.trip[index].tmin">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input size="small" v-model="asset_.auxContact.abs.trip[index].tmax">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                        </tr>
                ...
                <table class="table-strip-input-data" v-else-if="testData.limits === 'Relative'">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in Auxiliary_contact" :key="index">
                            ...
                            <td>
                                <el-input size="small" v-model="asset_.auxContact.rel.trip[index].tref">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input size="small" v-model="asset_.auxContact.rel.trip[index].tdef">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                        </tr>
                ...
                <table class="table-strip-input-data" v-if="testData.limits === 'Absolute'">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in miscellaneous" :key="index">
                            <td>{{ item }}</td>
                            <td>
                                <el-input v-if="index === 1" size="small" v-model="asset_.miscell.abs[index].min">
                                </el-input>
                                <el-input v-else size="small" v-model="asset_.miscell.abs[index].min">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input v-if="index === 1" size="small" v-model="asset_.miscell.abs[index].max">
                                </el-input>
                                <el-input v-else size="small" v-model="asset_.miscell.abs[index].max">
                                    <template #append>ms</template>
                                </el-input>
                            </td>
                        </tr>
                ...
                <table class="table-strip-input-data" v-else-if="testData.limits === 'Relative'">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in miscellaneous" :key="index">
                            <td>{{ item }}</td>
                            <td>
                                <el-input v-if="index === 1" size="small" v-model="asset_.miscell.rel[index].ref">
                                </el-input>
                                <el-input v-else size="small" v-model="asset_.miscell.rel[index].ref">
                                    <template v-if="item !== 1">ms</template>
                                </el-input>
                            </td>
                            <td>
                                <el-input v-if="index === 1" size="small" v-model="asset_.miscell.rel[index].dev">
                                </el-input>
                                <el-input v-else size="small" v-model="asset_.miscell.rel[index].dev">
                                    <template v-if="item !== 1">ms</template>
                                </el-input>
                            </td>
                        </tr>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      if (
        this.testData &&
        (!this.testData.table || Object.keys(this.testData.table).length === 0) &&
        this.assetData &&
        this.assetData.operating
      ) {
  ...
  computed: {
    ...
    numberOfCloseCoils() {
      if (this.assetData && this.assetData.operating) {
        const value =
          this.assetData.operating.numberCloseCoil || this.assetData.operating.number_of_close_coil
    ...
    numberOfCloseCoils: {
      handler: function (newVal) {
        ...
        if (this.testData && this.testData.table && !Array.isArray(this.testData.table)) {
          if (
            Object.keys(this.testData.table).length > 0 &&
            Object.keys(this.testData.table).length !== newVal
          ) {
            this.testData.table = {}
            this.$nextTick(() => {
              this.initializeTable()
  ...
  methods: {
    normalizeAssessmentLimits(data) {
      ...
      if (!normalized.openTime.abs || normalized.openTime.abs.length < 9) {
        normalized.openTime.abs = Array(9)
          .fill(null)
          .map((_, index) =>
      ...
      if (!normalized.openTime.rel || normalized.openTime.rel.length < 9) {
        normalized.openTime.rel = Array(9)
          .fill(null)
          .map((_, index) =>
    ...
    getInterruptersPerPhase() {
      if (this.assetData && this.assetData.circuitBreaker) {
        const value =
          this.assetData.circuitBreaker.interruptersPerPhase ||
    ...
    getNumberOfPhases() {
      if (this.assetData && this.assetData.circuitBreaker) {
        const value =
          this.assetData.circuitBreaker.numberOfPhases ||
    ...
    calculator() {
      /* eslint-disable */
      const circuitBreaker_ = JSON.parse(this.$selectedAsset()[0].circuitBreaker)
      ...
      this.testData.table.forEach((element, tableIndex) => {
        if (this.testData.limits === 'Absolute') {
          element.forEach((e, index) => {
            ...
            if (
              index % (circuitBreaker_.interruptersPerPhase * circuitBreaker_.numberOfPhases) ==
              0
            ) {
              if (
                parseFloat(e.closing_sync_between_phase.value) <
                  parseFloat(this.asset_.openTime.abs[5].tmin) ||
                (parseFloat(e.closing_sync_between_phase.value) >
                  parseFloat(this.asset_.openTime.abs[5].tmax) &&
                  e.closing_sync_between_phase.value)
              ) {
                for (
                  let j = 0;
                  j < circuitBreaker_.interruptersPerPhase * circuitBreaker_.numberOfPhases;
                  j++
                ) {
            ...
            if (e.assessment.value !== 'Fail') {
              if (index % circuitBreaker_.interruptersPerPhase == 0) {
                if (
                  parseFloat(e.closing_sync_between_interrupter.value) <
                    parseFloat(this.asset_.openTime.abs[4].tmin) ||
                  parseFloat(e.closing_sync_between_interrupter.value) >
                    parseFloat(this.asset_.openTime.abs[4].tmax)
                ) {
              ...
              if (e.assessment.value !== 'Fail') {
                if (
                  parseFloat(e.closing_time.value) < parseFloat(this.asset_.openTime.abs[3].tmin) ||
                  parseFloat(e.closing_time.value) > parseFloat(this.asset_.openTime.abs[3].tmax)
                ) {
        ...
        // --------------------------- RELATIVE ------------------------------
          ...
          element.forEach((e, index) => {
            ...
            if (
              ...
              if (
                parseFloat(e.closing_sync_between_phase.value) <
                parseFloat(this.asset_.openTime.rel[5].rref)
              ) {
                if (
                  parseFloat(e.closing_sync_between_phase.value) <
                  parseFloat(this.asset_.openTime.rel[5].rref) -
                    parseFloat(this.asset_.openTime.rel[5].tdevZ)
                ) {
                  for (
                    let j = 0;
                    j < circuitBreaker_.interruptersPerPhase * circuitBreaker_.numberOfPhases;
                ...
                if (
                  parseFloat(e.closing_sync_between_phase.value) >
                  parseFloat(this.asset_.openTime.rel[5].rref) +
                    parseFloat(this.asset_.openTime.rel[5].tdevN)
                ) {
                  for (
                    let j = 0;
                    j < circuitBreaker_.interruptersPerPhase * circuitBreaker_.numberOfPhases;
            ...
            if (e.assessment.value !== 'Fail') {
              if (index % circuitBreaker_.interruptersPerPhase == 0) {
                if (
                  parseFloat(e.closing_sync_between_interrupter.value) <
                  parseFloat(this.asset_.openTime.rel[4].rref)
                ) {
                  if (
                    parseFloat(e.closing_sync_between_interrupter.value) <
                    parseFloat(this.asset_.openTime.rel[4].rref) -
                      parseFloat(this.asset_.openTime.rel[4].tdevZ)
                  ) {
                    for (let j = 0; j < circuitBreaker_.interruptersPerPhase; j++) {
                      this.testData.table[tableIndex][index + j].assessment.value = 'Fail'
                    }
                  ...
                  if (
                    parseFloat(e.closing_sync_between_interrupter.value) >
                    parseFloat(this.asset_.openTime.rel[4].rref) +
                      parseFloat(this.asset_.openTime.rel[4].tdevN)
                  ) {
                    for (let j = 0; j < circuitBreaker_.interruptersPerPhase; j++) {
                      this.testData.table[tableIndex][index + j].assessment.value = 'Fail'
                    }
              ...
              if (e.assessment.value !== 'Fail') {
                if (
                  parseFloat(e.closing_time.value) < parseFloat(this.asset_.openTime.rel[3].rref)
                ) {
                  if (
                    parseFloat(e.closing_time.value) <
                    parseFloat(this.asset_.openTime.rel[3].rref) -
                      parseFloat(this.asset_.openTime.rel[3].tdevZ)
                  ) {
                    e.assessment.value = 'Fail'
                  ...
                  if (
                    parseFloat(e.closing_time.value) >
                    parseFloat(this.asset_.openTime.rel[3].rref) +
                      parseFloat(this.asset_.openTime.rel[3].tdevN)
                  ) {
                    e.assessment.value = 'Fail'
                  } else e.assessment.value = 'Pass'
                }
