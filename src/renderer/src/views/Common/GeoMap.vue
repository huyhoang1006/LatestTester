...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      let attributionLinks = document.querySelectorAll(
  ...
  methods: {
    ...
    async customLocationClick() {
      this.popupText =
        'Geo coordinate at (' + this.clickedLatLng.lat + ',' + this.clickedLatLng.lng + ')'
    },
    async reloadMap() {
      await this.$nextTick()
      const mapRef = this.$refs.map
      const map = mapRef?.leafletObject || mapRef?.mapObject
