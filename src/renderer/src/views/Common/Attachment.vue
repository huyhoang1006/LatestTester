<template>
  <div>
    ...
    <div
      class="border-main color-main"
      :style="{ height: height, overflow: 'auto', fontSize: '12px' }"
    >
      <table class="table-attachment">
        <tr class="tr-hover" v-for="(item, index) in rowData" :key="index">
          <td @click="onTdClick(index)" @dblclick="onTdDblClick" ref="table">
  ...
  methods: {
    ...
    selectRow(x) {
      ...
      this.$refs['table'].forEach((element) => {
        ;((element.style.backgroundColor = 'rgba(0, 0, 0, 0)'), (element.style.color = 'black'))
      })
      if (myDivObjBgColor.toString() === 'rgba(0, 0, 0, 0)') {
        ;((ref.style.backgroundColor = '#012596'), (ref.style.color = 'white'))
        this.rowCurrent = x
      } else {
        ;((ref.style.backgroundColor = 'rgba(0, 0, 0, 0)'), (ref.style.color = 'black'))
        this.rowCurrent = ''
      }
    },
    async upload() {
