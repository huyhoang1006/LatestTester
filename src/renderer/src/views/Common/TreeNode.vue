  ...
  <li class="tree-li">
    ...
    >
      ...
      <icon
        ...
        :badgeColor="iconProps.badgeColor"
      ></icon>
...
import {
  ...
  clearDragOver,
  endDrag
} from '@/views/TreeNode/Common/MoveNode/dragDropState'
...
export default {
  ...
  computed: {
    ...
    // gom cấu hình icon theo mode về 1 chỗ
    iconProps() {
      const mode = this.node ? this.node.mode : ''
      if (mode === 'substation')
        return { folderType: 'location', badgeColor: '146EBE', assetDetail: undefined }
      if (mode === 'voltageLevel')
        return { folderType: 'voltageLevel', badgeColor: '146EBE', assetDetail: undefined }
      if (mode === 'bay') return { folderType: 'bay', badgeColor: '146EBE', assetDetail: undefined }
      if (mode === 'asset')
        return { folderType: 'asset', badgeColor: '146EBE', assetDetail: this.node.asset }
