    ...
    <template v-if="tabs && tabs.length > 0">
      <div class="tabs-header">
        ...
        <div class="tabs-header-data" ref="tabsHeader" @scroll="checkScroll">
          <div
            ...
            class="tab-item"
            :class="{ active: compareTab(activeTab, tab) }"
            ref="tabItems"
          >
            <div class="icon-wrapper mgl-10">
              <icon
                v-if="tab.mode == 'substation'"
                size="16px"
                folderType="location"
                badgeColor="146EBE"
              ></icon>
              <icon
                v-else-if="tab.mode == 'voltageLevel'"
                size="16px"
                folderType="voltageLevel"
                badgeColor="146EBE"
              ></icon>
              <icon
                v-else-if="tab.mode == 'bay'"
                size="16px"
                folderType="bay"
                badgeColor="146EBE"
              ></icon>
              <icon
                v-else-if="tab.mode == 'asset'"
                size="16px"
                folderType="asset"
              ...
              ></icon>
              <icon
                v-else-if="tab.mode == 'job'"
                size="16px"
                folderType="job"
                badgeColor="FF0000"
              ></icon>
              <icon
                v-else-if="tab.mode == 'test'"
                size="16px"
                folderType="test"
                badgeColor="008001"
              ></icon>
              <icon v-else size="16px" folderType="building" badgeColor="008001"></icon>
              <span v-if="tab.mode == 'organisation'" class="tab-label">{{
                tab.aliasName || tab.name
              }}</span>
              <span v-else-if="tab.mode == 'substation'" class="tab-label">{{
                tab.aliasName || tab.name
              }}</span>
              <span v-else-if="tab.mode == 'voltageLevel'" class="tab-label">{{
                tab.aliasName || tab.name
              }}</span>
              <span v-else-if="tab.mode == 'bay'" class="tab-label">{{
                tab.aliasName || tab.name
              }}</span>
              <span v-else-if="tab.mode == 'asset'" class="tab-label">{{
                tab.apparatus_id || tab.serial_number
              }}</span>
      ...
      <div class="tabs-content">
        <div class="mgr-20 mgt-20 mgb-20 mgl-20">
          ...
          <div v-show="activeTab && (activeTab.mrid || activeTab.id)" class="tab-scroll-wrapper">
            <!-- KEEP-ALIVE BẮT BUỘC BỌC TRỰC TIẾP LÊN COMPONENT -->
            <keep-alive :max="20">
              <component
                ...
                :productAssetModelData="
                  getContext(activeTab.mrid || activeTab.id).productAssetModelData
                "
                :parent="getContext(activeTab.mrid || activeTab.id).parentOrganization"
                :locationData="getContext(activeTab.mrid || activeTab.id).locationData"
...
export default {
  ...
  data() {
    return {
      ...
      // Tạo vùng nhớ độc lập cho từng Tab
      tabContexts: {},
      ...
      handler(newTab) {
        ...
        this.$nextTick(() => {
          ...
          if (comp && this.pendingLoadData[id]) {
            ...
            delete this.pendingLoadData[id]
          }
        })
      },
      immediate: true,
  ...
  methods: {
    ...
    executeOrQueueLoadData(id, loadFn) {
      this.$nextTick(() => {
        ...
        if (comp) {
          ...
          this.pendingLoadData[id] = loadFn
        }
      })
    },
        ...
        if (tab.mode === 'substation') {
          ...
          if (savedData) {
            ...
            const [dataLocation, dataPerson, dataEntity] = await Promise.all([
              ...
              window.electronAPI.getSubstationEntityByMrid(tab.mrid, this.$userId(), tab.parentId)
            ])
          ...
          if (savedData) {
            ...
            if (tab.asset === 'Surge arrester') {
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getSurgeArresterEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
              ...
              if (data.success) {
                ...
                if (!BushingDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getCurrentTransformerEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const currentTransformerDto = currentTransformerMapper.mapEntityToDto(data.data)
                if (!currentTransformerDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getVoltageTransformerEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const vtDto = vtMapper.mapEntityToDto(data.data)
                if (!vtDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getDisconnectorEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const disconnectorDto = disconnectorMapper.disconnectorEntityToDto(data.data)
                if (!disconnectorDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getPowerCableEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const powerCableDto = PowerCableMapper.mapEntityToDto(data.data)
                if (!powerCableDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getRotatingMachineEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const rotatingMachineDto = RotatingMachineMapper.mapEntityToDto(data.data)
                if (!rotatingMachineDto.properties?.serial_no)
              ...
              if (data.success) {
                ...
                if (!capacitorDto.properties?.serial_no)
              ...
              if (data.success) {
                ...
                if (!breakerDto.properties?.serial_no)
              ...
              ctx.parentOrganization = { mrid: tab.parentId }
              const data = await window.electronAPI.getTransformerEntityByMrid(
                tab.mrid,
                tab.parentId
              )
              if (data.success) {
                const transformerDto = transformerMapper.transformerEntityToDto(data.data)
                if (!transformerDto.properties?.serial_no)
              ...
              if (data.success) {
                ...
                if (!reactorDto.properties?.serial_no)
          ...
          if (dataAsset.success) {
            ctx.assetData = dataAsset.data
            ctx.parentOrganization = dataAsset.data
            ...
            ctx.locationData = dataLocation.success ? dataLocation.data : {}
            ctx.productAssetModelData = dataProductAssetModel.success
              ? dataProductAssetModel.data
              : {}
          } else {
            ctx.assetData = {}
            ctx.locationData = {}
            ctx.productAssetModelData = {}
          }
          ...
          if (tab.job === 'Surge arrester') {
            ...
            ctx.assetData = dataSurgeArrester.success ? dataSurgeArrester.data : {}
            ...
            if (data.success) {
              ...
              for (const test of surgeArresterJobDto.testList) {
                for (const type of ctx.testTypeListData) {
            ...
            if (dataPowerCable.success && dataPowerCable.data) {
              ...
              ctx.assetData = PowerCableMapper.mapEntityToDto(dataPowerCable.data)
            } else {
              ctx.assetData = {}
            }
            ...
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataTransformer = await window.electronAPI.getTransformerEntityByMrid(
              tab.parentId
            )
            ctx.assetData = dataTransformer.success
              ? transformerMapper.transformerEntityToDto(dataTransformer.data)
              : {}
            ...
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataVoltageTransformer =
              await window.electronAPI.getVoltageTransformerEntityByMrid(tab.parentId)
            if (dataVoltageTransformer.success) {
              ctx.assetData = vtMapper.mapEntityToDto(dataVoltageTransformer.data)
              const [dataLocation, dataProductAssetModel] = await Promise.all([
                window.electronAPI.getLocationDetailByMrid(ctx.assetData.locationId),
                window.electronAPI.getProductAssetModelByMrid(ctx.assetData.productAssetModelId)
              ])
              ctx.locationData = dataLocation.success ? dataLocation.data : {}
              ctx.productAssetModelData = dataProductAssetModel.success
                ? dataProductAssetModel.data
            ...
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataCurrentTransformer =
              await window.electronAPI.getCurrentTransformerEntityByMrid(tab.parentId)
            ctx.assetData = dataCurrentTransformer.success
              ? currentTransformerMapper.mapEntityToDto(dataCurrentTransformer.data)
              : {}
            ...
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataDisconnector = await window.electronAPI.getDisconnectorEntityByMrid(
              tab.parentId
            )
            ctx.assetData = dataDisconnector.success
              ? disconnectorMapper.disconnectorEntityToDto(dataDisconnector.data)
              : {}
            ...
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataRotatingMachine = await window.electronAPI.getRotatingMachineEntityByMrid(
              tab.parentId
            )
            ctx.assetData = dataRotatingMachine.success
              ? RotatingMachineMapper.mapEntityToDto(dataRotatingMachine.data)
              : {}
            ...
            const dataTestType =
              await window.electronAPI.getProcedureByGenericAssetModel('Capacitor')
            ctx.testTypeListData = dataTestType.success ? dataTestType.data : []
            const dataCapacitor = await window.electronAPI.getCapacitorEntityByMrid(tab.parentId)
            ctx.assetData = dataCapacitor.success
              ? CapacitorMapper.mapEntityToDto(dataCapacitor.data)
              : {}
            ...
            ctx.assetData = dataBushing.success ? dataBushing.data : {}
            ...
            ctx.assetData = dataCircuitBreaker.success
              ? BreakerMapper.mapEntityToDto(dataCircuitBreaker.data)
              : {}
    ...
    // Dọn dẹp cache rác khi tab bị đóng
    closeTab(index) {
      const tabToClose = this.tabs[index]
      if (tabToClose) {
        const id = tabToClose.mrid || tabToClose.id
        delete this.tabContexts[id]
        delete this.pendingLoadData[id]
      }
