...
const REGISTRY = {
  ...
  'Voltage transformer': {
    ...
    save: (payload, deviceId) =>
      voltageTransformerJobAPI.createVoltageTransformerJob(payload, deviceId)
  },
  'Current transformer': {
    dtoKey: 'currentTransformerJobDto',
    toServer: currentTransformerJobServerMapper.mapDtoToServer,
    save: (payload, deviceId) =>
      currentTransformerJobAPI.createCurrentTransformerJob(payload, deviceId)
  },
  'Circuit breaker': {
...
export async function saveServerJobTab(component, tab) {
  ...
  try {
    ...
    const version =
      (response && response.data && response.data.version) !== undefined
    ...
    if (
      ...
      return {
        ...
        conflict: true,
        message:
          'Someone else saved this job after you opened it. ' +
