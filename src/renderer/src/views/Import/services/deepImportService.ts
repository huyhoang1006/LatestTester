...
export var deepImportService = {
  ...
  getLevelsToProcess: function (selectedMode, allLevelData) {
    ...
    if (!selectedMode || selectedMode === 'organisation') {
      ...
      for (var i = 0; i < LEVEL_ORDER.length; i++) {
        if (LEVEL_ORDER[i].mode === selectedMode) {
          skipUntil = i
          break
        }
      }
    ...
    // Thêm Asset levels
    var assetCatKeys = Object.keys(allLevelData).filter(function (k) {
      return k.startsWith('Asset_')
    })
    assetCatKeys.forEach(function (catKey) {
      var assetType = catKey.replace('Asset_', '')
      var node =
        FEATURE_TREE.Asset && FEATURE_TREE.Asset.children && FEATURE_TREE.Asset.children[assetType]
      result.push({
        id: 'asset',
        catKey: catKey,
        label: (node && node.label) || assetType,
        mode: 'asset',
        reqField: ASSET_REQ_FIELD,
        reqLabel: 'serial no.'
      })
    })

    // Thêm Job levels
    var jobCatKeys = Object.keys(allLevelData).filter(function (k) {
      return k.startsWith('Job_')
    })
    jobCatKeys.forEach(function (catKey) {
      ...
      result.push({
        ...
        mode: 'job',
        reqField: JOB_REQ_FIELD,
        reqLabel: 'job name (auto-generated if has tests)'
      })
    })
  ...
  validatePrerequisites: function (levelsToProcess, allLevelData) {
    ...
    for (var i = 0; i < levelsToProcess.length; i++) {
      ...
      var data: any = allLevelData[lv.catKey]
      ...
      if (lv.id === 'job' && !data[lv.reqField]) {
        var hasTests = Object.keys(data).some(function (k) {
          return k !== 'job_name'
        })
      ...
      if (data[lv.reqField] === undefined || data[lv.reqField] === '') {
        errors.push({
          level: lv.label,
          field: lv.reqLabel,
          message: lv.label + ' must have a ' + lv.reqLabel
        })
      }
    }
    return errors
  ...
  async findExistingByName(levelId, name, ctx, assetType, userId) {
    if (!userId)
      userId =
        ...
        (window as any).store.state.user &&
        (window as any).store.state.user.user_id
          ? (window as any).store.state.user.user_id
          : null
    if (levelId === 'org') {
      ...
      if (_orgCtxMrid) {
        ...
        var rs: any = await electronAPI.getParentOrganizationByParentMrid(_orgCtxMrid)
        if (!rs || !rs.success) return null
        var list = rs.data || []
        return (
          (list as any[]).find(function (o: any) {
            return (o.name || o.aliasName) === name
          }) || null
        )
      } else {
        // Chưa có ctx org → tìm trong root's children
        var rs: any = await electronAPI.getParentOrganizationByMrid(ROOT_MRID)
        ...
        var rs2 = await electronAPI.getParentOrganizationByParentMrid(rootOrg.mrid)
    ...
    if (levelId === 'sub') {
      ...
      var rs: any = await electronAPI.getSubstationsInOrganisationForUser(orgMrid, userId)
      if (!rs || !rs.success) return null
      var list = rs.data || []
      var found: any = list.find(function (s) {
        return s.name === name || s.aliasName === name
      })
      return found || null
    }
    if (levelId === 'vl') {
      ...
      var rs: any = await electronAPI.getVoltageLevelBySubstationId(subMrid)
      if (!rs || !rs.success) return null
      var list = rs.data || []
      return (
        list.find(function (vl) {
          return vl.name === name || vl.aliasName === name
        }) || null
      )
    }
    if (levelId === 'bay') {
      ...
      var subMrid = (ctx.sub && ctx.sub.mrid) || (ctx.substation && ctx.substation.mrid)
      var rs: any = await electronAPI.getBayByVoltageBySubstationId(vlMrid || null, subMrid || null)
      if (!rs || !rs.success) return null
      var list = rs.data || []
      return (
        list.find(function (b) {
          return b.name === name || b.aliasName === name
        }) || null
      )
    }
    if (levelId === 'asset') {
      ...
      for (var pi = 0; pi < psrIds.length; pi++) {
        ...
        if (kind === 'Surge arrester') {
          rs = await electronAPI.getSurgeArresterByPsrId(psrIds[pi])
        } else if (kind === 'Bushing') {
          rs = await electronAPI.getBushingByPsrId(psrIds[pi])
        } else {
          rs = await electronAPI.getAssetByPsrIdAndKind(psrIds[pi], kind)
        }
        ...
        var found: any = list.find(function (a) {
    ...
    if (levelId === 'job') {
      ...
      var rs: any = await electronAPI.getOldWorkByAssetId(assetMrid)
      if (!rs || !rs.success) return null
      var list = rs.data || []
      return (
        list.find(function (j) {
          return (j.name || j.job_name) === name
        }) || null
      )
    }
    return null
  ...
  traverseAndFillMrid: function (obj) {
    if (Array.isArray(obj)) {
      obj.forEach(function (item) {
        deepImportService.traverseAndFillMrid(item)
      })
      ...
      Object.values(obj).forEach(function (val) {
        deepImportService.traverseAndFillMrid(val)
      })
    }
    return obj
  },
  ...
  _getUserId: function () {
    // (window as any).store không tồn tại trong Electron context
    // userId nên được truyền từ caller (importHierarchy nhận userId param)
    // Hàm này chỉ là fallback cuối cùng
    try {
      if (typeof window !== 'undefined') {
        if ((window as any).__vue_store__)
          return ((window as any).__vue_store__.state.user || {}).user_id || null
        if ((window as any).store)
          return (((window as any).store.state || {}).user || {}).user_id || null
      }
    } catch (e: any) {}
    return null
  },
  _getUserName: function () {
    try {
      return (window as any).store &&
        (window as any).store.state &&
        (window as any).store.state.user
        ? (window as any).store.state.user.name
        : ''
    } catch (e: any) {
      return ''
    }
  ...
  _getField: function (data, suffix) {
    ...
    var found: any = keys.find(function (k) {
      return k === suffix || k.endsWith('_' + suffix)
    })
  ...
  async insertEntity(lv, data, ctx, _passedUserId) {
    ...
    if (lv.id === 'org') {
      ...
      var rootRs = await electronAPI.getParentOrganizationByMrid(ROOT_MRID)
      ...
      var hasTown =
        dto.city ||
        dto.state_or_province ||
        dto.country ||
        dto.district_or_town ||
        dto.ward_or_commune
      ...
      rs = await electronAPI.insertParentOrganizationEntity(entity)
      return {
        success: !!(rs && rs.success === true),
        mrid: this._extractMrid(rs),
    ...
    if (lv.id === 'sub') {
      if (!orgMrid)
        return { success: false, message: 'Cannot insert Substation: no Organisation context' }
      ...
      if ((dto.streetDetailId || dto.townDetailId) && !dto.streetAddressId)
        dto.streetAddressId = uuid.newUuid()
      if ((dto.streetAddressId || dto.locationName) && !dto.locationId)
        dto.locationId = uuid.newUuid()
      if ((dto.email || dto.fax) && !dto.electronicAddressId)
      ...
      if (dto.personId && dto.subsId && !dto.personSubstationId)
        dto.personSubstationId = uuid.newUuid()
      if (orgMrid && dto.locationId && !dto.organisationLocationId)
        dto.organisationLocationId = uuid.newUuid()
      if (orgMrid && dto.personId && !dto.organisationPersonId)
        dto.organisationPersonId = uuid.newUuid()
      if (orgMrid && dto.subsId && !dto.organisationPsrId) dto.organisationPsrId = uuid.newUuid()
      if (dto.subsId && userId && !dto.userIdentifiedObjectId)
      ...
      rs = await electronAPI.insertSubstationEntity(entity)
      return {
        success: !!(rs && rs.success === true),
        mrid: this._extractMrid(rs),
        message: rs && rs.message
    ...
    if (lv.id === 'vl') {
      if (!subMrid)
        return { success: false, message: 'Cannot insert VoltageLevel: no Substation context' }
      ...
      rs = await electronAPI.insertVoltageLevelEntity(entity)
      return {
        success: !!(rs && rs.success === true),
        mrid: this._extractMrid(rs),
        message: rs && rs.message
    ...
    if (lv.id === 'bay') {
      ...
      rs = await electronAPI.insertBayEntity(dto)
      return {
        success: !!(rs && rs.success === true),
        mrid: this._extractMrid(rs),
        message: rs && rs.message
    ...
    if (lv.id === 'asset') {
      ...
      if (!psrId)
        return { success: false, message: 'Cannot insert Asset: no Bay/Substation context' }
      ...
      if (dto.properties) {
        ...
        this._fillDtoFlat(dto.properties, data, [
          'type',
          'manufacturer',
          'manufacturer_type',
          'manufacturer_year',
      ...
      if (catKey === 'Asset_VoltageTransformerDto') {
        if (
          dto.vt_Configuration &&
          dto.vt_Configuration.windings &&
      ...
      if (catKey === 'Asset_CurrentTransformerDto') {
        ...
        if (
          ...
          typeof dto.ratings.system_voltage_type !== 'object'
        ) {
          ...
          dto.ctConfiguration.dataCT.push({
            ...
            fullTap: {
              table: {
                ...
                ipn: {
                  mrid: uuid.newUuid(),
                  value: mkNum(arrVal('fulltap_ipn', ci)),
                  unit: arrVal('fulltap_ipn_unit', ci) || null
                },
                isn: {
                  mrid: uuid.newUuid(),
                  value: mkNum(arrVal('fulltap_isn', ci)),
                  unit: arrVal('fulltap_isn_unit', ci) || null
                },
                inUse:
                  inUseRaw === '1' ||
                  inUseRaw === 'true' ||
              ...
              classRating: {
                ...
                operatingBurdenCos:
                  arrVal('op_burden_cos', ci) || arrVal('class_op_burden_cos', ci),
      ...
      if (catKey === 'Asset_TransformerDataDto') {
        ...
        if (!dto.oldTransformerEndInfo || dto.oldTransformerEndInfo.length === 0) {
          ...
          var assetType =
            this._getField(data, 'type') || (dto.properties && dto.properties.type) || ''
          var isThree
          if (assetType) {
            ...
            var hasSecTert = !!(
              this._getField(data, 'st_uk') ||
              this._getField(data, 'st_base_power') ||
              this._getField(data, 'st_base_voltage')
            )
        ...
        if (!vg.tert || typeof vg.tert !== 'object')
          vg.tert = { i: '', value: '', accessible: null }
        ...
        dto.testList = dto.testList.map(function (item) {
          ...
          if (newItem.testCondition && newItem.testCondition.condition) {
            ...
            Object.keys(newItem.testCondition.condition).forEach(function (ck) {
              ...
              _cond[ck] =
                cv && typeof cv === 'object' ? Object.assign({}, cv, { measurement_id: null }) : cv
            })
  ...
  async overwriteEntity(lv, existing, data, ctx, userId) {
    ...
    if (lv.id === 'org') {
      var rs: any = await electronAPI.getOrganisationEntityByMrid(mrid)
      if (!rs || !rs.success || !rs.data) return { success: false }
      var e = rs.data
      this._applyFlat(e, data, [
        'name',
        'aliasName',
        'tax_code',
        'street',
      ...
      return await electronAPI.insertParentOrganizationEntity(e) // upsert
    }
    if (lv.id === 'sub') {
      var rs: any = await electronAPI.getSubstationEntityByMrid(mrid)
      if (!rs || !rs.success || !rs.data) return { success: false }
      var e = rs.data
      this._applyFlat(e, data, [
        'name',
      ...
      return await electronAPI.insertSubstationEntity(e) // upsert
    }
    if (lv.id === 'vl') {
      var rs: any = await electronAPI.getVoltageLevelEntityByMrid(mrid)
      if (!rs || !rs.success || !rs.data) return { success: false }
      var e = rs.data
      this._applyFlat(e, data, [
        'name',
      ...
      return await electronAPI.insertVoltageLevelEntity(e) // upsert
    }
    if (lv.id === 'bay') {
      var rs: any = await electronAPI.getBayEntityByMrid(mrid)
      if (!rs || !rs.success || !rs.data) return { success: false }
      var e = rs.data
      this._applyFlat(e, data, [
        'name',
      ...
      return await electronAPI.insertBayEntity(e) // upsert
    }
    if (lv.id === 'asset') {
      // Upsert: truyền existing.mrid qua _overwriteMrid để insertEntity dùng mrid cũ
      return await this.insertEntity(
        lv,
        Object.assign({}, data, { _overwriteMrid: mrid }),
        ctx,
        userId
      )
    }
  ...
  analyzeDecisionsNeeded: function (allLevelData, levelsToProcess) {
    ...
    var hasOrg = !!(
      allLevelData['OrgEntityToOrgDto'] && Object.keys(allLevelData['OrgEntityToOrgDto']).length
    )
    void hasOrg
    var hasJobData = levelsToProcess.some(function (lv) {
      return (
        lv.id === 'job' && allLevelData[lv.catKey] && Object.keys(allLevelData[lv.catKey]).length
      )
    })
    ...
    var hasAssetSerial = levelsToProcess.some(function (lv) {
      return (
        lv.id === 'asset' && allLevelData[lv.catKey] && allLevelData[lv.catKey][ASSET_REQ_FIELD]
      )
    })
    ...
    if (
      ...
      !hasSubName &&
      !hasAssetSerial &&
      !decisions.some(function (d) {
        return d.levelId === 'sub'
      })
    ) {
      decisions.push({
        ...
        issue:
          'Job/Test data found but no Substation name or Asset serial number. ' +
  ...
  applyDecisions: function (decisions, allLevelData) {
    for (var i = 0; i < decisions.length; i++) {
      ...
      if (d.type === 'missing_name') {
        ...
        var assetKey = Object.keys(allLevelData).find(function (k) {
          return k.startsWith('Asset_')
        })
        if (!assetKey) {
    ...
    try {
      if (lv.id === 'org') {
        ...
        if (parentMrid) {
          ...
          var rs: any = await electronAPI.getParentOrganizationByParentMrid(parentMrid)
          list = rs && rs.success && rs.data ? (Array.isArray(rs.data) ? rs.data : [rs.data]) : []
        } else {
          // Không có parent → lấy root org, rồi tìm trong root's children
          var rootRs = await electronAPI.getParentOrganizationByMrid(ROOT_MRID)
          if (rootRs && rootRs.success && rootRs.data) {
            var rootOrg = rootRs.data
            var childRs = await electronAPI.getParentOrganizationByParentMrid(rootOrg.mrid)
            list =
              childRs && childRs.success && childRs.data
                ? Array.isArray(childRs.data)
                  ? childRs.data
                  : [childRs.data]
                : []
          }
        }
        var found: any = (list as any[]).find(function (o: any) {
          return (o.name || o.aliasName) === reqVal
        })
      ...
      if (lv.id === 'sub') {
        ...
        if (!orgMrid) return null
        var rs: any = await electronAPI.getSubstationsInOrganisationForUser(orgMrid, userId)
        var list: any[] = rs && rs.success && rs.data ? rs.data : []
        var found: any = (list as any[]).find(function (s: any) {
          return s.name === reqVal
        })
        return found ? found.mrid : null
      }
      if (lv.id === 'vl') {
        ...
        var rs: any = await electronAPI.getVoltageLevelBySubstationId(subMrid)
        var list: any[] =
          (rs as any) && (rs as any).success && (rs as any).data ? (rs as any).data : []
        var found: any = (list as any[]).find(function (vl: any) {
          return vl.name === reqVal
        })
        found as any
        return found ? found.mrid : null
      }
      if (lv.id === 'bay') {
        ...
        var subMrid = _sub && _sub.mrid
        var rs: any = await (window as any).electronAPI.getBayByVoltageBySubstationId(
          vlMrid || null,
          subMrid || null
        )
        var list: any[] = rs && rs.success && rs.data ? rs.data : []
        var found: any = (list as any[]).find(function (b: any) {
          return b.name === reqVal
        })
        return found ? found.mrid : null
      }
      if (lv.id === 'asset') {
        ...
        var rs: any = await (window as any).electronAPI.getAssetByPsrIdAndKind(psrId, _findKind)
        var list: any[] =
          (rs as any) && (rs as any).success && (rs as any).data
            ? Array.isArray((rs as any).data)
              ? (rs as any).data
              : [(rs as any).data]
            : []
        var found: any = (list as any[]).find(function (a: any) {
          return a.serial_no === reqVal || a.serial_number === reqVal
        })
        found as any
        return found ? found.mrid : null
      }
  ...
  _makeBaseMeasure: function (value) {
    return {
      mrid: uuid.newUuid(),
      data: { mrid: uuid.newUuid(), value: value || null, unit: null, multiplier: null }
    }
  },
  ...
  _buildTransformerArrays: function (dto, lvm) {
    ...
    var ps = this._buildImpedanceItems(
      lvm,
      'ps_uk',
      'ps_base_power',
      'ps_base_voltage',
    ...
    var pt = this._buildImpedanceItems(
      lvm,
      'pt_uk',
      'pt_base_power',
      'pt_base_voltage',
    ...
    var st = this._buildImpedanceItems(
      lvm,
      'st_uk',
      'st_base_power',
      'st_base_voltage',
    ...
    // voltage_ratings (vr_*)
    var vrLen = Math.max(
      Array.isArray(this._getField(lvm, 'vr_winding'))
        ? this._getField(lvm, 'vr_winding').length
        : 0,
      Array.isArray(this._getField(lvm, 'vr_voltage_ll'))
        ? this._getField(lvm, 'vr_voltage_ll').length
        : 0
    )
    if (vrLen > 0) {
      ...
      for (var i = 0; i < vrLen; i++) {
        dto.ratings.voltage_ratings.push({
          mrid: uuid.newUuid(),
          winding: Array.isArray(this._getField(lvm, 'vr_winding'))
            ? this._getField(lvm, 'vr_winding')[i] || ''
            : '',
          voltage_ll: {
            mrid: uuid.newUuid(),
            value: Array.isArray(this._getField(lvm, 'vr_voltage_ll'))
              ? this._getField(lvm, 'vr_voltage_ll')[i] || null
              : null,
            unit: null,
            multiplier: null
          },
          voltage_ln: {
            mrid: uuid.newUuid(),
            value: Array.isArray(this._getField(lvm, 'vr_voltage_ln'))
              ? this._getField(lvm, 'vr_voltage_ln')[i] || null
              : null,
            unit: null,
            multiplier: null
          },
          insul_level_ll: {
            mrid: uuid.newUuid(),
            value: Array.isArray(this._getField(lvm, 'vr_insul_level_ll'))
              ? this._getField(lvm, 'vr_insul_level_ll')[i] || null
              : null,
            unit: null,
            multiplier: null
          },
          insulation_class: Array.isArray(this._getField(lvm, 'vr_insulation_class'))
            ? this._getField(lvm, 'vr_insulation_class')[i] || ''
            : '',
          voltage_regulation: Array.isArray(this._getField(lvm, 'vr_voltage_regulation'))
            ? this._getField(lvm, 'vr_voltage_regulation')[i] || ''
            : ''
        })
      }
    }
    ...
    // power_ratings (pr_*)
    var prLen = Math.max(
      Array.isArray(this._getField(lvm, 'pr_rated_power'))
        ? this._getField(lvm, 'pr_rated_power').length
        : 0,
      Array.isArray(this._getField(lvm, 'pr_cooling_class'))
        ? this._getField(lvm, 'pr_cooling_class').length
        : 0
    )
    if (prLen > 0) {
      ...
      for (var i = 0; i < prLen; i++) {
        dto.ratings.power_ratings.push({
          ...
          rated_power: {
            mrid: uuid.newUuid(),
            value: Array.isArray(this._getField(lvm, 'pr_rated_power'))
              ? this._getField(lvm, 'pr_rated_power')[i] || null
              : null,
            unit: null,
            multiplier: null
          },
          cooling_class: Array.isArray(this._getField(lvm, 'pr_cooling_class'))
            ? this._getField(lvm, 'pr_cooling_class')[i] || ''
            : '',
          temp_rise_wind: {
            mrid: uuid.newUuid(),
            value: Array.isArray(this._getField(lvm, 'pr_temp_rise_wind'))
              ? this._getField(lvm, 'pr_temp_rise_wind')[i] || null
              : null,
            unit: null,
            multiplier: null
          }
      ...
      for (var i = 0; i < crLen; i++) {
        dto.ratings.current_ratings.push({
          ...
          prim: {
            ...
            data: {
              mrid: uuid.newUuid(),
              value: Array.isArray(this._getField(lvm, 'cr_prim'))
                ? this._getField(lvm, 'cr_prim')[i] || null
                : null,
              unit: null,
              multiplier: null
            }
          ...
          sec: {
            ...
            data: {
              mrid: uuid.newUuid(),
              value: Array.isArray(this._getField(lvm, 'cr_sec'))
                ? this._getField(lvm, 'cr_sec')[i] || null
                : null,
              unit: null,
              multiplier: null
            }
          ...
          tert: {
            ...
            data: {
              mrid: uuid.newUuid(),
              value: Array.isArray(this._getField(lvm, 'cr_tert'))
                ? this._getField(lvm, 'cr_tert')[i] || null
                : null,
              unit: null,
              multiplier: null
            }
  ...
  async importHierarchy(codeValueMap, tableData, selectedNode, overwrite, decisions, userId) {
    ...
    if (selectedNode && selectedNode.context) {
      ctx = Object.assign({} as any, selectedNode.context as any)
      // Normalize: tree dùng key dài (organisation, substation, voltageLevel)
      // code dùng key ngắn (org, sub, vl) → alias để tương thích
      ;(ctx as any).organisation
      ;(ctx as any).substation
      ;(ctx as any).voltageLevel
      if ((ctx as any).organisation && !(ctx as any).org)
        (ctx as any).org = (ctx as any).organisation
      if ((ctx as any).substation && !(ctx as any).sub) (ctx as any).sub = (ctx as any).substation
      if ((ctx as any).voltageLevel && !(ctx as any).vl) (ctx as any).vl = (ctx as any).voltageLevel
    }
    ...
    for (var i = 0; i < levelsToProcess.length; i++) {
      ...
      var isJobWithTests =
        lv.id === 'job' && occurrences.length === 0 && Object.keys(data).length > 0
      if (isJobWithTests)
        occurrences = [{ reqVal: this._randomJobName(), index: 0 }] as any[] as never[]
      ...
      if (occurrences.length === 0) {
        results.push({
          ...
          catKey: lv.catKey,
          skipped: true,
          reason: 'Missing required field: ' + lv.reqLabel
        })
        continue
      }

      for (var oi = 0; oi < occurrences.length; oi++) {
        var occ = occurrences[oi] as any
        ...
        try {
          ...
          if (existing) {
            if (overwrite) {
              var rs = await this.overwriteEntity(lv, existing, occData, ctx, _userId)
            ...
            var rs: any = await this.insertEntity(lv, occData, ctx, _userId)
            var ok: any = !!(rs && rs.success === true)
