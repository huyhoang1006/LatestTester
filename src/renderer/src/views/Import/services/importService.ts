...
export const importService = {
  ...
  async updateOrg(mrid, lvm) {
    const rs = await electronAPI.getOrganisationEntityByMrid(mrid)
    ...
    fields.forEach((f: any) => {
      if (lvm[f] !== undefined) this._apply(e, f, lvm[f])
    })
    return await electronAPI.updateOrganisation(e)
  },

  // ── Substation ───────────────────────────────────────────────────────────
  async updateSubstation(mrid, lvm) {
    const rs = await electronAPI.getSubstationEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch substation' }
    const e = rs.data
    const fields = [
      'name',
      'aliasName',
    ...
    return await electronAPI.updateSubstation(e)
  },

  // ── VoltageLevel ─────────────────────────────────────────────────────────
  async updateVoltageLevel(mrid, lvm) {
    const rs = await electronAPI.getVoltageLevelEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch voltageLevel' }
    const e = rs.data
    const fields = [
      'name',
      'comment',
    ...
    return await electronAPI.updateVoltageLevel(e)
  },

  // ── Bay ──────────────────────────────────────────────────────────────────
  async updateBay(mrid, lvm) {
    const rs = await electronAPI.getBayEntityByMrid(mrid)
    ...
    return await electronAPI.updateBay(e)
  },
  ...
  _applyProps(entity, lvm) {
    ...
    propKeys.forEach((k) => {
      if (lvm[k] !== undefined) target[k] = lvm[k]
    })
  },
  ...
  _applyArrayField(arr: any[], _index: number, field: string, lvm: any) {
  ...
  async updateVT(mrid, lvm) {
    const rs = await electronAPI.getVoltageTransformerEntityByMrid(mrid)
    ...
    ratScalar.forEach((f: any) => {
      if (lvm[f] !== undefined) e.ratings[f] = lvm[f]
    })
    // config
    ...
    if (vtArrFields.some((f) => lvm[f] !== undefined)) {
      ...
      const maxLen = Math.max(
        ...vtArrFields.map((f) =>
          Array.isArray(lvm[f]) ? lvm[f].length : lvm[f] !== undefined ? 1 : 0
        )
      )
      for (let i = 0; i < maxLen; i++) {
        if (!e.vt_Configuration.dataVT[i]) e.vt_Configuration.dataVT[i] = {}
      }
      vtArrFields.forEach((f) => this._applyArrayField(e.vt_Configuration.dataVT, 0, f, lvm))
    }
    return await electronAPI.updateVoltageTransformer(e)
  },

  // ── Current Transformer ──────────────────────────────────────────────────
  async updateCT(mrid, lvm) {
    const rs = await electronAPI.getCurrentTransformerEntityByMrid(mrid)
    ...
    return await electronAPI.updateCurrentTransformer(e)
  },

  // ── Transformer ───────────────────────────────────────────────────────────
  async updateTransformer(mrid, lvm) {
    const rs = await electronAPI.getTransformerEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch Transformer' }
    const e = rs.data
    this._applyProps(e, lvm)
    if (!e.winding_configuration) e.winding_configuration = {}
    const wcFields = ['phases', 'vector_group_custom', 'unsupported_vector_group']
    wcFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.winding_configuration[f] = lvm[f]
    })
    if (!e.ratings) e.ratings = {}
    const rScalar = ['rated_frequency']
    rScalar.forEach((f: any) => {
      if (lvm[f] !== undefined) e.ratings[f] = lvm[f]
    })
    if (!e.others) e.others = {}
    const othFields = ['category', 'status', 'tank_type', 'insulation_medium']
    othFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.others[f] = lvm[f]
    })
    ...
    const tcMap = {
      tap_mode: 'mode',
      tap_serial_no: 'serial_no',
      tap_manufacturer: 'manufacturer',
      tap_manufacturer_type: 'manufacturer_type',
      tap_winding: 'winding',
      tap_scheme: 'tap_scheme',
      no_of_taps: 'no_of_taps'
    }
    tcFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.tap_changers[tcMap[f]] = lvm[f]
    })
    return await electronAPI.updateTransformer(e)
  },

  // ── Circuit Breaker ──────────────────────────────────────────────────────
  async updateBreaker(mrid, lvm) {
    const rs = await electronAPI.getBreakerEntityByMrid(mrid)
    ...
    return await electronAPI.updateCircuitBreaker(e)
  },

  // ── Power Cable ──────────────────────────────────────────────────────────
  async updateCable(mrid, lvm) {
    const rs = await electronAPI.getPowerCableEntityByMrid(mrid)
    ...
    rFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.ratingsData[f] = lvm[f]
    })
    if (!e.othersData) e.othersData = {}
    const oFields = ['insulation_method', 'bonding_type', 'install_location', 'cable_length']
    oFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.othersData[f] = lvm[f]
    })
    return await electronAPI.updatePowerCable(e)
  },

  // ── Surge Arrester ───────────────────────────────────────────────────────
  async updateSurgeArrester(mrid, lvm) {
    const rs = await electronAPI.getSurgeArresterEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data) return { success: false, message: 'Cannot fetch SA' }
    const e = rs.data
    this._applyProps(e, lvm)
    if (lvm['unitStack'] !== undefined) {
      if (!e.ratings) e.ratings = {}
      e.ratings.unitStack = lvm['unitStack']
    }
    return await electronAPI.updateSurgeArrester(e)
  },

  // ── Reactor ──────────────────────────────────────────────────────────────
  async updateReactor(mrid, lvm) {
    const rs = await electronAPI.getReactorEntityByMrid(mrid)
    ...
    rFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.reactorRating[f] = lvm[f]
    })
    if (!e.reactorOther) e.reactorOther = {}
    if (lvm['insulation_type'] !== undefined)
      e.reactorOther.insulation_type = lvm['insulation_type']
    if (lvm['weight'] !== undefined) e.reactorOther.weight = lvm['weight']
    return await electronAPI.updateReactor(e)
  },

  // ── Capacitor ────────────────────────────────────────────────────────────
  async updateCapacitor(mrid, lvm) {
    const rs = await (electronAPI as any).getCapacitorEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch Capacitor' } as any
    const e = rs.data
    this._applyProps(e, lvm)
    if (!e.configsData) e.configsData = {}
    ...
    if (!e.ratings) e.ratings = {}
    const rFields = {
      cap_rated_voltage: 'rated_voltage',
      cap_rated_frequency: 'rated_frequency',
      cap_rated_current: 'rated_current',
      cap_rated_power: 'rated_power'
    }
    Object.entries(rFields).forEach(([lv, ef]) => {
      if (lvm[lv] !== undefined) e.ratings[ef] = lvm[lv]
    })
    return await (electronAPI as any).updateCapacitor(e)
  },

  // ── Disconnector ─────────────────────────────────────────────────────────
  async updateDisconnector(mrid, lvm) {
    const rs = await (electronAPI as any).getDisconnectorEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch Disconnector' }
    const e = rs.data
    this._applyProps(e, lvm)
    if (!e.ratings) e.ratings = {}
    const rMap = {
      dc_rated_voltage: 'rated_voltage',
      dc_rated_frequency: 'rated_frequency',
      dc_rated_current: 'rated_current',
      dc_short_time_withstand_current: 'short_time_withstand_current',
    ...
    Object.entries(rMap).forEach(([lv, ef]) => {
      if (lvm[lv] !== undefined) e.ratings[ef] = lvm[lv]
    })
    return await (electronAPI as any).updateDisconnector(e)
  },

  // ── Rotating Machine ─────────────────────────────────────────────────────
  async updateRotatingMachine(mrid, lvm) {
    const rs = await (electronAPI as any).getRotatingMachineEntityByMrid(mrid)
    if (!rs || !rs.success || !rs.data)
      return { success: false, message: 'Cannot fetch RotatingMachine' }
    const e = rs.data
    this._applyProps(e, lvm)
    if (!e.ratingsData) e.ratingsData = {}
    const rFields = [
    ...
    return await (electronAPI as any).updateRotatingMachine(e)
  },

  // ── Bushing ──────────────────────────────────────────────────────────────
  async updateBushing(mrid, lvm) {
    const rs = await (electronAPI as any).getBushingEntityByMrid(mrid, '')
    ...
    bFields.forEach((f: any) => {
      if (lvm[f] !== undefined) e.bushing[f] = lvm[f]
    })
    return (
      (await (electronAPI as any).updateBushingAsset?.(e)) || {
        success: false,
        message: 'updateBushingAsset not available'
      }
    )
  },
  ...
  async applyImportForCat(cat, lvm, ctx) {
    ...
    if (k === 'OrgEntityToOrgDto')
      return this.updateOrg(ctx.organisation && ctx.organisation.mrid, lvm)
    if (k === 'SubstationDto')
      return this.updateSubstation(ctx.substation && ctx.substation.mrid, lvm)
    if (k === 'VoltageLevelDto')
      return this.updateVoltageLevel(ctx.voltageLevel && ctx.voltageLevel.mrid, lvm)
  ...
  async importNode(codeValueMap: any, tableData: any, exportCategories: any, ctx: any) {
    const results: any[] = []
    for (const cat of exportCategories) {
      ...
      if (Object.keys(lvm).length === 0) {
        results.push({
          ...
          label: cat.label,
          skipped: true,
          reason: 'No matching data in Excel'
        })
        continue
      }
      try {
