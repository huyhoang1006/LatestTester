...
const regenStandardTree = (std, uuid) => {
  ...
  s.assessment_rule = (s.assessment_rule || []).map((r) => {
    ...
    return {
      ...r,
      mrid: nm,
      standard_id: r.standard_id === oldStdMrid ? s.mrid : r.standard_id || s.mrid
    }
  })
...
const importJobNode = async (branch, parentNode, deps) => {
  ...
  try {
    ...
    if (res && res.success) {
      return {
        mode: 'job',
        mrid: newJobDto.properties.mrid,
...
const importNormalNode = async (branch, parentNode, deps) => {
  ...
  if (!branch || !branch.type) {
    console.warn(
      '[importJson] Node missing "type" field — file format may be old/invalid. Node:',
      branch
    )
    return null
  }
  ...
  try {
    ...
    if (res && res.mrid && res.success !== false)
      return {
        mode: res.mode || nodeModeOf(branch.type),
        mrid: res.mrid,
        parentId: parentNode.mrid,
        asset: branch.asset
      }
    console.warn(
      `[importJson] insert FAILED type=${branch.type} mrid=${branchMrid(branch)} parent=${parentNode.mrid} message=`,
      res && res.message,
      '| result=',
      res
    )
...
const overwriteNode = async (branch, parentNode, deps) => {
  ...
  if (branch.type === 'job') {
    ...
    try {
      ...
      if (res && res.success)
        return { mode: 'job', mrid, parentId: parentNode.mrid, asset: branch.asset }
      return null
    } catch (err) {
...
const branchMrid = (branch) => {
  ...
  switch (branch.type) {
    case 'job':
      return d.properties && d.properties.mrid
    case 'organisation':
      return d.organisationId || (d.organisation && d.organisation.mrid) || d.mrid
    case 'substation':
      return d.subsId || (d.substation && d.substation.mrid) || d.mrid
    case 'voltageLevel':
      return (d.voltageLevel && d.voltageLevel.mrid) || d.voltageLevelId || d.mrid
    case 'bay':
      return (d.bay && d.bay.mrid) || d.bayId || d.mrid
    // asset types: DTO lồng, mrid ở data.properties.mrid
    default:
      return (d.properties && d.properties.mrid) || d.mrid
  }
}
  ...
  const pass2 = (obj) => {
    ...
    for (const k of Object.keys(obj)) {
      ...
      if (isRef && !ID_KEEP.has(k) && typeof v === 'string' && v) {
        if (idMap[v])
          obj[k] = idMap[v] // tham chiếu nội bộ → trỏ id mới
  ...
  switch (branch.type) {
    ...
    case 'bay': {
      ...
      if (parentNode && parentNode.mode === 'voltageLevel') {
        d.voltage_level = pmrid
        d.substation = null
      } else {
        d.substation = pmrid
        d.voltage_level = null
      }
      break
    }
    case 'job':
...
const importBranch = async (branch, parentNode, deps, decisions) => {
  ...
  if (!newNode) {
    console.warn(
      `%c[BRANCH] ✗ FAILED — skipping subtree: type=${branch && branch.type}`,
...
const isOrgIntoOrg = (roots, targetMode) =>
  targetMode === 'organisation' && (roots || []).some((r) => nodeModeOf(r.type) === 'organisation')
...
export const importBranchFromJSON = async (fileContent, targetNode, deps, decisions = null) => {
  ...
  try {
    ...
    if (fail === 0) {
      messageHandler &&
        messageHandler.success(
          isLegacyFlat
            ? `Imported ${ok} item(s) successfully`
            : `Imported ${ok} branch(es) successfully`
        )
    } else {
    ...
    if (closeLoading) (closeLoading as any)()
  }
}
