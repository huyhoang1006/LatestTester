<template>
  <div id="login" class="login-page" :style="{ backgroundImage: `url(${loginBackground})` }">
    <div class="app-container">
      <div class="sidebar">
        <div class="brand-section">
          <div class="brand-logo no-select">
            <img
              src="@/assets/images/atenergy_logo_dark.png"
      ...
      <div class="main-content">
        ...
        <el-form ref="form" :model="model" :rules="loginRules" @submit.prevent="login">
          ...
          <el-button :loading="loadingLogin" class="submit-btn" type="primary" native-type="submit"
            >Login</el-button
          >
        </el-form>
      </div>
      <div class="mobile-footer no-select">Standard by IEC 61968</div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async login() {
      ...
      const loginAddr = localStorage.getItem('LOGIN_ADDR') || this.$store?.state?.loginAddr || ''
      const serviceAddr = localStorage.getItem('SERVICE_ADDR') || this.$store?.state?.serviceAddr || ''
      if (!loginAddr || !serviceAddr) {
        this.$message.error('Vui lòng nhập Config server address (Login Domain & Service Domain) trước khi đăng nhập')
        return
      }
      try {
        new URL(loginAddr)
        ...
        this.$message.error('Config server address không hợp lệ — phải bắt đầu bằng http:// hoặc https://')
        return
      }
...
:deep(.el-input.is-focus .el-input__prefix .input-icon) {
  color: #ffffff;
}
...
:deep(.remember-checkbox.is-checked .el-checkbox__label) {
  color: #ffffff;
}
