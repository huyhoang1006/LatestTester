...
const nodeMrid = (node) => {
  ...
  switch (node.type) {
    ...
    default:
      return (d.properties && d.properties.mrid) || d.mrid // asset
  }
}
...
const pickName = (node) => {
  const d = node.data || {}
  if (node.type === 'job')
    return (d.properties && (d.properties.name || d.properties.mRID)) || d.name || ''
  // asset: tên ở properties.apparatus_id / serial_no
  if (d.properties)
    return d.properties.apparatus_id || d.properties.serial_no || d.properties.name || d.name || ''
