...
import {
  ...
  collectGraftBranches,
  isOrgIntoOrg,
  nodeModeOf
} from '@/views/Import/services/importJsonService'
...
export default {
  ...
  methods: {
    ...
    async handleImportJSONFromContext(node) {
      const targetNode =
        node || (this.selectedNodes && this.selectedNodes[this.selectedNodes.length - 1])
      if (!targetNode) {
        this.$message.warning('Please select a target node to import into')
        return
      }
      ...
      const walk = (item) => {
        ...
        for (const key of Object.keys(item)) {
          ...
          if (key === 'userId' || key === 'user_id') {
            ...
            hasUserSuffix(current) &&
            getUserSuffix(current) !== String(userId)
          ) {
    ...
    async _pickFileAndImport(targetNode) {
      try {
        ...
        console.log(
          '[IMPORT] targetNode:',
          JSON.stringify({
            mode: targetNode && targetNode.mode,
            mrid: targetNode && targetNode.mrid,
        ...
        console.log(
          '[IMPORT] fileResult.success:',
          fileResult && fileResult.success,
          '| message:',
          fileResult && fileResult.message
        )
        console.log(
          '[IMPORT] typeof fileResult.data:',
          typeof (fileResult && fileResult.data),
          '| isArray:',
          Array.isArray(fileResult && fileResult.data)
        )
    ...
    async handleGraftConfirm() {
      ...
      if (!ctx) return
      const roots = Array.isArray(ctx.fileContent)
        ? ctx.fileContent
        : ctx.fileContent && ctx.fileContent.roots
    ...
    async _proceedImport(content, targetNode, roots) {
      try {
        ...
        console.log(
          '[IMPORT] scan trên',
          branchesToScan.length,
          'nhánh đã ghép (roots gốc:',
          roots.length,
          ')'
        )
        ...
        if (Object.keys(autoDecisions).length) {
          console.log(
            '[IMPORT] auto create-new (khác nhánh):',
            Object.keys(autoDecisions).length,
            Object.keys(autoDecisions)
          )
        }
    ...
    _buildAssetInserters(api, extractMrid, self) {
      const A = {
        voltageTransformer: {
          map: vtDtoToEntity,
          ins: (o, e) => api.insertVoltageTransformerEntity(o, e)
        },
        currentTransformer: {
          map: ctDtoToEntity,
          ins: (o, e) => api.insertCurrentTransformerEntity(o, e)
        },
      ...
      for (const key of Object.keys(A)) {
        ...
        out[key] = async (dto, parent) => {
          ...
          const newMrid =
            (rs && rs.data && rs.data.asset && rs.data.asset.mrid) ||
          ...
          return {
            ...
            mrid: newMrid,
            mode: 'asset',
            message: rs && (rs.message || (rs.error && rs.error.message))
          }
        }
      }
      return out
    ...
    _nullifyEmptyFKs(obj) {
      ...
      if (Array.isArray(obj)) {
        for (const it of obj) this._nullifyEmptyFKs(it)
        return
      }
    ...
    _fillNullMrids(obj) {
      if (!obj || typeof obj !== 'object') return
      if (Array.isArray(obj)) {
        for (const it of obj) this._fillNullMrids(it)
        return
      }
      ...
      const deps = {
        ...
        nodeInserters: {
          organisation: async (dto, parent) => {
            ...
            const newMrid =
              (rs && rs.data && rs.data.organisation && rs.data.organisation.mrid) ||
              (entity.organisation && entity.organisation.mrid) ||
              dto.organisationId
          ...
          substation: async (dto, parent) => {
            ...
            console.log(
              '[INS sub] organisationPsr:',
              JSON.stringify(entity && entity.organisationPsr),
              '| parent=',
              parent.mrid
            )
            ...
            const newMrid =
              (rs && rs.data && rs.data.substation && rs.data.substation.mrid) ||
              (entity.substation && entity.substation.mrid) ||
              dto.subsId
            return {
              success: !!(rs && rs.success),
              mrid: newMrid,
          ...
          voltageLevel: async (dto, parent) => {
            ...
            console.log(
              '%c[INS vl] DTO (parent sub=' + parent.mrid + '):',
              'color:#4CAF50',
            ...
            const newMrid =
              (rs && rs.data && rs.data.voltageLevel && rs.data.voltageLevel.mrid) ||
              (entity.voltageLevel && entity.voltageLevel.mrid) ||
              dto.voltageLevelId
          ...
          bay: async (dto, parent) => {
            ...
            console.log(
              '%c[INS bay] DTO (parent=' + parent.mode + ':' + parent.mrid + '):',
              'color:#4CAF50',
              JSON.parse(JSON.stringify(dto))
            )
        ...
        jobImporters: {
          'Voltage transformer': {
            jobDtoToEntity: VTJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertVoltageTransformerJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertVoltageTransformerJob(e, e)
          },
          'Current transformer': {
            jobDtoToEntity: CTJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertCurrentTransformerJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertCurrentTransformerJob(e, e)
          },
          Transformer: {
            jobDtoToEntity: TFJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertTransformerJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertTransformerJob(e, e)
          },
          'Circuit breaker': {
            jobDtoToEntity: CBJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertCircuitBreakerJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertCircuitBreakerJob(e, e)
          },
          'Surge arrester': {
            jobDtoToEntity: SAJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertSurgeArresterJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertSurgeArresterJob(e, e)
          },
          Disconnector: {
            jobDtoToEntity: DCJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertDisconnectorJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertDisconnectorJob(e, e)
          },
          'Power cable': {
            jobDtoToEntity: PCJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertPowerCableJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertPowerCableJob(e, e)
          },
          'Rotating machine': {
            jobDtoToEntity: RMJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertRotatingMachineJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertRotatingMachineJob(e, e)
          },
          Capacitor: {
            jobDtoToEntity: CAPJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertCapacitorJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertCapacitorJob(e, e)
          },
          Reactor: {
            jobDtoToEntity: REJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertReactorJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertReactorJob(e, e)
          },
          Bushing: {
            jobDtoToEntity: BUJob.jobDtoToEntity,
            insert: (a, o, e) => a.insertBushingJob(self._emptyArraysClone(e), e),
            update: (a, e) => a.insertBushingJob(e, e)
          }
        },
        ...
        onProgress: ({ name, type }) => {
          ...
          if (this._importReporter) {
            ...
            this._importReporter.progress(
              `Importing ${type || 'node'} "${label}"`,
              this._importDone
            )
          }
        },
