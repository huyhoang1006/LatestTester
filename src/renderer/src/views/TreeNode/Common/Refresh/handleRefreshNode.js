      ...
      try {
        ...
        if (node.mode === 'organisation') {
          ...
          if (result.success && result.data) {
            ...
            node.name = updatedData.name || node.name
            node.aliasName = updatedData.aliasName || node.aliasName
            node.geo_x = updatedData.x_position
            node.geo_y = updatedData.y_position
            node.phone_no = updatedData.phoneNumber
            node.email = updatedData.email
          }
...
const result = await window.electronAPI.getSubstationEntityByMrid(node.mrid, userStore.user?.user_id, node.parentId)
          if (result.success && result.data) {
            ...
            // Update node properties
            node.name = updatedData.name || node.name
            node.type = updatedData.type || node.type
            node.generation = updatedData.generation || node.generation
            node.industry = updatedData.industry || node.industry
          }
          ...
          if (result && result.success && result.data) {
            ...
            node.serial_number = updatedData.properties?.serial_no || node.serial_number
            node.apparatus_id = updatedData.properties?.apparatus_id || node.apparatus_id
            node.manufacturer = updatedData.properties?.manufacturer || node.manufacturer
            node.type = updatedData.properties?.type || node.type
          }
        ...
        if (updatedData) {
          node._hasFullProperties = true
          node._cachedEntityData = updatedData
        }
      } catch (error) {
