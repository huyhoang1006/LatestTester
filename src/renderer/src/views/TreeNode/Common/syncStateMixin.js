...
export default {
  ...
  methods: {
    ...
    async applySyncStatesToTree(nodes) {
      ...
      const mrids = flatNodes.map((node) => this.getSyncNodeMrid(node)).filter(Boolean)
      ...
      flatNodes.forEach((node) => {
        ...
        if (state) {
          node.sync_status = state.sync_status
          node.sync_error = state.last_error || ''
          node._serverId = state.server_id || ''
        } else if (DIRTY_STATUSES.includes(node.sync_status)) {
          delete node.sync_status
          delete node.sync_error
        }
      })
    },
    ...
    updateTreeNodeSyncState(nodeOrMrid, status, options = {}) {
      ...
      if (status === 'synced') {
        ...
        if (options.error) node.sync_error = options.error
      }

      if (options.serverId) node._serverId = String(options.serverId)
    },
    ...
    async markSavedExistingResultDirtyIfChanged(result) {
      ...
      const node = this.findNodeByIdOrMrid
        ...
        : null
      const nodeType =
        this.getSyncNodeType(node) || (stateResponse.data && stateResponse.data.node_type) || ''
    ...
    async handleSyncStateChanged(event) {
      ...
      if (detail.existingOnly && window.electronAPI && window.electronAPI.getSyncStateByMrid) {
        ...
        if (!stateResponse || !stateResponse.success || !stateResponse.data) {
          ...
          if (!looksLikeServerId || detail.status !== 'dirty' || !window.electronAPI.markNodeDirty)
            return
