...
export default {
  methods: {
    ...
    async confirmMoveNode() {
      ...
      let sourceName =
        nodeToMove.apparatus_id ||
        nodeToMove.serial_number ||
        nodeToMove.serial_no ||
        nodeToMove.name ||
        'Unknown Item'
        ...
        .then(async () => {
          ...
          try {
            ...
            if (nodeToMove.mode === 'asset') {
              ...
              const targetLocRes = await window.electronAPI.getLocationByPowerSystemResourceMrid(
                newParent.mrid
              )
              ...
              if (assetEntity.success && assetEntity.data) {
                ...
                if (newLocationMrid) {
                  ...
                  const updateAssetRes = await window.electronAPI.updateAssetByMrid(
                    assetData.mrid,
                    assetData
                  )
                  if (!updateAssetRes.success) {
                    console.error(
                      'Lỗi khi cập nhật trường location của Asset:',
                      updateAssetRes.message
                    )
                  }
                }
              }
              ...
              if (nodeToMove.parentId) {
                const searchRes = await window.electronAPI.getAssetPsrByAssetIdAndPsrId(
                  nodeToMove.mrid,
                  nodeToMove.parentId
                )
              ...
              if (currentAssetPsr) {
                ...
                updateResult = await window.electronAPI.updateAssetPsr(
                  currentAssetPsr.mrid,
                  currentAssetPsr
                )
            ...
            // --- XỬ LÝ CÁC LOẠI KHÁC (Giữ nguyên logic cũ của bạn) ---
              ...
              const orgEntity = await window.electronAPI.getOrganisationEntityByMrid(
                nodeToMove.mrid
              )
              if (orgEntity.success) {
                orgEntity.data.organisation.parent_organization = newParent.mrid
                updateResult = await window.electronAPI.updateParentOrganizationByMrid(
                  nodeToMove.mrid,
                  orgEntity.data.organisation
                )
                success = updateResult.success
              }
              ...
              if (subData.success && subData.data.location) {
                ...
                if (locData.success) {
                  ...
                  updateResult = await window.electronAPI.updateLocationByMrid(
                    subData.data.location,
                    locData.data
                  )
                  success = updateResult.success
                }
            ...
            // ... (Voltage, Bay logic giữ nguyên) ...
              ...
              if (vl.success) {
                ...
                updateResult = await window.electronAPI.updateVoltageLevelByMrid(
                  nodeToMove.mrid,
                  vl.data
                )
                success = updateResult.success
            ...
            if (success) {
              // 1. Xóa khỏi cha cũ
              const sourceList = this.clientSlide
              ...
              if (newParentInTree) {
                ...
                newParentInTree._childrenFetched = false
                await this.fetchChildren(newParentInTree)
                newParentInTree.expanded = true
              }
