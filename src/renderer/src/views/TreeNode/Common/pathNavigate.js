  ...
  methods: {
    ...
    async goToPath(side, rawText) {
      ...
      for (let i = 0; i < segments.length; i++) {
        ...
        if (matches.length === 0) {
          ...
          this.$message.error(
            `Path does not exist: "${segments[i]}" not found under ${parentLabel}`
          )
          return false
        }
        if (matches.length > 1) {
          ...
          this.$message.error(
            `Path is ambiguous: ${matches.length} nodes named "${segments[i]}" under ${parentLabel(current)}`
          )
          return false
        }
    ...
    async revealPathTarget(side, node) {
      ...
      const showProperties =
        side === 'server' ? this.showPropertiesData : this.showPropertiesDataClient
      if (typeof showProperties === 'function') {
        try {
          await showProperties.call(this, node)
        } catch (error) {
