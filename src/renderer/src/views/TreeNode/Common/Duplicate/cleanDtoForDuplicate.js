export default {
  methods: {
    cleanDtoForDuplicate(dto) {
      ...
      if (
        dto.properties &&
        (dto.properties.kind === 'Circuit breaker' || dto.asset === 'Circuit breaker')
      ) {
        ...
        if (dto.operating) {
          if (Array.isArray(dto.operating.trip_coil_component)) {
            dto.operating.trip_coil_component.forEach((item) => {
              item.mrid = null
            })
          }
          if (Array.isArray(dto.operating.close_coil_component)) {
            dto.operating.close_coil_component.forEach((item) => {
              item.mrid = null
            })
          }
        }
      ...
      if (dto.operatingMechanismProductAssetModelId)
        dto.operatingMechanismProductAssetModelId = null
