            ...
            if ((!nodeName || nodeName.toString().trim() === '') && node.mode === 'asset' && node.mrid) {
                try {
                    ...
                    if (node.asset === 'Surge arrester') {
                        ...
                        entityRes = await window.electronAPI.getTransformerEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Circuit breaker') {
                        entityRes = await window.electronAPI.getBreakerEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Disconnector') {
                        entityRes = await window.electronAPI.getDisconnectorEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                        ...
                        entityRes = await window.electronAPI.getRotatingMachineEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Voltage transformer') {
                        entityRes = await window.electronAPI.getVoltageTransformerEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Current transformer') {
                        entityRes = await window.electronAPI.getCurrentTransformerEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Capacitor') {
                        entityRes = await window.electronAPI.getCapacitorEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Reactor') {
                        entityRes = await window.electronAPI.getReactorEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
                            nodeName = getNameFromEntity(entityRes.data.asset)
                        }
                    } else if (node.asset === 'Bushing') {
                        entityRes = await window.electronAPI.getBushingEntityByMrid(node.mrid, this.$userId(), node.parentId)
                        if (entityRes.success && entityRes.data) {
