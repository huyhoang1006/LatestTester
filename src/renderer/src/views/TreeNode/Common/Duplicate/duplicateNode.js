      ...
      if ((!nodeName || nodeName.toString().trim() === '') && node.mode === 'asset' && node.mrid) {
        try {
          ...
          if (node.asset === 'Surge arrester') {
            ...
            entityRes = await window.electronAPI.getBreakerEntityByMrid(
              node.mrid,
              this.$userId(),
              node.parentId
            )
            ...
            entityRes = await window.electronAPI.getReactorEntityByMrid(
              node.mrid,
              this.$userId(),
              node.parentId
            )
            ...
            entityRes = await window.electronAPI.getBushingEntityByMrid(
              node.mrid,
              this.$userId(),
              node.parentId
            )
