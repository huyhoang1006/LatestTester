  ...
  methods: {
    async processDuplicateAsset(node, apiGetEntity, mappingFunction, mixinObject, dataPropName) {
      try {
        ...
        const entityRes = await apiGetEntity(node.mrid, this.$userId(), node.parentId)
          ...
          impedanceArrays.forEach((key) => {
            if (Array.isArray(dto.impedances[key])) {
              dto.impedances[key].forEach((item) => {
                ...
                if (item.short_circuit_impedances_uk)
                  item.short_circuit_impedances_uk.mrid = this.generateUuid()
        ...
        if (dto.shortCircuitTestTransformerEndInfo) {
          dto.shortCircuitTestTransformerEndInfo.forEach(
            (info) => (info.mrid = this.generateUuid())
          )
        }
        ...
        if (!targetLocationId && ['substation', 'organisation', 'root'].includes(parentNode.mode)) {
          try {
            const locRes = await window.electronAPI.getLocationByPowerSystemResourceMrid(
              parentNode.mrid
            )
        ...
        Object.keys(context).forEach((key) => {
          if (
            ...
            !vuePrototypeProps.includes(key) &&
            !key.startsWith('$')
          ) {
        ...
        if (saveResult && saveResult.success) {
          ...
          if (resData) {
            ...
            if (mainObj) {
              ...
              if (node.mode === 'asset') {
                newNodeData.name =
                  dto.properties?.apparatus_id || mainObj.apparatus_id || mainObj.name || nextLabel
              ...
              if (node.mode === 'asset') {
                newNodeData.name =
                  dto.properties?.apparatus_id || resData.apparatus_id || resData.name || nextLabel
