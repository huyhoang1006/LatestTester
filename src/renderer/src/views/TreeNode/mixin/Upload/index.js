  ...
  methods: {
    async handleUploadNode() {
      ...
      if (node.mode === 'substation') {
        ...
        this.$confirm(
          `Upload Power Cable "${node.serial_number || node.name}" to Server?`,
          'Confirm Upload',
          {
            confirmButtonText: 'Upload',
            cancelButtonText: 'Cancel',
        ...
        this.$message.warning(
          'Currently only "Substation", "VoltageLevel" and "Power cable" upload are supported.'
        )
      }
    },
    ...
    async searchOrganisationDeep(parentId, targetMrid, maxDepth = 5, currentDepth = 0) {
      ...
      try {
        ...
        for (const child of children) {
          if (child.id) {
            const result = await this.searchOrganisationDeep(
              child.id,
              targetMrid,
              maxDepth,
              currentDepth + 1
    ...
    async findBayNumericIdGlobal(bayMrid) {
      try {
        ...
        const orgId =
          Array.isArray(orgRes) && orgRes.length > 0 ? orgRes[0].id || orgRes[0].mrid : orgRes?.id
    ...
    async processUploadVoltageLevel(node) {
      try {
        ...
        if (!numericOwnerId) {
          throw new Error(
            `Cannot find parent Substation on Server. Make sure Substation is uploaded first.`
          )
        }
    ...
    async processUploadBay(node) {
      try {
        ...
        if (!numericOwnerId || !ownerType) {
          throw new Error(
            `Cannot find Numeric ID for parent (UUID: ${node.parentId}) on Server. Make sure parent is uploaded first.`
          )
        }
