        ...
        try {
          ...
          if (node.mode == 'asset') {
            ...
            if (assetDisconnectorReturn.success && assetDisconnectorReturn.data) {
              const dataArray = Array.isArray(assetDisconnectorReturn.data)
                ? assetDisconnectorReturn.data
                : [assetDisconnectorReturn.data]
            ...
            const [organisationReturn, substationReturn] = await Promise.all([
              ...
              window.electronAPI.getSubstationsInOrganisationForUser(
                clickedRow.mrid,
                this.$userId()
              )
            ])
            ...
            if (
              organisationReturn.success &&
              organisationReturn.data &&
              organisationReturn.data.length > 0
            ) {
            ...
            if (
              substationReturn.success &&
              substationReturn.data &&
              substationReturn.data.length > 0
            ) {
          ...
          if (existingChildren.length > 0) {
            ...
            node.children = [...existingChildren, ...newChildrenToAdd]
          } else {
            node.children = newRows
          }
          // Đánh dấu đã fetch để tránh fetch lại không cần thiết
          node._childrenFetched = true
        } catch (error) {
