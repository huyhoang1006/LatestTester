  ...
  methods: {
    ...
    async processUploadSubstation(node) {
      try {
        ...
        const entityRes = await window.electronAPI.getSubstationEntityByMrid(
          node.mrid,
          userId,
          node.parentId
        )
    ...
    async processUploadVoltageLevel(node) {
      try {
        ...
        if (parentNode?.mode !== 'substation') {
          throw new Error(
            `VoltageLevel parent must be Substation. Current parent mode: ${parentNode?.mode}`
          )
        }
    ...
    async processUploadBay(node) {
      try {
        ...
        if (!signParent) {
          ...
          if (!ownerType) {
            throw new Error(
              `Cannot resolve ownerType for parent node with mode: ${parentNode?.mode}`
            )
          } else {
    ...
    async processUploadPowerCable(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Local power cable data not found.')
    ...
    async processUploadVoltageTransformer(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Voltage transformer data not found.')
        ...
        const response = await voltageAPI.createVoltageTransformer(
          serverPayload,
          node.parentId,
          ownerType
        )
        ...
        this.$message.success(
          `Upload Voltage Transformer "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadCurrentTransformer(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Current transformer data not found.')
        ...
        const response = await currentAPI.createCurrentTransformer(
          serverPayload,
          node.parentId,
          ownerType
        )
        ...
        this.$message.success(
          `Upload Current Transformer "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadCircuitBreaker(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Circuit breaker data not found.')
        ...
        const response = await circuitBreakerAPI.createCircuitBreaker(
          serverPayload,
          node.parentId,
          ownerType
        )
        ...
        this.$message.success(
          `Upload Circuit Breaker "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadDisconnector(node) {
      try {
        const entityRes = await window.electronAPI.getDisconnectorEntityByMrid(
          node.mrid,
          node.parentId
        )
    ...
    async processUploadSurgeArrester(node) {
      try {
        ...
        const response = await surgeArresterAPI.createSurgeArrester(
          serverPayload,
          node.parentId,
          ownerType
        )
        ...
        this.$message.success(
          `Upload Surge Arrester "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadVoltageTransformerJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Voltage transformer job data not found.')
        ...
        const response = await voltageTransformerJobAPI.createVoltageTransformerJob(
          serverPayload,
          node.parentId
        )
        ...
        this.$message.success(
          `Upload Voltage Transformer Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadCurrentTransformerJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Current transformer job data not found.')
        ...
        await this.attachJobBaseVersion(node, serverPayload)
        const response = await currentTransformerJobAPI.createCurrentTransformerJob(
          serverPayload,
          node.parentId
        )
        ...
        await this.saveJobBaseAfterUpload(node, dto, response)
        this.$message.success(
          `Upload Current Transformer Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadCircuitBreakerJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Circuit breaker job data not found.')
        ...
        await this.attachJobBaseVersion(node, serverPayload)
        const response = await circuitBreakerJobAPI.createCircuitBreakerJob(
          serverPayload,
          node.parentId
        )
        ...
        await this.saveJobBaseAfterUpload(node, dto, response)
        this.$message.success(
          `Upload Circuit Breaker Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadSurgeArresterJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Surge arrester job data not found.')
        ...
        await this.attachJobBaseVersion(node, serverPayload)
        const response = await surgeArresterJobAPI.createSurgeArresterJob(
          serverPayload,
          node.parentId
        )
        ...
        await this.saveJobBaseAfterUpload(node, dto, response)
        this.$message.success(
          `Upload Surge Arrester Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadDisconnectorJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Disconnector job data not found.')
        ...
        await this.attachJobBaseVersion(node, serverPayload)
        const response = await disconnectorJobAPI.createDisconnectorJob(
          serverPayload,
          node.parentId
        )
        ...
        await this.saveJobBaseAfterUpload(node, dto, response)
        this.$message.success(
          `Upload Disconnector Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    async processUploadTransformerJob(node) {
      try {
        ...
        if (!entityRes.success || !entityRes.data)
          throw new Error('Transformer job data not found.')
        ...
        await this.saveJobBaseAfterUpload(node, dto, response)
        this.$message.success(
          `Upload Transformer Job "${this.getUploadNodeName(node)}" successfully!`
        )
        return response
      } catch (error) {
    ...
    findUploadParentNode(node) {
      ...
      return (
        this.findNodeByIdOrMrid(node.parentId, this.organisationClientList) ||
    ...
    async saveAssetComponentToServer(component, assetType, serverTab = null) {
      ...
      const response = await this.createAssetDtoOnServer(
        assetType,
        dto,
        parentId,
        ownerType,
    ...
    applyServerTabIdToPayload(payload, serverTab) {
      ...
      if (payload.assetInfoResponseDTO)
        payload.assetInfoResponseDTO.id = payload.assetInfoResponseDTO.id || null
    ...
    async createAssetDtoOnServer(assetType, dto, parentId, ownerType, serverTab = null) {
      if (assetType === 'Transformer') {
        const response = await transformerAPI.createTransformer(
          this.applyServerTabIdToPayload(
            transformerMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          )
        )
        await this.uploadAssetMedia(
          assetType,
          dto,
          this.extractUploadedServerId(response) ||
            serverTab?.id ||
            serverTab?.mrid ||
            dto?.properties?.mrid
        )
      ...
      if (assetType === 'Bushing') {
        const response = await bushingAPI.createBushing(
          this.applyServerTabIdToPayload(
            bushingMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          )
        )
        await this.uploadAssetMedia(
      ...
      if (assetType === 'Surge arrester') {
        const response = await surgeArresterAPI.createSurgeArrester(
          this.applyServerTabIdToPayload(
            surgeArresterMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          ),
          parentId,
          ownerType
      ...
      if (assetType === 'Circuit breaker') {
        const response = await circuitBreakerAPI.createCircuitBreaker(
          this.applyServerTabIdToPayload(
            circuitBreakerMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          ),
          parentId,
          ownerType
      ...
      if (assetType === 'Current transformer') {
        const response = await currentAPI.createCurrentTransformer(
          this.applyServerTabIdToPayload(
            currentTransformerMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          ),
          parentId,
          ownerType
      ...
      if (assetType === 'Voltage transformer') {
        const response = await voltageAPI.createVoltageTransformer(
          this.applyServerTabIdToPayload(
            voltageTransformerMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          ),
          parentId,
          ownerType
      ...
      if (assetType === 'Power cable') {
        const response = await powerCableAPI.createPowerCable(
          this.applyServerTabIdToPayload(
            PowerCableServerMapper.mapDtoToServer(dto, ownerType),
            serverTab
          )
        )
        await this.uploadAssetMedia(
      ...
      if (assetType === 'Disconnector') {
        const response = await disconnectorAPI.createDisconnector(
          this.applyServerTabIdToPayload(
            disconnectorMappingServer.mapDtoToServer(dto, ownerType),
            serverTab
          )
        )
        await this.uploadAssetMedia(
    ...
    async uploadAssetMedia(assetType, dto, serverAssetId) {
      ...
      try {
        return await uploadAssetMediaFromAttachmentData(
          assetType,
          serverAssetId,
          attachmentData,
    ...
    async syncUploadedNodeServerId(node, response) {
      ...
      if (!oldId || oldId === newId) {
        ...
        if (window.electronAPI && window.electronAPI.markNodeSynced) {
          await window.electronAPI.markNodeSynced(
            newId,
            this.getSyncNodeType ? this.getSyncNodeType(node) : node.mode || '',
            toServerId(newId)
          )
        }
