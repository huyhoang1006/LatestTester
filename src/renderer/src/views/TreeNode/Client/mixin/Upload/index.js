  ...
  methods: {
    ...
    async runUploadWithLog(node, handler) {
      ...
      this.$message = {
        ...
        warning: (m) => {
          captured = captured || m
          originalMessage.warning(m)
        },
        error: (m) => {
          captured = captured || m
          originalMessage.error(m)
        }
      }
    ...
    async handleUploadNode() {
      ...
      const confirmAndRun = async (label, handler) => {
        ...
        this.$confirm(
          `Upload ${label} "${node.name || node.serial_number}" to Server?`,
          'Confirm Upload',
          {
          ...
          .then(() => this.runUploadWithLog(node, handler))
          .catch(() => {})
      }
      ...
      if (node.mode === 'substation') {
        ...
        this.$confirm(
          ...
          .then(() => this.runUploadWithLog(node, this.processUploadPowerCable))
          .catch(() => {})
    ...
    async executeUploadFullTree() {
      ...
      if (failed) {
        this.$message.warning(
          `${this.uploadFullTreeTitle}: ${results.length - failed}/${results.length} uploaded, ${failed} failed`
        )
      } else {
