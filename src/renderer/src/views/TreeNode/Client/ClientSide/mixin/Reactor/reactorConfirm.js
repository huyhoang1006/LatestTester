...
export default {
  methods: {
    async handleReactorConfirm() {
      ...
      const licenseCheck = await window.electronAPI.checkLicense('Reactor')
        ...
        if (reactor) {
          ...
          if (success) {
            ...
            if (this.isEditMode) {
              ...
              try {
                const entityRes = await window.electronAPI.getReactorEntityByMrid(
                  assetData.mrid,
                  data.assetPsr?.mrid
                )
                if (entityRes.success && entityRes.data) {
                  const dto = ReactorMapping.reactorEntityToDto(entityRes.data)
                  const dialogRef = this.$refs.reactorDialog
