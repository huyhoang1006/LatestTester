      ...
      if (node.mode != 'job') {
        ...
        try {
          if (node.mode == 'substation') {
            const entity = await window.electronAPI.getSubstationEntityByMrid(
              node.mrid,
              this.$userId(),
              node.parentId
            )
            ...
            if (node.asset === 'Surge arrester' || node.asset === 'Surge Arrester') {
              ...
              const entity = await window.electronAPI.getPowerCableEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              if (!deleteSign.success) {
                this.$message.error(
                  'Delete data failed: ' + (deleteSign.message || 'Unknown error')
                )
                return
              }
              // ✅ Xóa node khỏi cây organisationClientList
              ...
              const entity = await window.electronAPI.getDisconnectorEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getRotatingMachineEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getCapacitorEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getReactorEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getVoltageTransformerEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const deleteSign = await window.electronAPI.deleteVoltageTransformerEntity(
                entity.data
              )
              if (!deleteSign.success) {
                this.$message.error(
              ...
              const entity = await window.electronAPI.getCurrentTransformerEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const deleteSign = await window.electronAPI.deleteCurrentTransformerEntity(
                entity.data
              )
              if (!deleteSign.success) {
                this.$message.error(
              ...
              const entity = await window.electronAPI.getBushingEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getBreakerEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
              ...
              const entity = await window.electronAPI.getTransformerEntityByMrid(
                node.mrid,
                node.parentId
              )
              if (!entity.success) {
        ...
        try {
          if (node.job == 'Surge arrester') {
            ...
            const deleteSign = await window.electronAPI.deleteVoltageTransformerJobByMrid(
              entity.data
            )
            if (!deleteSign.success) {
              this.$message.error('Delete data failed')
            ...
            const deleteSign = await window.electronAPI.deleteCurrentTransformerJobByMrid(
              entity.data
            )
            if (!deleteSign.success) {
              this.$message.error('Delete data failed')
