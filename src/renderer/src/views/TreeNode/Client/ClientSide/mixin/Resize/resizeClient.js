export default {
  methods: {
    ...
    resizeClient(event) {
      ...
      const contextDataClient = this.getContextDataClientRef
        ? this.getContextDataClientRef()
        : this.$refs.contextDataClient
    ...
    resizeContentClient(event) {
      const propertiesClient = this.getPropertiesClientRef
        ? this.getPropertiesClientRef()
        : this.$refs.propertiesClient
      const contentDataClient = this.getContentDataClientRef
        ? this.getContentDataClientRef()
        : this.$refs.contentDataClient
      const contextDataClient = this.getContextDataClientRef
        ? this.getContextDataClientRef()
        : this.$refs.contextDataClient
      const contentClient = this.getContentClientRef
        ? this.getContentClientRef()
        : this.$refs.contentClient
