      ...
      if (node.asset != undefined) {
        ...
        if (node._cachedEntityData) {
          ...
          assetDetailData = {
            ...
            manufacturer_year:
              node._cachedEntityData.properties?.manufacturer_year ||
              node._cachedEntityData.properties?.manufacturing_year,
        ...
        const parentNode = node.parentId
          ? this.findNodeById(node.parentId, this.organisationClientList)
          : null
        if (parentNode) {
          ...
          if (parentNode.mode === 'substation') {
            ...
            if (parentNode._cachedEntityData) {
              ...
              try {
                ...
                // @ts-ignore
                const res = await window.electronAPI.getSubstationEntityByMrid(
                  parentNode.mrid,
                  this.$userId(),
                  parentNode.parentId
                )
          ...
          // Nếu parent là Bay hoặc VoltageLevel, lấy thông tin từ grandparent (Substation)
            ...
            const grandParentNode = parentNode.parentId
              ? this.findNodeById(parentNode.parentId, this.organisationClientList)
              : null
            if (grandParentNode && grandParentNode.mode === 'substation') {
              ...
              if (grandParentNode._cachedEntityData) {
                ...
                try {
                  // @ts-ignore
                  const res = await window.electronAPI.getSubstationEntityByMrid(
                    grandParentNode.mrid,
                    this.$userId(),
                    grandParentNode.parentId
                  )
                  if (res.success && res.data) {
                    parentDetailData = SubstationMapping.mapEntityToDto(res.data)
                    // Cache kết quả vào node để lần sau không cần gọi API
        ...
        const assetName =
          node.apparatus_id || node.serial_number || node.serial_no || node.name || 'Unknown'
        ...
        const jobNode = node.parentId
          ? this.findNodeById(node.parentId, this.organisationClientList)
          : null
        const assetNode = jobNode
          ? jobNode.parentId
            ? this.findNodeById(jobNode.parentId, this.organisationClientList)
            : null
          : null
        const locationNode = assetNode
          ? assetNode.parentId
            ? this.findNodeById(assetNode.parentId, this.organisationClientList)
            : null
          : null
        if (locationNode) {
