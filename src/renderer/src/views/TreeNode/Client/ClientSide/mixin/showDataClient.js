...
export default {
    methods: {
        ...
        async showDataClient(node) {
            console.log('[showDataClient] called with node:', node);
            try {
            ...
            if (node.asset != undefined) {
                    ...
                    if (parentNode.mode === 'substation') {
                        ...
                        if (parentNode._cachedEntityData) {
                            ...
                            try {
                                ...
                                // @ts-ignore
                                const res = await window.electronAPI.getSubstationEntityByMrid(parentNode.mrid, this.$userId(), parentNode.parentId)
                    ...
                    // Nếu parent là Bay hoặc VoltageLevel, lấy thông tin từ grandparent (Substation)
                        ...
                        if (grandParentNode && grandParentNode.mode === 'substation') {
                            ...
                            if (grandParentNode._cachedEntityData) {
                                ...
                                try {
                                    // @ts-ignore
                                    const res = await window.electronAPI.getSubstationEntityByMrid(grandParentNode.mrid, this.$userId(), grandParentNode.parentId)
                                    if (res.success && res.data) {
                                        parentDetailData = SubstationMapping.mapEntityToDto(res.data)
                                        // Cache kết quả vào node để lần sau không cần gọi API
                ...
                if (node.mode === 'substation') {
                    ...
                    if (node._cachedEntityData) {
                        ...
                        try {
                            // @ts-ignore
                            const res = await window.electronAPI.getSubstationEntityByMrid(node.mrid, this.$userId(), node.parentId)
                            if (res.success && res.data) {
