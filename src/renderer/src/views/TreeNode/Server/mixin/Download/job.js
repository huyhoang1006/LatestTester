...
async function saveJobChain(assetType, data, ctx) {
  ...
  const userId =
    ctx && ctx.$store && ctx.$store.state && ctx.$store.state.user
      ? ctx.$store.state.user.user_id
      : null
  const serverDto = normalizeJobDto(
    entry.toDto(jobRef._serverData),
    entry.linkKey,
    assetType,
    userId
  )
...
export const getVoltageTransformerJobChain = (id, parentId) =>
  fetchJobChain('Voltage transformer', id, parentId)
export const getCurrentTransformerJobChain = (id, parentId) =>
  fetchJobChain('Current transformer', id, parentId)
export const getCircuitBreakerJobChain = (id, parentId) =>
  fetchJobChain('Circuit breaker', id, parentId)
export const getDisconnectorJobChain = (id, parentId) => fetchJobChain('Disconnector', id, parentId)
export const getSurgeArresterJobChain = (id, parentId) =>
  fetchJobChain('Surge arrester', id, parentId)

export const downloadTransformerJobChain = (data, ctx) => saveJobChain('Transformer', data, ctx)
export const downloadVoltageTransformerJobChain = (data, ctx) =>
  saveJobChain('Voltage transformer', data, ctx)
export const downloadCurrentTransformerJobChain = (data, ctx) =>
  saveJobChain('Current transformer', data, ctx)
export const downloadCircuitBreakerJobChain = (data, ctx) =>
  saveJobChain('Circuit breaker', data, ctx)
export const downloadDisconnectorJobChain = (data, ctx) => saveJobChain('Disconnector', data, ctx)
export const downloadSurgeArresterJobChain = (data, ctx) =>
  saveJobChain('Surge arrester', data, ctx)
