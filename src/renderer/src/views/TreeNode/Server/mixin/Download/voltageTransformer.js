...
import {
  ...
  VOLTAGE_TRANSFORMER_FIELD_DEFS
...
export async function downloadVoltageTransformerChain(data, ctx) {
  ...
  if (!clientDto) {
    ...
    if (!baseDto) {
      ...
      const diffFields = detectConflicts(
        baseDto,
        clientDto,
        serverDto,
        VOLTAGE_TRANSFORMER_FIELD_DEFS
  ...
  const insertResult = await window.electronAPI.insertVoltageTransformerEntity(
    oldEntity,
    newEntity,
    serverDto
  )
  if (!insertResult.success)
    throw new Error(`Database Insert VoltageTransformer Error: ${insertResult.message}`)
}
