...
export async function downloadBushingChain(data, ctx) {
  ...
  const existingResult = await window.electronAPI.getBushingEntityByMrid(bu.mrid, data.parentBayId)
  const clientEntity = existingResult.success ? existingResult.data : null
  const clientDto = clientEntity ? BushingMapper.mapEntityToDto(clientEntity) : null
  ...
  const insertResult = await fetchWithRetry(() => window.electronAPI.insertBushingEntity(newEntity))
