/* eslint-disable */
import { getTransformerById } from '@/api/demo/index.js'
...
import TransformerEntity from '@/views/Flatten/Transformer/index.js'
import { fetchWithRetry } from './core-utils.js'
import {
  detectConflicts,
  applyResolved,
  mergeWithoutSnapshot,
  TRANSFORMER_FIELD_DEFS
...
export async function getTransformerChain(id, parentId) {
  try {
    const data = await fetchWithRetry(() => getTransformerById(id))
    return {
      transformer: {
        id: id,
        mrid: String(id),
...
export async function downloadTransformerChain(data, ctx) {
  ...
  if (!clientDto) {
    ...
    mergedDto.oldPowerTransformerInfoId =
      clientDto.oldPowerTransformerInfoId || serverDto.oldPowerTransformerInfoId
  ...
  if (!insertResult.success)
    throw new Error(`Database Insert Transformer Error: ${insertResult.message}`)
}
