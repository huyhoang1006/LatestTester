...
export async function processVoltageLevelDownload(node, ctx) {
  ...
  loadingStore.text = 'Đang tải dữ liệu voltage level...'

  const existingSub = ctx.findNodeById(node.parentId, ctx.organisationClientList)
  if (!existingSub) {
    const parentSubNode = node.parentArr?.find((p) => p.mode === 'substation') || {
      mrid: node.parentId,
      id: node.parentId,
      parentId: node.parentArr?.find((p) => p.mode === 'organisation')?.mrid,
...
async function prepareVoltageLevel(node) {
  ...
  try {
    ...
    throw new Error('Lỗi fetch API VoltageLevel')
  }
