...
import {
  ...
  CURRENT_TRANSFORMER_FIELD_DEFS
...
export async function downloadCurrentTransformerChain(data, ctx) {
  ...
  const insertResult = await window.electronAPI.insertCurrentTransformerEntity(
    oldEntity,
    newEntity,
    serverDto
  )
  if (!insertResult.success)
    throw new Error(`Database Insert CurrentTransformer Error: ${insertResult.message}`)
}
