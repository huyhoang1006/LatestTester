  ...
  >
    ...
    <div class="field-groups">
      <div v-for="group in fieldGroups" :key="group.label" class="field-group">
        ...
        <div v-for="field in group.fields" :key="field.key" class="field-row" :class="field.status">
          ...
          <template v-else-if="field.status === 'auto'">
            <div class="diff-cols">
              ...
              <div
                ...
                :class="{
                  changed: field.autoResolved === 'client' || field.autoResolved === 'both'
                }"
              >
              ...
              <div
                ...
                :class="{
                  changed: field.autoResolved === 'server' || field.autoResolved === 'both'
                }"
              >
    ...
    <template #footer>
      <span class="dialog-footer">
        ...
        <el-button type="primary" :disabled="hasUnresolved" @click="onConfirm">
          Confirm merge
        </el-button>
      </span>
    </template>
...
export default {
  ...
  computed: {
    ...
    autoCount() {
      return this.sourceFields.filter((f) => f.status === 'auto').length
    },
    conflictCount() {
      return this.sourceFields.filter((f) => f.status === 'conflict').length
    },
    unchangedCount() {
      return this.sourceFields.filter((f) => f.status === 'unchanged').length
    },
    unresolvedCount() {
      return this.sourceFields.filter(
        (f) => f.status === 'conflict' && (f.resolved === null || f.resolved === undefined)
      ).length
    },
    hasUnresolved() {
...
.diff-cell.changed.client {
  background: #ecf5ff;
  color: #409eff;
  font-weight: 500;
}
.diff-cell.changed.server {
  background: #fdf6ec;
  color: #e6a23c;
  font-weight: 500;
}
.diff-cell.conflict-val.client {
  background: #ecf5ff;
  color: #409eff;
}
.diff-cell.conflict-val.server {
  background: #fdf6ec;
  color: #e6a23c;
}
