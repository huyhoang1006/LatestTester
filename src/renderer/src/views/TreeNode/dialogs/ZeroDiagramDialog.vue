    ...
    >
      <main class="modern-workspace">
        ...
        <section v-if="directSubstationAssets.length > 0" class="modern-card direct-assets-card">
          <div class="card-label"><i class="fa-solid fa-microchip"></i> assets</div>
              ...
              >
                <div class="eq-content-wrapper">
                  <div class="eq-icon-container">
                    <component
                      :is="getIconComponent(eq.type)"
                      :iconName="getIconName(eq.type, eq.transformerType)"
                    />
                  </div>
                ...
                <div class="eq-floating-actions">
                  <i
                    ...
                    title="Edit"
                    @click.stop="handleAction('edit', eq)"
                  ></i>
                  <i
                    class="fa-solid fa-trash"
                    title="Delete"
                    @click.stop="handleAction('delete', eq)"
                  ></i>
                </div>
              </div>
            </el-tooltip>
        ...
        <div
          v-if="voltageLevels.length === 0 && directSubstationAssets.length === 0 && !loading"
          class="empty-state"
        >
...
export default {
  ...
  methods: {
    ...
    async fetchSubstationData() {
      ...
      try {
        if (this.isServer) {
          ...
          const directBays = Array.isArray(directBaysRes)
            ? directBaysRes
            : directBaysRes?.data || []
          ...
          const directBays =
            directBayRes.success && Array.isArray(directBayRes.data) ? directBayRes.data : []
        ...
        for (let index = 0; index < results.length; index++) {
          ...
          if (res?.success && res?.data) {
            ...
            const mappedAssetsPromises = dataArray.map(async (item) => {
              ...
              if (!transformerType && typeStr === 'Transformer' && item.mrid) {
                try {
                  ...
                  if (fullAssetRes?.success && fullAssetRes.data) {
                    displayName =
                      fullAssetRes.data.properties?.apparatus_id || fullAssetRes.data.name
                    transformerType =
                      fullAssetRes.data.properties?.transformer_type ||
              ...
              if (!displayName)
                displayName = item.apparatus_id || item.serial_number || item.serial_no || typeStr
    ...
    removeItemFromLocalState(eq) {
      ...
      const dIndex = this.directSubstationAssets.findIndex(
        (item) => (item.mrid && item.mrid === eq.mrid) || (item.id && item.id === eq.id)
      )
      if (dIndex !== -1) {
      ...
      for (const vl of this.voltageLevels) {
        if (vl.bays) {
          for (const bay of vl.bays) {
            if (bay.equipments) {
              const eIndex = bay.equipments.findIndex(
                (item) => (item.mrid && item.mrid === eq.mrid) || (item.id && item.id === eq.id)
              )
              if (eIndex !== -1) {
