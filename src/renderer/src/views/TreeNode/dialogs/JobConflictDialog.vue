  ...
  >
    <div class="summary">
      ...
      <span>
        ...
        {{ conflicts.length === 1 ? 'change needs' : 'changes need' }} your decision — everything
        else was merged automatically.
      </span>
    </div>

    <div class="bulk">
      <span class="bulk-label">Apply to all:</span>
      <el-button size="small" @click="chooseAll('client')">Keep mine</el-button>
      <el-button size="small" @click="chooseAll('server')">Take theirs</el-button>
    </div>
    ...
    <div class="groups">
      <div v-for="(group, gi) in groups" :key="gi" class="group">
        ...
        <div v-for="(item, ii) in group.items" :key="item.key" class="row">
          ...
          <div class="base" title="Value at last sync">was {{ show(item.base) }}</div>
        </div>
      </div>
    </div>
  ...
  computed: {
    groups() {
      ...
      for (const item of this.conflicts) {
        const title =
          item.breadcrumb && item.breadcrumb.length ? item.breadcrumb.join(' › ') : 'Job'
