<template>
  <el-dialog
    ...
    width="780px"
    class="app-dialog import-conflict-dialog"
  ...
  >
    <div class="conflict-body">
      ...
      <div class="apply-all-bar">
        ...
        <el-radio-group v-model="applyAll" size="small" @change="handleApplyAll">
          ...
          <div class="conflict-info">
            ...
            <div class="conflict-path">
              ...
              <span class="path-text">{{
                c.existsAt.pathText || c.existsAt.name || '(unknown location)'
              }}</span>
            </div>
            <div class="conflict-mrid">mRID: {{ c.mrid }}</div>
          </div>
          <div class="conflict-actions">
            <el-radio-group v-model="c.action" size="small">
            ...
            <div v-if="c.action === ACTION.USE_EXISTING" class="use-existing-warning">
              ...
              Using the existing node may cause issues — other data may reference it, or it may
              reference elsewhere. Its children will still be asked.
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    iconOf(type) {
      return TYPE_ICON[type] || 'fa-solid fa-circle'
    },
    handleApplyAll(val) {
      this.rows.forEach((r) => {
        r.action = val
...
:global(.conflict-actions .el-radio) {
  display: block;
  margin: 0 0 2px 0;
  font-size: 12px;
}
