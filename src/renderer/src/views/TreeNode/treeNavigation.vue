<template>
  <div class="explorer">
    <div v-show="clientSlide" class="toolbar">
      <TopBarClient
        v-model:pathMapClient="pathMapClient"
    ...
    <div class="resizable-sidebar" :class="{ 'full-height': activeWorkspaceTab !== 'tree' }">
      ...
      <TreeToolbar
        ...
        @upload="handleUploadNode"
        @download="handleDownloadNode"
        @delete="handleDeleteNode"
        @fmeca="handleClickFmeca"
        @move="handleMoveNode"
      ...
      <ClientTreePanel
        ref="clientPanel"
        v-show="activeWorkspaceTab === 'tree' && clientSlide"
        :organisationClientList="organisationClientList"
        v-model:selectedNodes="selectedNodes"
        @showLocationRoot="showLocationRoot"
        @show-addSubs="showAddSubs"
        ...
        @download="handleDownloadFromContext"
        @upload="handleUploadFromContext"
        @fmeca="handleFmecaFromContext"
        @show-equipment="handleShowEquipmentFromContext"
      />

      <ServerTreePanel
        ref="serverPanel"
        v-show="activeWorkspaceTab === 'tree' && !clientSlide"
        :ownerServerList="ownerServerList"
        v-model:selectedNodes="selectedNodes"
        @showOwnerServerRoot="showOwnerServerRoot"
        @fetch-children-server="fetchChildrenServer"
        @double-click-node-server="doubleClickNodeServer"
      ...
      <div
        @mousedown="startResizeClient"
        v-if="activeWorkspaceTab === 'tree' && clientSlide"
        ref="resizerClient"
        class="resizer"
      ></div>
      <div
      ...
      >
        ...
        <div ref="logBar" v-if="logSign" class="log-bar">
          <LogBar
            ...
            @reloadLog="reloadLogServer"
            @hideLogBar="hideLogBar"
          ></LogBar>
        </div>
      </div>
      <ContextDataClient
        v-show="activeWorkspaceTab === 'tree' && clientSlide"
        ref="contextDataClient"
        :activeTabClient="activeTabClient"
        :tabsClient="tabsClient"
        v-model:propertiesSignClient="propertiesSignClient"
        :propertiesClient="propertiesClient"
        :assetPropertySignClient="assetPropertySignClient"
        :assetPropertiesClient="assetPropertiesClient"
        :jobPropertySignClient="jobPropertySignClient"
        :jobPropertiesClient="jobPropertiesClient"
        v-model:logSignClient="logSignClient"
        :logDataClient="logDataClient"
        @update:activeTabClient="activeTabClient = $event"
        @tab-changed="handleTabSelect"
        @remove-tab-client="removeTabClient"
      ...
      <div v-show="activeWorkspaceTab === 'testingEquipment'" class="testing-equipment-workspace">
        ...
        <div v-else class="testing-equipment-detail-shell">
          <div class="testing-equipment-detail-bar">
            <button
              type="button"
              class="testing-equipment-back"
              @click="closeTestingEquipmentDetail"
            >
          ...
          <TestingEquipmentView
            ...
            :equipment="
              selectedTestingEquipment && !selectedTestingEquipment.mrid
                ? selectedTestingEquipment
                : null
            "
    ...
    <!-- Dialog Components -->
    <SubstationDialog
      ref="substationDialog"
      :visible="signSubs"
      @update:visible="signSubs = $event"
    ...
    <VoltageLevelDialog
      ref="voltageLevelDialog"
      :visible="signVoltageLevel"
      @update:visible="signVoltageLevel = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :isSaving="isSaving"
      @close="handleVoltageLevelCancel"
      @cancel="handleVoltageLevelCancel"
      @confirm="handleVoltageLevelConfirm"
    />

    <BayDialog
      ref="bayDialog"
      :visible="signBay"
      @update:visible="signBay = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :isSaving="isSaving"
      @close="handleBayCancel"
      @cancel="handleBayCancel"
      @confirm="handleBayConfirm"
    />

    <TransformerDialog
      ref="transformerDialog"
      :visible="signTransformer"
      @update:visible="signTransformer = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      ...
      @cancel="handleTransformerCancel"
      @confirm="handleTransformerConfirm"
    />

    <BushingDialog
      ref="bushingDialog"
      :visible="signBushing"
      @update:visible="signBushing = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleBushingCancel"
      @confirm="handleBushingConfirm"
    />

    <SurgeArresterDialog
      ref="surgeArresterDialog"
      :visible="signSurge"
      @update:visible="signSurge = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleSurgeCancel"
      @confirm="handleSurgeConfirm"
    />

    <CircuitBreakerDialog
      ref="circuitBreakerDialog"
      :visible="signCircuit"
      @update:visible="signCircuit = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleCircuitCancel"
      @confirm="handleCircuitConfirm"
    />

    <CurrentTransformerDialog
      ref="currentTransformerDialog"
      :visible="signCt"
      @update:visible="signCt = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleCtCancel"
      @confirm="handleCtConfirm"
    />

    <VoltageTransformerDialog
      ref="voltageTransformerDialog"
      :visible="signVt"
      @update:visible="signVt = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleVtCancel"
      @confirm="handleVtConfirm"
    />

    <PowerCableDialog
      ref="powerCableDialog"
      :visible="signPower"
      @update:visible="signPower = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handlePowerCancel"
      @confirm="handlePowerConfirm"
    />

    <DisconnectorDialog
      ref="disconnectorDialog"
      :visible="signDisconnector"
      @update:visible="signDisconnector = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleDisconnectorCancel"
      @confirm="handleDisconnectorConfirm"
    />

    <RotatingMachineDialog
      ref="rotatingMachineDialog"
      :visible="signRotating"
      @update:visible="signRotating = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleRotatingCancel"
      @confirm="handleRotatingConfirm"
    />

    <CapacitorDialog
      ref="capacitorDialog"
      :visible="signCapacitor"
      @update:visible="signCapacitor = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleCapacitorCancel"
      @confirm="handleCapacitorConfirm"
    />

    <ReactorDialog
      ref="reactorDialog"
      :visible="signReactor"
      @update:visible="signReactor = $event"
      :locationId="locationId"
      :parentOrganization="parentOrganization"
      :modal="!isDuplicating"
      :show-close="!isDuplicating"
      ...
      @cancel="handleReactorCancel"
      @confirm="handleReactorConfirm"
    />

    <JobDialog
      ref="jobDialog"
      :visible="signJob"
      @update:visible="signJob = $event"
      :checkJobType="checkJobType"
    ...
    <ExportDialog
      ...
      @update:visible="openExportDialog = $event"
    ...
    <ImportDialog
      ...
      @update:visible="openImportDialog = $event"
    ...
    <FmecaDialog
      :visible="signFmeca"
      @update:visible="signFmeca = $event"
      @close="handleFmecaCancel"
      @cancel="handleFmecaCancel"
      @confirm="handleFmecaConfirm"
    />
    ...
    <DownloadDialog
      ...
      @update:visible="downloadDialogVisible = $event"
      :moveTreeData="moveTreeData"
      :moveTreeProps="moveTreeProps"
      :expandedMoveKeys="expandedMoveKeys"
      :selectedDownloadTargetNode="selectedDownloadTargetNode"
      @close="() => (downloadDialogVisible = false)"
      @cancel="() => (downloadDialogVisible = false)"
      @confirm="confirmDownloadSelection"
      @node-click="handleDownloadTargetSelection"
      @node-expand="fetchChildren"
    />

    <ZeroDiagramDialog
      ref="zeroDiagramDialog"
      ...
      :checkChildrenMethod="checkChildren"
      @node-deleted="handleNodeDeletedFromDiagram"
      @delete-node="handleDeleteNodeFromDiagram"
      @edit-node="handleEditNodeFromDiagram"
    />
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    ...
    dialogClass() {
      return this.isDuplicatingx ? 'app-dialog ghost-dialog' : 'app-dialog'
    }
  },
  ...
  methods: {
      ...
      if (serverSign == true) {
        ...
        if (this.ownerServerList.length === 0 && typeof this.showOwnerServerRoot === 'function') {
          try {
            ...
            this.$message?.error?.(
              'Failed to load server tree: ' + (error.message || 'Unknown error')
            )
          }
        }
      } else {
    ...
    async handleDeleteNodeFromDiagram(node, callback) {
      try {
        ...
        this.$message = {
          ...
          warning: (msg) => {
            originalMessage.warning(msg)
          },
          info: (msg) => {
            originalMessage.info(msg)
          }
        }
    ...
    async handleEditNodeFromDiagram(nodeData) {
      try {
        ...
        this.editingNode =
          this.findNodeById(nodeData.mrid || nodeData.id, this.organisationClientList) || nodeData
        ...
        if (type === 'Transformer') {
          entityRes = await window.electronAPI.getTransformerEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          ...
          entityRes = await window.electronAPI.getCurrentTransformerEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getVoltageTransformerEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getPowerCableEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getDisconnectorEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getSurgeArresterEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getCapacitorEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
          ...
          entityRes = await window.electronAPI.getRotatingMachineEntityByMrid(
            nodeData.mrid,
            nodeData.psrId
          )
          if (entityRes.success && entityRes.data) {
    ...
    async openAssetDialogForEdit(assetType, dto) {
      ...
      if (this.editingNode && this.editingNode.parentId) {
        this.parentOrganization = this.findNodeById(
          this.editingNode.parentId,
    ...
    async showAddVoltageLevel(node) {
      try {
        const dataLocation = await window.electronAPI.getLocationByPowerSystemResourceMrid(
          node.mrid
        )
        if (dataLocation.success) {
    ...
    async showAddJob(node) {
      try {
        ...
        if (node.asset == 'Surge arrester') {
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Surge arrester')
          ...
          this.signJob = true
          this.$nextTick(() => {
            this.$refs.jobDialog.$refs.jobData.loadParameter(
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Power cable')
          if (dataTestType.success) this.testTypeListData = dataTestType.data
          else this.testTypeListData = []
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Disconnector')
          if (dataTestType.success) this.testTypeListData = dataTestType.data
          else this.testTypeListData = []
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Current transformer')
          const dataCurrentTransformer = await window.electronAPI.getCurrentTransformerEntityByMrid(
            node.mrid
          )
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Voltage transformer')
          const dataVoltageTransformer = await window.electronAPI.getVoltageTransformerEntityByMrid(
            node.mrid
          )
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Circuit breaker')
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Transformer')
          const dataTransformerEntity = await window.electronAPI.getTransformerEntityByMrid(
            node.mrid
          )
          ...
          const dataTestType =
            await window.electronAPI.getProcedureByGenericAssetModel('Rotating machine')
          const dataRotatingMachineEntity = await window.electronAPI.getRotatingMachineEntityByMrid(
            node.mrid
          )
        ...
        if (voltageLevel) {
          ...
          if (success) {
            ...
            if (this.organisationClientList && this.organisationClientList.length > 0) {
              ...
              const name =
                data.name ||
                data.voltageLevel?.name ||
                data.data?.voltageLevel?.name ||
                'Unnamed Voltage Level'
              const newRow = {
                mrid: mrid,
                name: name,
                parentId: this.parentOrganization.mrid,
