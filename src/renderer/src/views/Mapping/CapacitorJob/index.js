/* eslint-disable */
import CapacitorJobEntity from '@/views/Flatten/Job/Capacitor'
import CapacitorJobDto from '@/views/Dto/Job/Capacitor'
...
import CapacitorTestingEquipmentTestType from '@/views/Flatten/CapacitorTestingEquipmentTestType'
...
import capacitorTestMap from '@/config/testing-condition/Capacitor'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const capacitorTestingEquipmentTestType of dto.capacitorTestingEquipmentTestType) {
...
export const JobEntityToDto = (entity) => {
  ...
  dto.capacitorTestingEquipmentTestType = entity.capacitorTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      capacitorConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = capacitorTestMap[item.type].testId || ''
