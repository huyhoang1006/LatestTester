...
export function mapDtoToEntity(dto) {
  ...
  if (frequencyValue === 'Custom') {
    ...
    frequencyValue =
      dto.ratings.rated_frequency_custom || dto.ratings.rated_frequency.custom_value || ''
...
const mapDataVTtoArrayPotentialTransformerTable = (dto, entity) => {
  ...
  dto.vt_Configuration.dataVT.forEach((item, idx) => {
    ...
    newTable.rated_power_factor =
      rpfRaw !== null && rpfRaw !== '' && !isNaN(Number(rpfRaw))
        ? parseFloat(rpfRaw)
        : rpfRaw === '' || rpfRaw === null
          ? null
          : isNaN(Number(rpfRaw))
    ...
    newTable.potential_transformer_info_id =
      entity?.OldPotentialTransformerInfo?.mrid ?? entity?.assetInfoId ?? null
