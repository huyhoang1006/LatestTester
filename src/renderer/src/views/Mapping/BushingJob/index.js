/* eslint-disable */
import BushingJobEntity from '@/views/Flatten/Job/Bushing'
import BushingJobDto from '@/views/Dto/Job/Bushing'
...
import BushingTestingEquipmentTestType from '@/views/Flatten/BushingTestingEquipmentTestType'
...
import bushingTestMap from '@/config/testing-condition/Bushing'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const bushingTestingEquipmentTestType of dto.bushingTestingEquipmentTestType) {
...
export const JobEntityToDto = (entity) => {
  ...
  dto.bushingTestingEquipmentTestType = entity.bushingTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      bushingConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = bushingTestMap[item.type].testId || ''
