/* eslint-disable */
import ReactorJobEntity from '@/views/Flatten/Job/Reactor'
import ReactorJobDto from '@/views/Dto/Job/Reactor'
...
import ReactorTestingEquipmentTestType from '@/views/Flatten/ReactorTestingEquipmentTestType'
...
import reactorTestMap from '@/config/testing-condition/Reactor'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const reactorTestingEquipmentTestType of dto.reactorTestingEquipmentTestType) {
    const data = new ReactorTestingEquipmentTestType()
    data.mrid = reactorTestingEquipmentTestType.mrid || null
    data.testing_equipment_id = reactorTestingEquipmentTestType.testing_equipment_id || null
    data.test_type_id = reactorTestingEquipmentTestType.test_type_id || null
    entity.reactorTestingEquipmentTestType.push(data)
  }
...
export const JobEntityToDto = (entity) => {
  ...
  for (const testingEquipment of entity.testingEquipment) {
    ...
    data.test_type_reactor_id = []
    for (const reactorTestingEquipmentTestType of entity.reactorTestingEquipmentTestType) {
      if (reactorTestingEquipmentTestType.testing_equipment_id === data.mrid) {
        data.test_type_reactor_id.push(reactorTestingEquipmentTestType.test_type_id)
      }
    }
    dto.testingEquipmentData.push(data)
  }
  dto.reactorTestingEquipmentTestType = entity.reactorTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      reactorConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = reactorTestMap[item.type].testId || ''
