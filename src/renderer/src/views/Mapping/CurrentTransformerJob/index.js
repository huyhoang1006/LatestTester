...
import CurrentTransformerTestingEquipmentTestType from '@/views/Flatten/CurrentTransformerTestingEquipmentTestType'
  ...
  try {
    ...
    for (const currentTransformerTestingEquipmentTestType of dto.currentTransformerTestingEquipmentTestType) {
      ...
      data.testing_equipment_id =
        currentTransformerTestingEquipmentTestType.testing_equipment_id || null
        ...
        for (const key in tableData) {
          ...
          if (Array.isArray(rows)) {
            rows.forEach((data) => {
              ...
              testData.type = 'test'
              testData.title = key || null // Lưu tên table
              entity.testDataSet.push(testData)

              // Xử lý các measurement values
                ...
                if (typeof value === 'object') {
                  if (value.type === 'analog') {
                    ...
                    if (fieldKey == 'assessment') {
                      ...
                      discreteValue.value =
                        commonFunc.conditionIndicatorToValue(value.value) ?? null
                    }
                    ...
                    discreteValue.discrete = value['measurement_id']
                      ? value['measurement_id']
                      : null
...
export const JobEntityToDto = (entity) => {
  ...
  for (const testingEquipment of entity.testingEquipment) {
    ...
    for (const currentTransformerTestingEquipmentTestType of entity.currentTransformerTestingEquipmentTestType) {
      if (currentTransformerTestingEquipmentTestType.testing_equipment_id === data.mrid) {
        data.test_type_current_transformer_id.push(
          currentTransformerTestingEquipmentTestType.test_type_id
        )
      }
    }
    dto.testingEquipmentData.push(data)
  }
  dto.currentTransformerTestingEquipmentTestType =
  ...
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      currentTransformerConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testData.forEach((test) => {
      const tableKey = test.title || 'table1' // Nếu không có title thì dùng 'table1' (backward compatibility)
      if (!grouped[tableKey]) {
        grouped[tableKey] = []
      }
