/* eslint-disable */
import DisconnectorJobEntity from '@/views/Flatten/Job/Disconnector'
import DisconnectorJobDto from '@/views/Dto/Job/Disconnector'
...
import DisconnectorTestingEquipmentTestType from '@/views/Flatten/DisconnectorTestingEquipmentTestType'
...
import disconnectorTestMap from '@/config/testing-condition/Disconnector'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const disconnectorTestingEquipmentTestType of dto.disconnectorTestingEquipmentTestType) {
    const data = new DisconnectorTestingEquipmentTestType()
    data.mrid = disconnectorTestingEquipmentTestType.mrid || null
    data.testing_equipment_id = disconnectorTestingEquipmentTestType.testing_equipment_id || null
    data.test_type_id = disconnectorTestingEquipmentTestType.test_type_id || null
    entity.disconnectorTestingEquipmentTestType.push(data)
  }
        ...
        for (const [key, value] of Object.entries(data)) {
          if (typeof value === 'object') {
            if (value.type === 'analog') {
              ...
              stringValue.string_measurement = value['measurement_id']
...
export const JobEntityToDto = (entity) => {
  const dto = new DisconnectorJobDto()
  //job properties
  ...
  for (const testingEquipment of entity.testingEquipment) {
    ...
    data.test_type_disconnector_id = []
    for (const disconnectorTestingEquipmentTestType of entity.disconnectorTestingEquipmentTestType) {
      if (disconnectorTestingEquipmentTestType.testing_equipment_id === data.mrid) {
        data.test_type_disconnector_id.push(disconnectorTestingEquipmentTestType.test_type_id)
      }
    }
    dto.testingEquipmentData.push(data)
  }
  dto.disconnectorTestingEquipmentTestType = entity.disconnectorTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      disconnectorConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = disconnectorTestMap[item.type].testId || ''
    ...
    const testData = entity.testDataSet.filter(
      (x) => x.work_task === item.mrid && x.type === 'test'
    )
    const grouped = {}
    ...
    for (const key in grouped) {
      ...
      for (const test of grouped[key]) {
        ...
        const stringMeasutementValueData = entity.stringMeasurementValues.filter(
          (x) => x.procedure_dataset_id == test.mrid
        )
        ...
        const analogValueData = entity.analogValues.filter(
          (x) => x.procedure_dataset_id === test.mrid
        )
        ...
        const discreteValueData = entity.discreteValues.filter(
          (x) => x.procedure_dataset_id === test.mrid
        )
    ...
    const testDataCondition = entity.testDataSet.find(
      (x) => x.work_task === item.mrid && x.type === 'condition'
    )
    if (testDataCondition) {
      const rowData = {}
      const stringMeasutementValueData = entity.stringMeasurementValues.filter(
        (x) => x.procedure_dataset_id === testDataCondition.mrid
      )
