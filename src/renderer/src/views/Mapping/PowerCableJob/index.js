...
export const jobDtoToEntity = (dto) => {
  ...
  for (const powerCableTestingEquipmentTestType of dto.powerCableTestingEquipmentTestType) {
  ...
  for (const item of dto.testList) {
    ...
    // Xử lý nested table structure như SurgeArrester
    if (item.data.table && typeof item.data.table === 'object') {
      for (const key in item.data.table) {
        const rows = item.data.table[key]
        // Add safety check to ensure rows is an array
...
export const JobEntityToDto = (entity) => {
  ...
  dto.powerCableTestingEquipmentTestType = entity.powerCableTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      powerCableConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = powerCableTestMap[item.type]?.testId || ''
