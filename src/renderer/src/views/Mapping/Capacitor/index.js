...
export function mapDtoToEntity(dto) {
  ...
  if (dto.configsData.phase == '1') {
    ...
    if (
      dto.capacitance.capacitance &&
      dto.capacitance.capacitance.value &&
      dto.capacitance.capacitance.value.value
    ) {
  ...
  if (dto.configsData.phase == '1') {
    ...
    if (
      dto.dissipationFactor.dissipation_factor &&
      dto.dissipationFactor.dissipation_factor.value &&
      phase1ValueStr &&
      phase1ValueStr.trim() !== ''
    ) {
      ...
      dissipationFactorCapacitorInfo.value =
        dto.dissipationFactor.dissipation_factor.value.mrid || null
...
export function mapEntityToDto(entity) {
  ...
  dto.configsData.phase =
    entity.capacitor.phase_number > 0 ? String(entity.capacitor.phase_number) : ''
  ...
  for (const capInfo of entity.capacitanceCapacitorInfo) {
    ...
    if (capacitanceValue) {
      const phaseKey =
        capInfo.phase && capInfo.phase !== '1' ? `capacitance_${capInfo.phase}` : 'capacitance'
      if (dto.capacitance[phaseKey]) {
        ...
        dto.capacitance[phaseKey].value.unit = capacitanceValue.multiplier
          ? `${capacitanceValue.multiplier}|${capacitanceValue.unit}`
          : capacitanceValue.unit
  ...
  for (const dfInfo of entity.dissipationFactorCapacitorInfo) {
    ...
    if (percentValue) {
      const phaseKey =
        dfInfo.phase && dfInfo.phase !== '1'
          ? `dissipation_factor_${dfInfo.phase}`
          : 'dissipation_factor'
      if (dto.dissipationFactor[phaseKey]) {
        ...
        dto.dissipationFactor[phaseKey].value.unit = percentValue.multiplier
          ? `${percentValue.multiplier}|${percentValue.unit}`
          : percentValue.unit
...
export const capacitorDtoToEntity = mapDtoToEntity
export const capacitorEntityToDto = mapEntityToDto
