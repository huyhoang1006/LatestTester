...
export const mapDtoToEntity = (dto) => {
  ...
  entity.oldCurrentTransformerInfo.system_voltage_type =
  ...
  dto.ctConfiguration.dataCT.forEach((coreDto, index) => {
    ...
    coreInfo.common_tap =
      validCommonTap >= 1 && validCommonTap <= coreInfo.tap_count ? validCommonTap : 1
...
export const mapEntityToDto = (entity) => {
  ...
  const findUnitValue = (collection, mrid) =>
    (collection.find((item) => item && item.mrid === mrid) || {}).value
  ...
  dto.ratings.u_withstand_rms.value = findUnitValue(
    entity.voltage,
    dto.ratings.u_withstand_rms.mrid
  )

  dto.ratings.u_lightning_peak.mrid = entity.oldCurrentTransformerInfo.u_lightning_peak || ''
  dto.ratings.u_lightning_peak.value = findUnitValue(
    entity.voltage,
    dto.ratings.u_lightning_peak.mrid
  )
  ...
  dto.ratings.rating_factor_temp.value = findUnitValue(
    entity.temperature,
    dto.ratings.rating_factor_temp.mrid
  )

  // CT Configuration - validate cores (must be 1-9)
  const validCoreCount = parseInt(entity.oldCurrentTransformerInfo.core_count)
  dto.ctConfiguration.cores = (
    validCoreCount >= 1 && validCoreCount <= 9 ? validCoreCount : 1
  ).toString()
  ...
  dataCT.forEach((coreInfo) => {
    ...
    core.commonTap = (
      validCommonTap >= 1 && validCommonTap <= maxCommonTap ? validCommonTap : 1
    ).toString()
    ...
    const tapsForThisCore = (entity.CtTapInfo || []).filter(
      (t) => t.ct_core_info_id === coreInfo.mrid
    )
    ...
    tapsForThisCore.forEach((tapInfo) => {
      const tapObject = {
        ...
        classRating: {
          ...
          extended_burden:
            tapInfo.extended_burden === 1 ||
            tapInfo.extended_burden === '1' ||
            tapInfo.extended_burden === true,
