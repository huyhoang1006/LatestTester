...
export const disconnectorDtoToEntity = (dto) => {
  ...
  entity.attachment.id = dto.attachmentId || null
  entity.attachment.id_foreign = dto.properties.mrid || null
  if (dto.attachment && dto.attachment.path) {
    ...
    entity.attachment.path =
      typeof dto.attachment.path === 'string'
        ? dto.attachment.path
  ...
  if (dto.ratings) {
    ...
    if (
      ...
      dto.ratings.short_time_withstand_current.value
    ) {
      // Tạo mrid nếu chưa có
      if (!dto.ratings.short_time_withstand_current.mrid) {
        dto.ratings.short_time_withstand_current.mrid = uuid.newUuid()
      }
      entity.disconnectorInfo.short_time_withstand_current =
        dto.ratings.short_time_withstand_current.mrid
    ...
    if (
      ...
      dto.ratings.rated_duration_of_short_circuit.value
    ) {
      // Tạo mrid nếu chưa có
      if (!dto.ratings.rated_duration_of_short_circuit.mrid) {
        dto.ratings.rated_duration_of_short_circuit.mrid = uuid.newUuid()
      }
      entity.disconnectorInfo.rated_duration_short_circuit =
        dto.ratings.rated_duration_of_short_circuit.mrid
    ...
    if (
      ...
      dto.ratings.power_freq_withstand_voltage_earth_poles.value
    ) {
      // Tạo mrid nếu chưa có
      ...
      entity.disconnectorInfo.withstand_voltage_earth_poles =
        dto.ratings.power_freq_withstand_voltage_earth_poles.mrid
    ...
    if (
      ...
      dto.ratings.power_freq_withstand_voltage_isolating_distance.value
    ) {
      ...
      entity.disconnectorInfo.power_frequency_isolating_distance =
        dto.ratings.power_freq_withstand_voltage_isolating_distance.mrid
...
export const disconnectorEntityToDto = (entity) => {
  ...
  dto.properties.manufacturer_type =
    entity.disconnectorInfo && entity.disconnectorInfo.manufacturer_type
  ...
  if (ratedVoltage) {
    ...
    dto.ratings.rated_voltage.unit =
      [ratedVoltage.multiplier, ratedVoltage.unit].filter(Boolean).join('|') || ''
  }
  ...
  if (withstandEarthPoles) {
    ...
    dto.ratings.power_freq_withstand_voltage_earth_poles.unit =
      [withstandEarthPoles.multiplier, withstandEarthPoles.unit].filter(Boolean).join('|') || ''
  }
  ...
  if (powerFreqIsoDistance) {
    dto.ratings.power_freq_withstand_voltage_isolating_distance.mrid =
      powerFreqIsoDistance.mrid || ''
    dto.ratings.power_freq_withstand_voltage_isolating_distance.value =
      powerFreqIsoDistance.value || ''
    dto.ratings.power_freq_withstand_voltage_isolating_distance.unit =
      powerFreqIsoDistance.unit || ''
  }
  ...
  if (shortTimeWithstand) {
    ...
    dto.ratings.short_time_withstand_current.unit =
      [shortTimeWithstand.multiplier, shortTimeWithstand.unit].filter(Boolean).join('|') || ''
  }
