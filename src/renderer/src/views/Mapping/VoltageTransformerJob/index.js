/* eslint-disable */
import VoltageTransformerJobEntity from '@/views/Flatten/Job/VoltageTransformer'
import VoltageTransformerJobDto from '@/views/Dto/Job/VoltageTransformer'
...
import VoltageTransformerTestingEquipmentTestType from '@/views/Flatten/VoltageTransformerTestingEquipmentTestType'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const voltageTransformerTestingEquipmentTestType of dto.voltageTransformerTestingEquipmentTestType) {
    ...
    data.testing_equipment_id =
      voltageTransformerTestingEquipmentTestType.testing_equipment_id || null
  ...
  for (const item of dto.testList) {
    ...
    // Updated table mapping logic similar to SurgeArrester
    for (const key in item.data.table) {
      const rows = item.data.table[key]
          ...
          for (const [fieldKey, value] of Object.entries(data)) {
            if (typeof value === 'object') {
              if (value.type === 'analog') {
                ...
                analogValue.alias_name = fieldKey || null
                ...
                stringValue.alias_name = fieldKey || null
...
export const JobEntityToDto = (entity) => {
  const dto = new VoltageTransformerJobDto()
  //job properties
  ...
  for (const testingEquipment of entity.testingEquipment) {
    ...
    for (const voltageTransformerTestingEquipmentTestType of entity.voltageTransformerTestingEquipmentTestType) {
      if (voltageTransformerTestingEquipmentTestType.testing_equipment_id === data.mrid) {
        data.test_type_voltage_transformer_id.push(
          voltageTransformerTestingEquipmentTestType.test_type_id
        )
      }
    }
    dto.testingEquipmentData.push(data)
  }
  dto.voltageTransformerTestingEquipmentTestType =
  ...
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      voltageTransformerConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
