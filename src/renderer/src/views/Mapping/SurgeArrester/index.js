...
export function mapDtoToEntity(dto) {
  ...
  for (let data of dto.ratings.tableRating) {
    ...
    newRatedVoltage.unit =
      unitPartsRatedV.length > 1 ? unitPartsRatedV[1] : unitPartsRatedV[0] || null
    ...
    newMaxSystemVoltage.unit =
      unitPartsMaxSysV.length > 1 ? unitPartsMaxSysV[1] : unitPartsMaxSysV[0] || null
    ...
    newShortCurrent.unit =
      unitPartsShortTime.length > 1 ? unitPartsShortTime[1] : unitPartsShortTime[0] || null
    ...
    newRatedDuration.multiplier =
      unitPartsRatedDuration.length > 1 ? unitPartsRatedDuration[0] : null
    newRatedDuration.unit =
      unitPartsRatedDuration.length > 1
        ? unitPartsRatedDuration[1]
        : unitPartsRatedDuration[0] || null
    ...
    newPolesVoltage.unit =
      unitPartsPolesVoltage.length > 1 ? unitPartsPolesVoltage[1] : unitPartsPolesVoltage[0] || null
    ...
    newIsoVoltage.unit =
      unitPartsIsoVoltage.length > 1 ? unitPartsIsoVoltage[1] : unitPartsIsoVoltage[0] || null
...
export function mapEntityToDto(entity) {
  ...
  for (let i = 0; i < entity.assetUnit.length; i++) {
    const assetInfoUnit = entity.assetInfoUnit.find(
      (x) => x.mrid === entity.assetUnit[i].asset_info
    )
    const data = {
      mrid: '',
      assetInfoId: '',
...
export const surgeArresterDtoToEntity = mapDtoToEntity
export const surgeArresterEntityToDto = mapEntityToDto
