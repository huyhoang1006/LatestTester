/* eslint-disable */
import RotatingMachineJobEntity from '@/views/Flatten/Job/RotatingMachine'
import RotatingMachineJobDto from '@/views/Dto/Job/RotatingMachine'
...
import RotatingMachineTestingEquipmentTestType from '@/views/Flatten/RotatingMachineTestingEquipmentTestType'
...
export const jobDtoToEntity = (dto) => {
  ...
  for (const rotatingMachineTestingEquipmentTestType of dto.rotatingMachineTestingEquipmentTestType) {
...
export const JobEntityToDto = (entity) => {
  const dto = new RotatingMachineJobDto()
  //job properties
  ...
  for (const testingEquipment of entity.testingEquipment) {
    ...
    for (const rotatingMachineTestingEquipmentTestType of entity.rotatingMachineTestingEquipmentTestType) {
      if (rotatingMachineTestingEquipmentTestType.testing_equipment_id === data.mrid) {
        data.test_type_rotating_machine_id.push(
          rotatingMachineTestingEquipmentTestType.test_type_id
        )
      }
    }
    dto.testingEquipmentData.push(data)
  }
  dto.rotatingMachineTestingEquipmentTestType = entity.rotatingMachineTestingEquipmentTestType || []

  //test list
  for (const item of entity.workTasks) {
    let condition = commonFunc.buildEmptyTestCondition(
      rotatingMachineConditionMap[item.type]?.columns || []
    )
    let testTemplate = {
      mrid: item.mrid || '',
      name: item.name || '',
    ...
    testTemplate.testTypeId = rotatingMachineTestMap[item.type].testId || ''
