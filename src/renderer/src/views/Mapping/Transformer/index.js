...
export const transformerDtoToEntity = (dto) => {
  ...
  if (dto.winding_configuration.unsupported_vector_group) {
    entity.oldPowerTransformerInfo.vector_group =
      dto.winding_configuration.unsupported_vector_group || null
  }
  if (dto.winding_configuration.vector_group_custom) {
    entity.oldPowerTransformerInfo.vector_group =
      dto.winding_configuration.vector_group_custom || null
  ...
  if (dto.winding_configuration.vector_group_data) {
    entity.oldPowerTransformerInfo.vector_group = ''
    const getVal = (val) =>
      val === 'null' || val === null || val === undefined || val === '' ? '' : val
    if (dto.winding_configuration.vector_group.prim.includes('Spare I')) {
      entity.oldPowerTransformerInfo.vector_group += 'I'
    } else {
      entity.oldPowerTransformerInfo.vector_group += getVal(
        dto.winding_configuration.vector_group.prim
      )
    }
    entity.oldPowerTransformerInfo.vector_group += getVal(
      dto.winding_configuration.vector_group.sec.i
    )
    entity.oldPowerTransformerInfo.vector_group += getVal(
      dto.winding_configuration.vector_group.sec.value
    )
    entity.oldPowerTransformerInfo.vector_group += getVal(
      dto.winding_configuration.vector_group.tert.i
    )
    entity.oldPowerTransformerInfo.vector_group += getVal(
      dto.winding_configuration.vector_group.tert.value
    )
  ...
  if (dto.ratings.short_circuit.mrid && dto.ratings.short_circuit.mrid !== '') {
    ...
    entity.shortCircuitRating.power_transformer_info_id =
      entity.oldPowerTransformerInfo.mrid || null
      ...
      for (let j = 0; j < dto.shortCircuitTestTransformerEndInfo.length; j++) {
        if (dto.shortCircuitTestTransformerEndInfo[j].short_circuit_test_id === item.mrid) {
          ...
          shortCircuitTestTransformerEndInfo.mrid =
            dto.shortCircuitTestTransformerEndInfo[j].mrid || null
          shortCircuitTestTransformerEndInfo.transformer_end_info_id =
            transformer_end_info_second_id || null
          shortCircuitTestTransformerEndInfo.short_circuit_test_id =
            dto.shortCircuitTestTransformerEndInfo[j].short_circuit_test_id || null
      ...
      for (let j = 0; j < dto.shortCircuitTestTransformerEndInfo.length; j++) {
        if (dto.shortCircuitTestTransformerEndInfo[j].short_circuit_test_id === item.mrid) {
          ...
          shortCircuitTestTransformerEndInfo.mrid =
            dto.shortCircuitTestTransformerEndInfo[j].mrid || null
          shortCircuitTestTransformerEndInfo.transformer_end_info_id =
            transformer_end_info_third_id || null
          shortCircuitTestTransformerEndInfo.short_circuit_test_id =
            dto.shortCircuitTestTransformerEndInfo[j].short_circuit_test_id || null
          entity.shortCircuitTestTransformerEndInfo.push(shortCircuitTestTransformerEndInfo)
          break
  ...
  entity.zeroSequenceImpedance.base_power =
    dto.impedances.zero_sequence_impedance.base_power.mrid || null
  entity.zeroSequenceImpedance.base_voltage =
    dto.impedances.zero_sequence_impedance.base_voltage.mrid || null
  const basePower = new BasePower()
  ...
  baseVoltage.nominal_voltage =
    dto.impedances.zero_sequence_impedance.base_voltage.data.mrid || null
  const voltage = new Voltage()
  ...
  for (let key in dto.impedances.zero_sequence_impedance.zero_percent) {
    ...
    zeroSequenceImpedanceTable.mrid =
      dto.impedances.zero_sequence_impedance.zero_percent[key].mrid || null
    zeroSequenceImpedanceTable.zero_sequence_impedance =
      dto.impedances.zero_sequence_impedance.mrid || null
    zeroSequenceImpedanceTable.zero =
      dto.impedances.zero_sequence_impedance.zero_percent[key].data.mrid || null
    if (key == 'prim') {
      for (let i = 0; i < dto.oldTransformerEndInfo.length; i++) {
        if (dto.oldTransformerEndInfo[i].end_number === 1) {
  ...
  entity.tapChanger.oldTapChangerInfo.product_asset_model =
    dto.tap_changers.productAssetModelId || null
  ...
  entity.tapChanger.oldTapChangerInfo.power_transformer_info_id =
    dto.oldPowerTransformerInfoId || null
  ...
  for (const winding_data of winding) {
    for (const bushing of dto.bushing_data[winding_data]) {
      ...
      bushingAsset.oldBushingInfo.rated_impulse_withstand_voltage =
        bushing.insulation_level.mrid || null
      const voltage = new Voltage()
      ...
      bushingAsset.oldBushingInfo.rated_line_to_ground_voltage =
        bushing.voltage_l_ground.mrid || null
  ...
  for (const winding_data of winding) {
    for (const surge of dto.surge_arrester[winding_data]) {
      if (surge.sign == true) {
        ...
        surgeArresterAsset.surgeArrester.asset_system_code =
          surge.properties.asset_system_code || null
        ...
        surgeArresterAsset.oldSurgeArresterInfo.product_asset_model =
          surge.properties.productAssetModelId || null
        ...
        surgeArresterAsset.lifecycleDate.manufactured_date =
          surge.properties.manufacturer_year || null
...
export const transformerEntityToDto = (entity) => {
    ...
    for (const winding of dto.oldTransformerEndInfo) {
      if (winding.end_number == 1) {
        if (winding.spare == true) {
          ...
          dto.winding_configuration.vector_group.prim =
            connKind === 'I' || (connKind && connKind.startsWith('I')) ? connKind + phase : connKind
        }
  ...
  dto.impedances.zero_sequence_impedance.base_voltage.mrid =
    entity.zeroSequenceImpedance.base_voltage
  for (const baseVoltage of entity.baseVoltage) {
    if (baseVoltage.mrid == entity.zeroSequenceImpedance.base_voltage) {
      dto.impedances.zero_sequence_impedance.base_voltage.data.mrid =
        baseVoltage.nominal_voltage || ''
      for (const voltage of entity.voltage) {
        if (voltage.mrid == baseVoltage.nominal_voltage) {
  ...
  for (const zeroSequenceImpedanceTable of entity.zeroSequenceImpedanceTable) {
    if (
      zeroSequenceImpedanceTable.transformer_end_id == '' ||
      zeroSequenceImpedanceTable.transformer_end_id == null
    ) {
      dto.impedances.zero_sequence_impedance.zero_percent.zero.mrid =
        zeroSequenceImpedanceTable.mrid || ''
      dto.impedances.zero_sequence_impedance.zero_percent.zero.data.mrid =
        zeroSequenceImpedanceTable.zero || ''
      for (const percent of entity.percent) {
      ...
      for (const transformerEndInfo of entity.oldTransformerEndInfo) {
        if (transformerEndInfo.mrid == zeroSequenceImpedanceTable.transformer_end_id) {
          if (transformerEndInfo.end_number == 1) {
            dto.impedances.zero_sequence_impedance.zero_percent.prim.mrid =
              zeroSequenceImpedanceTable.mrid
            dto.impedances.zero_sequence_impedance.zero_percent.prim.data.mrid =
              zeroSequenceImpedanceTable.zero || ''
            for (const percent of entity.percent) {
              if (percent.mrid == zeroSequenceImpedanceTable.zero) {
                dto.impedances.zero_sequence_impedance.zero_percent.prim.data.value =
                  percent.value || ''
              }
            }
          }
          ...
          if (transformerEndInfo.end_number == 2) {
            dto.impedances.zero_sequence_impedance.zero_percent.sec.mrid =
              zeroSequenceImpedanceTable.mrid
            dto.impedances.zero_sequence_impedance.zero_percent.sec.data.mrid =
              zeroSequenceImpedanceTable.zero || ''
            for (const percent of entity.percent) {
              if (percent.mrid == zeroSequenceImpedanceTable.zero) {
                dto.impedances.zero_sequence_impedance.zero_percent.sec_tert.data.value =
                  percent.value || ''
              }
            }
          }
  ...
  dto.tap_changers.productAssetModelId =
    entity.tapChanger.oldTapChangerInfo.product_asset_model || ''
  ...
  for (const transformer_end_info of entity.oldTransformerEndInfo) {
    for (const bushing of entity.bushing) {
      if (bushing.oldBushingInfo.transformer_end_info == transformer_end_info.mrid) {
        ...
        bushingTemplate.insulation_level.mrid =
          bushing.oldBushingInfo.rated_impulse_withstand_voltage || ''
        ...
        bushingTemplate.voltage_l_ground.mrid =
          bushing.oldBushingInfo.rated_line_to_ground_voltage || ''
        for (const voltage of bushing.voltage) {
  ...
  for (const transformer_end_info of entity.oldTransformerEndInfo) {
    for (const surgeArresterData of entity.surgeArrester) {
      if (
        surgeArresterData.oldSurgeArresterInfo.transformer_end_info === transformer_end_info.mrid
      ) {
        const surgeTemplate = {
          sign: true,
          properties: {
        ...
        surgeTemplate.properties.asset_system_code =
          surgeArresterData.surgeArrester.asset_system_code || ''
        surgeTemplate.properties.lifecycleDateId = surgeArresterData.lifecycleDate.mrid || ''
        surgeTemplate.properties.manufacturer =
          surgeArresterData.productAssetModel.manufacturer || ''
        surgeTemplate.properties.serial_no = surgeArresterData.surgeArrester.serial_number || ''
        surgeTemplate.properties.manufacturer_year =
          surgeArresterData.lifecycleDate.manufactured_date || ''
        surgeTemplate.properties.productAssetModelId =
          surgeArresterData.productAssetModel.mrid || ''
        ...
        for (const assetUnit of surgeArresterData.assetUnit) {
          const assetUnitInfo = surgeArresterData.assetInfoUnit.find(
            (x) => x.mrid === assetUnit.asset_info
          )
          const tableUnit = {
            mrid: '',
            assetInfoId: '',
          ...
          for (const voltage of surgeArresterData.voltage) {
            if (voltage.mrid === tableUnit.voltageLl.mrid) {
              ...
              tableUnit.voltageLl.unit = voltage.multiplier
                ? voltage.multiplier + '|' + voltage.unit
                : voltage.unit
            }
          }
          ...
          for (const voltage of surgeArresterData.voltage) {
            if (voltage.mrid === tableUnit.voltageLn.mrid) {
              ...
              tableUnit.voltageLn.unit = voltage.multiplier
                ? voltage.multiplier + '|' + voltage.unit
                : voltage.unit
            }
          }
          ...
          for (const voltage of surgeArresterData.voltage) {
            if (voltage.mrid === tableUnit.mcovRating.mrid) {
              ...
              tableUnit.mcovRating.unit = voltage.multiplier
                ? voltage.multiplier + '|' + voltage.unit
                : voltage.unit
            }
          }
