...
const VT_DISCRETE_ALIAS_TO_VALUE = (() => {
  const map = {}
  const collect = (cfg) => {
  ...
  collect(voltageTransformerTestMap)
  collect(voltageTransformerConditionMap)
  return map
})()
...
const discreteToValue = (measurementId, value) => {
  ...
  const m = measurementId && VT_DISCRETE_ALIAS_TO_VALUE[measurementId]
...
// ═══════════════════════════════════════════════════════════════════════════════
// Mapper: DTO Job (Voltage Transformer) → server JSON (upload)
//
...
const mapCell = (cell) => {
  ...
  else if (cell.type === 'discrete')
    value = discreteToValue(cell.measurement_id, cell.value) // chữ → số
...
const mapAssessment = (testAssessment) => {
  ...
  let customizedStandards: any = null
  if (customizeId) {
    ...
    if (customizedStandards && !customizedStandards.length) customizedStandards = null
  }

  // Đã chọn tiêu chuẩn CỐ ĐỊNH → chỉ tham chiếu mrid (server đã seed, không gửi định nghĩa).
  // CHỈ ref đúng (các) loại đã chọn trong fixedStandards, không ref mọi template non-customized.
  let fixedStandardRefs: any = null
  if (hasFixed) {
    const chosenMrids = new Set(Object.values(fixedStandards))
    fixedStandardRefs = asm
      .filter((a) => a.type && a.type !== 'customized' && chosenMrids.has(a.mrid))
      .map((a) => ({ mrid: a.mrid || '', type: a.type, code: a.code || '' }))
    if (fixedStandardRefs && !fixedStandardRefs.length) fixedStandardRefs = null
  }
...
export const mapDtoToServer = (dto) => {
  ...
  return {
    ...
    testingEquipmentList: (dto.testingEquipmentData || []).map((e) => ({
      ...
      testTypeIds: e.test_type_voltage_transformer_id || []
    })),
...
export const mapServerToDto = (server) => {
  ...
  return {
    ...
    testingEquipmentData: (server.testingEquipmentData || server.testingEquipmentList || []).map(
      (e) => ({
        mrid: e.mrid || null,
        model: e.model || null,
    ...
    voltageTransformerTestingEquipmentTestType:
      server.voltageTransformerTestingEquipmentTestType || [],
