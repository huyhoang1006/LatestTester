...
const mapServerUnit = (serverObj) => {
  ...
  const multiplier =
    serverObj.multiplier === 'none' || !serverObj.multiplier ? '' : serverObj.multiplier
  return {
...
export const mapServerToDto = (serverData) => {
  ...
  if (acc.sheathVoltageLimiter) {
    ...
    dto.datasData.sheathLimitsData.max_continuous_operating_voltage = mapServerUnit(
      svl.maxContinuousOperatingVoltage
    )
    dto.datasData.sheathLimitsData.nominal_discharge_current = mapServerUnit(
      svl.nominalDischargeCurrent
    )
    dto.datasData.sheathLimitsData.high_current_impulse_withstand = mapServerUnit(
      svl.highCurrentImpulseWithstand
    ) // 40 kA
    dto.datasData.sheathLimitsData.long_duration_current_impulse_withstand = mapServerUnit(
      svl.longDurationCurrentImpulseWithstand
    ) // 200 A
    dto.datasData.sheathLimitsData.short_circuit_withstand = mapServerUnit(
      svl.shortCircuitWithstand
    ) // 5 kA
  }
  return dto
}
...
export const mapDtoToServer = (dto) => {
  ...
  else if (dto.configsData.cores.value !== 'Single')
    cCount = safeInt(dto.configsData.cores.value, 1)
  ...
  if (dto.datasData.sheathLimitsData.mrid) {
    server.accessories.sheathVoltageLimiter = {
      ...
      maxContinuousOperatingVoltage: mapDtoUnitToServer(
        dto.datasData.sheathLimitsData.max_continuous_operating_voltage
      ),
      nominalDischargeCurrent: mapDtoUnitToServer(
        dto.datasData.sheathLimitsData.nominal_discharge_current
      ),
      highCurrentImpulseWithstand: mapDtoUnitToServer(
        dto.datasData.sheathLimitsData.high_current_impulse_withstand
      ),
      longDurationCurrentImpulseWithstand: mapDtoUnitToServer(
        dto.datasData.sheathLimitsData.long_duration_current_impulse_withstand
      ),
      shortCircuitWithstand: mapDtoUnitToServer(
        dto.datasData.sheathLimitsData.short_circuit_withstand
      )
    }
  }
