...
const mapCell = (cell, fieldKey) => {
  ...
  if (cell.type === 'analog') value = num(cell.value)
    ...
    value = discreteToValue(measurementId, cell.value) // chữ → số
...
const numD = (v: any) => (v !== null && v !== undefined && v !== '' ? v : null)
void numD
...
const discreteToAlias = (measurementId, value) => {
  ...
  const m = measurementId && CB_DISCRETE_ALIAS_TO_VALUE[measurementId]
  if (m) {
    for (const alias in m)
      if (m[alias] === value || String(m[alias]) === String(value)) return alias
  }
  return value
}
...
const unmapCell = (cell) => {
  ...
  let value
  if (cell.type === 'discrete')
    value = discreteToAlias(measurementId, cell.value) // số → chữ
